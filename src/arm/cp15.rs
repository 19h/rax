...
impl Default for Cp15State {
    fn default() -> Self {
        Self {
            ...
            id_isar: [0x02101110, 0x13112111, 0x21232041, 0x11112131, 0x10011142, 0],
...
impl Cp15State {
    ...
    pub fn write(&mut self, enc: Cp15Encoding, value: u32) -> bool {
        match (enc.crn, enc.op1, enc.crm, enc.op2) {
            ...
            // CRn=1: System control
            (1, 0, 0, 0) => { self.sctlr = Sctlr::from_bits(value); true }
            (1, 0, 0, 1) => { self.actlr = value; true }
            (1, 0, 0, 2) => { self.cpacr = Cpacr::from_bits(value); true }
            (1, 0, 1, 0) => { self.scr = value; true }
            (1, 0, 1, 1) => { self.sder = value; true }
            (1, 0, 1, 2) => { self.nsacr = value; true }
            
            // CRn=2: Translation table
            (2, 0, 0, 0) => { self.ttbr0 = value as u64; true }
            (2, 0, 0, 1) => { self.ttbr1 = value as u64; true }
            (2, 0, 0, 2) => { self.ttbcr = value; true }
            
            // CRn=3: Domain access
            (3, 0, 0, 0) => { self.dacr = value; true }
            
            // CRn=5: Fault status
            (5, 0, 0, 0) => { self.dfsr = value; true }
            (5, 0, 0, 1) => { self.ifsr = value; true }
            (5, 0, 1, 0) => { self.adfsr = value; true }
            (5, 0, 1, 1) => { self.aifsr = value; true }
            
            // CRn=6: Fault address
            (6, 0, 0, 0) => { self.dfar = value; true }
            (6, 0, 0, 2) => { self.ifar = value; true }
            ...
            // CRn=9: Performance monitors
            (9, 0, 12, 0) => { self.pmcr = value; true }
            (9, 0, 12, 1) => { self.pmcntenset |= value; true }
            (9, 0, 12, 2) => { self.pmcntenset &= !value; true } // PMCNTENCLR
            (9, 0, 13, 0) => { self.pmccntr = value; true }
            (9, 0, 14, 0) => { self.pmuserenr = value; true }
            
            // CRn=13: Context
            (13, 0, 0, 0) => { self.fcseidr = value; true }
            (13, 0, 0, 1) => { self.contextidr = value; true }
            (13, 0, 0, 2) => { self.tpidrurw = value; true }
            (13, 0, 0, 3) => { self.tpidruro = value; true }
            (13, 0, 0, 4) => { self.tpidrprw = value; true }
            
            // CRn=14: Timer
            (14, 0, 0, 0) => { self.cntfrq = value; true }
            (14, 0, 2, 0) => { self.cntp_tval = value; true }
            (14, 0, 2, 1) => { self.cntp_ctl = value; true }
            (14, 0, 3, 0) => { self.cntv_tval = value; true }
            (14, 0, 3, 1) => { self.cntv_ctl = value; true }
...
impl fmt::Debug for Sctlr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "SCTLR {{ M:{} A:{} C:{} Z:{} I:{} V:{} EE:{} TE:{} bits:{:08x} }}",
            self.m() as u8, self.a() as u8, self.c() as u8, self.z() as u8,
            self.i() as u8, self.v() as u8, self.ee() as u8, self.te() as u8,
            self.bits)
    }
}
