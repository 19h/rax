//! Exact legacy SSE4.1 `INSERTPS` replay classification.
...
fn expected(rex: Option<u8>, modrm: u8, immediate: u8) -> X86LegacyInsertpsReplay {
    let rex = rex.unwrap_or(0);
    X86LegacyInsertpsReplay {
...
fn classifier_exhausts_opcode_modrm_and_canonical_frontiers() {
    ...
    let invalid: &[&[u8]] = &[
        &[0x0F, 0x3A, 0x21, 0xCA, 0xA5],             // missing mandatory 66
        &[0xF2, 0x0F, 0x3A, 0x21, 0xCA, 0xA5],       // wrong mandatory prefix
        &[0xF0, 0x66, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // LOCK
        &[0x67, 0x66, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // address prefix excluded
        &[0x64, 0x66, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // segment prefix excluded
        &[0x48, 0x66, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // REX not final
        &[0x66, 0x48, 0x49, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // duplicate REX
        &[0x66, 0xD5, 0x00, 0x0F, 0x3A, 0x21, 0xCA, 0xA5], // REX2
        &[0x66, 0x0F, 0x38, 0x21, 0xCA, 0xA5],       // wrong map
        &[0x66, 0x0F, 0x3A, 0x20, 0xCA, 0xA5],       // adjacent opcode
        &[0x66, 0x0F, 0x3A, 0x22, 0xCA, 0xA5],       // adjacent opcode
        &[0x66, 0x0F, 0x3A, 0x21, 0x0A, 0xA5],       // memory source
        &[0x66, 0x0F, 0x3A, 0x21, 0xCA],             // missing immediate
        &[0x66, 0x0F, 0x3A, 0x21, 0xCA, 0xA5, 0],    // trailing byte
        &[0xC4, 0xE3, 0x79, 0x21, 0xCA, 0xA5],       // VEX
        &[0x62, 0xF3, 0x7D, 0x08, 0x21, 0xCA, 0xA5], // EVEX
    ];
...
fn expected_len(immediate: u8, level: OptLevel) -> usize {
    let destination_lane = (immediate >> 4) & 3;
    let zero_mask = immediate & 0x0F;
    let merge_extracts = (0..4u8)
        .filter(|lane| zero_mask & (1 << lane) == 0 && *lane != destination_lane)
        .count();
    match level {
        OptLevel::O0 => 17 + merge_extracts,
        OptLevel::O1 | OptLevel::O2 => {
            15 + merge_extracts
                + usize::from(zero_mask & (1 << destination_lane) == 0)
                + usize::from(zero_mask != 0)
        }
    }
}
...
fn assert_span(function: &SmirFunction, bytes: &[u8], expected_end: usize) {
    for spans in [
        x86_legacy_insertps_replay_spans(&function.blocks[0], &function.x86_instruction_bytes),
...
fn exact_graph_validator_covers_all_immediates_rex_aliases_and_o0_o1_o2() {
    for immediate in u8::MIN..=u8::MAX {
        ...
        for level in LEVELS {
            ...
            let expected_end = expected_len(immediate, level);
            assert_eq!(
        ...
        .enumerate()
    {
        for (shape_index, modrm) in [0xC0, 0xC9, 0xCA, 0xFF].into_iter().enumerate() {
            ...
            for level in LEVELS {
                assert_span(
                    &function(&bytes, level),
                    &bytes,
                    expected_len(immediate, level),
                );
            }
        }
    }
...
fn mutation_count(kind: &OpKind) -> usize {
    match kind {
        ...
        OpKind::VBroadcast { .. } => 4,
        _ => panic!("unexpected INSERTPS graph operation: {kind:?}"),
    }
}
...
fn graph_validator_rejects_every_operation_field_hint_and_escaping_temporary() {
    for immediate in [0x00, 0x0F, 0x10, 0x15, 0xA5, 0xFF] {
        for level in LEVELS {
