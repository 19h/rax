    ...
    for p1 in u8::MIN..=u8::MAX {
        ...
        for opcode in [0x28, 0x29] {
            for modrm in 0xC0..=0xFF {
                ...
                let expected_destination = expected.then_some(if opcode == 0x28 {
...
fn classifier_rejects_memory_reserved_and_noncanonical_frontiers() {
    ...
    for map in 0u8..=31 {
        ...
        let bytes = vex_c4_encoding(0xE0 | map, 0xF9, 0x28, 0xC0);
        assert!(
            !X86InstructionBytes::new(&bytes)
                .unwrap()
                .is_vex_register_aligned_packed_fp_move(),
    ...
    let invalid: &[&[u8]] = &[
        ...
        &[0x66, 0x0F, 0x28, 0xC0],
        &[0xC5, 0x78, 0x28],
        &[0xC5, 0x78, 0x28, 0xC0, 0],
        &[0xC5, 0x70, 0x28, 0xC0],
        &[0xC5, 0xFA, 0x28, 0xC0],
        &[0xC4, 0xE1, 0xF9, 0x28],
        &[0xC4, 0xE1, 0xF9, 0x28, 0xC0, 0],
        &[0xC4, 0xE1, 0xE9, 0x28, 0xC0],
        &[0xC4, 0xE1, 0xFA, 0x28, 0xC0],
        &[0x62, 0xF1, 0x7C, 0x08, 0x28, 0xC0],
    ];
...
fn replay_spans_preserve_exact_bytes_and_remain_non_evex() {
    ...
    for bytes in [
        vex_c5_encoding(0x7C, 0x28, 0xCB).to_vec(),
        vex_c4_encoding(0x41, 0xFD, 0x29, 0xCB).to_vec(),
    ] {
        ...
        for spans in [
            x86_vex_aligned_packed_fp_move_replay_spans(&block, &provenance),
    ...
    assert!(x86_vex_aligned_packed_fp_move_replay_spans(&block, &HashMap::new()).is_empty());
}
