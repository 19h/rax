//! Exact classifier and span tests for AVX VEX immediate blends.

use super::*;

const OPCODES: [u8; 4] = [0x02, 0x0C, 0x0D, 0x0E];
...
fn classifier_exhaustively_covers_131_072_prefix_opcode_and_register_combinations() {
    ...
    assert_eq!(accepted, 114_688);
    assert_eq!(tested, 131_072);

    for imm in u8::MIN..=u8::MAX {
        let opcode = OPCODES[usize::from(imm) % OPCODES.len()];
        let w = opcode != 0x02 && imm & 1 != 0;
        let l = imm & 2 != 0;
        let bytes = encoding(0x40, w, imm >> 4, l, opcode, 0xCB, imm);
        assert_eq!(
            X86InstructionBytes::new(&bytes)
                .unwrap()
                .vex_register_immediate_blend_needs_avx2(),
            expected_requirement(opcode, w, l),
            "{bytes:02X?}"
        );
    }
    ...
    for (bytes, needs_avx2) in [
        ([0xC4, 0xE3, 0x69, 0x02, 0xCB, 0xA5], true),
        ([0xC4, 0x43, 0x2D, 0x02, 0xCB, 0x5A], true),
        ([0xC4, 0xE3, 0x69, 0x0C, 0xCB, 0x96], false),
        ([0xC4, 0x43, 0x2D, 0x0D, 0xCB, 0x0D], false),
        ([0xC4, 0x43, 0x2D, 0x0E, 0xCB, 0x69], true),
    ] {
        let instruction = X86InstructionBytes::new(&bytes).unwrap();
        assert_eq!(
            instruction.vex_register_immediate_blend_needs_avx2(),
            Some(needs_avx2),
            "{bytes:02X?}"
        );
    }
    assert_eq!(
        X86InstructionBytes::new(&[0xC4, 0xE3, 0x69, 0x02, 0xCB, 0xA5])
            .unwrap()
            .vex_immediate_blend_destination_index(),
        Some(1)
    );
    assert_eq!(
        X86InstructionBytes::new(&[0xC4, 0x43, 0x2D, 0x0E, 0xCB, 0x69])
            .unwrap()
            .vex_immediate_blend_destination_index(),
        Some(9)
    );
...
fn classifier_rejects_every_structural_and_reserved_frontier() {
    let canonical = encoding(0xE0, false, 0x0D, true, 0x0E, 0xCA, 0xA5);
    let mut invalid = vec![
        canonical[..5].to_vec(),
    ...
    for bytes in invalid {
        ...
        assert_eq!(instruction.vex_immediate_blend_destination_index(), None);
    }
}
...
fn dedicated_and_aggregate_spans_require_exact_contiguous_provenance() {
    let pc = 0xB1E0;
    let instruction =
        X86InstructionBytes::new(&encoding(0x40, true, 3, true, 0x0D, 0xFF, 0x96)).unwrap();
    let mut block = SmirBlock::new(BlockId(40), pc);
    block.push_op(SmirOp::new(OpId(0), pc, OpKind::Nop));
    block.push_op(SmirOp::new(OpId(1), pc, OpKind::Nop));
    let provenance = HashMap::from([((block.id, pc), instruction)]);

    for spans in [
        x86_vex_immediate_blend_replay_spans(&block, &provenance),
        x86_native_replay_spans(&block, &provenance),
    ] {
        let span = spans.get(&0).expect("exact VEX immediate-blend span");
