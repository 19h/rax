...
/// Exact fields for one deterministic VEX.128 `VMOVLPS`, `VMOVLPD`,
/// `VMOVHPS`, or `VMOVHPD` memory store.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct X86VexHalfMoveStoreEncoding {
    pub(crate) source: u8,
    /// Source qword lane written to the 8-byte memory operand.
    pub(crate) memory_lane: u8,
    /// Exact WIG bit retained for source-provenance validation.
    pub(crate) w: bool,
    /// `0` selects packed-single naming; `1` selects packed-double naming.
...
impl X86InstructionBytes {
    ...
    /// Validate one complete VEX.128 high/low 64-bit lane memory store.
    ///
    /// Map 0F opcode 13H stores source bits 63:0; opcode 17H stores source
    /// bits 127:64. Mandatory-prefix values `00b` and `01b` select the
    /// packed-single and packed-double names but have identical bit-transfer
    /// semantics. W is ignored. VEX.vvvv is reserved and must decode to zero
    /// (encoded `1111b`).
    ///
    /// Intel specifies `VEX.L=1` as #UD for all four forms, so only L=0 is
    /// admitted. Load opcodes 12H/16H and register ModR/M forms remain
    /// disjoint. The shared parser validates the complete memory image.
    pub(crate) fn vex_half_move_store_encoding(&self) -> Option<X86VexHalfMoveStoreEncoding> {
        let fields = self.vex_memory_fields()?;
        if fields.map != 1
            || fields.width_256
            || fields.source1 != 0
            || !matches!(fields.pp, 0 | 1)
            || !matches!(fields.opcode, 0x13 | 0x17)
        {
            return None;
        }

        Some(X86VexHalfMoveStoreEncoding {
            source: fields.destination,
            memory_lane: u8::from(fields.opcode == 0x17),
            w: fields.w,
            pp: fields.pp,
            opcode: fields.opcode,
        })
...
mod tests {
    ...
    #[test]
    fn classifies_all_192_store_source_format_opcode_and_form_cells() {
        let forms = [Form::C5, Form::C4 { w: false }, Form::C4 { w: true }];
        let mut classified = 0usize;
        for source in 0..16 {
            for pp in 0..=1 {
                for opcode in [0x13, 0x17] {
                    for form in forms {
                        let base = if form == Form::C5 { 3 } else { 11 };
                        let bytes = instruction(source, 0, base, pp, opcode, form);
                        assert_eq!(
                            X86InstructionBytes::new(&bytes)
                                .unwrap()
                                .vex_half_move_store_encoding(),
                            Some(X86VexHalfMoveStoreEncoding {
                                source,
                                memory_lane: u8::from(opcode == 0x17),
                                w: matches!(form, Form::C4 { w: true }),
                                pp,
                                opcode,
                            }),
        ...
        assert_eq!(classified, 16 * 2 * 2 * 3);
    }

    #[test]
    fn store_complete_addresses_and_reserved_fields_classify_fail_closed() {
        let valid_cases: &[(&[u8], X86VexHalfMoveStoreEncoding)] = &[
            (
        ...
        let valid = instruction(9, 0, 11, 1, 0x17, Form::C4 { w: true });
        let mut invalid = Vec::new();
        ...
        for bytes in invalid {
            assert_eq!(
                X86InstructionBytes::new(&bytes)
                    .and_then(|instruction| instruction.vex_half_move_store_encoding()),
                None,
                "{bytes:02X?}"
            );
        }
