//! Complete VEX packed-sign memory-source classification.

use super::X86InstructionBytes;
use crate::smir::ir::types::{VecElementType, VecWidth};

impl X86InstructionBytes {
    /// Validate one complete VEX `VPSIGNB`, `VPSIGNW`, or `VPSIGND`
    /// instruction whose control operand is memory and return
    /// `(destination, value source, element type, vector width, W)`.
    ///
    /// These forms use map 0F38 and mandatory prefix 66H. VEX.W is ignored by
    /// the architecture and retained here so callers can prove that both W
    /// selections were classified before canonicalizing native replay to W=0.
    /// The shared parser validates the complete ModR/M/SIB/displacement byte
    /// shape and permits only segment/address-size legacy prefixes.
    pub(crate) fn vex_memory_packed_sign_fields(
        &self,
...
mod tests {
    ...
    #[test]
    fn classifies_every_destination_source_width_w_and_element_cell() {
            ...
            for source1 in 0..16 {
                for elem in [VecElementType::I8, VecElementType::I16, VecElementType::I32] {
                    for width in [VecWidth::V128, VecWidth::V256] {
                        for base in [3, 11] {
                            for w in [false, true] {
                                let bytes = instruction(destination, source1, base, elem, width, w);
                                let metadata = X86InstructionBytes::new(&bytes).unwrap();
                                assert_eq!(
                                    metadata.vex_memory_packed_sign_fields(),
        ...
        assert_eq!(classified, 16 * 16 * 3 * 2 * 2 * 2);
    }

    #[test]
    fn accepts_complete_prefixed_sib_and_displacement_shape() {
        // addr32 FS: VPSIGNW ymm14,ymm9,[r14d+r15d*2+0x44332211]
        let bytes = [
            0x64, 0x67, 0xC4, 0x02, 0xB5, 0x09, 0xB4, 0x7E, 0x11, 0x22, 0x33, 0x44,
        ];
        let metadata = X86InstructionBytes::new(&bytes).unwrap();
        assert_eq!(
            metadata.vex_memory_packed_sign_fields(),
    ...
    fn malformed_or_semantically_different_encodings_fail_closed() {
        let valid = instruction(3, 9, 11, VecElementType::I16, VecWidth::V128, false);
        ...
        cases.push(vec![0xC5, 0xB1, 0x09, 0x5B, 0x20]);
