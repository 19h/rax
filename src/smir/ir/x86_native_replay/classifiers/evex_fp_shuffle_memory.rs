//! EVEX VSHUFPS/VSHUFPD memory-source classification.

use super::X86InstructionBytes;
use super::evex_memory::{memory_operand_end, vector_legacy_prefix_len};
use crate::smir::ir::types::{MemWidth, VecElementType, VecWidth};

/// Native replay selected for one exact EVEX VSHUFPS/VSHUFPD memory source.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum X86EvexFpShuffleMemoryReplay {
...
/// Exact EVEX VSHUFPS/VSHUFPD memory encoding and its byte-validated native
/// replay.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct X86EvexFpShuffleMemoryEncoding {
    ...
    pub(crate) replay: X86EvexFpShuffleMemoryReplay,
    pub(crate) memory_size: u32,
    pub(crate) needs_avx512vl: bool,
}

impl X86InstructionBytes {
    /// Validate one EVEX VSHUFPS/VSHUFPD memory source and select an exact
    ...
    /// The unconstrained imm8 byte is preserved exactly. Segment/address-size
    /// prefixes and APX B4/X4 address extensions remain confined to helper
    /// address evaluation.
    pub(crate) fn evex_fp_shuffle_memory_encoding(&self) -> Option<X86EvexFpShuffleMemoryEncoding> {
        ...
        let memory_size = match replay {
            X86EvexFpShuffleMemoryReplay::Vector { .. } => width.bytes(),
            X86EvexFpShuffleMemoryReplay::Broadcast { memory_width, .. } => memory_width.bytes(),
        };

        Some(X86EvexFpShuffleMemoryEncoding {
            width,
            elem,
            destination,
            source1,
