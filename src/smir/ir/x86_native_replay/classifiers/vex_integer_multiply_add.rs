//! Complete VEX packed-integer multiply-add memory-source classification.

use super::X86InstructionBytes;
use crate::smir::ir::types::{VecElementType, VecWidth};

impl X86InstructionBytes {
    /// Validate one complete VEX `VPMADDUBSW` or `VPMADDWD` instruction whose
    /// second source operand is memory and return
    /// `(destination, first source, source element type, width, W)`.
    ///
    /// `VPMADDUBSW` uses map 0F38 and opcode 04H; `VPMADDWD` uses map 0F and
    /// opcode F5H. Both require mandatory prefix 66H and specify WIG. The
    /// shared memory parser validates every prefix, ModR/M, SIB, displacement,
    /// and complete-instruction boundary before this semantic classification.
    pub(crate) fn vex_memory_integer_multiply_add_fields(
        &self,
    ) -> Option<(u8, u8, VecElementType, VecWidth, bool)> {
        let fields = self.vex_memory_fields()?;
        if fields.pp != 1 {
            return None;
        }
        let source_element = match (fields.map, fields.opcode) {
            (2, 0x04) => VecElementType::I8,
            (1, 0xF5) => VecElementType::I16,
            _ => return None,
        };
        Some((
            fields.destination,
            fields.source1,
            source_element,
            if fields.width_256 {
                VecWidth::V256
            } else {
                VecWidth::V128
            },
            fields.w,
        ))
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn map_and_opcode(source_element: VecElementType) -> (u8, u8) {
        match source_element {
            VecElementType::I8 => (2, 0x04),
            VecElementType::I16 => (1, 0xF5),
            _ => unreachable!("packed-integer multiply-add source element"),
        }
    }

    fn vex2_instruction(destination: u8, source1: u8, base: u8, width: VecWidth) -> Vec<u8> {
        assert!(base < 8);
        let l = u8::from(width == VecWidth::V256);
        vec![
            0xC5,
            (if destination < 8 { 0x80 } else { 0 }) | (((!source1) & 0x0F) << 3) | (l << 2) | 1,
            0xF5,
            0x40 | ((destination & 7) << 3) | base,
            0x20,
        ]
    }

    fn vex3_instruction(
        destination: u8,
        source1: u8,
        base: u8,
        source_element: VecElementType,
        width: VecWidth,
        w: bool,
    ) -> Vec<u8> {
        let (map, opcode) = map_and_opcode(source_element);
        let l = u8::from(width == VecWidth::V256);
        vec![
            0xC4,
            (if destination < 8 { 0x80 } else { 0 })
                | 0x40
                | (if base < 8 { 0x20 } else { 0 })
                | map,
            (u8::from(w) << 7) | (((!source1) & 0x0F) << 3) | (l << 2) | 1,
            opcode,
            0x40 | ((destination & 7) << 3) | (base & 7),
            0x20,
        ]
    }

    #[test]
    fn classifies_every_destination_source_kind_width_form_and_w_cell() {
        let mut classified = 0usize;
        for destination in 0..16 {
            for source1 in 0..16 {
                for source_element in [VecElementType::I8, VecElementType::I16] {
                    for width in [VecWidth::V128, VecWidth::V256] {
                        if source_element == VecElementType::I16 {
                            let bytes = vex2_instruction(destination, source1, 3, width);
                            let metadata = X86InstructionBytes::new(&bytes).unwrap();
                            assert_eq!(
                                metadata.vex_memory_integer_multiply_add_fields(),
                                Some((destination, source1, source_element, width, false)),
                                "{bytes:02X?}"
                            );
                            classified += 1;
                        }

                        for base in [3, 11] {
                            for w in [false, true] {
                                let bytes = vex3_instruction(
                                    destination,
                                    source1,
                                    base,
                                    source_element,
                                    width,
                                    w,
                                );
                                let metadata = X86InstructionBytes::new(&bytes).unwrap();
                                assert_eq!(
                                    metadata.vex_memory_integer_multiply_add_fields(),
                                    Some((destination, source1, source_element, width, w)),
                                    "{bytes:02X?}"
                                );
                                classified += 1;
                            }
                        }
                    }
                }
            }
        }
        assert_eq!(classified, 16 * 16 * 2 * (4 + 5));
    }

    #[test]
    fn accepts_complete_prefixed_sib_and_displacement_shapes() {
        // addr32 FS: VPMADDUBSW ymm14,ymm9,[r14d+r15d*2+0x44332211]
        let bytes = [
            0x64, 0x67, 0xC4, 0x02, 0xB5, 0x04, 0xB4, 0x7E, 0x11, 0x22, 0x33, 0x44,
        ];
        let metadata = X86InstructionBytes::new(&bytes).unwrap();
        assert_eq!(
            metadata.vex_memory_integer_multiply_add_fields(),
            Some((14, 9, VecElementType::I8, VecWidth::V256, true))
        );

        // GS: VPMADDWD xmm14,xmm9,[r14+r15*2+0x44332211]
        let bytes = [
            0x65, 0xC4, 0x01, 0xB1, 0xF5, 0xB4, 0x7E, 0x11, 0x22, 0x33, 0x44,
        ];
        let metadata = X86InstructionBytes::new(&bytes).unwrap();
        assert_eq!(
            metadata.vex_memory_integer_multiply_add_fields(),
            Some((14, 9, VecElementType::I16, VecWidth::V128, true))
        );
    }

    #[test]
    fn malformed_or_semantically_different_memory_encodings_fail_closed() {
        let valid = vex3_instruction(3, 9, 11, VecElementType::I8, VecWidth::V128, false);
        let mut cases = Vec::new();

        let mut wrong_map = valid.clone();
        wrong_map[1] = (wrong_map[1] & !0x1F) | 1;
        cases.push(wrong_map);

        let mut wrong_prefix = valid.clone();
        wrong_prefix[2] = (wrong_prefix[2] & !3) | 2;
        cases.push(wrong_prefix);

        let mut wrong_opcode = valid.clone();
        wrong_opcode[3] = 0x05;
        cases.push(wrong_opcode);

        let mut register_source = valid.clone();
        register_source[4] |= 0xC0;
        register_source.truncate(5);
        cases.push(register_source);

        let mut trailing = valid.clone();
        trailing.push(0);
        cases.push(trailing);

        let mut truncated = valid.clone();
        truncated.pop();
        cases.push(truncated);

        let mut forbidden_legacy_prefix = valid;
        forbidden_legacy_prefix.insert(0, 0x66);
        cases.push(forbidden_legacy_prefix);

        // Map 0F38 cannot be represented by VEX2.
        cases.push(vec![0xC5, 0xF1, 0x04, 0x40, 0x20]);

        for bytes in cases {
            let metadata = X86InstructionBytes::new(&bytes).unwrap();
            assert_eq!(
                metadata.vex_memory_integer_multiply_add_fields(),
                None,
                "{bytes:02X?}"
            );
        }
    }
}
