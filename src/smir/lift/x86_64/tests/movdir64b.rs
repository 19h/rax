...
fn legacy_movdir64b_rejects_invalid_and_incomplete_encodings() {
    for (bytes, frontier) in [
        (&[0x0F, 0x38, 0xF8][..], 3),
        (&[0x0F, 0x38, 0xF8, 0x08][..], 3),
        (&[0xF2, 0x0F, 0x38, 0xF8][..], 4),
        (&[0xF3, 0x0F, 0x38, 0xF8][..], 4),
        (&[0xF0, 0x66, 0x0F, 0x38, 0xF8][..], 5),
        (&[0xF0, 0x66, 0x0F, 0x38, 0xF8, 0x08][..], 5),
        (&[0xF2, 0x66, 0x0F, 0x38, 0xF8, 0x08][..], 5),
        (&[0xF3, 0x66, 0x0F, 0x38, 0xF8, 0x08][..], 5),
        (&[0x66, 0x0F, 0x38, 0xF8, 0xC8][..], 5),
    ] {
        let result = lift_single(bytes).expect("invalid MOVDIR64B must strictly lift to #UD");
        assert_invalid_opcode_trap(&result, frontier);
    }
...
fn apx_movdir64b_rejects_reserved_fields_and_reports_absolute_incomplete_lengths() {
    for (bytes, name) in [
        (&[0x62, 0xEC, 0xFD, 0x08, 0xF8][..], "W=1"),
        (&[0x62, 0xEC, 0x7D, 0x18, 0xF8][..], "ND"),
        (&[0x62, 0xEC, 0x7D, 0x0C, 0xF8][..], "NF"),
        (&[0x62, 0xEC, 0x7D, 0x88, 0xF8][..], "z"),
        (&[0x62, 0xEC, 0x7D, 0x28, 0xF8][..], "LL"),
        (&[0x62, 0xEC, 0x7D, 0x09, 0xF8][..], "aaa"),
        (&[0x62, 0xEC, 0x75, 0x08, 0xF8][..], "V3:0"),
        (&[0x62, 0xEC, 0x7D, 0x00, 0xF8][..], "V4"),
    ] {
        let result = lift_single(bytes).unwrap_or_else(|error| panic!("{name}: {error:?}"));
        assert_invalid_opcode_trap(&result, 5);
    }

    let register_source = lift_single(&[0x62, 0xEC, 0x7D, 0x08, 0xF8, 0xC1])
