...
fn lift_legacy_vex_evex_comi_ucomi() {
    ...
    for p2 in [0x08, 0x28, 0x48, 0x18, 0x38, 0x58, 0x78] {
        let llig = lift_single(&[0x62, 0xF1, 0x7C, p2, 0x2E, 0xD1]).unwrap();
        assert!(matches!(
            llig.ops.last().unwrap().kind,
            OpKind::X86FpCompare {
                elem: VecElementType::F32,
                signaling: false,
                suppress_exceptions,
                ..
            } if suppress_exceptions == (p2 & 0x10 != 0)
        ));
    }
    ...
    for bytes in [
        ...
        &[0x62, 0xF1, 0x7C, 0x09, 0x2F, 0xC1][..], // reserved opmask
        &[0x62, 0xF1, 0x7C, 0x88, 0x2F, 0xC1][..], // reserved zeroing
        &[0x62, 0xF1, 0x7C, 0x68, 0x2F, 0xC1][..], // reserved no-SAE L'L=11b
        &[0x62, 0xF1, 0x7C, 0x18, 0x2F, 0x01][..], // EVEX.b memory
        &[0x62, 0xF1, 0x7C, 0x68, 0x2F, 0x01][..], // memory no-SAE L'L=11b
    ] {
        assert!(matches!(
            lift_single(bytes),
            Err(LiftError::InvalidEncoding { .. } | LiftError::Unsupported { .. })
