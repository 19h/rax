...
fn lift_through_calls_retains_state_backed_memory_targets_and_rejects_virtual_addresses() {
    ...
    // addr32 CALL [EAX] requires a zero-extension temporary. That virtual value
    // cannot be reconstructed by the helper ABI, so the instruction remains an
    // exact interpreter frontier and its pre-ops are not partially retained.
