...
fn lifted_aesni_vaes_executes_fips_vectors_lanes_upper_state_and_faults() {
    ...
    let round1 = block([
        0x89, 0xD8, 0x10, 0xE8, 0x85, 0x5A, 0xCE, 0x68, 0x2D, 0x18, 0x43, 0xD8, 0xCB, 0x12, 0x8F,
        0xE4,
    ]);
    let round9 = block([
        0xBD, 0x6E, 0x7C, 0x3D, 0xF2, 0xB5, 0x77, 0x9E, 0x0B, 0x61, 0x21, 0x6E, 0x8B, 0x10, 0xB6,
        0x89,
    ]);
    ...
    let keygen_expected = block([
        0xF2, 0x6B, 0x6F, 0xC5, 0x6A, 0x6F, 0xC5, 0xF2, 0xFE, 0xD7, 0xAB, 0x76, 0xD6, 0xAB, 0x76,
        0xFE,
    ]);
    if let ArchRegState::X86_64(x86) = &mut ctx.arch_regs {
        x86.xmm[0] = [upper; 16];
...
fn lifted_pmaddwd_executes_wrap_widths_masks_complete_memory_and_faults() {
    fn packed_words(values: &[u16], fill: u64) -> VecValue {
        ...
        for (lane, value) in values.iter().copied().enumerate() {
            let shift = (lane % 4) * 16;
            out[lane / 4] = (out[lane / 4] & !(0xFFFFu64 << shift)) | (u64::from(value) << shift);
        }
        out
    }
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF1, 0x75, 0x09, 0xF5, 0x00], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn lifted_packed_shift_count_executes_full_counts_masks_mem128_and_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF1, 0x75, 0x09, 0xE1, 0x00], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn lifted_immediate_blends_execute_raw_masks_repetition_aliases_and_faults() {
    ...
    fn blend(first: &[u8], second: &[u8], elem_bytes: usize, imm: u8, repeat_128: bool) -> Vec<u8> {
    ...
    let fault = execute_lifted_x86(&[0xC4, 0xE3, 0x65, 0x0C, 0x08, 0xA5], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn lifted_sm3_executes_schedule_rounds_aliases_memory_and_faults() {
    ...
    fn msg2(old: [u32; 4], first: [u32; 4], second: [u32; 4]) -> [u32; 4] {
        let mut result =
            std::array::from_fn(|index| first[index].rotate_left(7) ^ second[index] ^ old[index]);
        result[3] ^=
    ...
    let fault = execute_lifted_x86(&[0xC4, 0x63, 0x21, 0xDE, 0x08, 0x00], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn lifted_fp_unpack_family_executes_lane_blocks_masks_broadcast_and_e4nf_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xE1, 0x6C, 0x43, 0x14, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
    ...
    for exponent in [1u64, 2, 256, 1023, 1024, 1792, 2045, 2046] {
        for segment in 0u64..32 {
            for tail in [1u64, (1 << 46) - 1, (1 << 47) - 1] {
                ...
                let actual =
                    f64::from_bits(SmirInterpreter::x86_simd_rsqrt14(bits, X86_SIMD_F64, 0).bits);
