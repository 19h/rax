...
fn executes_evex_map5_fp16_arithmetic_masks_and_zeroes_upper_state() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF5, 0x6C, 0x0C, 0x5E, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_fp16_minmax_selection_exceptions_masks_and_scalar_merge() {
    ...
    let scalar = execute_lifted_x86(&[0x62, 0xF5, 0x6E, 0x08, 0x5D, 0xCB], &mut ctx, &mut memory);
...
fn executes_evex_map5_fp16_embedded_rounding_independent_of_mxcsr() {
    for (p2, mxcsr_rounding, expected_positive, expected_negative) in [
        ...
        let result = execute_lifted_x86(&[0x62, 0xF5, 0x6C, p2, 0x58, 0xCB], &mut ctx, &mut memory);
...
fn lifted_evex_vmovsh_executes_aliases_masks_load_store_and_fault_suppression() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF5, 0x7E, 0x0A, 0x10, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF5, 0x7E, 0x0A, 0x11, 0x10], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
...
fn lifted_evex_fp16_scalar_arithmetic_executes_ops_masks_aliases_and_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF5, 0x6E, 0x0A, 0x5E, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_packed_funnel_shifts_all_forms_masks_memory_and_faults() {
    ...
    for (bytes, bits, left, variable, immediate) in cases {
        ...
        if let ArchRegState::X86_64(x86) = &ctx.arch_regs {
            for lane in 0..lanes as u8 {
                let primary =
                    SmirInterpreter::get_lane(if variable { &old } else { &second }, lane, bits);
                let fill =
                    SmirInterpreter::get_lane(if variable { &second } else { &third }, lane, bits);
...
fn executes_evex_integer_test_masks_all_elements_polarities_masks_and_faults() {
    ...
    for (bytes, bits, inverted) in cases {
        ...
        let nonzero = (0..lanes).fold(0u64, |mask, lane| mask | u64::from(lane % 3 == 0) << lane);
        if let ArchRegState::X86_64(x86) = &ctx.arch_regs {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF2, 0x75, 0x52, 0x27, 0x20], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_pair_intersect_duplicates_pairing_and_e4nf_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF2, 0x77, 0x50, 0x68, 0x30], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_packed_fp_arithmetic_masks_broadcasts_aliases_and_e4_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF1, 0x6C, 0x49, 0x5C, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_packed_sqrt_masks_zeroes_and_suppresses_e4_faults() {
    ...
    if let ArchRegState::X86_64(x86) = &mut ctx.arch_regs {
        for lane in 0..16u8 {
            ...
            SmirInterpreter::set_lane(&mut x86.xmm[0], lane, 32, (root * root).to_bits().into());
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF1, 0x7C, 0x49, 0x51, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_mask_blends_select_sources_zero_and_suppress_e4_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF2, 0x6D, 0xC9, 0x64, 0x08], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: false, .. })
    ));
...
fn executes_evex_integer_narrow_all_modes_ratios_masks_aliases_memory_and_faults() {
    ...
    let fault = execute_lifted_x86(&[0x62, 0xF2, 0x7E, 0x09, 0x31, 0x10], &mut ctx, &mut memory);
    assert!(matches!(
        fault,
        BlockResult::Exit(ExitReason::MemoryFault { write: true, .. })
    ));
