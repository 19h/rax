...
fn lifted_evex_fp_class_executes_all_classes_daz_masks_and_fault_suppression() {
    ...
    if let ArchRegState::X86_64(x86) = &mut ctx.arch_regs {
        for lane in 0..16u8 {
            SmirInterpreter::set_lane(&mut x86.xmm[17], lane, 32, classes[usize::from(lane % 8)]);
        }
        x86.k[2] = u64::MAX;
        x86.k[3] = mask;
        x86.mxcsr = 0x1F80;
...
fn x86_fp16_complex_preserves_boundary_rounding_pair_masks_scalar_copy_and_masked_exceptions() {
    ...
    for (round, expected) in [
        ...
        assert!(matches!(
            SmirInterpreter::new().execute_block(&mut directed, &mut memory, &function.blocks[0]),
            BlockResult::Exit(ExitReason::Halt)
        ));
    ...
    assert!(matches!(
        SmirInterpreter::new().execute_block(&mut masked_merge, &mut memory, &function.blocks[0]),
        BlockResult::Exit(ExitReason::Halt)
    ));
...
fn x86_fixup_imm_exact_semantics_cover_tokens_responses_daz_and_reports() {
    ...
    let qnan = SmirInterpreter::x86_simd_fixup_imm(dest32, 0xFFC1_2345, 2, X86_SIMD_F32, 0x1F80, 0);
...
fn x86_fp16_approx_exhaustively_satisfies_special_cases_and_error_bound() {
    ...
    for exponent in -14..=15 {
        ...
        if exponent % 2 == 0 {
            let expected_rsqrt = SmirInterpreter::x86_f32_to_fp16(2.0f32.powi(-exponent / 2), 0);
...
fn lifted_avx512_four_fma_executes_sequential_rounding_masks_scalar_and_aliases() {
    ...
    if let ArchRegState::X86_64(x86) = &mut ctx.arch_regs {
        ...
        for lane in 0..16u8 {
            ...
            for (reg, value) in [(4, 2.0f32), (5, 3.0), (6, 4.0), (7, 5.0)] {
                SmirInterpreter::set_lane(&mut x86.xmm[reg], lane, 32, u64::from(value.to_bits()));
            }
        }
    }
    assert!(matches!(
    ...
    if let ArchRegState::X86_64(x86) = &mut ctx.arch_regs {
        ...
        for lane in 0..16u8 {
            ...
            for reg in 4..=7 {
                SmirInterpreter::set_lane(&mut x86.xmm[reg], lane, 32, u64::from(1.0f32.to_bits()));
            }
        }
    }
