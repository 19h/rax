...
impl Aarch64Lowerer {
    ...
    pub(crate) fn lower_x86_shift_reg(
        &mut self,
        dst: u8,
        src: u8,
        amount: u8,
        ...
        if width != OpWidth::W32 {
            return self.lower_shift_reg(dst, src, amount, shift, width);
        }

        // AArch64 W-register variable shifts use the low five count bits,
        // exactly matching x86 32-bit scalar shifts. The generic SMIR path
        // deliberately treats counts 32..63 as overshifts for non-x86 source
        // architectures, so x86-register operations must bypass that guard.
        let opcode2 = match shift {
            ShiftOp::Lsl => 0b1000,
            ShiftOp::Lsr => 0b1001,
            ShiftOp::Asr => 0b1010,
            ...
            ShiftOp::Rrx => {
                return Err(LowerError::UnsupportedOp {
                    op: "AArch64 native x86 RRX variable shift".into(),
                });
            }
        };
        self.emit_dp2(dst, src, amount, opcode2, width)
    ...
    pub(crate) fn lower_shift(
        ...
        let x86_register_count = matches!(
            (dst, src, amount),
            (
                VReg::Arch(ArchReg::X86(dst)),
                VReg::Arch(ArchReg::X86(src)),
                SrcOperand::Reg(VReg::Arch(ArchReg::X86(amount))),
            ) if dst.gpr_index().is_some()
                && src.gpr_index().is_some()
                && amount.gpr_index().is_some()
        );
        ...
        match amount {
            ...
            SrcOperand::Reg(reg) => {
                let amount = Self::gpr_arm_or_x86(*reg)?;
                if x86_register_count {
                    self.lower_x86_shift_reg(dst, src, amount, shift, width)
                } else {
                    self.lower_shift_reg(dst, src, amount, shift, width)
                }
            }
            other => Err(LowerError::UnsupportedOp {
                op: format!("AArch64 native shift amount {other:?}"),
