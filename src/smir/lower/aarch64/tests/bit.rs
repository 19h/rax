...
fn issue_31_cwd_subword_x86_dst_merges_low_bits_preserves_upper() {
    ...
    let (regs, _, _) = run_aarch64_code(&code16, &[(hd, 0xDEAD_BEEF_0000_1234), (hs, 0x8000)], 0);
    assert_eq!(
...
fn lowers_x86_subword_shift_rotate_flags_before_partial_merge() {
    ...
    for width in [OpWidth::W8, OpWidth::W16] {
        ...
        for (right, register_count) in [(false, false), (false, true), (true, false), (true, true)]
        {
            let amount = if register_count {
                SrcOperand::Reg(x86(X86Reg::Rcx))
            } else {
    ...
    for width in [OpWidth::W8, OpWidth::W16] {
        for right in [false, true] {
            for carry_in in [false, true] {
                ...
                let expected = (initial & !width_mask(width)) | (expected_low & width_mask(width));
