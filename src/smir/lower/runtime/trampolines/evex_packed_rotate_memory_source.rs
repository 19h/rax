...
fn exact_rotate_or_identity(
    ...
    let exact_rotate = match op.kind {
    ...
    // O1/O2 may fold an unmasked immediate rotate whose reduced count is zero
    // to an exact vector copy. Source-byte provenance retains the rotate
    // direction, element width, and original imm8 needed for native replay.
    let identity = encoding.source.is_none()
        && encoding.writemask.is_none()
        && !encoding.zeroing
        && encoding
            .immediate
            .is_some_and(|amount| u32::from(amount) % (encoding.elem.bytes() * 8) == 0);
    identity
        && op.x86_hint.is_none()
        && matches!(
            op.kind,
...
fn unmasked_vector_sequence(
    ...
    if rotate.guest_pc != load.guest_pc
        || !exact_rotate_or_identity(rotate, loaded, encoding)
...
fn masked_broadcast_sequence(
    ...
    if rotate.guest_pc != guest_pc || !exact_rotate_or_identity(rotate, loaded, encoding) {
        return None;
    }
    offset += 1;
