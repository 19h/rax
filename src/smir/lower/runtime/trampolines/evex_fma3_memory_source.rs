...
/// Exact contiguous unmasked EVEX scalar FMA3 memory-source decomposition
...
/// EVEX memory source. Exact instruction provenance binds the LLIG hint,
/// element type, architectural operands, opcode semantics, and a normalized
/// host-stack memory rewrite. Every virtual intermediate must have exactly one
/// definition and one use in the complete block.
///
/// Classification is O(1) time and O(1) auxiliary space. Callers build the
/// global definition/use maps once in O(N) time and O(V) space for N operations
/// and V virtual registers.
