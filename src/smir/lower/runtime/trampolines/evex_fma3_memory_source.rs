...
pub(crate) fn x86_jit_evex_packed_fma3_memory_sequence(
    ...
    if fma.guest_pc != load.guest_pc
        ...
        || vector_index(&fma_op.src1, width) != Some(encoding.destination)
        || vector_index(&fma_op.src2, width) != Some(encoding.source1)
        || fma_op.src3 != loaded
        || fma_op.mask.is_some()
        || fma_op.elem != elem
        || fma_op.kind != vex_fma3_kind(encoding.opcode)?
        || fma_op.order != vex_fma3_order(encoding.opcode)?
        || fma_op.round != FpRoundMode::Dynamic
        || fma_op.lanes != width.lanes(elem) as u8
        || fma.x86_hint
