//! Fail-closed helper-backed VEX `VDPPS`/`VDPPD` memory-source admission.
...
/// Exact contiguous VEX floating-point dot-product memory-source
/// decomposition consumed by the helper-backed x86-64 lowerer.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct X86JitVexFpDotProductMemorySequence {
...
/// Validate the complete two-op `VLoad`/`X86DotProduct` decomposition for one
/// AVX `VDPPS` or `VDPPD` memory source. Source-byte provenance binds the
/// architectural operands, element type, vector width, imm8, and ignored W
/// bit. The load may retain its original unaligned hint or an aligned hint
/// proven by vector-alignment inference. The loaded virtual must have exactly
/// one definition and one use.
///
...
pub(crate) fn x86_jit_vex_fp_dot_product_memory_sequence(
    ...
    let (encoded_destination, encoded_source1, encoded_elem, encoded_width, immediate, w) =
        instruction.vex_memory_fp_dot_product_fields()?;
    if (
        encoded_destination,
        encoded_source1,
        encoded_elem,
        encoded_width,
        immediate,
    ) != (destination, source1, elem, width, imm)
    {
        return None;
    }
