//! Fail-closed helper-backed EVEX VFIXUPIMM memory admission.
...
use crate::smir::ir::{
    X86EvexFixupImmMemoryEncoding, X86EvexFixupImmMemoryReplay, X86InstructionBytes,
};
...
/// VFIXUPIMM memory lowerer.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct X86JitEvexFixupImmMemorySequence {
    pub(crate) consumed: usize,
    pub(crate) address_offset: usize,
    pub(crate) memory_size: u32,
    pub(crate) encoding: X86EvexFixupImmMemoryEncoding,
}
...
fn exact_fixup(
    ...
    memory_source: VReg,
    encoding: X86EvexFixupImmMemoryEncoding,
) -> bool {
    ...
    matches!(
        ...
        OpKind::X86FixupImm {
            ...
            && suppress_exceptions == encoding.suppress_exceptions
            && op.x86_hint == Some(X86OpHint::EvexOp {
                map: X86VecMap::Map0F3A,
                pp: X86SsePrefix::OpSize,
                opcode: if encoding.scalar { 0x55 } else { 0x54 },
                width: encoding.width,
...
fn unmasked_vector_sequence(
...
) -> Option<X86JitEvexFixupImmMemorySequence> {
    if !matches!(encoding.replay, X86EvexFixupImmMemoryReplay::Vector { .. })
    ...
    let fixup = block.ops.get(index + 1)?;
    let consumed = 2;
    if fixup.guest_pc != load.guest_pc
        || !exact_fixup(fixup, loaded, encoding)
        || !no_following_same_pc(block, index, consumed, load.guest_pc)
    {
        return None;
    }
    Some(X86JitEvexFixupImmMemorySequence {
        consumed,
        address_offset: 0,
...
fn unmasked_broadcast_sequence(
    ...
    let fixup = block.ops.get(index + 2)?;
    let consumed = 3;
    if fixup.guest_pc != load.guest_pc
        || !exact_fixup(fixup, loaded, encoding)
        || !no_following_same_pc(block, index, consumed, load.guest_pc)
...
fn masked_broadcast_sequence(
    ...
    let fixup = block.ops.get(index + offset)?;
    if fixup.guest_pc != guest_pc || !exact_fixup(fixup, loaded, encoding) {
        return None;
    }
    offset += 1;
...
fn scalar_sequence(
    ...
    if !matches!(encoding.replay, X86EvexFixupImmMemoryReplay::Scalar { .. }) || !encoding.scalar {
        return None;
    }
    let first = block.ops.get(index)?;
    let guest_pc = first.guest_pc;
    ...
    let condition = if let Some(mask_index) = encoding.writemask {
    ...
    let loaded = match broadcast.kind {
        OpKind::VBroadcast {
            ...
            elem,
            lanes: 1,
        } if broadcast.x86_hint.is_none() && actual_scalar == scalar && elem == encoding.elem => {
            dst
        }
        _ => return None,
    };
...
/// scalar AVX-512 VFIXUPIMM memory source.
///
...
/// helper address, full imm8, SAE/MXCSR behavior, and the single architectural
...
/// O(V) space.
pub(crate) fn x86_jit_evex_fixup_imm_memory_sequence(
    block: &crate::smir::ir::SmirBlock,
    index: usize,
    allow_mem: bool,
    ...
    match encoding.replay {
        ...
        X86EvexFixupImmMemoryReplay::Broadcast { .. } if encoding.writemask.is_some() => {
