//! Exact helper-backed VEX packed-logic memory-source coverage.
...
impl LogicMemoryCase {
    ...
    /// Exact register-direct instruction emitted after the helper fills the
    /// internal transfer slot and the lowerer imports it through `scratch`.
    fn emitted_logic_bytes(self) -> Vec<u8> {
...
fn vector(index: u8, width: VecWidth) -> VReg {
    x86(match width {
        ...
        _ => unreachable!("VEX packed logic has only 128-/256-bit forms"),
    })
}

fn expected_address(case: LogicMemoryCase) -> Address {
    Address::BaseOffset {
...
fn assert_exact_pair(ops: &[SmirOp], case: LogicMemoryCase) {
    let [load, consumer] = ops else {
        panic!("{case:?}: expected exact VLoad + logic pair, got {ops:?}")
    };
    ...
    let expected_destination = vector(case.destination(), case.width);
    let expected_source1 = vector(case.source1(), case.width);
    let operands_match = match (&consumer.kind, case.kind) {
        (
            OpKind::VAnd {
                dst,
                src1,
    ...
    assert!(
        operands_match,
        "{case:?}: unexpected logic consumer {consumer:?}"
    );
}

fn lift_case(case: LogicMemoryCase) -> SmirFunction {
...
fn lower(function: &SmirFunction, case: LogicMemoryCase) -> (Vec<u8>, usize) {
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("helper-backed VEX logic lowering failed: {error:?}"));
    assert!(result.relocations.is_empty());
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed VEX logic"),
        result.entry_offset,
    )
}
...
fn all_72_c4_c5_wig_width_family_and_logic_shapes_are_lifted_admitted_and_lowered() {
    ...
    for case in cases {
        for level in LEVELS {
            ...
            let expected = case.emitted_logic_bytes();
            assert!(
...
fn classifier_and_lowerer_fail_closed_for_every_pair_invariant() {
    ...
    if let OpKind::VAnd { width, .. } = &mut width_mismatch.blocks[0].ops[1].kind {
        *width = VecWidth::V256;
    }
    ...
    if let OpKind::VAnd { dst, .. } = &mut high_destination.blocks[0].ops[1].kind {
    ...
    let malformed = [
        ...
        ("load/consumer width mismatch", width_mismatch),
...
#[cfg(target_arch = "x86_64")]
fn full_guest_regs(case: LogicMemoryCase, ordinal: usize) -> GuestRegs {
    ...
    registers.zmm[usize::from(case.source1())] = std::array::from_fn(|word| {
        0x00FF_F0F0_3333_5555u64.rotate_left((word * 9 + ordinal) as u32)
    });
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_memory_logic_matches_interpreter_and_is_fault_precise_noncommitting() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX memory-logic differential: host lacks AVX");
        return;
    }
