//! Exact helper-backed AVX-IFMA VEX memory-source coverage.
...
use crate::smir::lower::runtime::{
    ...
    x86_jit_vex_binary_memory_sequence, x86_jit_vex_ifma52_memory_sequence,
    ...
    fn bytes(self) -> Vec<u8> {
        vec![
            ...
            (if self.operands.destination < 8 {
                ...
                | 2,
            0x80 | (((!self.operands.source1) & 0x0F) << 3)
...
fn assert_exact_chain(ops: &[SmirOp], case: MemoryCase) {
    let [load, consumer] = ops else {
        panic!("expected VLoad + VMultiplyAdd52 for {case:?}, got {ops:?}")
    };
    ...
    let OpKind::VMultiplyAdd52 {
        ...
        panic!("{case:?}: expected VMultiplyAdd52, got {consumer:?}")
    };
...
fn sequence(
    ...
    allow_mem: bool,
) -> Option<crate::smir::lower::runtime::X86JitVexIfma52MemorySequence> {
    let (definitions, uses) = virtual_counts(function);
    x86_jit_vex_ifma52_memory_sequence(
        &function.blocks[0],
        0,
        allow_mem,
...
fn lower(function: &SmirFunction, case: MemoryCase) -> (Vec<u8>, usize) {
    ...
    assert_eq!(
        ...
        std::is_x86_feature_detected!("avx") && x86_host_has_avx_ifma(),
        "{case:?}"
    );
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("helper-backed AVX-IFMA lowering failed: {error:?}"));
    assert!(result.relocations.is_empty());
    (
        lowerer.finalize().expect("finalize helper-backed AVX-IFMA"),
        result.entry_offset,
    )
}

#[test]
fn every_width_opcode_operand_alias_and_optimizer_shape_is_admitted_and_lowered() {
    let cases = all_cases();
    assert_eq!(cases.len(), 2 * 2 * 6);
    ...
    assert_eq!(lowered, 2 * 2 * 6 * LEVELS.len());
}

#[test]
fn all_1_024_destination_source_width_opcode_cells_are_admitted_and_lowered_after_o2() {
    ...
    for high in [false, true] {
        ...
        for width in [VecWidth::V128, VecWidth::V256] {
            ...
            for source1 in 0u8..16 {
                let p1 = 0x80 | (((!source1) & 0x0F) << 3) | (l << 2) | 1;
                for destination in 0u8..16 {
                    let p0 = (if destination < 8 { 0x80 } else { 0 }) | 0x60 | 2;
                    let bytes = [0xC4, p0, p1, opcode, 0x02 | ((destination & 7) << 3)];
                    let function = optimize(lift_bytes(&bytes), OptLevel::O2);
                    let actual = sequence(&function, true).unwrap_or_else(|| {
                        panic!("{bytes:02X?}: defined AVX-IFMA form was rejected")
                    });
...
fn sib_rip_relative_segment_absolute_and_addr32_forms_are_admitted_and_lowered() {
    // Independently assembled by LLVM 23 with `{vex}` and `+avxifma`.
    let encodings = [
        (&[0xC4, 0xE2, 0xE9, 0xB4, 0x0C, 0x8B][..], 1, 2),
        (
            &[0xC4, 0x42, 0xA9, 0xB4, 0x8C, 0xD3, 0x78, 0x56, 0x34, 0x12][..],
            9,
            10,
        ),
        (&[0xC4, 0xE2, 0xE9, 0xB4, 0x0D, 0x20, 0, 0, 0][..], 1, 2),
        (&[0x64, 0xC4, 0xE2, 0xE9, 0xB4, 0x4B, 0x20][..], 1, 2),
        (&[0x65, 0xC4, 0xC2, 0xE9, 0xB4, 0x4C, 0xD3, 0x20][..], 1, 2),
        (
            &[0xC4, 0xE2, 0xE9, 0xB4, 0x0C, 0x25, 0x78, 0x56, 0x34, 0x12][..],
            1,
            2,
        ),
        (&[0x67, 0xC4, 0xE2, 0xE9, 0xB4, 0x4C, 0x8B, 0x20][..], 1, 2),
    ];
...
fn classifier_and_gate_fail_closed_for_every_chain_and_provenance_invariant() {
    ...
    assert_mutation_rejected(&base, "accumulator differs from destination", |function| {
        if let OpKind::VMultiplyAdd52 { acc, .. } = &mut function.blocks[0].ops[1].kind {
            *acc = vector(4, case.width);
        }
    });
    assert_mutation_rejected(&base, "consumer bypasses loaded source", |function| {
        if let OpKind::VMultiplyAdd52 { src2, .. } = &mut function.blocks[0].ops[1].kind {
            *src2 = vector(4, case.width);
        }
    });
    assert_mutation_rejected(&base, "consumer width mismatch", |function| {
        if let OpKind::VMultiplyAdd52 { width, .. } = &mut function.blocks[0].ops[1].kind {
            *width = VecWidth::V128;
        }
    });
    assert_mutation_rejected(&base, "wrong high-product selection", |function| {
        if let OpKind::VMultiplyAdd52 { high, .. } = &mut function.blocks[0].ops[1].kind {
            *high = !*high;
        }
    });
    assert_mutation_rejected(&base, "unexpected mask", |function| {
        if let OpKind::VMultiplyAdd52 { mask, .. } = &mut function.blocks[0].ops[1].kind {
            *mask = Some(x86(X86Reg::K(1)));
        }
    });
    assert_mutation_rejected(&base, "unexpected zeroing", |function| {
        if let OpKind::VMultiplyAdd52 { zeroing, .. } = &mut function.blocks[0].ops[1].kind {
            *zeroing = true;
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only source", |function| {
        if let OpKind::VMultiplyAdd52 { src1, .. } = &mut function.blocks[0].ops[1].kind {
    ...
    let mut bytes = case.bytes();
    bytes[3] ^= 1;
    assert_mutation_rejected(&base, "encoded high-low mismatch", |function| {
        replace_instruction_bytes(function, &bytes);
    });
    let mut bytes = case.bytes();
...
fn initial_memory_state(case: MemoryCase, ordinal: usize) -> IfmaState {
...
#[test]
fn memory_interpretation_matches_intel_equations_for_every_variant_alias_and_boundary() {
    let cases = all_cases();
    ...
    assert_eq!(interpreted, 2 * 2 * 6 * DIFFERENTIAL_LEVELS.len());
}
...
#[cfg(target_arch = "x86_64")]
fn guest_regs(initial: &IfmaState) -> GuestRegs {
    GuestRegs {
        gpr: initial.gprs,
...
#[cfg(target_arch = "x86_64")]
fn patch_ifma_to_vxorps(code: &mut [u8], case: MemoryCase) {
...
#[cfg(target_arch = "x86_64")]
#[test]
fn patched_native_boundary_executes_all_success_and_fault_frontiers_with_exact_state() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping patched AVX-IFMA helper boundary: host lacks AVX");
        return;
    }
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in DIFFERENTIAL_LEVELS {
            ...
            patch_ifma_to_vxorps(&mut code, case);
            let exec =
...
#[cfg(target_arch = "x86_64")]
const MEMORY_CHILD_ENV: &str = "RAX_VEX_IFMA52_MEMORY_CHILD";

#[cfg(target_arch = "x86_64")]
#[test]
fn native_memory_sources_match_intel_equations_on_an_avx_ifma_host() {
    ...
    if !std::is_x86_feature_detected!("avx") || !x86_host_has_avx_ifma() {
        eprintln!("skipping native AVX-IFMA memory differential: host feature unavailable");
        return;
    }

    let test_name = "smir::lower::runtime::jit_gate_tests::vex_ifma52_replay::\
                     memory_source::native_memory_sources_match_intel_equations_on_an_avx_ifma_host";
    let output = std::process::Command::new(std::env::current_exe().unwrap())
        ...
        .output()
        .expect("spawn isolated AVX-IFMA memory differential");
    assert!(
        output.status.success(),
        "isolated AVX-IFMA memory differential failed: {}\nstdout: {}\nstderr: {}",
        output.status,
