//! Exact helper-backed EVEX VSHUFPS/VSHUFPD memory coverage.
...
use crate::smir::ir::{
    SmirBlock, SmirFunction, Terminator, X86EvexFpShuffleMemoryReplay, X86InstructionBytes,
};
use crate::smir::lift::x86_64::X86_64Lifter;
use crate::smir::lift::{ControlFlow, LiftContext, SmirLifter};
use crate::smir::lower::SmirLowerer;
use crate::smir::lower::runtime::{
    X86JitEvexFpShuffleMemorySequence, is_native_clobber_safe_excluding,
    is_x86_aarch64_native_clobber_safe_excluding, uses_x86_native_vectors_excluding,
    x86_jit_evex_fp_shuffle_memory_sequence, x86_native_replay_feature_requirements,
...
impl ShuffleKind {
    const ALL: [Self; 2] = [
        Self::new("VSHUFPS", VecElementType::F32),
        Self::new("VSHUFPD", VecElementType::F64),
    ];

    const fn new(name: &'static str, elem: VecElementType) -> Self {
        Self { name, elem }
    }
...
fn memory_encoding(case: FpShuffleMemoryCase, sib: bool) -> Vec<u8> {
...
fn register_encoding(case: FpShuffleMemoryCase, scratch: u8) -> Vec<u8> {
    assert!(scratch < 16);
    let p0 = 0x41
        | if case.destination & 8 == 0 { 0x80 } else { 0 }
        | if case.destination & 16 == 0 { 0x10 } else { 0 }
...
fn stack_encoding(case: FpShuffleMemoryCase) -> Vec<u8> {
...
fn lift_bytes(bytes: &[u8]) -> SmirFunction {
    ...
    function.x86_instruction_bytes.insert(
        (BlockId(0), PC),
        X86InstructionBytes::new(bytes).expect("EVEX floating shuffle provenance"),
    );
    function
}

fn lift_case(case: FpShuffleMemoryCase) -> SmirFunction {
...
fn sequence(function: &SmirFunction, allow_mem: bool) -> Option<X86JitEvexFpShuffleMemorySequence> {
    let (definitions, uses) = virtual_counts(function);
    let index = usize::from(matches!(
        function.blocks[0].ops.first().map(|op| &op.kind),
        Some(OpKind::X86RequireApx)
    ));
    x86_jit_evex_fp_shuffle_memory_sequence(
        &function.blocks[0],
        index,
        allow_mem,
...
fn lower(function: &SmirFunction, case: FpShuffleMemoryCase) -> (Vec<u8>, usize) {
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("{case:?}: EVEX floating shuffle lowering: {error:?}"));
    assert!(result.relocations.is_empty(), "{case:?}");
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed EVEX floating shuffle"),
        result.entry_offset,
    )
}

fn case_immediate(kind: ShuffleKind, width: VecWidth, destination: u8, source1: u8) -> u8 {
    destination
        .wrapping_mul(17)
        .wrapping_add(source1.wrapping_mul(29))
        .wrapping_add(width.bytes() as u8)
        .wrapping_add(kind.pp().wrapping_mul(0x5B))
}

fn all_cases() -> Vec<FpShuffleMemoryCase> {
            ...
            for (destination, source1) in [(0, 0), (9, 10), (17, 17)] {
                for control in MaskControl::ALL {
                    for tuple in TupleKind::ALL {
                        cases.push(FpShuffleMemoryCase {
                            kind,
                            width,
                            destination,
                            source1,
...
#[test]
fn all_108_shuffle_memory_cells_optimize_admit_and_lower_exactly() {
    let cases = all_cases();
...
#[test]
fn type_e4nf_shuffle_graphs_always_preserve_one_exact_tuple_access() {
...
#[test]
fn shuffle_sequence_fails_closed_for_provenance_selector_mask_and_ssa_mutations() {
    ...
    // Bit zero controls a VSHUFPD lane at every admitted vector width. High
    // bits outside a narrower vector's lane count are architecturally ignored
    // and may legitimately map to the same canonical graph.
    *bytes.last_mut().unwrap() ^= 0x01;
    ...
    let load_index = function.blocks[0]
        ...
        .unwrap();
    let broadcast_indices = function.blocks[0]
        .ops
        .iter()
        .enumerate()
        .filter(|(_, op)| matches!(op.kind, OpKind::VBroadcast { .. }))
        .nth(1)
        .map(|(index, _)| index)
        .unwrap();
    ...
    let selector = wrong_selector.blocks[0]
        .ops
        .iter_mut()
        .skip(broadcast_indices + 1)
        .find(|op| matches!(op.kind, OpKind::Mov { src: SrcOperand::Imm(value), .. } if value != 0))
        .unwrap();
    if let OpKind::Mov {
    ...
    let mut escaped_indices = function.clone();
    let indices = match escaped_indices.blocks[0].ops[broadcast_indices].kind {
    ...
    mutations.push(("escaped selector vector", escaped_indices));
...
#[test]
fn shuffle_full_tuple_and_unmasked_commit_fail_closed_under_mutation() {
    let case = FpShuffleMemoryCase {
    ...
    broadcast_provenance
        .x86_instruction_bytes
        .insert((BlockId(0), PC), X86InstructionBytes::new(&bytes).unwrap());
    mutations.push(("broadcast provenance over full tuple", broadcast_provenance));
...
#[test]
fn shuffle_segment_addr32_rip_disp8_and_apx_addresses_admit_and_lower() {
    let x86 = |register| VReg::Arch(ArchReg::X86(register));
    let full = FpShuffleMemoryCase {
        kind: ShuffleKind::ALL[0],
        width: VecWidth::V512,
        destination: 17,
        source1: 18,
    ...
    let broadcast = FpShuffleMemoryCase {
        kind: ShuffleKind::ALL[1],
        tuple: TupleKind::Broadcast,
        immediate: 0xA5,
        ..full
    ...
    rip[5] = (rip[5] & 0x38) | 5;
    rip.insert(rip.len() - 1, 0x20);
    rip.insert(rip.len() - 1, 0);
    rip.insert(rip.len() - 1, 0);
    rip.insert(rip.len() - 1, 0);
...
#[test]
fn shuffle_rejects_the_avx_only_state_bridge() {
    for tuple in TupleKind::ALL {
        let case = FpShuffleMemoryCase {
            kind: ShuffleKind::ALL[usize::from(tuple.is_broadcast())],
            width: VecWidth::V512,
            destination: 17,
            source1: 18,
            control: MaskControl::Zero,
        ...
        let error = lowerer
            .lower_function(&function)
            .expect_err("AVX-only state bridge must reject EVEX floating shuffles");
        assert!(
