//! Native replay coverage for register-only EVEX floating-point comparisons.
...
#[cfg(target_arch = "x86_64")]
fn initial_state(kind: CompareKind, mxcsr: u32) -> CompareState {
    let masks = [
...
#[cfg(target_arch = "x86_64")]
fn interpret(bytes: &[u8], initial: &CompareState) -> CompareState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native(bytes: &[u8], initial: &CompareState) -> CompareState {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;
    ...
    let exec = ExecMem::new(&code).expect("map EVEX floating-point compare replay");
...
#[cfg(target_arch = "x86_64")]
#[test]
fn replay_matches_interpreter_for_all_predicates_formats_sae_masks_extensions_and_mxcsr() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!(
            "skipping native EVEX floating-point compare differential: host lacks AVX-512F/BW"
        );
        return;
    }
    ...
    for kind in CompareKind::ALL {
        for (ll, suppress_exceptions) in CompareKind::CONTROLS {
            ...
            for predicate in 0u8..32 {
                let case = available_controls * 32 + usize::from(predicate);
                let (destination, source1, source2) = operands[case % operands.len()];
                let writemask = writemasks[(case / operands.len()) % writemasks.len()];
                let bytes = encoding(
                    kind,
                    ll,
                    suppress_exceptions,
                ...
                // Native vector execution is admitted only while all SIMD
                // exception masks remain set. Vary pre-existing status, RC,
                // DAZ, and FTZ; comparisons may add invalid/denormal status.
                let prior_status = if case % 3 == 0 { 1 << 5 } else { 0 };
                let rc = ((case as u32) & 3) << 13;
                let denormal_controls = if case & 1 == 0 {
                    0
                } else {
                    (1 << 6) | (1 << 15)
                };
                let initial = initial_state(kind, 0x1F80 | prior_status | rc | denormal_controls);
                let interpreted = interpret(&bytes, &initial);
                let native = execute_native(&bytes, &initial);
                assert_eq!(native, interpreted, "{kind:?} {bytes:02X?} case={case}");
                executed += 1;
            }
        }
    }
    ...
    assert!(
        ...
        "feature-selected floating-point comparison controls"
    );

    // Destination/writemask aliasing must consume the incoming mask before
    // overwriting it; both high vector sources alias as well.
    let alias = encoding(CompareKind::PackedF32, 2, false, 5, 25, 25, 5, 19);
    let alias_initial = initial_state(CompareKind::PackedF32, 0x1F80);
    assert_eq!(
        execute_native(&alias, &alias_initial),
        interpret(&alias, &alias_initial),
        "{alias:02X?}"
    );
    executed += 1;

    // Force scalar QNaN/SNaN inputs through active non-SAE, active SAE, and
    // masked-off non-SAE cases. This isolates invalid/denormal status updates,
    // SAE suppression, and inactive-lane exception suppression for each
    // element format instead of relying on the packed-lane pattern sweep.
    let mut scalar_exception_cases = 0usize;
    for kind in [
        CompareKind::ScalarF16,
        CompareKind::ScalarF32,
        CompareKind::ScalarF64,
    ] {
        if kind.fields().4 && !has_fp16 {
            continue;
        }
    ...
    assert_eq!(
        executed,
        available_controls * 32 + 1 + scalar_exception_cases
    );
}
