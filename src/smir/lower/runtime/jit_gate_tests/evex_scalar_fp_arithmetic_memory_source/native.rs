...
fn native_cases() -> Vec<ScalarFpMemoryCase> {
...
#[test]
fn native_scalar_fp_memory_matches_interpretation_faults_and_mask_suppression() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!(
            "skipping native scalar EVEX floating-point arithmetic differential: \
        ...
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            if case.control != MaskControl::None {
                let mut suppressed_context = ScalarMemoryContext {
                    value: source ^ 0xFFFF_FFFF_FFFF_FFFF,
                    ok: 0,
                    ..ScalarMemoryContext::default()
                };
    ...
    assert!(successes >= 7 * 2 * 3 * 2);
    assert!(suppressions >= 7 * 2 * 2 * 2);
}
