//! Native replay coverage for register-only EVEX opmask-to-vector conversions.
...
fn mask_to_vector_function(bytes: &[u8]) -> crate::smir::ir::SmirFunction {
...
#[test]
fn mask_to_vector_replay_admits_and_emits_192_register_encodings() {
    ...
    assert_eq!(
        mask_to_vector_encoding((0x38, true, 2), 25, 7, false, false),
        [0x62, 0x62, 0xFE, 0x48, 0x38, 0xCF]
    );
    ...
    for shape in shapes {
        for (destination, source) in [(1, 0), (9, 2), (17, 5), (25, 7)] {
            for ignored_x in [false, true] {
                for ignored_b in [false, true] {
                    let bytes =
                        mask_to_vector_encoding(shape, destination, source, ignored_x, ignored_b);
                    let (needs_vl, needs_dq) = crate::smir::ir::X86InstructionBytes::new(&bytes)
                        .unwrap()
                        .evex_register_mask_to_vector_requirements()
                        .unwrap_or_else(|| panic!("{bytes:02X?}"));
                    let mut function = mask_to_vector_function(&bytes);
                    if admitted == 0 {
        ...
        // mask expansion sequence into raw native replay.
        let register = mask_to_vector_encoding(shape, 1, 0, false, false);
        let mut memory = register;
        memory[5] &= 0x3F;
        let mut memory_metadata = mask_to_vector_function(&register);
...
#[cfg(target_arch = "x86_64")]
fn interpret_mask_to_vector(bytes: &[u8], initial: &MaskToVectorState) -> MaskToVectorState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native_mask_to_vector(bytes: &[u8], initial: &MaskToVectorState) -> MaskToVectorState {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;

    let function = mask_to_vector_function(bytes);
    let mut lowerer = X86_64Lowerer::new();
    let lowered = lowerer
        .lower_function(&function)
        .unwrap_or_else(|error| panic!("{bytes:02X?}: {error:?}"));
    ...
    let exec = ExecMem::new(&code).expect("map EVEX mask-to-vector replay");
...
#[cfg(target_arch = "x86_64")]
fn assert_native_mask_to_vector_matches_interpreter(bytes: &[u8], initial: &MaskToVectorState) {
    let interpreted = interpret_mask_to_vector(bytes, initial);
    let native = execute_native_mask_to_vector(bytes, initial);
    assert_eq!(native, interpreted, "{bytes:02X?}");
}

#[cfg(target_arch = "x86_64")]
#[test]
fn mask_to_vector_replay_matches_interpreter_for_all_shapes_extensions_and_aliases() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native mask-to-vector differential: host lacks AVX-512F/BW");
        return;
    }
    ...
    for (shape_index, shape) in mask_to_vector_shapes().into_iter().enumerate() {
        if (shape.2 != 2 && !has_vl) || (shape.0 == 0x38 && !has_dq) {
            continue;
        }
        for (destination_index, destination) in destinations.into_iter().enumerate() {
            for (ignored_index, (ignored_x, ignored_b)) in ignored_fields.into_iter().enumerate() {
                ...
                let bytes =
                    mask_to_vector_encoding(shape, destination, source, ignored_x, ignored_b);
                assert_native_mask_to_vector_matches_interpreter(&bytes, &initial);
                executed += 1;
            }
        }
    }
    ...
    assert_eq!(
        executed,
        shapes_per_opcode * if has_dq { 2 } else { 1 } * 16
    );
}
