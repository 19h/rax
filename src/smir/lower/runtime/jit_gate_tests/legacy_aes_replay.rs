//! Native replay coverage for register-only legacy AES-NI instructions.

use super::*;
use crate::smir::ir::types::{FunctionId, SourceArch, X86AesOp};
...
fn function(bytes: &[u8], level: OptLevel, halt: bool) -> SmirFunction {
    ...
    function.x86_instruction_bytes.insert(
        (BlockId(0), PC),
        X86InstructionBytes::new(bytes).expect("legacy AES provenance"),
    );
...
#[test]
fn feature_requirements_select_aes_avx_and_the_ymm16_bridge_only() {
    let bytes = canonical_encoding(0xDC, false, 9, 11, 0, 0x0A);
    ...
    expected.needs_aes = true;
    assert_eq!(requirements, expected);

    #[cfg(target_arch = "x86_64")]
    assert_eq!(
        requirements.x86_host_supported(),
        std::is_x86_feature_detected!("avx") && std::is_x86_feature_detected!("aes")
    );
...
#[test]
fn all_13_056_o0_o2_rex_register_graphs_lower_to_the_exact_source_instruction() {
    ...
    for rex in [None].into_iter().chain((0x40..=0x4F).map(Some)) {
        for (_, opcode, keygen) in OPERATIONS {
            for modrm in 0xC0..=0xFF {
                let bytes = encoding(opcode, keygen, rex, modrm, modrm ^ rex.unwrap_or(0));
...
fn native_cases() -> Vec<NativeCase> {
    const OPERANDS: [(u8, u8); 6] = [(1, 3), (9, 11), (1, 1), (15, 0), (0, 15), (8, 8)];
    let mut cases = Vec::new();
    for (operation_index, (_, opcode, keygen)) in OPERATIONS.into_iter().enumerate() {
...
#[cfg(target_arch = "x86_64")]
fn initial_state(case: NativeCase, ordinal: usize) -> AesState {
...
#[cfg(target_arch = "x86_64")]
fn interpret(case: NativeCase, initial: &AesState, level: OptLevel) -> AesState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native(case: NativeCase, initial: &AesState, level: OptLevel) -> AesState {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;
    ...
    let exec = ExecMem::new(&code).expect("map legacy AES replay");
...
#[cfg(target_arch = "x86_64")]
const CHILD_RANGE_ENV: &str = "RAX_LEGACY_AES_CHILD_RANGE";
...
fn run_child_range(test_name: &str, range: std::ops::Range<usize>) -> std::process::Output {
    std::process::Command::new(std::env::current_exe().expect("current unit-test executable"))
        ...
        .output()
        .expect("run isolated native legacy AES differential")
}
...
fn run_isolated_native_differential(test_name: &str) {
    ...
    panic!(
        "isolated native legacy AES failure at case {start}/{}: {case:?} {:02X?}; \
...
fn native_replay_matches_o0_o2_interpretation_for_all_operations_aliases_and_full_state() {
    if !std::is_x86_feature_detected!("avx") || !std::is_x86_feature_detected!("aes") {
        eprintln!("skipping native legacy AES differential: host lacks AVX or AES");
        return;
    }
    run_isolated_native_differential(
        "smir::lower::runtime::jit_gate_tests::legacy_aes_replay::\
