//! Exact helper-backed EVEX opmask-selector blend memory coverage.
...
use crate::smir::ir::{
    SmirBlock, SmirFunction, Terminator, X86EvexMaskBlendMemoryReplay, X86InstructionBytes,
};
use crate::smir::lift::x86_64::X86_64Lifter;
use crate::smir::lift::{ControlFlow, LiftContext, SmirLifter};
use crate::smir::lower::SmirLowerer;
use crate::smir::lower::runtime::{
    X86JitEvexMaskBlendMemorySequence, is_native_clobber_safe_excluding,
    is_x86_aarch64_native_clobber_safe_excluding, uses_x86_native_vectors_excluding,
    x86_jit_evex_mask_blend_memory_sequence, x86_native_replay_feature_requirements,
...
impl BlendKind {
    ...
    const fn opcode(self) -> u8 {
        match self {
            Self::PackedDword | Self::PackedQword => 0x64,
            Self::PackedSingle | Self::PackedDouble => 0x65,
            Self::PackedByte | Self::PackedWord => 0x66,
        }
    }

    const fn elem(self) -> VecElementType {
        match self {
            Self::PackedByte => VecElementType::I8,
            Self::PackedWord => VecElementType::I16,
            Self::PackedDword | Self::PackedSingle => VecElementType::I32,
            Self::PackedQword | Self::PackedDouble => VecElementType::I64,
        }
    }
    ...
    const fn w(self) -> bool {
        matches!(
            self,
            Self::PackedQword | Self::PackedDouble | Self::PackedWord
        )
    }

    const fn allows_broadcast(self) -> bool {
        !matches!(self, Self::PackedByte | Self::PackedWord)
    }
}
...
impl MaskBlendMemoryCase {
    ...
    fn expected_replay(self) -> Vec<u8> {
        if self.broadcast() || self.selector() != 0 {
            stack_encoding(self)
        } else {
...
fn vector(index: u8, width: VecWidth) -> VReg {
    VReg::Arch(ArchReg::X86(match width {
        ...
        _ => unreachable!("EVEX mask-blend vector width"),
    }))
}

fn memory_encoding(case: MaskBlendMemoryCase, sib: bool) -> Vec<u8> {
    assert!(case.destination < 32 && case.source1 < 32);
    assert!(case.selector() < 8 && (!case.zeroing() || case.selector() != 0));
    assert!(!case.broadcast() || case.kind.allows_broadcast());
    let p0 = 0x02
        | 0x60
        | if case.destination & 8 == 0 { 0x80 } else { 0 }
        | if case.destination & 16 == 0 { 0x10 } else { 0 };
    let p1 = if case.kind.w() { 0x80 } else { 0 } | (((!case.source1) & 0x0F) << 3) | 0x05;
...
fn stack_encoding(case: MaskBlendMemoryCase) -> Vec<u8> {
    let p0 = 0x62
        | if case.destination & 8 == 0 { 0x80 } else { 0 }
        | if case.destination & 16 == 0 { 0x10 } else { 0 };
    let p1 = if case.kind.w() { 0x80 } else { 0 } | (((!case.source1) & 0x0F) << 3) | 0x05;
...
fn register_encoding(case: MaskBlendMemoryCase, scratch: u8) -> Vec<u8> {
    assert!(scratch < 16);
    let p0 = 0x42
        | if case.destination & 8 == 0 { 0x80 } else { 0 }
        | if case.destination & 16 == 0 { 0x10 } else { 0 }
...
fn lift_bytes(bytes: &[u8]) -> SmirFunction {
    ...
    function.x86_instruction_bytes.insert(
        (BlockId(0), PC),
        X86InstructionBytes::new(bytes).expect("EVEX mask-blend provenance"),
    );
    function
}

fn lift_case(case: MaskBlendMemoryCase) -> SmirFunction {
...
fn lower(function: &SmirFunction, case: MaskBlendMemoryCase) -> (Vec<u8>, usize) {
    ...
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed EVEX mask blend"),
        result.entry_offset,
    )
}

fn all_cases() -> Vec<MaskBlendMemoryCase> {
            ...
            for (destination, source1) in [(0, 0), (9, 10), (17, 18)] {
                for form in [SourceForm::Vector, SourceForm::Broadcast] {
                    ...
                    for control in MaskControl::ALL {
                        cases.push(MaskBlendMemoryCase {
                            kind,
                            width,
                            destination,
                            source1,
...
#[test]
fn mask_blend_rewrites_match_six_independent_llvm_23_anchors() {
    let anchors: &[(&[u8], &[u8])] = &[
        (
            &[0x62, 0xF2, 0x6D, 0x08, 0x65, 0x08],
            &[0x62, 0xF2, 0x6D, 0x08, 0x65, 0xC8],
        ),
        (
            &[0x62, 0x72, 0xAD, 0xAB, 0x65, 0x08],
            &[0x62, 0x72, 0xAD, 0xAB, 0x65, 0x0C, 0x24],
        ),
        (
            &[0x62, 0xE2, 0x6D, 0x41, 0x64, 0x08],
            &[0x62, 0xE2, 0x6D, 0x41, 0x64, 0x0C, 0x24],
        ),
        (
            &[0x62, 0x62, 0xAD, 0xD2, 0x64, 0x0C, 0x24],
            &[0x62, 0x62, 0xAD, 0xD2, 0x64, 0x0C, 0x24],
        ),
        (
            &[0x62, 0xE2, 0x6D, 0x43, 0x66, 0x08],
            &[0x62, 0xE2, 0x6D, 0x43, 0x66, 0x0C, 0x24],
        ),
        (
            &[0x62, 0x72, 0xAD, 0xAC, 0x66, 0x0C, 0x24],
            &[0x62, 0x72, 0xAD, 0xAC, 0x66, 0x0C, 0x24],
        ),
    ];
    for (memory, llvm) in anchors {
        ...
        let replay = match encoding.replay {
            ...
            | X86EvexMaskBlendMemoryReplay::MaskedVector { stack_instruction } => stack_instruction,
        };
...
#[test]
fn mask_blend_classifier_exhausts_1_843_200_operand_control_and_apx_cells() {
                    ...
                    for form in [SourceForm::Vector, SourceForm::Broadcast] {
                        ...
                        for selector in 0..8u8 {
                            for zeroing in [false, true] {
                                if zeroing && selector == 0 {
                                    continue;
                                }
                                let case = MaskBlendMemoryCase {
                                    kind,
                                    width,
                                    destination,
                                    source1,
                                    form,
                                    control: if selector == 0 {
                                        MaskControl::None
                                    } else if zeroing {
                                        MaskControl::Zero
                                    } else {
                                ...
                                canonical[3] =
                                    (canonical[3] & !0x87) | selector | (u8::from(zeroing) << 7);
                                    ...
                                    for index_high in [false, true] {
                                        ...
                                        match encoding.replay {
                                            X86EvexMaskBlendMemoryReplay::Vector {
                                                ...
                                                assert_eq!(selector, 0, "{bytes:02X?}");
                                            ...
                                            X86EvexMaskBlendMemoryReplay::MaskedVector {
                                                ...
                                                assert_ne!(selector, 0, "{bytes:02X?}");
                                                assert_eq!(form, SourceForm::Vector);
                                            }
                                        }
                                        accepted += 1;
...
#[test]
fn mask_blend_classifier_rejects_reserved_non_owned_and_trailing_shapes() {
    let case = MaskBlendMemoryCase {
    ...
    let mut byte_broadcast = MaskBlendMemoryCase {
...
#[test]
fn all_270_mask_blend_memory_cells_optimize_admit_and_lower_exactly() {
    let cases = all_cases();
    ...
    for case in cases {
        for level in LEVELS {
            ...
            assert_eq!(
                exact.encoding.selector,
                (case.selector() != 0).then_some(case.selector())
            );
...
#[test]
fn mask_blend_sequence_fails_closed_for_provenance_and_graph_mutations() {
    for case in [
        MaskBlendMemoryCase {
            kind: BlendKind::PackedByte,
            width: VecWidth::V512,
        ...
        bytes[3] = (bytes[3] & !7) | if case.selector() == 1 { 2 } else { 1 };
        wrong_provenance
            .x86_instruction_bytes
            .insert((BlockId(0), PC), X86InstructionBytes::new(&bytes).unwrap());
        assert_rejected("wrong selector provenance", &wrong_provenance);
...
#[test]
fn mask_blend_segment_addr32_rip_and_apx_addresses_admit_and_lower() {
    let x86 = |register| VReg::Arch(ArchReg::X86(register));
    let vector_case = MaskBlendMemoryCase {
        kind: BlendKind::PackedSingle,
        width: VecWidth::V128,
        destination: 1,
        source1: 2,
    ...
    let broadcast_case = MaskBlendMemoryCase {
        form: SourceForm::Broadcast,
        control: MaskControl::Merge,
        ..vector_case
    };
...
#[test]
fn mask_blend_rejects_the_avx_only_state_bridge() {
    let case = MaskBlendMemoryCase {
        kind: BlendKind::PackedByte,
        width: VecWidth::V512,
        destination: 17,
    ...
    let error = lowerer
        .lower_function(&function)
        .expect_err("AVX-only state bridge must reject EVEX mask blends");
    assert!(
