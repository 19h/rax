...
fn evex_broadcast_xor_sequence_fails_closed_for_provenance_and_graph_mutations() {
    ...
    mutations.push(("predicate negation", predicate_negation));

    let mut predicate_or = base.clone();
    if let OpKind::Or { src2, .. } = &mut predicate_or.blocks[0].ops[3].kind {
        *src2 = crate::smir::ir::types::SrcOperand::Imm(0);
    }
    mutations.push(("predicate combination", predicate_or));

    let mut predicate_shift = base.clone();
    if let OpKind::Shr { amount, .. } = &mut predicate_shift.blocks[0].ops[4].kind {
        *amount = crate::smir::ir::types::SrcOperand::Imm(62);
    }
    mutations.push(("predicate normalization", predicate_shift));
    ...
    mutations.push(("xor hint", xor_hint));
