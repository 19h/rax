//! Exact helper-backed packed EVEX FP16 arithmetic memory-source coverage.
...
use crate::smir::ir::types::{
    ...
    SourceArch, VReg, VecElementType, VecWidth, VirtualId, X86Reg,
};
use crate::smir::ir::{
    SmirBlock, SmirFunction, Terminator, X86EvexPackedFp16ArithmeticMemoryReplay,
    X86InstructionBytes,
};
...
use crate::smir::lower::runtime::{
    GuestRegs, X86JitEvexPackedFp16ArithmeticMemorySequence, is_native_clobber_safe_excluding,
    is_x86_aarch64_native_clobber_safe_excluding, uses_x86_native_vectors_excluding,
    x86_jit_evex_packed_fp16_arithmetic_memory_sequence, x86_native_replay_feature_requirements,
...
fn vector(index: u8, width: VecWidth) -> VReg {
    VReg::Arch(ArchReg::X86(match width {
        ...
        _ => unreachable!("packed EVEX FP16 arithmetic width"),
    }))
}
...
fn lift_case(case: Fp16MemoryCase) -> SmirFunction {
    ...
    function.x86_instruction_bytes.insert(
        (BlockId(0), PC),
        X86InstructionBytes::new(&bytes).expect("packed FP16 memory provenance"),
    );
    function
}
...
fn sequence(
    ...
    x86_jit_evex_packed_fp16_arithmetic_memory_sequence(
        &function.blocks[0],
        0,
        allow_mem,
...
fn lower(function: &SmirFunction, case: Fp16MemoryCase) -> (Vec<u8>, usize) {
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("{case:?}: packed FP16 memory lowering: {error:?}"));
    assert!(result.relocations.is_empty(), "{case:?}");
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed packed FP16 arithmetic"),
        result.entry_offset,
    )
}

fn all_cases() -> Vec<Fp16MemoryCase> {
...
#[test]
fn packed_fp16_memory_classifier_exhaustively_rewrites_2_211_840_control_and_apx_address_cells() {
                                    ...
                                    for index_high in [false, true] {
                                        ...
                                        match encoding.replay {
                                            ...
                                            X86EvexPackedFp16ArithmeticMemoryReplay::Vector {
                                                ...
                                                assert_eq!(
                                                    register_instruction
                                                        .evex_register_packed_fp16_arithmetic_needs_vl(),
                                                    Some(ll != 2),
                                                    "{bytes:02X?}"
                                                );
                                            }
    ...
    assert_eq!(accepted, 6 * 3 * 32 * 32 * 2 * 15 * 2 * 2);

    for opcode in 0..=u8::MAX {
        let bytes = memory_encoding(0x58, 0, 1, 0, 0, false, false, 3);
...
#[test]
fn packed_fp16_encodings_match_six_independent_llvm_23_anchors() {
    for (actual, llvm) in [
        (
            register_encoding(0x58, 0, 1, 2, 0, 0, false).to_vec(),
            vec![0x62, 0xF5, 0x74, 0x08, 0x58, 0xC2],
        ),
        (
            register_encoding(0x59, 16, 17, 0, 1, 0, false).to_vec(),
            vec![0x62, 0xE5, 0x74, 0x20, 0x59, 0xC0],
        ),
        (
            register_encoding(0x5C, 31, 30, 0, 2, 0, false).to_vec(),
            vec![0x62, 0x65, 0x0C, 0x40, 0x5C, 0xF8],
        ),
        (
            stack_encoding(0x5E, 0, 1, 0, 3, true, true).to_vec(),
            vec![0x62, 0xF5, 0x74, 0x9B, 0x5E, 0x04, 0x24],
        ),
        (
            stack_encoding(0x5D, 16, 17, 1, 7, false, false).to_vec(),
            vec![0x62, 0xE5, 0x74, 0x27, 0x5D, 0x04, 0x24],
        ),
        (
            stack_encoding(0x5F, 31, 30, 2, 5, true, false).to_vec(),
            vec![0x62, 0x65, 0x0C, 0xC5, 0x5F, 0x3C, 0x24],
        ),
    ] {
...
#[test]
fn packed_fp16_memory_classifier_rejects_reserved_non_owned_and_trailing_shapes() {
    let valid = memory_encoding(0x58, 0, 1, 0, 1, false, false, 3).to_vec();
    ...
    let X86EvexPackedFp16ArithmeticMemoryReplay::MaskedVector { stack_instruction } =
        encoding.replay
    else {
    ...
    assert_eq!(
        stack_instruction.as_slice(),
        stack_encoding(0x58, 0, 1, 0, 1, false, false)
    );
}

#[test]
fn packed_fp16_apx_r16_r17_sib_address_lifts_admits_and_lowers_exactly() {
    // VADDPH xmm16{k1},xmm17,[r16+r17*2+16]. The disp8 value 1 is
    // compressed by the 16-byte full-vector tuple.
    let bytes = [0x62, 0xED, 0x70, 0x01, 0x58, 0x44, 0x48, 0x01];
    let mut lifter = X86_64Lifter::strict();
    let mut context = LiftContext::new(SourceArch::X86_64);
    let result = lifter
        .lift_insn(PC, &bytes, &mut context)
        .expect("APX-extended packed FP16 memory source");
    ...
    for level in LEVELS {
        ...
        let expected = [0x62, 0xE5, 0x74, 0x01, 0x58, 0x04, 0x24];
        assert_eq!(stack_instruction.as_slice(), expected, "{level:?}");

        let case = Fp16MemoryCase {
            opcode: 0x58,
            width: VecWidth::V128,
            source1: 17,
...
#[test]
fn all_324_packed_fp16_memory_cells_optimize_admit_and_lower_exactly() {
    let cases = all_cases();
...
#[test]
fn masked_vector_lowering_stages_disjoint_16_bit_lanes_and_rejects_avx_only_bridge() {
    let case = Fp16MemoryCase {
        opcode: 0x5E,
        width: VecWidth::V512,
        source1: 17,
    ...
    assert!(
        code.windows(5)
            .any(|window| window == [0x66, 0x8B, 0x44, 0x24, 0x48]),
        "16-bit scalar helper return must be read from the disjoint staging slot"
    );
    assert!(
        code.windows(5)
            .any(|window| window == [0x66, 0x89, 0x44, 0x24, 0x46]),
        "lane 31 must end at payload bytes 62..63"
    );
...
#[test]
fn packed_fp16_memory_sequence_fails_closed_for_provenance_and_graph_mutations() {
    let cases = [
        Fp16MemoryCase {
            opcode: 0x58,
            width: VecWidth::V128,
    ...
    for case in cases {
        ...
        let arithmetic = wrong_round.blocks[0]
            .ops
            .iter_mut()
            .find(|op| matches!(op.kind, OpKind::VFP16Arith { .. }))
            .unwrap();
        let OpKind::VFP16Arith { round, .. } = &mut arithmetic.kind else {
            unreachable!()
        };
        *round = FpRoundMode::RoundNearest;
        ...
        let arithmetic = wrong_source.blocks[0]
            ...
            .find(|op| matches!(op.kind, OpKind::VFP16Arith { .. }))
            .unwrap();
        let OpKind::VFP16Arith { src1, .. } = &mut arithmetic.kind else {
            unreachable!()
        };
...
fn initial_registers(case: Fp16MemoryCase, ordinal: usize) -> GuestRegs {
    ...
    if case.destination() == case.source1 {
        registers.zmm[0] = std::array::from_fn(|word| {
...
#[test]
fn packed_fp16_memory_o0_o1_o2_interpretation_is_exactly_equivalent() {
    let cases = all_cases();
...
extern "C" fn lane_load_helper(
    ...
    assert!(offset + 2 <= context.value.len());
    LoadResult {
        value: u16::from_le_bytes(context.value[offset..offset + 2].try_into().unwrap()) as u64,
        ok: 1,
    }
}
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_packed_fp16_memory_matches_interpretation_faults_and_mask_suppression() {
    ...
    {
        eprintln!(
            "skipping native packed FP16 arithmetic memory differential: \
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            let active_mask = if case.mask() == 0 {
                (1u64 << case.width.lanes(VecElementType::F16)) - 1
            } else {
            ...
            let expected_addresses: Vec<u64> = if case.broadcast() {
                ...
                (0..case.width.lanes(VecElementType::F16))
                    .filter(|lane| active_mask & (1 << lane) != 0)
                    .map(|lane| 0x2000 + u64::from(lane) * 2)
                    .collect()
            };
            assert_eq!(
            ...
            let fail_address = if case.broadcast() { 0x2000 } else { 0x2004 };
            let mut context = LaneMemoryContext {
                base: 0x2000,
                value: bytes,
