//! Exact helper-backed AVX-VNNI-INT8/INT16 memory-source coverage.
...
use crate::smir::lower::runtime::{
    ...
    x86_jit_vex_binary_memory_sequence, x86_jit_vex_integer_dot_ext_memory_sequence,
...
impl MemoryCase {
    fn pp_opcode(self) -> (u8, u8) {
        self.kind.pp_opcode(self.saturate)
    }
...
fn vector(index: u8, width: VecWidth) -> VReg {
    x86(match width {
        ...
        _ => unreachable!("VEX extended dot products are 128 or 256 bits"),
    })
}
...
fn assert_exact_chain(ops: &[SmirOp], case: MemoryCase) {
    let [load, consumer] = ops else {
        panic!("expected VLoad + VDotProductExt for {case:?}, got {ops:?}")
    };
    ...
    let expected_elem = if case.kind.int16() {
        VecElementType::I16
    } else {
        VecElementType::I8
    };
    ...
    let OpKind::VDotProductExt {
        ...
        panic!("{case:?}: expected VDotProductExt, got {consumer:?}")
    };
    ...
    assert_eq!(*src_elem, expected_elem, "{case:?}");
    assert_eq!(*acc_elem, VecElementType::I32, "{case:?}");
    assert_eq!(*width, case.width, "{case:?}");
    assert_eq!(*actual_src1_signed, src1_signed, "{case:?}");
    assert_eq!(*actual_src2_signed, src2_signed, "{case:?}");
...
fn sequence(
    ...
    allow_mem: bool,
) -> Option<crate::smir::lower::runtime::X86JitVexIntegerDotExtMemorySequence> {
    let (definitions, uses) = virtual_counts(function);
    x86_jit_vex_integer_dot_ext_memory_sequence(
        &function.blocks[0],
        0,
        allow_mem,
...
fn lower(function: &SmirFunction, case: MemoryCase) -> (Vec<u8>, usize) {
    ...
    assert_eq!(
        requirements.needs_avx_vnni_int8,
        !case.kind.int16(),
        "{case:?}"
    );
    assert_eq!(
        requirements.needs_avx_vnni_int16,
        case.kind.int16(),
        "{case:?}"
    );
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("helper-backed dot-product lowering failed: {error:?}"));
    assert!(result.relocations.is_empty());
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed VEX extended dot product"),
        result.entry_offset,
    )
}
...
fn every_kind_width_operand_alias_and_optimizer_shape_is_lifted_admitted_and_lowered() {
    ...
    for case in cases {
        for level in LEVELS {
            ...
            let (pp, opcode) = case.pp_opcode();
            let expected_prefix = match pp {
                0 => X86SsePrefix::None,
            ...
            assert_eq!(binary.prefix, expected_prefix, "{level:?} {case:?}");
            assert_eq!(binary.opcode, opcode, "{level:?} {case:?}");
            assert!(!binary.w, "{level:?} {case:?}");
            assert!(!binary.needs_avx2, "{level:?} {case:?}");
            assert!(!binary.needs_fma, "{level:?} {case:?}");
            assert_eq!(actual.int16, case.kind.int16(), "{level:?} {case:?}");
            assert_eq!(
    ...
    assert_eq!(lowered, 6 * 2 * 2 * 6 * LEVELS.len());
}

#[test]
fn all_3_072_decoder_census_cells_are_admitted_and_lowered_after_o2() {
                ...
                for source1 in 0u8..16 {
                    ...
                    for destination in 0u8..8 {
                        ...
                        let actual = sequence(&function, true).unwrap_or_else(|| {
                            panic!("{bytes:02X?}: defined extended dot product was rejected")
                        });
                        ...
                        assert_eq!(actual.binary.memory_size, width.bytes(), "{bytes:02X?}");
                        assert_eq!(actual.int16, kind.int16(), "{bytes:02X?}");
                        admitted += 1;
    ...
    assert_eq!(admitted, 3_072);
    assert_eq!(lowered, 3_072);
}

#[test]
fn sib_rip_relative_segment_absolute_and_addr32_forms_are_admitted_and_lowered() {
    // Independently assembled by LLVM 23 with +avxvnniint8.
    let encodings = [
        (&[0xC4, 0xE2, 0x6B, 0x50, 0x0C, 0x8B][..], 1, 2),
        (
            &[0xC4, 0x02, 0x2B, 0x50, 0x8C, 0xD3, 0x78, 0x56, 0x34, 0x12][..],
            9,
            10,
        ),
        (&[0xC4, 0xE2, 0x6B, 0x50, 0x0D, 0x20, 0, 0, 0][..], 1, 2),
        (&[0x64, 0xC4, 0xE2, 0x6B, 0x50, 0x4B, 0x20][..], 1, 2),
        (&[0x65, 0xC4, 0x82, 0x6B, 0x50, 0x4C, 0xD3, 0x20][..], 1, 2),
        (
            &[0xC4, 0xE2, 0x6B, 0x50, 0x0C, 0x25, 0x78, 0x56, 0x34, 0x12][..],
            1,
            2,
        ),
        (&[0x67, 0xC4, 0xE2, 0x6B, 0x50, 0x4C, 0x8B, 0x20][..], 1, 2),
    ];
    ...
    for (bytes, destination, source1) in encodings {
        for level in DIFFERENTIAL_LEVELS {
            ...
            assert_eq!(admitted.binary.memory_size, VecWidth::V128.bytes());
            assert!(!admitted.int16);
            assert!(is_native_clobber_safe_excluding(
                &function,
                &HashMap::new(),
                true
...
fn register_rewrite_matches_all_independent_llvm_23_encodings() {
    ...
    let cases = [
        (DotKind::ByteSs, false, VecWidth::V128, low),
        (DotKind::ByteSs, true, VecWidth::V256, high),
        (DotKind::ByteSu, false, VecWidth::V128, low),
        (DotKind::ByteSu, true, VecWidth::V256, high),
        (DotKind::ByteUu, false, VecWidth::V128, low),
        (DotKind::ByteUu, true, VecWidth::V256, high),
        (DotKind::WordSu, false, VecWidth::V128, low),
        (DotKind::WordSu, true, VecWidth::V256, high),
        (DotKind::WordUs, false, VecWidth::V128, low),
        (DotKind::WordUs, true, VecWidth::V256, high),
        (DotKind::WordUu, false, VecWidth::V128, low),
        (DotKind::WordUu, true, VecWidth::V256, high),
    ];
    let expected = [
        [0xC4, 0x62, 0x2B, 0x50, 0xC8],
        [0xC4, 0x62, 0x0F, 0x51, 0xF8],
        [0xC4, 0x62, 0x2A, 0x50, 0xC8],
        [0xC4, 0x62, 0x0E, 0x51, 0xF8],
        [0xC4, 0x62, 0x28, 0x50, 0xC8],
        [0xC4, 0x62, 0x0C, 0x51, 0xF8],
        [0xC4, 0x62, 0x2A, 0xD2, 0xC8],
        [0xC4, 0x62, 0x0E, 0xD3, 0xF8],
        [0xC4, 0x62, 0x29, 0xD2, 0xC8],
        [0xC4, 0x62, 0x0D, 0xD3, 0xF8],
        [0xC4, 0x62, 0x28, 0xD2, 0xC8],
        [0xC4, 0x62, 0x0C, 0xD3, 0xF8],
    ];
...
fn classifier_and_gate_fail_closed_for_every_chain_and_provenance_invariant() {
    ...
    assert_mutation_rejected(&base, "accumulator differs from destination", |function| {
        if let OpKind::VDotProductExt { acc, .. } = &mut function.blocks[0].ops[1].kind {
            *acc = vector(4, case.width);
        }
    });
    assert_mutation_rejected(&base, "consumer bypasses loaded source", |function| {
        if let OpKind::VDotProductExt { src2, .. } = &mut function.blocks[0].ops[1].kind {
            *src2 = vector(4, case.width);
        }
    });
    assert_mutation_rejected(&base, "consumer width mismatch", |function| {
        if let OpKind::VDotProductExt { width, .. } = &mut function.blocks[0].ops[1].kind {
            *width = VecWidth::V128;
        }
    });
    assert_mutation_rejected(&base, "wrong source element", |function| {
        if let OpKind::VDotProductExt { src_elem, .. } = &mut function.blocks[0].ops[1].kind {
            *src_elem = VecElementType::I8;
        }
    });
    assert_mutation_rejected(&base, "wrong accumulator element", |function| {
        if let OpKind::VDotProductExt { acc_elem, .. } = &mut function.blocks[0].ops[1].kind {
            *acc_elem = VecElementType::I64;
        }
    });
    assert_mutation_rejected(&base, "wrong first-source signedness", |function| {
        if let OpKind::VDotProductExt { src1_signed, .. } = &mut function.blocks[0].ops[1].kind {
            *src1_signed = !*src1_signed;
        }
    });
    assert_mutation_rejected(&base, "wrong second-source signedness", |function| {
        if let OpKind::VDotProductExt { src2_signed, .. } = &mut function.blocks[0].ops[1].kind {
            *src2_signed = !*src2_signed;
        }
    });
    assert_mutation_rejected(&base, "wrong saturation", |function| {
        if let OpKind::VDotProductExt { saturate, .. } = &mut function.blocks[0].ops[1].kind {
            *saturate = !*saturate;
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only source", |function| {
        if let OpKind::VDotProductExt { src1, .. } = &mut function.blocks[0].ops[1].kind {
            *src1 = vector(16, case.width);
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only destination", |function| {
        if let OpKind::VDotProductExt { dst, acc, .. } = &mut function.blocks[0].ops[1].kind {
            *dst = vector(16, case.width);
            *acc = *dst;
        }
    });
    assert_mutation_rejected(&base, "destination namespace mismatch", |function| {
        if let OpKind::VDotProductExt { dst, acc, .. } = &mut function.blocks[0].ops[1].kind {
...
fn memory_interpretation_matches_sdm_equations_for_every_variant_alias_and_boundary() {
    ...
    assert_eq!(interpreted, 6 * 2 * 2 * 6 * DIFFERENTIAL_LEVELS.len());
}
...
#[cfg(target_arch = "x86_64")]
#[test]
fn patched_native_boundary_executes_288_successes_and_faults_with_exact_state() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping patched AVX-VNNI helper boundary: host lacks AVX");
        return;
    }
    ...
    eprintln!(
        "executed {successes} successful and {faults} faulting patched AVX-VNNI memory cases"
    );
}

#[cfg(target_arch = "x86_64")]
const MEMORY_CHILD_RANGE_ENV: &str = "RAX_VEX_INTEGER_DOT_EXT_MEMORY_CHILD_RANGE";

#[cfg(target_arch = "x86_64")]
fn memory_child_range() -> Option<std::ops::Range<usize>> {
    let value = std::env::var(MEMORY_CHILD_RANGE_ENV).ok()?;
    let (start, end) = value
        .split_once(':')
        .unwrap_or_else(|| panic!("invalid {MEMORY_CHILD_RANGE_ENV}: {value}"));
    Some(
        start
            .parse::<usize>()
            .unwrap_or_else(|_| panic!("invalid {MEMORY_CHILD_RANGE_ENV} start: {value}"))
            ..end
                .parse::<usize>()
                .unwrap_or_else(|_| panic!("invalid {MEMORY_CHILD_RANGE_ENV} end: {value}")),
    )
}

#[cfg(target_arch = "x86_64")]
fn supported_native_memory_cases() -> Vec<(OptLevel, MemoryCase)> {
...
#[cfg(target_arch = "x86_64")]
fn execute_real_native_range(cases: &[(OptLevel, MemoryCase)], range: std::ops::Range<usize>) {
    use crate::smir::lower::runtime::ExecMem;
...
#[cfg(target_arch = "x86_64")]
fn run_memory_child_range(test_name: &str, range: std::ops::Range<usize>) -> std::process::Output {
    std::process::Command::new(std::env::current_exe().expect("current unit-test executable"))
        .arg(test_name)
        .arg("--exact")
        .arg("--nocapture")
        .env(
            MEMORY_CHILD_RANGE_ENV,
            format!("{}:{}", range.start, range.end),
        )
        .output()
        .expect("run isolated native AVX-VNNI memory differential")
}

#[cfg(target_arch = "x86_64")]
#[test]
fn native_memory_sources_match_sdm_model_when_host_supports_feature() {
    let cases = supported_native_memory_cases();
    if cases.is_empty() {
        eprintln!(
            "skipping native AVX-VNNI memory differential: \
             host lacks AVX-VNNI-INT8 and AVX-VNNI-INT16"
        );
        return;
    }
    if let Some(range) = memory_child_range() {
        execute_real_native_range(&cases, range);
        return;
    }

    let test_name = "smir::lower::runtime::jit_gate_tests::vex_integer_dot_ext_replay::\
        memory_source::native_memory_sources_match_sdm_model_when_host_supports_feature";
    let whole = run_memory_child_range(test_name, 0..cases.len());
    ...
    while end - start > 1 {
        ...
        if run_memory_child_range(test_name, start..middle)
            .status
            .success()
        {
            start = middle;
    ...
    let singleton = run_memory_child_range(test_name, start..end);
    let (level, case) = cases[start];
    panic!(
        "isolated native AVX-VNNI memory failure at case {start}/{}: \
