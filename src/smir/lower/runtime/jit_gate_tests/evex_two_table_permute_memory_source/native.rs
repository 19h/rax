...
extern "C" fn scalar_load_helper(
    ...
    LoadResult {
        value: match context.elem_bytes {
            ...
            _ => unreachable!("two-table-permute scalar helper width"),
        },
        ok: 1,
    }
}
...
fn selected_cases() -> [PermuteMemoryCase; 12] {
    [
        PermuteMemoryCase {
...
#[test]
fn native_two_table_permute_memory_matches_interpreter_and_faults_before_commit() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!(
            "skipping native EVEX two-table-permute memory differential: host lacks AVX-512F/BW"
        );
        return;
    }
