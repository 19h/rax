...
fn selected_cases() -> [PsadbwMemoryCase; 6] {
    [
        PsadbwMemoryCase {
            width: VecWidth::V128,
            destination: 1,
...
#[test]
fn native_psadbw_matches_interpreter_restores_scratch_and_preserves_e4nf_faults() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native EVEX VPSADBW differential: host lacks AVX-512F/BW");
        return;
    }
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            let bytes = memory_bytes(case, ordinal + 401);
