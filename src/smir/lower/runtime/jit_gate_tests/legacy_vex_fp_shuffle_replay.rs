...
fn replay_features_distinguish_legacy_sse_from_avx_vex() {
    for (case, expected_avx) in [
        ...
        // The shared vector-state boundary uses ZMM and KMOVQ even when the
        // replayed instruction itself needs only SSE/SSE2 or AVX.
...
fn replay_matches_intel_o0_o2_state_for_widths_controls_aliases_and_upper_lanes() {
    ...
    {
        eprintln!(
            "skipping native legacy/VEX floating-shuffle differential: host lacks AVX/AVX-512F/BW"
        );
        return;
    }
