...
fn lane_mask(elem: VecElementType) -> u64 {
    match elem {
        ...
        _ => unreachable!("packed-abs element"),
    }
}
...
fn wrapping_abs(value: u64, elem: VecElementType) -> u64 {
    let mask = lane_mask(elem);
    let sign = 1u64 << (u32::from(elem.bytes()) * 8 - 1);
    if value & sign == 0 {
        value & mask
...
pub(super) fn initial_state(case: PackedAbsMemoryCase, ordinal: usize) -> SemanticState {
...
pub(super) fn memory_bytes(case: PackedAbsMemoryCase, ordinal: usize) -> [u8; 64] {
...
fn manual(case: PackedAbsMemoryCase, initial: &SemanticState, memory: &[u8; 64]) -> SemanticState {
    ...
    for lane in 0..lanes {
        let value = if mask & (1u64 << lane) != 0 {
            wrapping_abs(
                memory_lane(memory, if case.broadcast() { 0 } else { lane }, case.elem),
                case.elem,
            )
...
#[test]
fn all_72_packed_abs_cells_match_manual_semantics_at_o0_o1_o2() {
    let cases = all_cases();
...
#[test]
fn signed_minimum_wraps_and_empty_masks_suppress_type_e4_faults_without_commit() {
    for case in [
        PackedAbsMemoryCase {
        ...
        for level in LEVELS {
            ...
            let mut empty = initial_state(case, 3);
            ...
            let mut active = initial_state(case, 3);
