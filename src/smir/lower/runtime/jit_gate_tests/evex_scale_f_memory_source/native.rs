...
extern "C" fn lane_load_helper(
    ...
    LoadResult {
        value: match context.lane_bytes {
            ...
            _ => unreachable!("VSCALEF scalar helper width"),
        },
        ok: 1,
    }
}
...
fn selected_cases() -> Vec<ScaleFMemoryCase> {
...
fn host_supports(case: ScaleFMemoryCase) -> bool {
...
#[test]
fn native_scale_f_matches_interpreter_helpers_faults_masks_and_mxcsr() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native VSCALEF differential: host lacks AVX-512F/BW");
        return;
    }
    ...
    assert!(successes >= 36);
    assert!(suppressions >= 18);
}

#[test]
fn native_apx_guard_precedes_scale_f_memory_and_destination_commit() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native APX-address VSCALEF: host lacks AVX-512F/BW");
        return;
    }
    ...
    let (code, entry) = lower(&function, case);
    let exec = ExecMem::new(&code).expect("map APX-address VSCALEF");
