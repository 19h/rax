//! Exact helper-backed packed EVEX FP32/FP64 arithmetic memory-source coverage.
...
use crate::smir::ir::{
    SmirBlock, SmirFunction, Terminator, X86EvexPackedFpArithmeticMemoryReplay, X86InstructionBytes,
};
...
fn vector(index: u8, width: VecWidth) -> VReg {
    VReg::Arch(ArchReg::X86(match width {
        ...
        _ => unreachable!("packed EVEX FP32/FP64 arithmetic width"),
    }))
}
...
fn memory_encoding(
    ...
    let (w, pp) = match elem {
        VecElementType::F32 => (0, 0),
        VecElementType::F64 => (0x80, 1),
        _ => panic!("packed FP arithmetic uses binary32 or binary64 elements"),
    };
...
fn lift_case(case: FpMemoryCase) -> SmirFunction {
    ...
    function.x86_instruction_bytes.insert(
        (BlockId(0), PC),
        X86InstructionBytes::new(&bytes).expect("packed FP32/FP64 memory provenance"),
    );
    function
}
...
fn lower(function: &SmirFunction, case: FpMemoryCase) -> (Vec<u8>, usize) {
    ...
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed packed FP32/FP64 arithmetic"),
        result.entry_offset,
    )
}
...
#[test]
fn packed_fp_memory_classifier_exhaustively_rewrites_4_423_680_control_and_apx_address_cells() {
    ...
    assert_eq!(accepted, 6 * 2 * 3 * 32 * 32 * 2 * 15 * 2 * 2);

    for opcode in 0..=u8::MAX {
        let bytes = memory_encoding(VecElementType::F32, 0x58, 0, 1, 0, 0, false, false, 3);
...
#[test]
fn packed_fp_encodings_match_six_independent_llvm_23_anchors() {
    for (actual, llvm) in [
        (
            register_encoding(VecElementType::F32, 0x58, 16, 1, 2, 0, 0, false).to_vec(),
            vec![0x62, 0xE1, 0x74, 0x08, 0x58, 0xC2],
        ),
        (
            register_encoding(VecElementType::F64, 0x59, 16, 17, 0, 1, 0, false).to_vec(),
            vec![0x62, 0xE1, 0xF5, 0x20, 0x59, 0xC0],
        ),
        (
            register_encoding(VecElementType::F32, 0x5C, 31, 30, 0, 2, 0, false).to_vec(),
            vec![0x62, 0x61, 0x0C, 0x40, 0x5C, 0xF8],
        ),
        (
            stack_encoding(VecElementType::F32, 0x5E, 0, 1, 0, 3, true, true).to_vec(),
            vec![0x62, 0xF1, 0x74, 0x9B, 0x5E, 0x04, 0x24],
        ),
        (
            stack_encoding(VecElementType::F64, 0x5D, 16, 17, 1, 7, false, false).to_vec(),
            vec![0x62, 0xE1, 0xF5, 0x27, 0x5D, 0x04, 0x24],
        ),
        (
            stack_encoding(VecElementType::F64, 0x5F, 31, 30, 2, 5, true, false).to_vec(),
            vec![0x62, 0x61, 0x8D, 0xC5, 0x5F, 0x3C, 0x24],
        ),
    ] {
...
#[test]
fn packed_fp_memory_classifier_rejects_reserved_non_owned_and_trailing_shapes() {
    let valid = memory_encoding(VecElementType::F32, 0x58, 0, 1, 0, 1, false, false, 3).to_vec();
    ...
    let X86EvexPackedFpArithmeticMemoryReplay::MaskedVector { stack_instruction } = encoding.replay
    else {
        panic!("prefixed masked vector selected wrong replay")
    };
    assert_eq!(
        stack_instruction.as_slice(),
        stack_encoding(VecElementType::F32, 0x58, 0, 1, 0, 1, false, false,)
    );
}

#[test]
fn packed_fp_apx_r16_r17_sib_address_lifts_admits_and_lowers_exactly() {
    // VADDPS xmm16{k1},xmm17,[r16+r17*2+16]. The disp8 value 1 is
    // compressed by the 16-byte full-vector tuple.
    let bytes = [0x62, 0xE9, 0x70, 0x01, 0x58, 0x44, 0x48, 0x01];
    let mut lifter = X86_64Lifter::strict();
    let mut context = LiftContext::new(SourceArch::X86_64);
    let result = lifter
        .lift_insn(PC, &bytes, &mut context)
        .expect("APX-extended packed FP32/FP64 memory source");
    ...
    for level in LEVELS {
        ...
        let X86EvexPackedFpArithmeticMemoryReplay::MaskedVector { stack_instruction } =
            exact.encoding.replay
        else {
            panic!("{level:?}: APX masked vector selected wrong replay")
        };
        let expected = [0x62, 0xE1, 0x74, 0x01, 0x58, 0x04, 0x24];
...
#[test]
fn all_648_packed_fp_memory_cells_optimize_admit_and_lower_exactly() {
    let cases = all_cases();
    ...
    assert_eq!(lowerings, 648 * LEVELS.len());
}

#[test]
fn masked_vector_lowering_stages_disjoint_32_and_64_bit_lanes_and_rejects_avx_only_bridge() {
    for (elem, staging_load, final_store) in [
        ...
        assert!(
            ...
            "{elem:?}: scalar helper return must be read from the disjoint staging slot"
        );
        assert!(
...
#[test]
fn packed_fp_memory_sequence_fails_closed_for_provenance_and_graph_mutations() {
    let cases = [
        FpMemoryCase {
            opcode: 0x58,
            elem: VecElementType::F32,
    ...
    for case in cases {
        ...
        wrong_provenance.x86_instruction_bytes.insert(
            ...
            X86InstructionBytes::new(&memory_encoding(
                case.elem,
                if case.opcode == 0x58 { 0x59 } else { 0x58 },
                case.destination(),
                case.source1,
                case.ll(),
                case.mask(),
        ...
        let arithmetic = wrong_round.blocks[0]
            .ops
            .iter_mut()
            .find(|op| matches!(op.kind, OpKind::X86FpBinary { .. }))
            .unwrap();
        let OpKind::X86FpBinary { round, .. } = &mut arithmetic.kind else {
            unreachable!()
        };
        *round = FpRoundMode::RoundNearest;
        ...
        let arithmetic = suppressed.blocks[0]
            .ops
            .iter_mut()
            .find(|op| matches!(op.kind, OpKind::X86FpBinary { .. }))
            .unwrap();
        ...
        *suppress_exceptions = true;
        assert_rejected("suppressed exceptions", &suppressed);

        let mut wrong_source = function.clone();
        let arithmetic = wrong_source.blocks[0]
            .ops
            .iter_mut()
            .find(|op| matches!(op.kind, OpKind::X86FpBinary { .. }))
            .unwrap();
        let OpKind::X86FpBinary { src1, .. } = &mut arithmetic.kind else {
            unreachable!()
        };
...
fn initial_registers(case: FpMemoryCase, ordinal: usize) -> GuestRegs {
    let mut registers = GuestRegs {
        ...
        zmm: std::array::from_fn(|register| {
            std::array::from_fn(|word| match case.elem {
                VecElementType::F32 => {
                    let low = 0x3F80_0000u64 | ((register as u64) << 8) | word as u64;
                    let high = 0x4000_0000u64 | ((register as u64) << 8) | (word as u64 + 0x40);
                    (high << 32) | low
                }
                VecElementType::F64 => {
                    0x3FF0_0000_0000_0000u64 | ((register as u64) << 16) | word as u64
                }
                _ => unreachable!(),
            })
        }),
    ...
    if case.destination() == case.source1 {
        registers.zmm[0] = std::array::from_fn(|word| match case.elem {
            VecElementType::F32 => {
                ((0x4040_0000u64 | word as u64) << 32) | (0x3F00_0000u64 | (word as u64 + 0x80))
            }
            VecElementType::F64 => 0x4008_0000_0000_0000u64 | word as u64,
            _ => unreachable!(),
        });
    }
    registers
...
fn memory_value(case: FpMemoryCase, ordinal: usize) -> [u64; 8] {
    std::array::from_fn(|word| match case.elem {
        VecElementType::F32 => {
            let low = 0x3FC0_0000u64 | ((ordinal as u64) << 8) | word as u64;
            let high = 0x4020_0000u64 | ((ordinal as u64) << 8) | (word as u64 + 0x20);
            (high << 32) | low
        }
        VecElementType::F64 => 0x3FF8_0000_0000_0000u64 | ((ordinal as u64) << 16) | word as u64,
        _ => unreachable!(),
    })
}
...
#[test]
fn packed_fp_memory_o0_o1_o2_interpretation_is_exactly_equivalent() {
    let cases = all_cases();
    ...
    assert_eq!(comparisons, 648 * LEVELS.len());
}
...
extern "C" fn lane_load_helper(
    ...
    LoadResult {
        value: match context.lane_bytes {
            ...
            _ => unreachable!("packed FP scalar helper width"),
        },
        ok: 1,
    }
}
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_packed_fp_memory_matches_interpretation_faults_and_mask_suppression() {
    use super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!(
            "skipping native packed FP32/FP64 arithmetic memory differential: \
