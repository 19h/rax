...
fn native_cases() -> Vec<ScalarConvertMemoryCase> {
...
#[test]
fn native_scalar_convert_memory_matches_interpretation_faults_and_mask_suppression() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!(
            "skipping native scalar EVEX floating-point conversion differential: \
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        if case.conversion.needs_fp16() && !std::is_x86_feature_detected!("avx512fp16") {
            continue;
        }
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            assert_eq!(
                context.last_size,
                case.conversion.memory_size() as u64,
                "{level:?} {case:?}"
            );
    ...
    assert!(successes >= 2 * 3 * 2); // classic F32/F64 conversions without FP16.
    assert!(suppressions >= 2 * 2 * 2);
}
