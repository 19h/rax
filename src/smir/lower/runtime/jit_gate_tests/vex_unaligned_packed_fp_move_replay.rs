//! Native replay coverage for register-only VEX `VMOVUPS`/`VMOVUPD`.
...
fn replay_uses_the_avx_ymm16_bridge_without_avx512() {
    for case in [
        ...
        {
            let mut expected = std::is_x86_feature_detected!("avx");
            #[cfg(target_os = "macos")]
            {
...
#[test]
fn replay_admits_and_emits_208_optimized_legal_encodings_and_fails_closed() {
...
#[cfg(target_arch = "x86_64")]
const CHILD_RANGE_ENV: &str = "RAX_VEX_UNALIGNED_PACKED_FP_MOVE_CHILD_RANGE";
...
fn replay_matches_intel_o0_o2_state_for_directions_lengths_aliases_and_upper_lanes() {
    ...
    if running_under_rosetta() {
        eprintln!(
            "skipping native VEX packed-move differential: Rosetta raises #UD for \
             register-only VEX VMOVUPS/VMOVUPD"
        );
        return;
    }
    ...
    run_isolated_native_differential(
        "smir::lower::runtime::jit_gate_tests::vex_unaligned_packed_fp_move_replay::\
