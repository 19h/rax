//! Independent immediate-blend model and native helper-boundary differential.
...
/// Select raw element bits under imm8. `VPBLENDW` repeats the same eight mask
/// bits in each 128-bit block; the other forms use one bit per destination
/// element. Bytes above the VEX vector length are zeroed.
fn architectural_destination(
    ...
    let block_lanes = 16 / element_bytes;
    for lane in 0..lanes {
        let bit = if case.blend.repeat_128() {
            lane % block_lanes
        } else {
            lane
        };
        let source = if (case.immediate >> bit) & 1 != 0 {
            &memory
        } else {
            &source1
        };
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_blends_match_independent_model_interpreter_and_precise_faults() {
    ...
    eprintln!(
        "executed {successes} successful and {faults} faulting native VEX immediate-blend cases"
    );
}
