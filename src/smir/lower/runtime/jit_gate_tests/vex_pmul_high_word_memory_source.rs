//! Exact helper-backed VEX VPMULHUW/VPMULHW memory-source coverage.
...
fn vector(index: u8, width: VecWidth) -> VReg {
    x86(match width {
        ...
        _ => unreachable!("VEX VPMULHUW/VPMULHW has only 128-/256-bit forms"),
    })
}

fn expected_address(case: PmulHighWordMemoryCase) -> Address {
    Address::BaseOffset {
...
fn assert_exact_pair(ops: &[SmirOp], case: PmulHighWordMemoryCase) {
    ...
    assert_eq!(*signed1, case.signed, "{case:?}");
    assert_eq!(*signed2, case.signed, "{case:?}");
    assert_eq!(*shift_left, 0, "{case:?}");
    assert!(!*round, "{case:?}");
    assert_eq!(*sat_bits, 0, "{case:?}");
    assert_eq!(*out_shift, 16, "{case:?}");
}

fn lift_case(case: PmulHighWordMemoryCase) -> SmirFunction {
...
fn lower(function: &SmirFunction, case: PmulHighWordMemoryCase) -> (Vec<u8>, usize) {
    ...
    let result = lowerer.lower_function(function).unwrap_or_else(|error| {
        panic!("helper-backed VEX VPMULHUW/VPMULHW lowering failed: {error:?}")
    });
    assert!(result.relocations.is_empty());
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed VEX VPMULHUW/VPMULHW"),
        result.entry_offset,
    )
}
...
fn every_kind_width_form_alias_and_optimizer_shape_is_lifted_admitted_and_lowered() {
    ...
    assert_eq!(lowered, 24 * LEVELS.len());
}
...
fn classifier_and_lowerer_fail_closed_for_every_pair_invariant() {
    let case = PmulHighWordMemoryCase {
        signed: true,
        width: VecWidth::V128,
        form: VexForm::Vex3W0,
        alias: false,
    ...
    assert_mutation_rejected(&base, "consumer bypasses temporary", |function| {
        if let OpKind::VMulShiftSat { src2, .. } = &mut function.blocks[0].ops[1].kind {
            *src2 = vector(2, VecWidth::V128);
        }
    });
    assert_mutation_rejected(&base, "non-word element", |function| {
        if let OpKind::VMulShiftSat { src_elem, .. } = &mut function.blocks[0].ops[1].kind {
            *src_elem = VecElementType::I8;
        }
    });
    assert_mutation_rejected(&base, "nonintegral lane geometry", |function| {
        if let OpKind::VMulShiftSat { lanes, .. } = &mut function.blocks[0].ops[1].kind {
            *lanes -= 1;
        }
    });
    assert_mutation_rejected(&base, "mixed signedness", |function| {
        if let OpKind::VMulShiftSat { signed2, .. } = &mut function.blocks[0].ops[1].kind {
            *signed2 = false;
        }
    });
    assert_mutation_rejected(&base, "shifted input", |function| {
        if let OpKind::VMulShiftSat { shift_left, .. } = &mut function.blocks[0].ops[1].kind {
            *shift_left = 1;
        }
    });
    assert_mutation_rejected(&base, "rounding enabled", |function| {
        if let OpKind::VMulShiftSat { round, .. } = &mut function.blocks[0].ops[1].kind {
            *round = true;
        }
    });
    assert_mutation_rejected(&base, "saturation enabled", |function| {
        if let OpKind::VMulShiftSat { sat_bits, .. } = &mut function.blocks[0].ops[1].kind {
            *sat_bits = 16;
        }
    });
    assert_mutation_rejected(&base, "wrong output shift", |function| {
        if let OpKind::VMulShiftSat { out_shift, .. } = &mut function.blocks[0].ops[1].kind {
            *out_shift = 15;
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only destination", |function| {
        if let OpKind::VMulShiftSat { dst, .. } = &mut function.blocks[0].ops[1].kind {
            *dst = vector(16, VecWidth::V128);
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only first source", |function| {
        if let OpKind::VMulShiftSat { src1, .. } = &mut function.blocks[0].ops[1].kind {
...
fn read_u16(bytes: &[u8], lane: usize) -> u16 {
    u16::from_le_bytes(bytes[lane * 2..lane * 2 + 2].try_into().unwrap())
}

fn write_u16(bytes: &mut [u8], lane: usize, value: u16) {
    bytes[lane * 2..lane * 2 + 2].copy_from_slice(&value.to_le_bytes());
}

fn model_lane(first: u16, second: u16, signed: bool) -> u16 {
    if signed {
        (i32::from(first as i16) * i32::from(second as i16) >> 16) as u16
    } else {
        (u32::from(first) * u32::from(second) >> 16) as u16
    }
}

fn operand_vectors(case: PmulHighWordMemoryCase) -> ([u64; 8], [u64; 8]) {
    ...
    for lane in 0..lanes {
        ...
        write_u16(&mut source1, lane, first);
        write_u16(&mut source2, lane, second);
    }
...
#[cfg(target_arch = "x86_64")]
fn full_guest_regs(case: PmulHighWordMemoryCase, ordinal: usize) -> GuestRegs {
...
fn expected_success(
    ...
    for lane in 0..lanes {
        write_u16(
            ...
            lane,
            model_lane(
                read_u16(&source1_bytes, lane),
                read_u16(&source2_bytes, lane),
                case.signed,
            ),
        );
    }
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_high_word_multiply_matches_model_interpreter_and_precise_faults() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX VPMULHUW/VPMULHW memory differential: host lacks AVX");
        return;
    }
