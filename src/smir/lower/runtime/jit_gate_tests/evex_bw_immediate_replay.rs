//! Native replay coverage for register-only EVEX VPALIGNR/VDBPSADBW.
...
#[test]
fn replay_admits_and_emits_324_legal_register_encodings() {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;

    assert_eq!(
        encoding((0x0F, 2, true), 25, 26, 27, 2, true, 0x1F),
        [0x62, 0x03, 0xAD, 0xC2, 0x0F, 0xCB, 0x1F]
    );
    ...
    for shape in shapes() {
        for (destination, source1, source2) in operands {
            for (mask, zeroing) in [(0u8, false), (1, false), (2, true)] {
                for immediate in [0u8, 0x1F, 0xFF] {
                    let bytes = encoding(
                        shape,
                        destination,
                        source1,
        ...
        let register = encoding(shape, 1, 2, 3, 1, false, 0x1F);
...
#[cfg(target_arch = "x86_64")]
fn interpret(bytes: &[u8], initial: &BwImmediateState) -> BwImmediateState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native(bytes: &[u8], initial: &BwImmediateState) -> BwImmediateState {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;
    ...
    let exec = ExecMem::new(&code).expect("map AVX-512BW immediate replay");
...
fn replay_matches_interpreter_for_shapes_wig_extensions_aliases_masks_and_edges() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native AVX-512BW immediate differential: host lacks AVX-512F/BW");
        return;
    }
    let has_vl = std::is_x86_feature_detected!("avx512vl");
    let initial = BwImmediateState {
    ...
    let mut maximum_sad = initial.clone();
    maximum_sad.vectors[2] = [0; 8];
    maximum_sad.vectors[3] = [u64::MAX; 8];
    let bytes = encoding((0x42, 2, false), 1, 2, 3, 0, false, 0xFF);
    let native = execute_native(&bytes, &maximum_sad);
    assert_eq!(native, interpret(&bytes, &maximum_sad), "{bytes:02X?}");
    assert_eq!(native.vectors[1], [0x03FC_03FC_03FC_03FC; 8]);

    for immediate in [0x20, 0xFF] {
        let bytes = encoding((0x0F, 2, true), 1, 2, 3, 0, false, immediate);
        let native = execute_native(&bytes, &initial);
        assert_eq!(native, interpret(&bytes, &initial), "{bytes:02X?}");
        assert_eq!(native.vectors[1], [0; 8], "{bytes:02X?}");
    }
    ...
    for shape in shapes() {
        ...
        for (destination, source1, source2) in operands {
            for (mask, zeroing) in [(0u8, false), (1, false), (2, true)] {
                for immediate in [0u8, 1, 15, 16, 31, 0xFF] {
                    let bytes = encoding(
                        shape,
                        destination,
                        source1,
    ...
    let available_shapes = if has_vl { 9 } else { 3 };
