//! Independent variable-blend model and native helper-boundary differential.
...
fn memory_vector(ordinal: usize) -> [u64; 8] {
    ...
    for (index, byte) in memory.iter_mut().enumerate() {
        *byte = 0x2Du8
            .wrapping_add((ordinal as u8).wrapping_mul(0x1D))
            .wrapping_add((index as u8).wrapping_mul(11));
    }
    bytes_to_words(memory)
}

fn mask_vector(case: BlendMemoryCase, ordinal: usize) -> [u64; 8] {
    let mut mask = [0u8; 64];
    let element_bytes = case.blend.element().bytes() as usize;
    let lanes = 64 / element_bytes;
    for lane in 0..lanes {
        let start = lane * element_bytes;
        for byte in 0..element_bytes {
            mask[start + byte] = 0x15u8
                .wrapping_add((ordinal as u8).wrapping_mul(0x23))
                .wrapping_add((lane as u8).wrapping_mul(0x11))
                .wrapping_add(byte as u8);
        }
        let negative = (lane + ordinal + usize::from(case.ignored_low)) % 3 != 1;
        let sign_byte = start + element_bytes - 1;
        mask[sign_byte] = (mask[sign_byte] & 0x7F) | if negative { 0x80 } else { 0 };
    }
    bytes_to_words(mask)
}

/// Select raw element bits from memory when the signed mask element is
/// negative and from source 1 otherwise. The mask is tested only through its
/// element most-significant bit. Bytes above the VEX vector length are zeroed.
fn architectural_destination(
    ...
    for lane in 0..lanes {
        let start = lane * element_bytes;
        let sign_byte = start + element_bytes - 1;
        let source = if mask[sign_byte] & 0x80 != 0 {
            &memory
        } else {
            &source1
        };
...
#[cfg(target_arch = "x86_64")]
fn full_guest_regs(case: BlendMemoryCase, ordinal: usize) -> GuestRegs {
    ...
    registers.zmm[usize::from(case.mask)] = mask_vector(case, ordinal);
...
fn expected_success(
    ...
    let mask = registers.zmm[usize::from(case.mask)];
    registers.zmm[usize::from(case.destination)] =
        architectural_destination(case, source1, memory, mask);
...
fn assert_interpreter_matches(
    ...
    let memory_bytes = words_to_bytes(memory_value);
    memory.load(
        address as usize,
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_blends_match_independent_model_interpreter_aliases_and_precise_faults() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX variable-blend memory differential: host lacks AVX");
        return;
    }
    ...
    let cases = semantic_cases()
        .into_iter()
        .filter(|case| avx2 || !case.blend.needs_avx2(case.width))
        .collect::<Vec<_>>();
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in DIFFERENTIAL_LEVELS {
            ...
            let fault_ordinal = ordinal ^ 0x55;
    ...
    eprintln!(
        "executed {successes} successful and {faults} faulting native VEX variable-blend memory cases"
    );
}
