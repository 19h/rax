...
fn lane_mask(elem: VecElementType) -> u64 {
    match elem {
        ...
        _ => unreachable!("chunk-shuffle element width"),
    }
}
...
pub(super) fn initial_state(case: ChunkShuffleMemoryCase, ordinal: usize) -> SemanticState {
...
pub(super) fn memory_bytes(case: ChunkShuffleMemoryCase, ordinal: usize) -> [u8; 64] {
...
fn memory_lane(case: ChunkShuffleMemoryCase, bytes: &[u8; 64], lane: usize) -> u64 {
...
pub(super) fn manual(
    ...
    let chunks = usize::try_from(case.width.bytes() / 16).unwrap();
    let chunk_lanes = usize::try_from(16 / elem.bytes()).unwrap();
    let lanes = chunks * chunk_lanes;
    ...
    for destination_chunk in 0..chunks {
        let (from_second, selector) = if chunks == 2 {
            if destination_chunk == 0 {
                (false, usize::from(case.immediate & 1))
            } else {
                (true, usize::from((case.immediate >> 1) & 1))
            }
        } else if destination_chunk < 2 {
            (
                false,
                usize::from((case.immediate >> (destination_chunk * 2)) & 3),
            )
        } else {
            (
                true,
        ...
        for chunk_lane in 0..chunk_lanes {
            let source_lane = selector * chunk_lanes + chunk_lane;
            let value = if from_second {
                memory_lane(case, memory, source_lane)
            } else {
                get_lane(&source1, source_lane, elem)
            };
            set_lane(
                &mut raw,
                destination_chunk * chunk_lanes + chunk_lane,
                elem,
                value,
            );
        }
...
#[test]
fn all_144_cells_match_manual_exact_bits_at_o0_o1_o2() {
    let cases = all_cases();
    ...
    assert_eq!(comparisons, 144 * LEVELS.len());
}

#[test]
fn every_imm8_matches_manual_at_every_kind_width_tuple_and_opt_level() {
    let mut comparisons = 0usize;
    for (kind_index, kind) in ChunkKind::ALL.into_iter().enumerate() {
        for width in [VecWidth::V256, VecWidth::V512] {
            for tuple in TupleKind::ALL {
                for immediate in u8::MIN..=u8::MAX {
                    ...
                    let ordinal = usize::from(immediate) + kind_index * 257;
                    let initial = initial_state(case, ordinal);
                    let bytes = memory_bytes(case, ordinal);
                    let expected = manual(case, &initial, &bytes);
                    for level in LEVELS {
    ...
    assert_eq!(comparisons, 4 * 2 * 2 * 256 * LEVELS.len());
}

#[test]
fn empty_masks_do_not_suppress_e4nf_faults_or_commit_state() {
    let mut faults = 0usize;
    for (ordinal, kind) in ChunkKind::ALL.into_iter().enumerate() {
        for tuple in TupleKind::ALL {
            for width in [VecWidth::V256, VecWidth::V512] {
                for control in [MaskControl::Merge, MaskControl::Zero] {
                    let case = ChunkShuffleMemoryCase {
                        kind,
                        width,
                        destination: [1, 9, 17, 25][ordinal],
                        source1: [2, 10, 17, 26][ordinal],
                        control,
                        tuple,
                        immediate: [0x00, 0x4E, 0xA5, 0xFF][ordinal],
                    };
                    for level in LEVELS {
                        let function = optimize(lift_case(case), level);
    ...
    assert_eq!(faults, 4 * 2 * 2 * 2 * LEVELS.len());
}
...
fn empty_masks_still_perform_one_successful_access_before_merge_or_zero() {
    ...
    assert_eq!(comparisons, 4 * 2 * 2 * LEVELS.len());
}
