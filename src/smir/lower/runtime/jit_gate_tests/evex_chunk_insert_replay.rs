//! Native replay coverage for register-only EVEX VINSERTF*/VINSERTI* chunks.
...
fn replay_admits_and_emits_432_legal_register_encodings() {
    ...
    assert_eq!(
        encoding((0x1A, false, 2), 25, 26, 27, 2, true, 0xFF),
        [0x62, 0x03, 0x2D, 0xC2, 0x1A, 0xCB, 0xFF]
    );
    ...
    for shape in shapes() {
        ...
        let register = encoding(shape, 1, 2, 3, 1, false, 0xFF);
...
#[cfg(target_arch = "x86_64")]
fn interpret(bytes: &[u8], initial: &InsertState) -> InsertState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native(bytes: &[u8], initial: &InsertState) -> InsertState {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;
    ...
    let exec = ExecMem::new(&code).expect("map EVEX chunk-insert replay");
...
fn replay_matches_interpreter_for_shapes_extensions_aliases_masks_and_immediates() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native EVEX chunk-insert differential: host lacks AVX-512F/BW");
        return;
    }
    let has_vl = std::is_x86_feature_detected!("avx512vl");
    let has_dq = std::is_x86_feature_detected!("avx512dq");
    let initial = InsertState {
