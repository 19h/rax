...
fn selected_cases() -> [MultiShiftMemoryCase; 8] {
    [
...
#[test]
fn native_multishift_matches_manual_and_enforces_unconditional_e4nf_faults() {
    ...
    {
        eprintln!(
            "skipping native VPMULTISHIFTQB memory differential: host lacks AVX-512F/BW/VBMI"
        );
        return;
    }
    ...
    let expected_empty_mask_cases = cases.iter().filter(|case| case.mask() != 0).count() * 2;
    ...
    assert_eq!(empty_mask_successes, expected_empty_mask_cases);
    assert_eq!(empty_mask_faults, empty_mask_successes);
}
