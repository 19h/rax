...
extern "C" fn lane_load_helper(
    ...
    LoadResult {
        value: match context.lane_bytes {
            ...
            _ => unreachable!("VPTERNLOG scalar helper width"),
        },
        ok: 1,
    }
}
...
fn selected_cases() -> Vec<TernaryMemoryCase> {
    let mut cases = Vec::new();
    let immediates = [0x00, 0x01, 0x69, 0x96, 0xE8, 0xFF];
...
#[test]
fn native_ternary_logic_matches_interpreter_helpers_faults_and_mask_suppression() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native VPTERNLOG differential: host lacks AVX-512F/BW");
        return;
    }
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in LEVELS {
            ...
            let lane_count = case.width.lanes(case.elem);
            let applicable_mask = (1u64 << lane_count) - 1;
            let active_mask = if case.mask() == 0 {
                applicable_mask
            } else {
                initial.masks[usize::from(case.mask())] & applicable_mask
            };
...
#[test]
fn native_apx_address_guard_precedes_memory_and_destination_commit() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native APX-address VPTERNLOG: host lacks AVX-512F/BW");
        return;
    }
    ...
    let (code, entry) = lower(&function, case);
    let exec = ExecMem::new(&code).expect("map APX-address VPTERNLOG");
