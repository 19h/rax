...
fn expected_vector_scratch(case: ConvertCase, memory: &[u8; 64]) -> [u64; 8] {
    let mut scratch = [0u8; 64];
    let size = case.source_width().bytes() as usize;
    scratch[..size].copy_from_slice(&memory[..size]);
    bytes_to_words(scratch)
}

#[test]
fn native_packed_conversions_match_interpreter_helpers_faults_and_mask_suppression() {
    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native packed conversion differential: host lacks AVX-512F/BW");
        return;
    }
    let has_dq = std::is_x86_feature_detected!("avx512dq");
    let cases: Vec<_> = selected_cases()
        .into_iter()
        .filter(|case| !case.spec.needs_avx512dq() || has_dq)
        .collect();
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        ...
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            successes += 1;
            reconstructed_broadcasts += usize::from(case.spec == SPECS[2]);
    ...
    // Keep the conversion helpers honest about byte/word marshalling.
    assert_eq!(
