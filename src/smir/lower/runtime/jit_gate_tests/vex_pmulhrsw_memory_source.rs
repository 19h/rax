//! Exact helper-backed VEX VPMULHRSW memory-source coverage.
...
struct PmulhrswMemoryCase {
    width: VecWidth,
    w: bool,
    alias: bool,
}

impl PmulhrswMemoryCase {
    ...
    fn emitted_bytes(self) -> Vec<u8> {
        ...
        vec![
            ...
            (if destination < 8 { 0x80 } else { 0 })
                ...
                | 2,
            // VPMULHRSW is WIG; the lowerer emits canonical W=0.
...
fn all_cases() -> Vec<PmulhrswMemoryCase> {
    let mut cases = Vec::new();
    for width in [VecWidth::V128, VecWidth::V256] {
        for w in [false, true] {
            for alias in [false, true] {
                cases.push(PmulhrswMemoryCase { width, w, alias });
            }
        }
    }
    cases
...
fn vector(index: u8, width: VecWidth) -> VReg {
    x86(match width {
        ...
        _ => unreachable!("VEX VPMULHRSW has only 128-/256-bit forms"),
    })
}

fn expected_address(case: PmulhrswMemoryCase) -> Address {
    Address::BaseOffset {
...
fn assert_exact_pair(ops: &[SmirOp], case: PmulhrswMemoryCase) {
    let [load, consumer] = ops else {
        panic!("{case:?}: expected exact VLoad + VMulShiftSat pair, got {ops:?}")
    };
    ...
    let OpKind::VMulShiftSat {
        ...
        panic!("{case:?}: expected VMulShiftSat consumer, got {consumer:?}")
    };
    ...
    assert_eq!(*src_elem, VecElementType::I16, "{case:?}");
    assert_eq!(
        *lanes,
        case.width.lanes(VecElementType::I16) as u8,
        "{case:?}"
    );
    assert!(*signed1, "{case:?}");
    assert!(*signed2, "{case:?}");
    assert_eq!(*shift_left, 0, "{case:?}");
    assert!(*round, "{case:?}");
    assert_eq!(*sat_bits, 0, "{case:?}");
    assert_eq!(*out_shift, 15, "{case:?}");
}

fn lift_case(case: PmulhrswMemoryCase) -> SmirFunction {
...
fn lower(function: &SmirFunction, case: PmulhrswMemoryCase) -> (Vec<u8>, usize) {
    ...
    let result = lowerer
        ...
        .unwrap_or_else(|error| panic!("helper-backed VEX VPMULHRSW lowering failed: {error:?}"));
    assert!(result.relocations.is_empty());
    (
        lowerer
            .finalize()
            .expect("finalize helper-backed VEX VPMULHRSW"),
        result.entry_offset,
    )
}

#[test]
fn all_wig_width_and_alias_shapes_are_lifted_admitted_and_lowered() {
    let cases = all_cases();
    ...
    for case in cases {
        for level in LEVELS {
            ...
            assert_eq!(actual.opcode, 0x0B, "{level:?} {case:?}");
    ...
    assert_eq!(lowered, 8 * LEVELS.len());
}
...
fn classifier_and_lowerer_fail_closed_for_every_pair_invariant() {
    ...
    if let OpKind::VMulShiftSat { src2, .. } = &mut wrong_source2.blocks[0].ops[1].kind {
        *src2 = vector(2, VecWidth::V128);
    }

    let mut wrong_element = base.clone();
    if let OpKind::VMulShiftSat { src_elem, .. } = &mut wrong_element.blocks[0].ops[1].kind {
        *src_elem = VecElementType::I8;
    }

    let mut wrong_lanes = base.clone();
    if let OpKind::VMulShiftSat { lanes, .. } = &mut wrong_lanes.blocks[0].ops[1].kind {
        *lanes -= 1;
    }

    let mut unsigned_first = base.clone();
    if let OpKind::VMulShiftSat { signed1, .. } = &mut unsigned_first.blocks[0].ops[1].kind {
        *signed1 = false;
    }

    let mut unsigned_second = base.clone();
    if let OpKind::VMulShiftSat { signed2, .. } = &mut unsigned_second.blocks[0].ops[1].kind {
        *signed2 = false;
    }

    let mut shifted_input = base.clone();
    if let OpKind::VMulShiftSat { shift_left, .. } = &mut shifted_input.blocks[0].ops[1].kind {
        *shift_left = 1;
    }

    let mut no_rounding = base.clone();
    if let OpKind::VMulShiftSat { round, .. } = &mut no_rounding.blocks[0].ops[1].kind {
        *round = false;
    }

    let mut saturating = base.clone();
    if let OpKind::VMulShiftSat { sat_bits, .. } = &mut saturating.blocks[0].ops[1].kind {
        *sat_bits = 16;
    }

    let mut wrong_output_shift = base.clone();
    if let OpKind::VMulShiftSat { out_shift, .. } = &mut wrong_output_shift.blocks[0].ops[1].kind {
        *out_shift = 16;
    }

    let mut high_destination = base.clone();
    if let OpKind::VMulShiftSat { dst, .. } = &mut high_destination.blocks[0].ops[1].kind {
        *dst = vector(16, VecWidth::V128);
    }

    let mut high_source1 = base.clone();
    if let OpKind::VMulShiftSat { src1, .. } = &mut high_source1.blocks[0].ops[1].kind {
        *src1 = vector(16, VecWidth::V128);
    }

    let mut wrong_namespace = base.clone();
    if let OpKind::VMulShiftSat { dst, .. } = &mut wrong_namespace.blocks[0].ops[1].kind {
    ...
    let mut encoded_opcode = case.bytes();
    encoded_opcode[3] = 0x0A;
    let mut byte_opcode_mismatch = base.clone();
    replace_instruction_bytes(&mut byte_opcode_mismatch, &encoded_opcode);
    ...
    let malformed = [
        ...
        ("non-word element", wrong_element),
        ("nonintegral lane geometry", wrong_lanes),
        ("unsigned first source", unsigned_first),
        ("unsigned second source", unsigned_second),
        ("shifted input", shifted_input),
        ("rounding disabled", no_rounding),
        ("saturation enabled", saturating),
        ("wrong output shift", wrong_output_shift),
        ...
        ("encoded opcode mismatch", byte_opcode_mismatch),
    ];
...
fn model_lane(first: i16, second: i16) -> i16 {
    let product = i32::from(first) * i32::from(second);
    (((product >> 14) + 1) >> 1) as i16
}

fn operand_vectors(case: PmulhrswMemoryCase) -> ([u64; 8], [u64; 8]) {
    let mut source1 = [0xC3; 64];
    let mut source2 = [0x5A; 64];
    let lanes = case.width.bytes() as usize / 2;
    let values = [
        (0, 0),
        (1, 1),
        (i16::MAX, i16::MAX),
    ...
    for lane in 0..lanes {
        ...
        write_i16(&mut source1, lane, first);
        write_i16(&mut source2, lane, second);
    }
...
#[cfg(target_arch = "x86_64")]
fn full_guest_regs(case: PmulhrswMemoryCase, ordinal: usize) -> GuestRegs {
...
fn expected_success(
    ...
    for lane in 0..lanes {
        write_i16(
            ...
            lane,
            model_lane(
                read_i16(&source1_bytes, lane),
                read_i16(&source2_bytes, lane),
            ),
        );
    }
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_pmulhrsw_matches_independent_model_interpreter_and_precise_faults() {
    use crate::smir::lower::runtime::ExecMem;

    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX VPMULHRSW memory differential: host lacks AVX");
        return;
    }
    ...
    eprintln!(
        "executed {successes} successful and {faults} faulting native VEX VPMULHRSW memory cases"
    );
}
