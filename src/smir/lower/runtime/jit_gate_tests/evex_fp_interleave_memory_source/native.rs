...
fn selected_cases() -> [FpInterleaveMemoryCase; 8] {
    [
        FpInterleaveMemoryCase {
...
#[test]
fn native_interleave_matches_interpreter_restores_scratch_and_preserves_e4nf_faults() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native EVEX floating interleave differential: host lacks AVX-512F/BW");
        return;
    }
    ...
    for (ordinal, case) in cases.into_iter().enumerate() {
        for level in [OptLevel::O0, OptLevel::O2] {
            ...
            match case.tuple {
                ...
                TupleKind::Broadcast => {
                    let memory_width = case.kind.memory_width();
                    let mut context = ScalarMemoryContext {
                        value: scalar_value(&bytes, memory_width),
                        ok: true,
                        calls: 0,
                        last_address: 0,
                        last_size: 0,
