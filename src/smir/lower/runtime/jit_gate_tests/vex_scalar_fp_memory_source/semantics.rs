...
fn fp_bits(case: ScalarFpCase, seed: usize) -> u64 {
    const F32: [u32; 8] = [
        0x0000_0000,
        0x8000_0000,
        0x7F80_0000,
    ...
    match case.family {
        ScalarFpFamily::Ss => u64::from(F32[seed % F32.len()]),
        ScalarFpFamily::Sd => F64[seed % F64.len()],
    }
}

fn full_guest_regs(case: ScalarFpCase, seed: usize) -> GuestRegs {
    ...
    let bits = fp_bits(case, seed);
    if case.family == ScalarFpFamily::Ss {
        registers.zmm[usize::from(case.vector)][0] =
            (registers.zmm[usize::from(case.vector)][0] & 0xFFFF_FFFF_0000_0000) | bits;
    } else {
        registers.zmm[usize::from(case.vector)][0] = bits;
    }
    registers.gpr[usize::from(case.base)] = 0x3000 + ((seed & 0x1F) as u64) * 0x40;
    registers
}

fn effective_address(registers: &GuestRegs, case: ScalarFpCase) -> u64 {
...
fn interpreted_load_vectors(
    ...
    memory.load(
        address as usize,
        &bytes[..case.family.memory_width().bytes() as usize],
    );
    let result =
...
fn interpreted_store_value(
    ...
    let size = case.family.memory_width().bytes() as usize;
...
fn expected_load_scratch(case: ScalarFpCase, value: u64) -> [u64; 8] {
    let mut scratch = [0; 8];
    scratch[0] = if case.family == ScalarFpFamily::Ss {
        value & 0xFFFF_FFFF
    } else {
        value
    };
...
fn expected_store_scratch(initial: &GuestRegs, case: ScalarFpCase, value: u64) -> [u64; 8] {
...
fn assert_helper_observation(
    ...
    assert_eq!(
        context.last_size,
        case.family.memory_width().bytes(),
        "{label} {case:?}"
    );
...
fn native_cases() -> Vec<NativeCase> {
    ...
    for level in [OptLevel::O0, OptLevel::O2] {
        for form in VexForm::ALL {
            for family in ScalarFpFamily::ALL {
                for width_256 in [false, true] {
                    if !family.accepts_width_256(width_256) {
                        continue;
                    }
                    for kind in [
                        X86VexScalarFpMemoryKind::Load,
    ...
    assert_eq!(cases.len(), 2 * 3 * (2 + 4) * 16);
    ...
    assert!(
        cases
            .iter()
            .any(|case| case.instruction.family == ScalarFpFamily::Sd
                && case.instruction.width_256)
    );
    cases
}

const CHILD_RANGE_ENV: &str = "RAX_VEX_SCALAR_FP_MEMORY_CHILD_RANGE";
...
fn execute_case_range(cases: &[NativeCase], range: std::ops::Range<usize>) {
    ...
    for native_case in &cases[range] {
        ...
        let value = fp_bits(case, native_case.seed);
        ...
        let semantic_value = match case.kind {
            X86VexScalarFpMemoryKind::Load => {
        ...
        let mut context = MemoryContext {
            ...
            last_size: 0,
            last_zero_upper: 0,
            value: fp_bits(case, native_case.seed ^ 0x55),
            observed: 0,
        };
        ...
        let mut expected = initial;
        let semantic_value = match case.kind {
            X86VexScalarFpMemoryKind::Load => context.value,
            X86VexScalarFpMemoryKind::Store => {
                let stored = interpreted_store_value(&function, &initial, address, case);
                expected.vector_scratch = expected_store_scratch(&initial, case, stored);
                stored
    ...
    eprintln!(
        "executed {successes} successful and {faults} faulting native VEX scalar-floating memory cases"
    );
}
...
fn run_child_range(test_name: &str, range: std::ops::Range<usize>) -> std::process::Output {
    std::process::Command::new(std::env::current_exe().expect("current unit-test executable"))
        ...
        .output()
        .expect("run isolated native VEX scalar-floating memory differential")
}
...
fn run_isolated_native_differential(test_name: &str) {
    ...
    panic!(
        "isolated native VEX scalar-floating memory failure at case {start}/{}: \
...
#[test]
fn native_special_values_match_o0_o2_interpreter_and_fault_without_guest_commit() {
    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX scalar-floating memory differential: host lacks AVX");
        return;
    }
    run_isolated_native_differential(
        "smir::lower::runtime::jit_gate_tests::vex_scalar_fp_memory_source::semantics::\
         native_special_values_match_o0_o2_interpreter_and_fault_without_guest_commit",
    );
}
