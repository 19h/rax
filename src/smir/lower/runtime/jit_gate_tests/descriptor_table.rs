//! Fail-closed native admission and ABI layout for x86 descriptor tables.
...
use crate::smir::lower::x86_64::{
    x86_descriptor_table_load_shape_valid, x86_descriptor_table_store_shape_valid,
};
use crate::smir::lower::{
    X86_GUEST_DESCRIPTOR_LOAD_FN_OFFSET, X86_GUEST_DESCRIPTOR_STORE_FN_OFFSET,
    X86_GUEST_PMC_FN_OFFSET,
};
...
fn load(addr: Address, requires_apx: bool, next_pc: u64) -> OpKind {
...
#[test]
fn descriptor_table_helper_offsets_are_append_only_and_match_guest_layout() {
    assert_eq!(
    ...
    assert_eq!(
        std::mem::offset_of!(GuestRegs, descriptor_load_fn),
        X86_GUEST_DESCRIPTOR_LOAD_FN_OFFSET as usize
    );
    assert_eq!(
        X86_GUEST_DESCRIPTOR_LOAD_FN_OFFSET,
        X86_GUEST_DESCRIPTOR_STORE_FN_OFFSET + 8
    );
    assert_eq!(GuestRegs::default().descriptor_load_fn, 0);
}
...
#[test]
fn x86_descriptor_load_gate_requires_memory_helpers_and_accepts_exact_addresses() {
    for (addr, requires_apx, next_pc) in [
        (Address::Absolute(0x4000), false, 0x1003),
        (Address::Direct(x86(X86Reg::Rsp)), false, 0x1004),
        (
            Address::BaseIndexScale {
                base: Some(x86(X86Reg::Rbp)),
                index: x86(X86Reg::R31),
        ...
        let op = load(addr, requires_apx, next_pc);
        let function = function(op.clone());
        assert!(op.is_jit_safe(), "{op:?}");
        assert!(x86_descriptor_table_load_shape_valid(
            &function.blocks[0].ops[0]
        ));
        assert!(!gate(op.clone(), false), "{op:?}");
        assert!(gate(op, true));
...
#[test]
fn x86_descriptor_load_gate_rejects_malformed_ir_and_all_aarch64_hosts() {
    for malformed in [
        load(Address::Direct(VReg::virt(0)), false, 0x1003),
        load(Address::Direct(arm_x(0)), false, 0x1003),
        load(Address::Direct(x86(X86Reg::R31)), false, 0x1003),
        load(Address::Direct(x86(X86Reg::Rax)), false, 0x1000),
        load(Address::Direct(x86(X86Reg::Rax)), false, 0x1002),
        load(Address::Direct(x86(X86Reg::Rax)), false, 0x1010),
        load(
            Address::BaseIndexScale {
                base: Some(x86(X86Reg::Rax)),
                index: x86(X86Reg::Rcx),
    ...
    for op in [
        load(Address::Absolute(0x4000), false, 0x1003),
        load(Address::Direct(x86(X86Reg::Rsp)), false, 0x1003),
    ] {
        let function = function(op.clone());
        assert!(!is_x86_aarch64_native_clobber_safe_excluding(
            &function,
...
#[test]
fn x86_descriptor_load_survives_o2_and_remains_admitted() {
    let mut builder = FunctionBuilder::new(FunctionId(0), 0x1000);
    builder.push_op(
        0x1000,
        load(Address::Direct(x86(X86Reg::Rsp)), false, 0x1003),
    ...
    assert_eq!(
        function.blocks[0]
            .ops
            .iter()
            .filter(|op| matches!(op.kind, OpKind::X86DescriptorTableLoad(..)))
            .count(),
        2
    );
    assert!(is_native_clobber_safe_excluding(
