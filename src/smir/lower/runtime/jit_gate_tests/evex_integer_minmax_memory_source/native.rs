...
fn selected_cases() -> [IntegerMinMaxMemoryCase; 16] {
    std::array::from_fn(|index| {
        let kind = MinMaxKind::ALL[index];
        let width = if index == 10 {
            // Exercise the complete 64-bit opmask with byte lane 63 active.
            VecWidth::V512
        } else {
            match index % 3 {
                0 => VecWidth::V128,
        ...
        let form = if kind.allows_broadcast() && index % 2 != 0 {
            SourceForm::Broadcast
        } else {
            SourceForm::Vector
        };
        let control = match index % 3 {
            0 => MaskControl::None,
            1 => MaskControl::Merge,
            _ => MaskControl::Zero,
        };
        IntegerMinMaxMemoryCase {
            ...
            form,
            control,
            wig_w: kind.is_wig() && index % 2 != 0,
        }
    })
}

fn applicable_mask(case: IntegerMinMaxMemoryCase, state: &SemanticState) -> u64 {
    if case.mask() == 0 {
        return u64::MAX;
    }
...
fn scalar_addresses(case: IntegerMinMaxMemoryCase, state: &SemanticState) -> Vec<u64> {
...
#[test]
fn native_integer_minmax_matches_interpreter_helpers_and_precise_type_e4_faults() {
    use super::super::vex_fma3_memory_source::{VectorMemoryContext, vector_load_helper};

    if !std::is_x86_feature_detected!("avx512f") || !std::is_x86_feature_detected!("avx512bw") {
        eprintln!("skipping native EVEX integer min/max differential: host lacks AVX-512F/BW");
        return;
    }
