...
pub(super) fn initial_state(case: IntegerMinMaxMemoryCase, ordinal: usize) -> SemanticState {
...
pub(super) fn memory_bytes(case: IntegerMinMaxMemoryCase, ordinal: usize) -> [u8; 64] {
...
fn lane_mask(elem: VecElementType) -> u64 {
    match elem {
        ...
        _ => unreachable!("EVEX integer min/max element"),
    }
}
...
fn minmax_lane(kind: MinMaxKind, source1: u64, source2: u64) -> u64 {
    let choose_source1 = if kind.signed {
        let source1 = signed_value(source1, kind.elem);
        let source2 = signed_value(source2, kind.elem);
        if kind.minimum {
            source1 < source2
        } else {
            source1 > source2
    ...
    if choose_source1 { source1 } else { source2 }
}
...
#[test]
fn all_864_integer_minmax_cells_match_manual_semantics_at_o0_o1_o2() {
    let cases = all_cases();
