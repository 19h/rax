...
fn assert_exact_pair(ops: &[SmirOp], case: InterleaveMemoryCase) {
    ...
    let OpKind::VInterleave {
        ...
        panic!("{case:?}: expected VInterleave, got {consumer:?}")
    };
...
fn classifier_and_lowerer_fail_closed_for_every_pair_invariant() {
    ...
    assert_mutation_rejected(&base, "consumer bypasses loaded value", |function| {
        if let OpKind::VInterleave { src2, .. } = &mut function.blocks[0].ops[1].kind {
            *src2 = vector(2, VecWidth::V128);
        }
    });
    assert_mutation_rejected(&base, "wrong interleave element", |function| {
        if let OpKind::VInterleave { elem, .. } = &mut function.blocks[0].ops[1].kind {
            *elem = VecElementType::I16;
        }
    });
    assert_mutation_rejected(&base, "wrong interleave lane count", |function| {
        if let OpKind::VInterleave { lanes, .. } = &mut function.blocks[0].ops[1].kind {
            *lanes -= 1;
        }
    });
    assert_mutation_rejected(&base, "wrong interleave block size", |function| {
        if let OpKind::VInterleave { block_lanes, .. } = &mut function.blocks[0].ops[1].kind {
            *block_lanes -= 1;
        }
    });
    assert_mutation_rejected(&base, "wrong interleave half", |function| {
        if let OpKind::VInterleave { high, .. } = &mut function.blocks[0].ops[1].kind {
            *high = false;
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only first source", |function| {
        if let OpKind::VInterleave { src1, .. } = &mut function.blocks[0].ops[1].kind {
            *src1 = vector(16, VecWidth::V128);
        }
    });
    assert_mutation_rejected(&base, "high EVEX-only destination", |function| {
        if let OpKind::VInterleave { dst, .. } = &mut function.blocks[0].ops[1].kind {
    ...
    let mut bytes = case.bytes();
    bytes[3] = 0x62;
    assert_mutation_rejected(&base, "encoded interleave-half mismatch", |function| {
        replace_instruction_bytes(function, &bytes);
    });
    let mut bytes = case.bytes();
...
fn model_result(case: InterleaveMemoryCase, source1: [u64; 8], source2: [u64; 8]) -> [u64; 8] {
    ...
    let mut result = [0; 64];
    let block_lanes = 16usize / case.kind.elem.bytes() as usize;
    let blocks = case.width.bytes() as usize / 16;
    for block in 0..blocks {
        let output_base = block * block_lanes;
        let input_base = output_base + if case.kind.high { block_lanes / 2 } else { 0 };
        for lane in 0..block_lanes / 2 {
            write_lane(
                &mut result,
                case.kind.elem,
                output_base + lane * 2,
                read_lane(&source1, case.kind.elem, input_base + lane),
            );
            write_lane(
                &mut result,
                case.kind.elem,
                output_base + lane * 2 + 1,
                read_lane(&source2, case.kind.elem, input_base + lane),
            );
        }
    }
