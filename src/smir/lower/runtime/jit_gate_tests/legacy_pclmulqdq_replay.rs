//! Native replay coverage for register-only legacy `PCLMULQDQ`.
//!
//! Encoding, qword selection, upper-state preservation, and lack of rFLAGS or
//! MXCSR effects follow Intel SDM Order No. 325383-092US (June 2026), Vol. 2B,
//! pp. 4-242--4-244, and AMD APM 26568 Rev. 3.26 (January 2026), pp. 435--437.
...
#[test]
fn feature_requirements_select_pclmulqdq_and_avx_ymm16_state() {
    ...
    assert_eq!(
        ...
        std::is_x86_feature_detected!("pclmulqdq") && std::is_x86_feature_detected!("avx")
    );
...
fn assert_exact_replay_without_upper_clear(code: &[u8], bytes: &[u8]) {
    ...
    // mov rax,[rbp+state]. Legacy PCLMULQDQ preserves YMM[255:128].
    assert!(
...
fn admission_fails_closed_for_missing_mismatched_memory_and_prefixed_provenance() {
    ...
    let memory_bytes = [0x66, 0x0F, 0x3A, 0x44, 0x00, 0xA5];
...
#[cfg(target_arch = "x86_64")]
fn initial_state(case: NativeCase) -> PclmulqdqState {
    PclmulqdqState {
...
#[cfg(target_arch = "x86_64")]
fn expected_state(case: NativeCase, initial: &PclmulqdqState) -> PclmulqdqState {
    let mut expected = initial.clone();
    let lhs = initial.vectors[case.destination()][usize::from(case.immediate & 1)];
    let rhs = initial.vectors[case.source()][usize::from((case.immediate >> 4) & 1)];
    let (low, high) = carryless_multiply(lhs, rhs);
    expected.vectors[case.destination()][0] = low;
    expected.vectors[case.destination()][1] = high;
    expected
}

#[cfg(target_arch = "x86_64")]
fn interpret(case: NativeCase, initial: &PclmulqdqState) -> PclmulqdqState {
...
#[cfg(target_arch = "x86_64")]
fn execute_native(case: NativeCase, initial: &PclmulqdqState) -> PclmulqdqState {
    use crate::smir::lower::SmirLowerer;
    ...
    let code = lowerer
        ...
        .unwrap_or_else(|error| panic!("{case:?} {bytes:02X?}: {error:?}"));
    assert_exact_replay_without_upper_clear(&code, &bytes);
    let exec = ExecMem::new(&code).expect("map legacy PCLMULQDQ replay");
...
#[cfg(target_arch = "x86_64")]
const CHILD_RANGE_ENV: &str = "RAX_LEGACY_PCLMULQDQ_CHILD_RANGE";
...
fn run_child_range(test_name: &str, range: std::ops::Range<usize>) -> std::process::Output {
    std::process::Command::new(std::env::current_exe().expect("current unit-test executable"))
        ...
        .output()
        .expect("run isolated native legacy PCLMULQDQ differential")
}
...
fn run_isolated_native_differential(test_name: &str) {
    ...
    panic!(
        "isolated native legacy PCLMULQDQ failure at case {start}/{}: \
...
fn all_13056_native_cases_match_o0_o1_o2_interpretation_equation_and_full_state() {
    if !std::is_x86_feature_detected!("pclmulqdq") || !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native legacy PCLMULQDQ differential: host lacks PCLMULQDQ or AVX");
        return;
    }
    run_isolated_native_differential(
        "smir::lower::runtime::jit_gate_tests::legacy_pclmulqdq_replay::\
