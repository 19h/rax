//! Native replay coverage for register-destination AVX/AVX2 VEX 128-bit
//! chunk extracts.
...
impl ExtractKind {
    fn opcode(self) -> u8 {
        if self == Self::Integer { 0x39 } else { 0x19 }
    }
...
fn encoding(case: ExtractCase) -> [u8; 6] {
    ...
    [
        ...
        0x7D,
        case.kind.opcode(),
        0xC0 | ((case.source & 7) << 3) | (case.destination & 7),
        case.immediate,
    ]
}
...
fn cases() -> Vec<ExtractCase> {
    ...
    for kind in [ExtractKind::Floating, ExtractKind::Integer] {
...
#[test]
fn replay_admits_and_emits_240_optimized_representatives_and_fails_closed() {
    ...
    assert_eq!(lowered, 240);

    let case = ExtractCase {
        kind: ExtractKind::Integer,
        ignored_x: true,
        destination: 9,
        source: 11,
        immediate: 0xFE,
...
#[test]
fn ymm16_replay_clears_the_exact_rm_destination_state_backed_upper_half() {
    use crate::smir::lower::SmirLowerer;
    use crate::smir::lower::x86_64::X86_64Lowerer;

    for case in [
        ExtractCase {
            kind: ExtractKind::Floating,
            ignored_x: false,
            destination: 9,
            source: 3,
            immediate: 0x7E,
...
fn architectural_expected(case: ExtractCase, initial: &ExtractState) -> ExtractState {
    ...
    let source_word = usize::from(case.immediate & 1) * 2;
    let destination = &mut expected.vectors[usize::from(case.destination)];
    destination.fill(0);
    destination[..2].copy_from_slice(&source[source_word..source_word + 2]);
    expected
}
...
#[test]
fn interpreter_matches_intel_o0_o2_equations_for_all_lanes_aliases_and_ignored_bits() {
    let cases = cases();
    assert_eq!(cases.len(), 120);
...
fn execute_native(
    ...
    let exec = ExecMem::new(&code).expect("map VEX chunk-extract replay");
...
#[cfg(target_arch = "x86_64")]
const CHILD_RANGE_ENV: &str = "RAX_VEX_CHUNK_EXTRACT_CHILD_RANGE";
...
#[cfg(target_arch = "x86_64")]
fn native_cases() -> Vec<ExtractCase> {
...
fn run_child_range(test_name: &str, range: std::ops::Range<usize>) -> std::process::Output {
    std::process::Command::new(std::env::current_exe().expect("current unit-test executable"))
        ...
        .output()
        .expect("run isolated native VEX chunk-extract differential")
}
...
fn run_isolated_native_differential(test_name: &str) {
    ...
    let bytes = encoding(case);
    panic!(
        "isolated native VEX chunk-extract failure at case {start}/{}: \
...
#[cfg(target_arch = "x86_64")]
#[test]
fn replay_matches_intel_o0_o2_state_for_all_lanes_aliases_and_ignored_bits() {
    if !std::is_x86_feature_detected!("avx") {
        eprintln!("skipping native VEX chunk-extract differential: host lacks AVX");
        return;
    }
    run_isolated_native_differential(
        "smir::lower::runtime::jit_gate_tests::vex_chunk_extract_replay::\
         replay_matches_intel_o0_o2_state_for_all_lanes_aliases_and_ignored_bits",
    );
}
