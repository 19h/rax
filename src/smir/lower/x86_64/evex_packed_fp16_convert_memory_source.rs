//! Helper-backed packed AVX-512-FP16 conversion memory lowering.
...
use crate::smir::ir::X86EvexPackedFp16ConvertMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_packed_fp16_convert_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedFp16ConvertMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated FP16 conversion broadcast owns its scalar memory op"),
        };
        {
    ...
    /// Fuse one exact packed AVX-512-FP16 conversion memory source.
    ///
    /// Unmasked vectors perform the exact 4-/8-/16-/32-/64-byte helper read
    /// into the reserved vector transfer slot and execute a byte-validated
    /// register rewrite. Broadcasts issue at most one 2-/4-/8-byte helper
    /// read. Writemasked vectors issue ascending per-active-lane helper reads
    /// and execute only after the complete source is staged, so a helper fault
    /// cannot commit destination or MXCSR state.
    pub(crate) fn try_lower_jit_evex_packed_fp16_convert_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_fp16_convert_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "packed AVX-512-FP16 conversion memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512-FP16 state".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexPackedFp16ConvertMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    OpKind::VLoad { addr, .. } | OpKind::Lea { addr, .. } => addr,
                    _ => unreachable!("validated FP16 conversion vector owns its memory address"),
                };
            ...
            X86EvexPackedFp16ConvertMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_packed_fp16_convert_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedFp16ConvertMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    OpKind::Lea { addr, .. } => addr,
                    _ => unreachable!("validated masked FP16 conversion owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
                ...
                let mask = sequence
                    .encoding
                    .writemask
                    .expect("validated masked-vector FP16 conversion");
                let source_elem = sequence.encoding.kind.source_elem();
