//! Helper-backed EVEX packed binary32/binary64 arithmetic memory lowering.

use std::collections::HashMap;

use super::{X86_64Lowerer, X86Cond, X86Emitter};
use crate::smir::ir::SmirBlock;
use crate::smir::ir::X86EvexPackedFpArithmeticMemoryReplay;
...
impl X86_64Lowerer {
    fn evex_fp_arithmetic_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
            VecWidth::V128 => PhysReg::Xmm(index),
            VecWidth::V256 => PhysReg::Ymm(index),
            VecWidth::V512 => PhysReg::Zmm(index),
            _ => unreachable!("validated packed binary32/binary64 width"),
        }
    }

    fn evex_fp_arithmetic_element_widths(
        elem: VecElementType,
        ...
        match elem {
            ...
            _ => Err(LowerError::InvalidOperand {
                op: "EVEX packed FP arithmetic memory source".to_string(),
    ...
    fn emit_evex_masked_fp_arithmetic_lane_helper(
        &mut self,
        guest_pc: u64,
        address: &Address,
        mask: u8,
        ...
        let lane_offset = i32::try_from(lane).expect("at most 16 FP lanes") * lane_bytes;
        ...
        // one 4/8-byte architectural lane into the payload. MOV preserves
        // guest flags; the saved guest RAX is restored after staging.
        self.code.emit_u8(0x50); // push guest RAX
        {
            let mut emitter = X86Emitter::new(&mut self.code);
            emitter.emit_mov_rm(
    ...
    fn emit_evex_fp_arithmetic_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedFpArithmeticMemorySequence,
        stack_instruction: crate::smir::ir::X86InstructionBytes,
    ) -> Result<(), LowerError> {
        let (memory_width, _, _) = Self::evex_fp_arithmetic_element_widths(sequence.encoding.elem)?;
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated FP broadcast sequence owns its scalar memory op"),
        };
        {
            let mut emitter = X86Emitter::new(&mut self.code);
    ...
    /// Fuse one exact packed AVX-512 binary32/binary64 arithmetic memory
    /// decomposition.
    ///
    ...
    /// active lanes, accumulate them outside architectural state, and commit
    /// the destination once every active load succeeds. Any helper fault exits
    /// at the source guest PC before arithmetic or destination modification.
    pub(crate) fn try_lower_jit_evex_packed_fp_arithmetic_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_fp_arithmetic_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX packed FP arithmetic memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 arithmetic".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexPackedFpArithmeticMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated FP vector sequence starts with VLoad"),
                };
                ...
                let scratch_reg =
                    Self::evex_fp_arithmetic_phys_reg(scratch, sequence.encoding.width);
                self.code.emit_u8(0x50); // push guest RAX
            ...
            X86EvexPackedFpArithmeticMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_fp_arithmetic_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedFpArithmeticMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    OpKind::Lea { addr, .. } => addr,
                    _ => unreachable!("validated masked FP vector sequence owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                ...
                let (memory_width, copy_width, lane_bytes) =
                    Self::evex_fp_arithmetic_element_widths(sequence.encoding.elem)?;
                for lane in 0..lanes {
                    self.emit_evex_masked_fp_arithmetic_lane_helper(
                        block.ops[index].guest_pc,
                        address,
                        mask,
                        lane,
