//! Helper-backed VEX `VPALIGNR` memory-source lowering.
...
impl X86_64Lowerer {
    fn vex_alignr_memory_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
    ...
    /// Fuse the exact semantic decomposition for one VEX `VPALIGNR` memory
    /// source. The MMU helper commits only the nonarchitectural vector transfer
    /// slot. A byte-validated register-source instruction consumes that value
    /// from a borrowed low vector register, which is restored completely
    /// before native execution continues.
    pub(crate) fn try_lower_jit_vex_alignr_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) = crate::smir::lower::runtime::x86_jit_vex_alignr_memory_sequence(
            block,
            index,
            true,
        ...
        let address = match &block.ops[index].kind {
            ...
            _ => unreachable!("validated VEX VPALIGNR sequence starts with VLoad"),
        };
        ...
        let scratch = Self::vex_alignr_memory_phys_reg(scratch_index, sequence.width);
        let destination = Self::vex_alignr_memory_phys_reg(sequence.destination, sequence.width);
        let source1 = Self::vex_alignr_memory_phys_reg(sequence.source1, sequence.width);
