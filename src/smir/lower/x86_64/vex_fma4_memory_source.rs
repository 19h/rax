//! Helper-backed AMD VEX FMA4 memory-source lowering.
...
impl X86_64Lowerer {
    fn vex_fma4_memory_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
    ...
    /// Fuse one exact scalar or packed FMA4 memory-source decomposition.
    ///
    ...
    /// continuation. Guest MXCSR remains active for the fused operation, and
    /// the enclosing vector trampoline captures accrued status.
    pub(crate) fn try_lower_jit_vex_fma4_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) = crate::smir::lower::runtime::x86_jit_vex_fma4_memory_sequence(
            block,
            index,
            true,
        ...
        let address = match &block.ops[index].kind {
            ...
            _ => unreachable!("validated FMA4 sequence starts with a memory load"),
        };
        ...
        let scratch =
            Self::vex_fma4_memory_phys_reg(sequence.encoding.scratch, sequence.encoding.width);
