//! Helper-backed EVEX packed-integer comparison/test memory lowering.
...
use crate::smir::ir::X86EvexPackedIntegerMaskMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_integer_mask_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedIntegerMaskMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated integer-mask broadcast owns its scalar memory op"),
        };
        {
        ...
        let inactive = if let Some(mask) = sequence.encoding.writemask {
            ...
            debug_assert!(lanes < 64, "only dword/quadword forms permit broadcast");
    ...
    /// Fuse one exact EVEX packed-integer comparison/test memory
    /// decomposition.
    ///
    ...
    /// one scalar helper access. Writemasked vectors issue ascending 1/2/4/8-
    /// byte helper loads only for active lanes and execute the native operation
    /// only after every load succeeds. A helper fault therefore leaves the K
    /// destination unchanged.
    pub(crate) fn try_lower_jit_evex_packed_integer_mask_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_integer_mask_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "packed EVEX integer comparison/test memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 comparison/test".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexPackedIntegerMaskMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated integer-mask vector sequence starts with VLoad"),
                };
            ...
            X86EvexPackedIntegerMaskMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_integer_mask_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedIntegerMaskMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated masked integer comparison/test owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
                ...
                let mask = sequence
                    .encoding
                    .writemask
                    .expect("validated masked-vector integer-mask replay");
