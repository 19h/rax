//! Helper-backed packed EVEX floating-point comparison memory lowering.
...
use crate::smir::ir::X86EvexPackedFpCompareMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_fp_compare_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedFpCompareMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated FP comparison broadcast owns its scalar memory op"),
        };
        {
    ...
    /// Fuse one exact packed EVEX floating-point comparison memory
    /// decomposition.
    ///
    ...
    /// one scalar helper access. Writemasked vectors issue ascending 2/4/8-byte
    /// helper loads only for active lanes and execute the comparison only after
    /// every load succeeds. A helper fault therefore leaves K destination and
    /// MXCSR unchanged; successful replay retains native IE/DE status updates.
    pub(crate) fn try_lower_jit_evex_packed_fp_compare_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_fp_compare_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "packed EVEX FP comparison memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 comparison".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexPackedFpCompareMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated FP comparison vector sequence starts with VLoad"),
                };
            ...
            X86EvexPackedFpCompareMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_fp_compare_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedFpCompareMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated masked FP comparison owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
                ...
                let mask = sequence
                    .encoding
                    .writemask
                    .expect("validated masked-vector comparison replay");
