//! Helper-backed EVEX VPSHUFD/VPSHUFHW/VPSHUFLW memory lowering.

use std::collections::HashMap;

use super::{X86_64Lowerer, X86Emitter};
use crate::smir::ir::SmirBlock;
use crate::smir::ir::X86EvexLaneShuffleMemoryReplay;
...
impl X86_64Lowerer {
    fn evex_lane_shuffle_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
    ...
    fn emit_evex_lane_shuffle_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexLaneShuffleMemorySequence,
        ...
        let address = match &block.ops[index + sequence.address_offset].kind {
            ...
            _ => unreachable!("validated EVEX lane shuffle owns its dword load"),
        };
        {
    ...
    /// Fuse one exact EVEX VPSHUFD/VPSHUFHW/VPSHUFLW memory decomposition.
    ///
    /// Type E4NF/E4NF.nb requires one complete 16/32/64-byte vector or 4-byte
    ...
    /// unused low vector register; dword broadcasts replay from `[rsp]{1toN}`.
    pub(crate) fn try_lower_jit_evex_lane_shuffle_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) = crate::smir::lower::runtime::x86_jit_evex_lane_shuffle_memory_sequence(
            block,
            index,
            true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX packed lane-shuffle memory source".to_string(),
        ...
        match sequence.encoding.replay {
            X86EvexLaneShuffleMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated EVEX lane shuffle sequence starts with VLoad"),
                };
                ...
                let scratch_reg =
                    Self::evex_lane_shuffle_phys_reg(scratch, sequence.encoding.width);
            ...
            X86EvexLaneShuffleMemoryReplay::Broadcast {
                memory_width,
                stack_instruction,
            } => self.emit_evex_lane_shuffle_broadcast_replay(
                block,
                index,
                sequence,
                memory_width,
