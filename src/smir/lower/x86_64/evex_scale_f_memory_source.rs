//! Helper-backed EVEX VSCALEFPD/PS/PH/SD/SS/SH memory lowering.
...
use crate::smir::ir::X86EvexScaleFMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_scale_f_stack_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexScaleFMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated VSCALEF stack replay owns its scalar memory op"),
        };
        {
    ...
    /// Fuse one exact packed or scalar AVX-512 VSCALEF memory decomposition.
    ///
    ...
    /// `[rsp]` memory form. Writemasked packed vectors issue ascending
    /// 2/4/8-byte helper loads only for active lanes. Every helper completes
    /// before native VSCALEF executes, so a fault exits at the source guest PC
    /// without changing the destination or MXCSR; successful replay retains
    /// native dynamic rounding and sticky exception reporting.
    pub(crate) fn try_lower_jit_evex_scale_f_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) = crate::smir::lower::runtime::x86_jit_evex_scale_f_memory_sequence(
            block,
            index,
            true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX VSCALEF memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 VSCALEF".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexScaleFMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated VSCALEF vector sequence starts with VLoad"),
                };
            ...
            | X86EvexScaleFMemoryReplay::Scalar { stack_instruction } => {
                self.emit_evex_scale_f_stack_replay(block, index, sequence, stack_instruction)?;
            }
            X86EvexScaleFMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    OpKind::Lea { addr, .. } => addr,
                    _ => unreachable!("validated masked VSCALEF vector sequence owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
                ...
                let mask = sequence
                    .encoding
                    .writemask
                    .expect("validated masked-vector VSCALEF replay");
