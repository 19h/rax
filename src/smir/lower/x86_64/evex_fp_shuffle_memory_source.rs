//! Helper-backed EVEX VSHUFPS/VSHUFPD memory lowering.

use std::collections::HashMap;

use super::{X86_64Lowerer, X86Emitter};
use crate::smir::ir::SmirBlock;
use crate::smir::ir::X86EvexFpShuffleMemoryReplay;
...
impl X86_64Lowerer {
    fn evex_fp_shuffle_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
    ...
    fn emit_evex_fp_shuffle_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexFpShuffleMemorySequence,
        ...
        let address = match &block.ops[index + sequence.address_offset].kind {
            ...
            _ => unreachable!("validated EVEX floating shuffle owns its scalar load"),
        };
        {
    ...
    /// Fuse one exact EVEX VSHUFPS/VSHUFPD memory decomposition.
    ///
    ...
    pub(crate) fn try_lower_jit_evex_fp_shuffle_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) = crate::smir::lower::runtime::x86_jit_evex_fp_shuffle_memory_sequence(
            block,
            index,
            true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX floating-point shuffle memory source".to_string(),
        ...
        match sequence.encoding.replay {
            X86EvexFpShuffleMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated EVEX floating shuffle sequence starts with VLoad"),
                };
                ...
                let scratch_reg = Self::evex_fp_shuffle_phys_reg(scratch, sequence.encoding.width);
            ...
            X86EvexFpShuffleMemoryReplay::Broadcast {
                memory_width,
                stack_instruction,
            } => self.emit_evex_fp_shuffle_broadcast_replay(
                block,
                index,
                sequence,
                memory_width,
