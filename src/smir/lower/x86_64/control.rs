...
impl X86_64Lowerer {
    ...
    pub(crate) fn lower_block(&mut self, block: &SmirBlock) -> Result<(), LowerError> {
        ...
        while idx < end_idx {
            ...
            if self.mem_helpers {
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_vpcom_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_vbit_select_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_xop_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_evex_logic_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_evex_broadcast_interleave_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_vex_fma4_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
                ...
                #[cfg(feature = "smir-jit")]
                if let Some(consumed) = self.try_lower_jit_vex_vpermil2_memory_source(
                    block,
                    idx,
                    &virtual_definitions,
                    &virtual_uses,
