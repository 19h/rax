//! Helper-backed EVEX packed funnel-shift memory lowering.

use std::collections::HashMap;

use super::{X86_64Lowerer, X86Cond, X86Emitter};
use crate::smir::ir::SmirBlock;
use crate::smir::ir::X86EvexPackedFunnelShiftMemoryReplay;
...
impl X86_64Lowerer {
    fn evex_packed_funnel_shift_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
            VecWidth::V128 => PhysReg::Xmm(index),
            VecWidth::V256 => PhysReg::Ymm(index),
            VecWidth::V512 => PhysReg::Zmm(index),
            _ => unreachable!("validated packed funnel-shift width"),
        }
    }

    fn evex_packed_funnel_shift_element_widths(
        elem: VecElementType,
        ...
        match elem {
            ...
            _ => Err(LowerError::InvalidOperand {
                op: "EVEX packed funnel-shift memory source".to_string(),
    ...
    fn emit_evex_masked_packed_funnel_shift_lane_helper(
        &mut self,
        guest_pc: u64,
        address: &Address,
        mask: u8,
        ...
        let lane_offset = i32::try_from(lane).expect("at most 32 funnel-shift lanes") * lane_bytes;
    ...
    fn emit_evex_packed_funnel_shift_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedFunnelShiftMemorySequence,
        stack_instruction: crate::smir::ir::X86InstructionBytes,
    ) -> Result<(), LowerError> {
        let (memory_width, _, _) =
            Self::evex_packed_funnel_shift_element_widths(sequence.encoding.elem)?;
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated funnel-shift broadcast owns its scalar memory op"),
        };
        {
            let mut emitter = X86Emitter::new(&mut self.code);
    ...
    /// Fuse one exact packed AVX-512 VBMI2 funnel-shift memory decomposition.
    ///
    /// Unmasked vectors use the reserved nonarchitectural vector transfer slot
    /// and a byte-validated register rewrite. Broadcasts issue at most one
    /// scalar helper access and replay an exact `[rsp]{1toN}` form.
    /// Writemasked vectors issue ascending 2/4/8-byte helper loads only for
    /// active lanes and commit the destination after every load succeeds.
    pub(crate) fn try_lower_jit_evex_packed_funnel_shift_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_funnel_shift_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX packed funnel-shift memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 funnel shifts".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexPackedFunnelShiftMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated funnel-shift vector sequence starts with VLoad"),
                };
                ...
                let scratch_reg =
                    Self::evex_packed_funnel_shift_phys_reg(scratch, sequence.encoding.width);
                self.code.emit_u8(0x50); // push guest RAX
            ...
            X86EvexPackedFunnelShiftMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_packed_funnel_shift_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedFunnelShiftMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => {
                        unreachable!("validated masked funnel-shift vector sequence owns its LEA")
                    }
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                ...
                let (memory_width, copy_width, lane_bytes) =
                    Self::evex_packed_funnel_shift_element_widths(sequence.encoding.elem)?;
                for lane in 0..lanes {
                    self.emit_evex_masked_packed_funnel_shift_lane_helper(
                        block.ops[index].guest_pc,
                        address,
                        mask,
                        lane,
