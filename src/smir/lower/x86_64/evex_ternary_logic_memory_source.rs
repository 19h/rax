//! Helper-backed EVEX VPTERNLOGD/Q memory lowering.
...
use crate::smir::ir::X86EvexTernaryLogicMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_ternary_logic_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexTernaryLogicMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated ternary-logic broadcast owns its scalar memory op"),
        };
        {
    ...
    /// Fuse one exact VPTERNLOGD/Q memory-source decomposition.
    ///
    ...
    pub(crate) fn try_lower_jit_evex_ternary_logic_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_ternary_logic_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX ternary-logic memory source".to_string(),
                operand: "AVX-only vector bridge cannot carry AVX-512 ternary logic".to_string(),
            });
        }
        ...
        match sequence.encoding.replay {
            X86EvexTernaryLogicMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated ternary-logic vector sequence starts with VLoad"),
                };
            ...
            X86EvexTernaryLogicMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_ternary_logic_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexTernaryLogicMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated masked ternary logic owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
