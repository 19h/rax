//! Helper-backed EVEX packed per-element variable-shift memory lowering.
...
use crate::smir::ir::X86EvexPackedVariableShiftMemoryReplay;
...
impl X86_64Lowerer {
    fn emit_evex_packed_variable_shift_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexPackedVariableShiftMemorySequence,
        ...
        let address = match &block.ops[address_index].kind {
            ...
            _ => unreachable!("validated variable-shift broadcast owns its scalar memory op"),
        };
        {
    ...
    /// Fuse one exact packed AVX-512 per-element variable-shift memory
    /// decomposition.
    ///
    ...
    pub(crate) fn try_lower_jit_evex_packed_variable_shift_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_packed_variable_shift_memory_sequence(
                block,
                index,
                true,
        ...
        if self.avx_ymm16_vector_state {
            return Err(LowerError::InvalidOperand {
                op: "EVEX packed variable-shift memory source".to_string(),
        ...
        match sequence.encoding.replay {
            X86EvexPackedVariableShiftMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated variable-shift vector sequence starts with VLoad"),
                };
            ...
            X86EvexPackedVariableShiftMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_packed_variable_shift_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexPackedVariableShiftMemoryReplay::MaskedVector { stack_instruction } => {
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated masked variable shift owns its LEA"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                    emitter.emit_lea(
