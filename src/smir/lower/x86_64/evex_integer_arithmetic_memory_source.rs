...
impl X86_64Lowerer {
    fn evex_integer_arithmetic_phys_reg(index: u8, width: VecWidth) -> PhysReg {
        match width {
            VecWidth::V128 => PhysReg::Xmm(index),
            VecWidth::V256 => PhysReg::Ymm(index),
            VecWidth::V512 => PhysReg::Zmm(index),
            _ => unreachable!("validated EVEX packed integer arithmetic width"),
        }
    }

    fn evex_integer_arithmetic_element_widths(
        elem: VecElementType,
        ...
        match elem {
            ...
            _ => Err(LowerError::InvalidOperand {
                op: "EVEX packed integer arithmetic memory source".to_string(),
    ...
    fn emit_evex_integer_arithmetic_lane_helper(
        &mut self,
        guest_pc: u64,
        address: &Address,
        mask: u8,
    ...
    fn emit_evex_integer_arithmetic_broadcast_replay(
        &mut self,
        block: &SmirBlock,
        index: usize,
        sequence: crate::smir::lower::runtime::X86JitEvexIntegerArithmeticMemorySequence,
        stack_instruction: crate::smir::ir::X86InstructionBytes,
    ) -> Result<(), LowerError> {
        let (memory_width, _, _) =
            Self::evex_integer_arithmetic_element_widths(sequence.encoding.elem)?;
    ...
    /// native arithmetic/destination commit until every required access
    /// succeeds.
    pub(crate) fn try_lower_jit_evex_integer_arithmetic_memory_source(
        &mut self,
        block: &SmirBlock,
        index: usize,
        ...
        let Some(sequence) =
            crate::smir::lower::runtime::x86_jit_evex_integer_arithmetic_memory_sequence(
                block,
                index,
                true,
        ...
        match sequence.encoding.replay {
            X86EvexIntegerArithmeticMemoryReplay::Vector {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated integer vector sequence starts with VLoad"),
                };
                ...
                let scratch_reg =
                    Self::evex_integer_arithmetic_phys_reg(scratch, sequence.encoding.width);
                self.code.emit_u8(0x50); // push guest RAX
            ...
            X86EvexIntegerArithmeticMemoryReplay::Broadcast { stack_instruction } => {
                self.emit_evex_integer_arithmetic_broadcast_replay(
                    block,
                    index,
                    sequence,
                    stack_instruction,
            ...
            X86EvexIntegerArithmeticMemoryReplay::MaskedVector { stack_instruction } => {
                ...
                let address = match &block.ops[index + sequence.address_offset].kind {
                    ...
                    _ => unreachable!("validated masked integer sequence owns its address"),
                };
                {
                    let mut emitter = X86Emitter::new(&mut self.code);
                ...
                for lane in 0..lanes {
                    self.emit_evex_integer_arithmetic_lane_helper(
                        block.ops[index].guest_pc,
                        address,
                        mask,
                        lane,
