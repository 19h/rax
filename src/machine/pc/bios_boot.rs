...
/// Parse + load an El-Torito ISO's boot image into `mem` and return the
/// per-VM real-mode CPU/BIOS state that the VMM will attach to the boot vCPU.
pub fn arm_real_mode_boot(mem: &GuestMemoryMmap, iso: Vec<u8>) -> Result<RealModeBoot, String> {
    setup_real_mode_boot(mem, iso)
}
