...
use rax::vm::runtime::Vmm;
use rax::vm::snapshot::Snapshot;
...
pub fn run() -> Result<()> {
    ...
    let filter = if cli.gdb_trace {
        base_filter.add_directive("rax::debug::gdb=trace".parse().unwrap())
    } else {
        base_filter
    };
    ...
    if config.profile {
        let profiling_config = rax::observability::profiling::ProfilingConfig {
            track_memory: true,
            live_interval: config.profile_interval.unwrap_or(10_000_000),
            json_path: config.profile_output.clone(),
        };
        rax::observability::profiling::init(profiling_config);
    }
    ...
    #[cfg(feature = "profiling")]
    {
        ctrlc::set_handler(move || {
            ...
            rax::observability::profiling::shutdown();
            std::process::exit(0);
        })
