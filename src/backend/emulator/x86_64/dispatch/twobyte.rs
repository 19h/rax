...
impl X86_64Vcpu {
    ...
    /// Execute 0x0F 0x38 opcodes (3-byte escape - SSSE3, SSE4.1, AES-NI)
        ...
        match opcode3 {
            ...
            0x00 => {
                ...
                if ctx.operand_size_override {
                    ...
                    self.regs.xmm[xmm_dst][0] = u64::from_le_bytes(result[0..8].try_into().unwrap());
                    self.regs.xmm[xmm_dst][1] = u64::from_le_bytes(result[8..16].try_into().unwrap());
                } else {
                    ...
                    self.regs.mm[mm_dst] = u64::from_le_bytes(result);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x01 => {
                ...
                if ctx.operand_size_override {
                    ...
                    self.regs.xmm[xmm_dst][0] = (d[0] as u64) | ((d[1] as u64) << 16) | ((d[2] as u64) << 32) | ((d[3] as u64) << 48);
                    self.regs.xmm[xmm_dst][1] = (d[4] as u64) | ((d[5] as u64) << 16) | ((d[6] as u64) << 32) | ((d[7] as u64) << 48);
                } else {
                    // MMX version
                    let mm_dst = reg as usize & 0x7;
                    let src = if is_memory { self.read_mem(addr, 8)? } else { self.regs.mm[rm as usize & 0x7] };
                    let dst = self.regs.mm[mm_dst];
                    let d = [
                    ...
                    self.regs.mm[mm_dst] = (d[0] as u64) | ((d[1] as u64) << 16) | ((d[2] as u64) << 32) | ((d[3] as u64) << 48);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x02 => {
                ...
                if ctx.operand_size_override {
                    ...
                    self.regs.xmm[xmm_dst][0] = (d[0] as u64) | ((d[1] as u64) << 32);
                    self.regs.xmm[xmm_dst][1] = (d[2] as u64) | ((d[3] as u64) << 32);
                } else {
                    let mm_dst = reg as usize & 0x7;
                    let src = if is_memory { self.read_mem(addr, 8)? } else { self.regs.mm[rm as usize & 0x7] };
                    let dst = self.regs.mm[mm_dst];
                    ...
                    self.regs.mm[mm_dst] = (d0 as u64) | ((d1 as u64) << 32);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x04 => {
                ...
                if ctx.operand_size_override {
                    ...
                    for i in 0..8 {
                        result[i] = maddubs(dst_bytes[i*2], src_bytes[i*2] as i8,
                                           dst_bytes[i*2+1], src_bytes[i*2+1] as i8) as u16;
                    }
                    self.regs.xmm[xmm_dst][0] = (result[0] as u64) | ((result[1] as u64) << 16) | ((result[2] as u64) << 32) | ((result[3] as u64) << 48);
                    self.regs.xmm[xmm_dst][1] = (result[4] as u64) | ((result[5] as u64) << 16) | ((result[6] as u64) << 32) | ((result[7] as u64) << 48);
                } else {
                    let mm_dst = reg as usize & 0x7;
                    let src = if is_memory { self.read_mem(addr, 8)? } else { self.regs.mm[rm as usize & 0x7] };
                    let dst = self.regs.mm[mm_dst];
                    ...
                    self.regs.mm[mm_dst] = (result[0] as u64) | ((result[1] as u64) << 16) | ((result[2] as u64) << 32) | ((result[3] as u64) << 48);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x0A => {
                ...
                if ctx.operand_size_override {
                    ...
                    let d0 = sign_dword(dst_lo as u32, src_lo as i32);
                    let d1 = sign_dword((dst_lo >> 32) as u32, (src_lo >> 32) as i32);
                    let d2 = sign_dword(dst_hi as u32, src_hi as i32);
                    let d3 = sign_dword((dst_hi >> 32) as u32, (src_hi >> 32) as i32);
                    ...
                    let src = if is_memory { self.read_mem(addr, 8)? } else { self.regs.mm[rm as usize & 0x7] };
                    let dst = self.regs.mm[mm_dst];
                    let d0 = sign_dword(dst as u32, src as i32);
                    let d1 = sign_dword((dst >> 32) as u32, (src >> 32) as i32);
                    self.regs.mm[mm_dst] = (d0 as u64) | ((d1 as u64) << 32);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            0x10 => {
                ...
                let mask_lo = self.regs.xmm[0][0];
                let mask_hi = self.regs.xmm[0][1];
                let dst_lo = self.regs.xmm[xmm_dst][0];
                let dst_hi = self.regs.xmm[xmm_dst][1];
            ...
            0x14 => {
                ...
                self.regs.xmm[xmm_dst][0] = (if m0 { s0 } else { d0 }) | ((if m1 { s1 } else { d1 }) << 32);
                self.regs.xmm[xmm_dst][1] = (if m2 { s2 } else { d2 }) | ((if m3 { s3 } else { d3 }) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x15 => {
                ...
                self.regs.xmm[xmm_dst][0] = if m0 { src_lo } else { dst_lo };
                self.regs.xmm[xmm_dst][1] = if m1 { src_hi } else { dst_hi };
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x17 => {
                ...
                self.regs.rflags &= !(flags::bits::AF | flags::bits::OF | flags::bits::PF |
                                      flags::bits::SF | flags::bits::ZF | flags::bits::CF);
                if and_result == 0 {
            ...
            0x1E => {
                ...
                if ctx.operand_size_override {
                    ...
                    let d0 = ((src_lo as u32) as i32).abs() as u32;
                    let d1 = (((src_lo >> 32) as u32) as i32).abs() as u32;
                    let d2 = ((src_hi as u32) as i32).abs() as u32;
                    let d3 = (((src_hi >> 32) as u32) as i32).abs() as u32;
                    self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                    self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                } else {
                    let mm_dst = reg as usize & 0x7;
                    let src = if is_memory { self.read_mem(addr, 8)? } else { self.regs.mm[rm as usize & 0x7] };
                    let d0 = ((src as u32) as i32).abs() as u32;
                    let d1 = (((src >> 32) as u32) as i32).abs() as u32;
                    self.regs.mm[mm_dst] = (d0 as u64) | ((d1 as u64) << 32);
                }
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            0x21 => {
                ...
                self.regs.xmm[xmm_dst][0] = (b0 as u64) | ((b1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (b2 as u64) | ((b3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x22 => {
                ...
                let src = if is_memory {
                    ...
                    self.regs.xmm[rm as usize][0] & 0xFFFF
                };
                let b0 = (src & 0xFF) as i8 as i64 as u64;
                let b1 = ((src >> 8) & 0xFF) as i8 as i64 as u64;
                self.regs.xmm[xmm_dst][0] = b0;
                self.regs.xmm[xmm_dst][1] = b1;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x23 => {
                ...
                self.regs.xmm[xmm_dst][0] = (w0 as u64) | ((w1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (w2 as u64) | ((w3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x24 => {
                ...
                self.regs.xmm[xmm_dst][0] = w0;
                self.regs.xmm[xmm_dst][1] = w1;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x28 => {
                ...
                self.regs.xmm[xmm_dst][0] = r0 as u64;
                self.regs.xmm[xmm_dst][1] = r1 as u64;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x29 => {
                ...
                let dst_lo = self.regs.xmm[xmm_dst][0];
                let dst_hi = self.regs.xmm[xmm_dst][1];
                self.regs.xmm[xmm_dst][0] = if dst_lo == src_lo { u64::MAX } else { 0 };
                self.regs.xmm[xmm_dst][1] = if dst_hi == src_hi { u64::MAX } else { 0 };
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x2A => {
                ...
                if !is_memory {
                    return Err(Error::Emulator("MOVNTDQA requires memory operand".to_string()));
                }
                let xmm_dst = reg as usize;
            ...
            0x2B => {
                ...
                let d = [
                    sat_u16(dst_lo as i32), sat_u16((dst_lo >> 32) as i32),
                    sat_u16(dst_hi as i32), sat_u16((dst_hi >> 32) as i32),
                    sat_u16(src_lo as i32), sat_u16((src_lo >> 32) as i32),
                    sat_u16(src_hi as i32), sat_u16((src_hi >> 32) as i32),
                ];
                self.regs.xmm[xmm_dst][0] = (d[0] as u64) | ((d[1] as u64) << 16) | ((d[2] as u64) << 32) | ((d[3] as u64) << 48);
                self.regs.xmm[xmm_dst][1] = (d[4] as u64) | ((d[5] as u64) << 16) | ((d[6] as u64) << 32) | ((d[7] as u64) << 48);
                self.regs.rip += ctx.cursor as u64;
            ...
            0x31 => {
                ...
                let b2 = (src >> 16) & 0xFF;
                let b3 = (src >> 24) & 0xFF;
                self.regs.xmm[xmm_dst][0] = b0 | (b1 << 32);
                self.regs.xmm[xmm_dst][1] = b2 | (b3 << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x32 => {
                ...
                self.regs.xmm[xmm_dst][0] = src & 0xFF;
                self.regs.xmm[xmm_dst][1] = (src >> 8) & 0xFF;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x33 => {
                ...
                self.regs.xmm[xmm_dst][0] = w0 | (w1 << 32);
                self.regs.xmm[xmm_dst][1] = w2 | (w3 << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x34 => {
                ...
                self.regs.xmm[xmm_dst][0] = src & 0xFFFF;
                self.regs.xmm[xmm_dst][1] = (src >> 16) & 0xFFFF;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x35 => {
                ...
                self.regs.xmm[xmm_dst][0] = src & 0xFFFFFFFF;
                self.regs.xmm[xmm_dst][1] = (src >> 32) & 0xFFFFFFFF;
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x37 => {
                ...
                let dst_lo = self.regs.xmm[xmm_dst][0];
                let dst_hi = self.regs.xmm[xmm_dst][1];
                self.regs.xmm[xmm_dst][0] = if (dst_lo as i64) > (src_lo as i64) { u64::MAX } else { 0 };
                self.regs.xmm[xmm_dst][1] = if (dst_hi as i64) > (src_hi as i64) { u64::MAX } else { 0 };
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            }
            ...
            0x39 => {
                ...
                let d0 = (dst_lo as i32).min(src_lo as i32) as u32;
                let d1 = ((dst_lo >> 32) as i32).min((src_lo >> 32) as i32) as u32;
                let d2 = (dst_hi as i32).min(src_hi as i32) as u32;
                let d3 = ((dst_hi >> 32) as i32).min((src_hi >> 32) as i32) as u32;
                self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            0x3B => {
                ...
                let d0 = (dst_lo as u32).min(src_lo as u32);
                let d1 = ((dst_lo >> 32) as u32).min((src_lo >> 32) as u32);
                let d2 = (dst_hi as u32).min(src_hi as u32);
                let d3 = ((dst_hi >> 32) as u32).min((src_hi >> 32) as u32);
                self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            0x3D => {
                ...
                let d0 = (dst_lo as i32).max(src_lo as i32) as u32;
                let d1 = ((dst_lo >> 32) as i32).max((src_lo >> 32) as i32) as u32;
                let d2 = (dst_hi as i32).max(src_hi as i32) as u32;
                let d3 = ((dst_hi >> 32) as i32).max((src_hi >> 32) as i32) as u32;
                self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            0x3F => {
                ...
                let d0 = (dst_lo as u32).max(src_lo as u32);
                let d1 = ((dst_lo >> 32) as u32).max((src_lo >> 32) as u32);
                let d2 = (dst_hi as u32).max(src_hi as u32);
                let d3 = ((dst_hi >> 32) as u32).max((src_hi >> 32) as u32);
                self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            // PMULLD - Multiply Packed Signed Dwords (0x40)
            0x40 => {
                if !ctx.operand_size_override {
                ...
                let d0 = (dst_lo as i32).wrapping_mul(src_lo as i32) as u32;
                let d1 = ((dst_lo >> 32) as i32).wrapping_mul((src_lo >> 32) as i32) as u32;
                let d2 = (dst_hi as i32).wrapping_mul(src_hi as i32) as u32;
                let d3 = ((dst_hi >> 32) as i32).wrapping_mul((src_hi >> 32) as i32) as u32;
                self.regs.xmm[xmm_dst][0] = (d0 as u64) | ((d1 as u64) << 32);
                self.regs.xmm[xmm_dst][1] = (d2 as u64) | ((d3 as u64) << 32);
                self.regs.rip += ctx.cursor as u64;
                Ok(None)
            ...
            // PHMINPOSUW - Packed Horizontal Minimum of Unsigned Word (0x41)
            0x41 => {
                if !ctx.operand_size_override {
                ...
                for i in 0..4 {
                    let w = ((src_lo >> (i * 16)) & 0xFFFF) as u16;
                    if w < min_val {
                        min_val = w;
                        min_idx = i as u16;
                    }
                ...
                for i in 0..4 {
                    let w = ((src_hi >> (i * 16)) & 0xFFFF) as u16;
                    if w < min_val {
                        min_val = w;
                        min_idx = (i + 4) as u16;
                    }
                }
                // Result: bits 0-15 = minimum value, bits 16-18 = index, rest = 0
