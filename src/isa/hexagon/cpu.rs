...
use crate::vm::vcpu::{CpuState, HexagonRegisters, VCpu, VcpuExit};
...
impl HexagonVcpu {
    ...
    fn execute_insn(
        ...
        match insn {
            ...
            DecodedInsn::JumpSet { dst, value, offset } => {
                let val = match value {
                    crate::isa::hexagon::decode::JumpSetSrc::Reg(reg) => self.regs.r[reg as usize],
                    crate::isa::hexagon::decode::JumpSetSrc::Imm(imm) => imm,
                };
                new_r[dst as usize] = Some(val);
                let target = pcrel_base.wrapping_add(offset as u32) & !0x3;
                self.set_branch(branch, target, false)?;
    ...
    fn try_sem(
        ...
        let (usr_or, v_writes, q_writes) = {
            let mut ctx = super::semantics::SemCtx {
                regs: &self.regs,
                new_r: &mut *new_r,
                new_p: &mut *new_p,
                vnew: &self.new_v,
            ...
            if !super::semantics::dispatch(&dop, &mut ctx) {
