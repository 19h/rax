...
//! (`tests/suites/differential/hexagon/scalar.rs`). See `semantics/float.rs`
//! and `semantics/alu.rs` for the
//! established pattern; this file only adds ops that the earlier wave left out.
//!
//! QEMU softfloat configuration (mirrored from `semantics/float.rs`):
