...
//! verified bit-exact against the qemu-hexagon oracle
//! (`tests/suites/differential/hexagon/scalar.rs`).
//! See `semantics/alu.rs` and `semantics/mpy.rs` for the established pattern.
...
pub fn exec(op: Opcode, d: &DecodedOp, ctx: &mut SemCtx) -> bool {
    ...
    // these map to the decoded `s`/`t` fields (mirroring semantics/alu.rs, oracle-verified).
