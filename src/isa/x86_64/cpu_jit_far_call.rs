//! Native-helper bridge for fault-precise indirect far CALL.

use super::X86_64Vcpu;
use crate::smir::ir::types::OpWidth;

/// Execute one long-mode `FF /3` through the owning vCPU. Encoding bits 1:0
/// select the far-pointer offset width, bit two records REX2/APX, and bit three
/// records an SS-based pointer. Failure restores speculative verifier
/// bookkeeping and hands the original guest PC to direct replay.
#[cfg(all(feature = "smir-jit", target_arch = "x86_64"))]
pub(super) unsafe extern "C" fn rax_jit_far_call(
    ...
    if vcpu
        .call_far_long_mode(pointer_address, width, encoding & 0x8 != 0, return_pc, true)
        .is_err()
    {
