...
impl Mmu {
    ...
    pub fn drain_mem_records(&mut self, out: &mut Vec<crate::vm::vcpu::MemRecord>) {
    ...
    #[inline]
    fn record(&mut self, access: crate::vm::vcpu::MemAccess, addr: u64, size: u8, value: u64) {
        if self.mem_rec_on && !self.fetch_active {
            self.mem_rec.push(crate::vm::vcpu::MemRecord {
                access,
                addr,
                size,
                value,
    ...
    pub(super) fn record_fetch(&mut self, addr: u64, size: u8) {
        if self.mem_rec_on {
            self.mem_rec.push(crate::vm::vcpu::MemRecord {
                access: crate::vm::vcpu::MemAccess::Exec,
                addr,
                size,
                value: 0,
            });
    ...
    pub fn read(&mut self, vaddr: u64, buf: &mut [u8], sregs: &SystemRegisters) -> Result<()> {
        ...
        if self.mem_rec_on && !self.fetch_active {
            ...
            self.mem_rec.push(crate::vm::vcpu::MemRecord {
                access: crate::vm::vcpu::MemAccess::Read,
                addr: vaddr,
    ...
    pub fn write(&mut self, vaddr: u64, buf: &[u8], sregs: &SystemRegisters) -> Result<()> {
        ...
        if self.mem_rec_on && !self.fetch_active {
            let value = Self::le_value(buf);
            self.mem_rec.push(crate::vm::vcpu::MemRecord {
                access: crate::vm::vcpu::MemAccess::Write,
                addr: vaddr,
                size: len.min(255) as u8,
                value,
            });
    ...
    pub fn read_u8(&mut self, vaddr: u64, sregs: &SystemRegisters) -> Result<u8> {
        ...
        self.record(crate::vm::vcpu::MemAccess::Read, vaddr, 1, v as u64);
        Ok(v)
    }
    ...
    pub fn read_u16(&mut self, vaddr: u64, sregs: &SystemRegisters) -> Result<u16> {
        ...
        if (vaddr & 0xFFF) <= 0xFFE {
            ...
            self.record(crate::vm::vcpu::MemAccess::Read, vaddr, 2, v as u64);
            Ok(v)
        } else {
            let mut buf = [0u8; 2];
    ...
    pub fn read_u32(&mut self, vaddr: u64, sregs: &SystemRegisters) -> Result<u32> {
        ...
        if (vaddr & 0xFFF) <= 0xFFC {
            ...
            self.record(crate::vm::vcpu::MemAccess::Read, vaddr, 4, v as u64);
            Ok(v)
        } else {
            let mut buf = [0u8; 4];
            self.read(vaddr, &mut buf, sregs)?;
    ...
    pub fn read_u64(&mut self, vaddr: u64, sregs: &SystemRegisters) -> Result<u64> {
        ...
        if (vaddr & 0xFFF) <= 0xFF8 {
            ...
            self.record(crate::vm::vcpu::MemAccess::Read, vaddr, 8, v);
            Ok(v)
        } else {
            let mut buf = [0u8; 8];
            self.read(vaddr, &mut buf, sregs)?;
    ...
    pub fn write_u8(&mut self, vaddr: u64, value: u8, sregs: &SystemRegisters) -> Result<()> {
        ...
        self.record(crate::vm::vcpu::MemAccess::Write, vaddr, 1, value as u64);
        Ok(())
    }
    ...
    pub fn write_u16(&mut self, vaddr: u64, value: u16, sregs: &SystemRegisters) -> Result<()> {
        ...
        if (vaddr & 0xFFF) <= 0xFFE {
            ...
            self.record(crate::vm::vcpu::MemAccess::Write, vaddr, 2, value as u64);
            Ok(())
        } else {
    ...
    pub fn write_u32(&mut self, vaddr: u64, value: u32, sregs: &SystemRegisters) -> Result<()> {
        ...
        if (vaddr & 0xFFF) <= 0xFFC {
            ...
            self.record(crate::vm::vcpu::MemAccess::Write, vaddr, 4, value as u64);
            Ok(())
        } else {
            self.write(vaddr, &value.to_le_bytes(), sregs)
        }
    ...
    pub fn write_u64(&mut self, vaddr: u64, value: u64, sregs: &SystemRegisters) -> Result<()> {
        ...
        if (vaddr & 0xFFF) <= 0xFF8 {
            ...
            self.record(crate::vm::vcpu::MemAccess::Write, vaddr, 8, value);
            Ok(())
        } else {
            self.write(vaddr, &value.to_le_bytes(), sregs)
        }
