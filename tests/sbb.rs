...
fn test_sbb_al_imm8_no_carry() {
    ...
    assert_eq!(regs.rax & 0xFF, 10, "SBB AL, 5: 15 - 5 - 0 = 10");
}
...
fn test_sbb_al_imm8_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 9, "SBB AL, 5: 15 - 5 - 1 = 9");
}
...
fn test_sbb_al_imm8_underflow_with_carry() {
    // Signed underflow: 0x80 - 0 - 1 = 0x7F
    let code = [0x1c, 0x00, 0xf4];
    let mut regs = Registers::default();
    regs.rax = 0x80;
    regs.rflags = 0x2 | flags::bits::CF;
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 0x7F, "SBB AL, 0: 0x80 - 0 - 1 = 0x7F");
...
fn test_sbb_al_imm8_zero_result() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 0, "SBB AL, 9: 10 - 9 - 1 = 0");
...
fn test_sbb_eax_imm32_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x11111110, "SBB EAX, 0x12345678: with CF=1");
}
...
fn test_sbb_rax_imm32_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    // 100 - (-1) - 1 = 100
    assert_eq!(regs.rax, 100, "SBB RAX, -1: 100 - (-1) - 1 = 100");
}
...
fn test_sbb_rm8_imm8_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 15, "SBB CL, 10: 25 - 10 - 0 = 15");
}
...
fn test_sbb_rm8_imm8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 14, "SBB CL, 10: 25 - 10 - 1 = 14");
}
...
fn test_sbb_rm8_imm8_memory_with_carry() {
    ...
    assert_eq!(read_mem_u8(&mem), 39, "SBB [RBX], 10: 50 - 10 - 1 = 39");
}
...
fn test_sbb_rm32_imm32_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x11111110, "SBB ECX, 0x12345678: with CF=1");
}
...
fn test_sbb_rm64_imm32_register_with_carry() {
    ...
    assert_eq!(regs.rcx, 0x100000000, "SBB RCX, -1: with CF=1");
}
...
fn test_sbb_rm32_imm8_positive_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 89, "SBB ECX, 10: 100 - 10 - 1 = 89");
}
...
fn test_sbb_rm32_imm8_negative_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 109, "SBB ECX, -10: 100 - (-10) - 1 = 109");
}
...
fn test_sbb_rm8_r8_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 10, "SBB AL, CL: 15 - 5 - 0 = 10");
}
...
fn test_sbb_rm8_r8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 9, "SBB AL, CL: 15 - 5 - 1 = 9");
}
...
fn test_sbb_rm8_r8_memory_with_carry() {
    ...
    let _ = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u8(&mem), 29, "SBB [RBX], CL: 50 - 20 - 1 = 29");
}
...
fn test_sbb_rm32_r32_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x12345678, "SBB EAX, ECX with CF=0");
}
...
fn test_sbb_rm32_r32_memory_with_carry() {
    ...
    let _ = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u32(&mem), 0x12344677, "SBB [RBX], ECX with CF=1");
}
...
fn test_sbb_r8_rm8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 9, "SBB CL, AL: 15 - 5 - 1 = 9");
}
...
fn test_sbb_r8_rm8_memory_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 29, "SBB CL, [RBX]: 50 - 20 - 1 = 29");
}
...
fn test_sbb_r32_rm32_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x22222221, "SBB ECX, EAX: with CF=1");
}
...
fn test_sbb_r64_rm64_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x2222222222222221, "SBB RCX, RAX with CF=1");
}
...
fn test_sbb_r32_rm32_memory_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x12344677, "SBB ECX, [RBX] with CF=1");
}
...
fn test_sbb_chain_64bit_subtraction() {
    // Test subtracting two 64-bit values using 32-bit SBB chain
    ...
    regs.rax = 0x00000000; // Low word of first number
    regs.rcx = 0x00000001; // Low word of second number
    regs.rbx = 0x00000001; // High word of first number
...
fn test_sbb_chain_with_borrow_propagation() {
    // Test borrow propagation through SBB chain
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xFFFFFFFF, "Low 32 bits with borrow");
    assert_eq!(regs.rbx, 0, "High 32 bits: 1 - 0 - 1 (borrow)");
}
...
fn test_sbb_overflow_with_carry() {
    // Signed overflow: 0x80 - 0x00 - 1 = 0x7F
    let code = [0x1c, 0x00, 0xf4]; // SBB AL, 0
    let mut regs = Registers::default();
    regs.rax = 0x80;
    regs.rflags = 0x2 | flags::bits::CF;
...
fn test_sbb_r8_extended_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r8, 99, "SBB R8D, 50: 150 - 50 - 1 = 99");
}
