//! End-to-end native-JIT coverage for register-only legacy SSE4.1 `INSERTPS`.
...
fn i32_lane(vector: [u64; 2], lane: usize) -> u32 {
    (vector[lane / 2] >> ((lane % 2) * 32)) as u32
}

fn set_i32_lane(vector: &mut [u64; 2], lane: usize, value: u32) {
    let shift = (lane % 2) * 32;
...
fn apply(xmm: &mut [[u64; 2]; 16], rex: u8, modrm: u8, immediate: u8) {
    ...
    let selected = i32_lane(xmm[source], usize::from(immediate >> 6));
    set_i32_lane(
        &mut xmm[destination],
        usize::from((immediate >> 4) & 3),
        selected,
    );
    for lane in 0..4 {
        if immediate & (1 << lane) != 0 {
            set_i32_lane(&mut xmm[destination], lane, 0);
        }
    }
}
...
fn run_case(code: &[u8], profile: usize, manual_xmm: [[u64; 2]; 16], label: &str) {
    ...
    assert_eq!(expected.xmm, manual_xmm, "{label}: direct lane equation");
...
#[test]
fn jit_all_128_scanner_legacy_insertps_gaps_match_direct_and_lane_equations() {
    ...
    for (rex_index, rex) in SCANNER_REX_IMAGES.into_iter().enumerate() {
        ...
        for modrm in 0xC0..=0xFF {
            code.push(0x66);
            code.extend(rex);
            code.extend([0x0F, 0x3A, 0x21, modrm, 0]);
        ...
        run_case(
            ...
            profile,
            manual_xmm,
            &format!("INSERTPS scanner rex={rex:02X?}"),
        );
    }
...
#[test]
fn jit_all_256_immediate_controls_match_direct_and_lane_equations() {
    assert!(std::is_x86_feature_detected!("sse4.1"));
    assert!(std::is_x86_feature_detected!("avx"));
    let mut cases = 0usize;
    for chunk_start in (0usize..256).step_by(15) {
        ...
        for (destination, immediate) in (chunk_start..usize::min(chunk_start + 15, 256)).enumerate()
        {
            ...
            code.extend([0x66, rex, 0x0F, 0x3A, 0x21, modrm, immediate as u8]);
        ...
        run_case(
            ...
            profile,
            manual_xmm,
            &format!("INSERTPS immediates {chunk_start}..{}", chunk_start + 15),
        );
    }
    assert_eq!(cases, 256);
}
