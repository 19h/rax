//! End-to-end native-JIT coverage for register-only legacy SSE2/SSE3 lane
//! shuffles and duplicate moves.

use super::*;

const IMMEDIATE: u8 = 0xA5;
...
fn apply(xmm: &mut [[u64; 2]; 16], family: Family, prefix: &[u8], modrm: u8) {
    let rex = rex(prefix);
    let destination = usize::from(((modrm >> 3) & 7) | ((rex & 0x04) << 1));
    let source = usize::from((modrm & 7) | ((rex & 0x01) << 3));
    // Snapshot the source before updating the destination so source/destination
    // aliasing follows the architectural read-before-write rule.
    let source_words = xmm[source];
    let source_bytes = [source_words[0].to_le_bytes(), source_words[1].to_le_bytes()].concat();
    let width = family.element_bytes();
    let mut result = [0u8; 16];
    for lane in 0..family.lanes() {
        let selector = family.selector(lane);
        ...
        result[destination_start..destination_start + width]
            .copy_from_slice(&source_bytes[source_start..source_start + width]);
    }
    xmm[destination] = [
...
/// The independent iced-x86 scanner reports six families, each with two
/// canonical mandatory-prefix images (without REX and with ignored REX.W)
/// across all 64 register ModR/M cells:
///
/// 6 families × 2 prefix images × 64 ModR/M cells = 768 newly admitted cells.
#[test]
fn jit_all_768_scanner_legacy_lane_shuffle_gaps_match_direct_and_intel_equations() {
    assert!(std::is_x86_feature_detected!("avx"));
    assert!(std::is_x86_feature_detected!("sse3"));

    let groups: &[(Family, &[u8])] = &[
        (Family::MovDDup, &[0xF2]),
        (Family::MovDDup, &[0xF2, 0x48]),
        (Family::MovShDup, &[0xF3]),
        (Family::MovShDup, &[0xF3, 0x48]),
        (Family::MovSlDup, &[0xF3]),
        (Family::MovSlDup, &[0xF3, 0x48]),
        (Family::PshufD, &[0x66]),
        (Family::PshufD, &[0x66, 0x48]),
        (Family::PshufHighW, &[0xF3]),
        (Family::PshufHighW, &[0xF3, 0x48]),
        (Family::PshufLowW, &[0xF2]),
        (Family::PshufLowW, &[0xF2, 0x48]),
    ];
    ...
    for (profile, &(family, prefix)) in groups.iter().enumerate() {
        ...
        for modrm in 0xC0..=0xFF {
            apply(&mut manual_xmm, family, prefix, modrm);
        }
