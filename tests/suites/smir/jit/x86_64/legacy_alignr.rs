//! End-to-end native-JIT coverage for register-only legacy SSSE3 XMM
//! `PALIGNR`.
...
fn append_encoding(code: &mut Vec<u8>, prefix: &[u8], modrm: u8) {
    code.extend_from_slice(prefix);
    code.extend_from_slice(&[0x0F, 0x3A, 0x0F, modrm, IMMEDIATE]);
}
...
fn apply(xmm: &mut [[u64; 2]; 16], prefix: &[u8], modrm: u8) {
    let rex = rex(prefix);
    let destination = usize::from(((modrm >> 3) & 7) | ((rex & 0x04) << 1));
    let source = usize::from((modrm & 7) | ((rex & 0x01) << 3));
    let low = xmm[source];
    let high = xmm[destination];
    let mut composite = [0u8; 32];
    composite[..8].copy_from_slice(&low[0].to_le_bytes());
    composite[8..16].copy_from_slice(&low[1].to_le_bytes());
    composite[16..24].copy_from_slice(&high[0].to_le_bytes());
    composite[24..].copy_from_slice(&high[1].to_le_bytes());
    let offset = usize::from(IMMEDIATE);
    let result = &composite[offset..offset + 16];
    xmm[destination] = [
...
/// cells:
///
/// 2 prefix images × 64 ModR/M cells = 128 newly admitted cells.
#[test]
fn jit_all_128_scanner_legacy_palignr_gaps_match_direct_and_intel_equation() {
    assert!(std::is_x86_feature_detected!("avx"));
    assert!(std::is_x86_feature_detected!("ssse3"));

    let groups: &[&[u8]] = &[&[0x66], &[0x66, 0x48]];
    let mut cases = 0usize;
    for (profile, &prefix) in groups.iter().enumerate() {
        let mut code = Vec::new();
        for modrm in 0xC0..=0xFF {
            append_encoding(&mut code, prefix, modrm);
            cases += 1;
        }
        code.push(0xF4);
        let label = format!("PALIGNR {prefix:02X?}");

        let mut direct = make_vcpu_code(&code);
        let initial = setup(&mut direct, profile);
        let mut manual_xmm = initial.xmm;
        for modrm in 0xC0..=0xFF {
            apply(&mut manual_xmm, prefix, modrm);
        }
