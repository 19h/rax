...
use rax::isa::hexagon::HexagonVcpu;
use rax::smir::ir::types::{ArchReg, BlockId, HexagonReg, OpId, SourceArch};
use rax::smir::{
...
use rax::vm::vcpu::{CpuState, HexagonRegisters, VCpu, VcpuExit};
...
fn cf_cmpjump_nv_target_is_branch_word_relative() {
    ...
    assert_eq!(
        words.len(),
        2,
        "new-value compare jump must be a two-word packet"
    );
...
fn lift_extract_insert_rp_edges() {
    run_on_large_lifter_stack(
        "lift_extract_insert_rp_edges",
        lift_extract_insert_rp_edges_inner,
    );
}
...
fn audit_final_scalar_gap_rescan() {
    run_on_large_lifter_stack(
        "audit_final_scalar_gap_rescan",
        audit_final_scalar_gap_rescan_inner,
    );
}
