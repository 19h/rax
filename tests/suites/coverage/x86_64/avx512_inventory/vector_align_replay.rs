//! Intel-inventory coverage for register-only EVEX VALIGND/Q.

use super::*;

#[test]
fn register_evex_vector_align_replay_closes_24_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["valignd", "valignq"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        let w = match row.key.w {
            EvexW::W0 => false,
            EvexW::W1 => true,
            EvexW::WIg => panic!("VALIGN EVEX row unexpectedly uses WIG: {}", row.cell),
        };
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_vector_align_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible vector align");
    ...
    let register = [0x62, 0xF3, 0x6D, 0x08, 0x03, 0xC8, 0x03];
