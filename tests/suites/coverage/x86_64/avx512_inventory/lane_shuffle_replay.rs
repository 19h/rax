//! Intel-inventory coverage for register-only EVEX one-source lane shuffles.

use super::*;

#[test]
fn register_evex_lane_shuffle_replay_closes_72_generated_lift_lower_gaps() {
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_lane_shuffle_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible one-source lane shuffle");
    ...
    assert_eq!(register_forms, 72);
    assert_eq!(memory_forms, 18);

    // Exhaust map/opcode/pp/W/L'L/length admission against independently
    // parsed Intel rows. WIG rows contribute both architectural W values.
    for map in 0u8..=7 {
