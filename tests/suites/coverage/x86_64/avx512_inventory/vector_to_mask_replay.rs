//! Intel-inventory coverage for register-only EVEX vector-to-opmask conversions.

use super::*;

#[test]
fn register_evex_vector_to_mask_replay_closes_48_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["vpmovb2m", "vpmovd2m", "vpmovq2m", "vpmovw2m"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        let w = match row.key.w {
            ...
            EvexW::W1 => true,
            EvexW::WIg => panic!(
                "vector-to-mask EVEX row unexpectedly uses WIG: {}",
                row.cell
            ),
        };
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_vector_to_mask_requirements();
            match variant.mode {
                EvexAsmMode::Register => {
                    assert_eq!(
                        classified,
                        Some((row.key.vl != EvexVl::Vl512, row.key.opcode == 0x39)),
                        "{} ({bytes:02X?})",
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible vector-to-mask conversion");
