//! Intel-inventory coverage for EVEX VMOVLPS/LPD/HPS/HPD memory replay.
...
fn assert_memory_half_move_lifts_admits_and_lowers(
    row: &EvexSpecRow,
    bytes: &[u8],
    level: OptLevel,
) {
    ...
    assert_eq!(
        result
            ...
            .count(),
        1,
        "{} {level:?}: exact 8-byte Type-E9NF source",
        row.cell
    );
    ...
    assert!(
        ...
        "{} {level:?}: memory half-move was not admitted ({bytes:02X?})",
        row.cell
    );
    ...
    assert!(
        ...
        "{} {level:?}: missing stack replay {expected:02X?}",
        row.cell
    );
}

#[test]
fn memory_evex_half_move_replay_closes_all_4_intel_rows_at_o0_o1_o2() {
    ...
    for row in avx512_spec_evex_rows().into_iter().filter(|row| {
        expected_mnemonics.contains(&row.key.mnemonic) && matches!(row.key.opcode, 0x12 | 0x16)
    }) {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            for level in [OptLevel::O0, OptLevel::O1, OptLevel::O2] {
                assert_memory_half_move_lifts_admits_and_lowers(&row, &bytes, level);
                lowerings += 1;
            }
            cells += 1;
        }
    ...
    // Memory r/m operands do not have the four vector-register r/m extension
    // buckets used by the register-only inventory partition.
    assert_eq!(cells, 4);
