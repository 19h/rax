//! Intel-inventory coverage for register-only EVEX 128-bit chunk shuffles.

use super::*;

#[test]
fn register_evex_chunk_shuffle_replay_closes_32_generated_lift_lower_gaps() {
    let expected_mnemonics =
        set_from_slice(&["vshuff32x4", "vshuff64x2", "vshufi32x4", "vshufi64x2"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        let w = match row.key.w {
            EvexW::W0 => false,
            EvexW::W1 => true,
            EvexW::WIg => panic!("EVEX chunk-shuffle row unexpectedly uses WIG: {}", row.cell),
        };
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_chunk_shuffle_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible chunk shuffle");
    ...
    assert_eq!(expected_shapes.len(), 8);
    assert_eq!(register_forms, 32);
    assert_eq!(memory_forms, 8);

    // Exhaust map/opcode/pp/W/L'L/length and every R/X/B/R' combination
    // against independently parsed Intel rows. Every vector extension channel
    // is architectural for register sources.
    ...
    let register = [0x62, 0xF3, 0x6D, 0x08, 0x23, 0xC8, 0x03];
