//! Intel-inventory coverage for register-only EVEX VPSHUFB/VPMADDUBSW/VPMADDWD.

use super::*;

#[test]
fn register_evex_bw_shuffle_madd_replay_closes_36_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["vpmaddubsw", "vpmaddwd", "vpshufb"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_bw_shuffle_madd_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible AVX-512BW operation");
    ...
    let register = [0x62, 0xF2, 0x6D, 0x08, 0x00, 0xC8];
