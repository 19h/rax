//! Intel-inventory coverage for Type-E4 packed expand memory replay.

use super::*;

fn assert_expand_memory_lifts_admits_and_lowers(row: &EvexSpecRow, bytes: &[u8], level: OptLevel) {
    ...
    assert!(
        ...
        "{} {level:?}: expand memory replay was not admitted ({bytes:02X?})",
        row.cell
    );
...
#[test]
fn expand_memory_replay_closes_all_54_intel_inventory_cells_at_o0_o1_o2() {
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        unmasked[3] &= !0x87;
        let mut merge = base;
        merge[3] = (merge[3] & !0x87) | 1;
        let mut zero = merge.clone();
        zero[3] |= 0x80;
        for bytes in [&unmasked, &merge, &zero] {
            for level in [OptLevel::O0, OptLevel::O1, OptLevel::O2] {
                assert_expand_memory_lifts_admits_and_lowers(&row, bytes, level);
                lowerings += 1;
            }
            control_cells += 1;
        }
    ...
    assert_eq!(rows, 18);
    assert_eq!(control_cells, 54);
    assert_eq!(lowerings, 54 * 3);
}
