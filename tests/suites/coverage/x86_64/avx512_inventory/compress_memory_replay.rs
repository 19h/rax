//! Intel-inventory coverage for Type-E4 packed compress memory replay.

use super::*;

fn assert_compress_memory_lifts_admits_and_lowers(
    row: &EvexSpecRow,
    bytes: &[u8],
    level: OptLevel,
) {
    ...
    assert!(
        ...
        "{} {level:?}: compress memory replay was not admitted ({bytes:02X?})",
        row.cell
    );
...
#[test]
fn compress_memory_replay_closes_all_36_intel_inventory_cells_at_o0_o1_o2() {
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for bytes in [&unmasked, &masked] {
            for level in [OptLevel::O0, OptLevel::O1, OptLevel::O2] {
                assert_compress_memory_lifts_admits_and_lowers(&row, bytes, level);
                lowerings += 1;
            }
            control_cells += 1;
        }
