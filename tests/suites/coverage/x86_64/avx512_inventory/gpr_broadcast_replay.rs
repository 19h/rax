//! Intel-inventory coverage for register-only EVEX GPR-source broadcasts.

use super::*;

#[test]
fn register_evex_gpr_broadcast_replay_closes_24_generated_lift_lower_gaps() {
    ...
    for row in avx512_spec_evex_rows().into_iter().filter(|row| {
        expected_mnemonics.contains(&row.key.mnemonic)
            && avx512_spec::evex_rm_register_class(row) == EvexRmRegisterClass::Gpr
    }) {
        seen_mnemonics.insert(row.key.mnemonic.clone());
        let w = match row.key.w {
            EvexW::W0 => false,
            EvexW::W1 => true,
            EvexW::WIg => panic!("GPR-broadcast EVEX row unexpectedly uses WIG: {}", row.cell),
        };
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_gpr_broadcast_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible GPR broadcast");
    ...
    // against independently parsed Intel rows. The ModR/M source is RAX or R8
    // according to B; X is ignored, while R/R' extend the vector destination.
    ...
    for (source, expected) in [(4u8, None), (5, None), (12, Some(false)), (13, Some(false))] {
        for ignored_x in [false, true] {
            let mut p0 = if source >= 8 { 0xD2 } else { 0xF2 };
            if ignored_x {
                p0 &= !0x40;
            }
            let bytes = [0x62, p0, 0xFD, 0x48, 0x7C, 0xC0 | (source & 7)];
            assert_eq!(
                X86InstructionBytes::new(&bytes)
                    .unwrap()
                    .evex_register_gpr_broadcast_needs_vl(),
