//! Intel-inventory coverage for register-only EVEX opmask-to-vector broadcasts.

use super::*;

#[test]
fn register_evex_mask_broadcast_replay_closes_6_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["vpbroadcastmb2q", "vpbroadcastmw2d"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        let w = match row.key.w {
            ...
            EvexW::W1 => true,
            EvexW::WIg => panic!(
                "mask-broadcast EVEX row unexpectedly uses WIG: {}",
                row.cell
            ),
        };
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_mask_broadcast_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible mask broadcast");
