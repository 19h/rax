//! Intel-inventory closure for Type-E6NF/E9NF EVEX extraction to memory.

use super::*;

fn assert_extract_memory_lifts_admits_and_lowers(
    row: &EvexSpecRow,
    bytes: &[u8],
    level: OptLevel,
    control: &str,
    ...
    assert!(
        ...
        "{} {control} {level:?}: extraction memory replay was not admitted ({bytes:02X?})",
        row.cell
    );
    assert!(
        !is_native_clobber_safe_excluding(&function, &excluded, false),
        "{} {control} {level:?}: extraction memory replay bypassed the memory gate",
        row.cell
    );
...
#[test]
fn extract_memory_replay_closes_all_32_intel_inventory_cells_at_o0_o1_o2() {
    let chunk_mnemonics = set_from_slice(&[
        "vextractf32x4",
        "vextractf32x8",
        "vextractf64x2",
        "vextractf64x4",
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        assert_eq!(base[1] & 7, 3, "{} ({base:02X?})", row.cell);
        ...
        for &w in widths {
            let mut width_bytes = base.clone();
            width_bytes[2] = (width_bytes[2] & !0x80) | (u8::from(w) << 7);
            if chunk_mnemonics.contains(&row.key.mnemonic) {
                for (control, mask) in [("unmasked", 0u8), ("masked-k1", 1u8)] {
                    let mut bytes = width_bytes.clone();
                    bytes[3] = (bytes[3] & !0x87) | mask;
                    for level in [OptLevel::O0, OptLevel::O1, OptLevel::O2] {
                        assert_extract_memory_lifts_admits_and_lowers(&row, &bytes, level, control);
                        lowerings += 1;
                    }
                    chunk_cells += 1;
                }
                ...
                width_bytes[3] &= !0x87;
                let control = if w { "scalar-W1" } else { "scalar-W0" };
                for level in [OptLevel::O0, OptLevel::O1, OptLevel::O2] {
                    assert_extract_memory_lifts_admits_and_lowers(
                        &row,
                        &width_bytes,
                        level,
                        control,
    ...
    assert_eq!(rows, 17);
    assert_eq!(scalar_cells, 8);
    assert_eq!(chunk_cells, 24);
    assert_eq!(scalar_cells + chunk_cells, 32);
