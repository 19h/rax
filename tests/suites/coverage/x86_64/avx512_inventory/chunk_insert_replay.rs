//! Intel-inventory coverage for register-only EVEX VINSERTF*/VINSERTI* chunks.

use super::*;

#[test]
fn register_evex_chunk_insert_replay_closes_48_generated_lift_lower_gaps() {
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for &w in widths {
            ...
            assert!(previous.is_none(), "duplicate Intel insert shape: {row:?}");
        }

        for variant in evex_case_variants_for_row(&row) {
            let bytes = raw_evex_spec_bytes_for_variant(&row, variant);
            let instruction = X86InstructionBytes::new(&bytes).unwrap();
            let classified = instruction.evex_register_chunk_insert_requirements();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible EVEX chunk insert");
    ...
    let register = [0x62, 0xF3, 0x6D, 0x28, 0x18, 0xC8, 0xFF];
