//! Intel-inventory coverage for register-only EVEX GFNI instructions.

use super::*;

#[test]
fn register_evex_gfni_replay_closes_36_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["vgf2p8affineinvqb", "vgf2p8affineqb", "vgf2p8mulb"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        let widths: &[bool] = match row.key.w {
            EvexW::W0 => &[false],
            EvexW::W1 => &[true],
            EvexW::WIg => panic!("EVEX GFNI row unexpectedly uses WIG: {}", row.cell),
        };
        for &w in widths {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_gfni_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible EVEX GFNI operation");
    ...
    let register = [0x62, 0xF3, 0xFD, 0x09, 0xCE, 0xC8, 0x63];
    ...
    for bytes in [memory, embedded_broadcast, reserved_zeroing] {
        assert_eq!(
            X86InstructionBytes::new(&bytes)
                .unwrap()
                .evex_register_gfni_needs_vl(),
