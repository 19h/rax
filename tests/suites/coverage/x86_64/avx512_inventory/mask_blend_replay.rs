//! Intel-inventory coverage for register-only EVEX opmask-selector blends.

use super::*;

#[test]
fn register_evex_mask_blend_replay_closes_72_generated_lift_lower_gaps() {
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_mask_blend_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible mask blend");
