//! Intel-inventory coverage for register-only EVEX VPCLMULQDQ.

use super::*;

#[test]
fn register_evex_vpclmulqdq_replay_closes_12_generated_lift_lower_gaps() {
    let expected_mnemonics = set_from_slice(&["vpclmulqdq"]);
        ...
        .filter(|row| expected_mnemonics.contains(&row.key.mnemonic))
    {
        ...
        for variant in evex_case_variants_for_row(&row) {
            ...
            let classified = instruction.evex_register_vpclmulqdq_needs_vl();
            match variant.mode {
                EvexAsmMode::Register => {
                    ...
                    let code = lowerer
                        .finalize()
                        .expect("finalize replay-eligible EVEX VPCLMULQDQ operation");
    ...
    let register = [0x62, 0xF3, 0x7D, 0x08, 0x44, 0xC8, 0x03];
    ...
    zeroing[3] |= 0x80;
    for bytes in [memory, embedded_control, masked, zeroing] {
        assert_eq!(
            X86InstructionBytes::new(&bytes)
                .unwrap()
                .evex_register_vpclmulqdq_needs_vl(),
