...
use rax::isa::x86_64::{X86_64Vcpu, flags};
...
fn run_kvm(
    ...
    let to_kvm_seg = |s: &rax::vm::vcpu::Segment| kvm_segment {
        base: s.base,
        limit: s.limit,
        selector: s.selector,
        type_: s.type_,
...
// CF=0 instead of the operand's sign bit. Fixed in
// src/isa/x86_64/execute/shift/core.rs (SAR arm: CF = sign bit when count >
// width). SHR/SHL leave
...
fn run_smir(code: &[u8], init: &Registers, scratch_init: &[u8; 64]) -> Result<SmirOutcome, String> {
    use rax::smir::interpret::SmirInterpreter;
    use rax::smir::ir::context::{ExitReason, SmirContext};
    use rax::smir::ir::flags::MaterializedFlags;
    use rax::smir::ir::memory::{FlatMemory, MemoryError, SmirMemory};
    use rax::smir::ir::types::{ArchReg, SourceArch, X86Reg};
...
// ===========================================================================
//
// The SMIR lowerer (src/smir/lower/x86_64/mod.rs) emits x86-64 machine code with a
...
fn smir_native_m0_add() {
    use rax::smir::ir::flags::FlagUpdate;
    use rax::smir::ir::ops::OpKind;
    use rax::smir::ir::types::{ArchReg, FunctionId, OpWidth, SrcOperand, VReg, X86Reg};
...
fn run_smir_lowered(
    ...
    use rax::smir::ir::flags::MaterializedFlags;
    use rax::smir::ir::memory::{FlatMemory, MemoryError, SmirMemory};
    use rax::smir::ir::types::{ArchReg, BlockId, FunctionId, SourceArch, X86Reg};
