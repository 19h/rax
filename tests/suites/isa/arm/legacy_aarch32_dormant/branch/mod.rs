...
use super::arithmetic::{exec_one, make_insn};
...
fn test_branch_forward() {
    ...
    cpu.regs[15] = 0x1000;
    let insn = make_insn(
        rax::isa::arm::Mnemonic::B,
        0xEA000040,
        Some(rax::isa::arm::Condition::AL),
        false,
    );
    let result = exec_one(&mut cpu, &mut mem, &insn);
    if let rax::isa::arm::ExecResult::Branch(target) = result {
...
fn test_branch_conditional_eq() {
    ...
    let insn = make_insn(
        rax::isa::arm::Mnemonic::B,
        0x0A000040,
        Some(rax::isa::arm::Condition::EQ),
        false,
    );
    let result = exec_one(&mut cpu, &mut mem, &insn);
    assert!(matches!(result, rax::isa::arm::ExecResult::Branch(_)));
}
...
fn test_branch_with_negative_offset() {
    let mut cpu = rax::isa::arm::Armv7Cpu::new();
    let mut mem = rax::isa::arm::FlatMemory::new(0x10000, 0);
    cpu.regs[15] = 0x10000;
    let insn = make_insn(
        rax::isa::arm::Mnemonic::B,
        0xEAFFEFF0,
...
fn test_bl_link_value() {
    let mut cpu = rax::isa::arm::Armv7Cpu::new();
    let mut mem = rax::isa::arm::FlatMemory::new(0x2000, 0);
    cpu.regs[15] = 0x2000;
    cpu.regs[14] = 0;
    let insn = make_insn(
        rax::isa::arm::Mnemonic::BL,
