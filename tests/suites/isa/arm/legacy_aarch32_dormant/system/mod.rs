...
fn test_svc_exception() {
    ...
    let insn = make_insn(
        rax::isa::arm::Mnemonic::SVC,
        0xEF000080,
        Some(rax::isa::arm::Condition::AL),
        false,
    );
    let result = exec_one(&mut cpu, &mut mem, &insn);
    if let rax::isa::arm::ExecResult::Exception(rax::isa::arm::ExceptionType::SupervisorCall(imm)) = result {
        assert_eq!(imm, 0x80);
    } else {
