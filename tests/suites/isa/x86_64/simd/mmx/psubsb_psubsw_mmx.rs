...
//! Flags affected: None
//!
//! Reference: docs/psubsb:psubsw.txt
...
fn test_psubsb_basic() {
    ...
    write_mm_via_mem(&mem, 0x2000, 0x50463C32281E140A);
...
fn test_psubsb_positive_saturation() {
    let code = vec![
        ...
        0x00, 0x0f, 0xe8, 0xc1, 0x0f, 0x7f, 0x04, 0x25, 0x10, 0x20, 0x00, 0x00, 0xf4,
    ];
...
fn test_psubsb_mixed_signs() {
    ...
    write_mm_via_mem(&mem, 0x2000, 0x0A14FFF0E6DC8060);
    write_mm_via_mem(&mem, 0x2008, 0x0564F0F01428A050);
...
fn test_psubsb_alternating_pattern() {
    ...
    // Alternating +/- pattern
    write_mm_via_mem(&mem, 0x2000, 0x7F807F807F807F80);
    write_mm_via_mem(&mem, 0x2008, 0x017F017F017F017F);
...
fn test_psubsb_memory_operand() {
    ...
    write_mm_via_mem(&mem, 0x2000, 0x5040302010000000);
    write_mm_via_mem(&mem, 0x2008, 0x1010101010000000);
...
fn test_psubsb_boundary_cases() {
    ...
    // Edge cases: 127-(-128) should saturate to 127, -128-127 should saturate to -128
    write_mm_via_mem(&mem, 0x2000, 0x7F80FF007F800000);
    write_mm_via_mem(&mem, 0x2008, 0x807F01007F000000);
...
fn test_psubsw_positive_saturation() {
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, 0x0f, 0x6f, 0x0c, 0x25, 0x08, 0x20, 0x00,
        0x00, 0x0f, 0xe9, 0xc1, 0x0f, 0x7f, 0x04, 0x25, 0x10, 0x20, 0x00, 0x00, 0xf4,
    ];
...
fn test_psubsw_mixed_signs() {
    ...
    // Mix of positive and negative values
    write_mm_via_mem(&mem, 0x2000, 0x0064FFF0E700A000);
    write_mm_via_mem(&mem, 0x2008, 0x00C8FF388000D000);
...
fn test_psubsw_alternating_pattern() {
    ...
    write_mm_via_mem(&mem, 0x2008, 0x00017FFF00017FFF);
...
fn test_psubsw_memory_operand() {
    ...
    write_mm_via_mem(&mem, 0x2000, 0x03E807D00BB80FA0);
    write_mm_via_mem(&mem, 0x2008, 0x006400C801900320);
...
fn test_psubsw_boundary_cases() {
    ...
    // Edge cases: 32767-(-32768) should saturate to 32767, -32768-32767 should saturate to -32768
    write_mm_via_mem(&mem, 0x2000, 0x7FFF8000FF000000);
    write_mm_via_mem(&mem, 0x2008, 0x80007FFF01000000);
...
fn test_psubsw_large_values() {
    ...
    // Large values
    write_mm_via_mem(&mem, 0x2000, 0x5000500050005000);
...
fn test_psubsw_small_negative_values() {
    ...
    write_mm_via_mem(&mem, 0x2000, 0xFFF0FFF0FFF0FFF0); // -16 in all words
    write_mm_via_mem(&mem, 0x2008, 0xFFFCFFFCFFFCFFFC); // -4 in all words
...
fn test_psubsw_sequential() {
    ...
    write_mm_via_mem(&mem, 0x2010, 0x0100010001000100);
...
fn test_psubsw_register_reuse() {
    ...
    assert_eq!(
        ...
        "PSUBSW: register reuse (self-subtract)"
    );
}
