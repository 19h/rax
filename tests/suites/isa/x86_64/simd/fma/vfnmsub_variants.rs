...
fn test_vfnmsub132ps_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0x9e, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFNMSUB132PS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfnmsub213pd_ymm0_ymm1_mem() {
    let code = [
        0xc4, 0xe2, 0xf5, 0xae, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFNMSUB213PD YMM0, YMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfnmsub231ss_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0xbf, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFNMSUB231SS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfnmsub132sd_xmm15_xmm8_mem() {
    let code = [
        0xc4, 0x62, 0xb9, 0x9f, 0x3c, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFNMSUB132SD XMM15, XMM8, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
