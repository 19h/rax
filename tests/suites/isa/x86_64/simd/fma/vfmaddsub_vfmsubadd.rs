...
fn test_vfmaddsub132ps_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0x96, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADDSUB132PS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmaddsub213pd_ymm0_ymm1_mem() {
    let code = [
        0xc4, 0xe2, 0xf5, 0xa6, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADDSUB213PD YMM0, YMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmsubadd132ps_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0x97, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMSUBADD132PS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmsubadd231pd_ymm0_ymm1_mem() {
    let code = [
        0xc4, 0xe2, 0xf5, 0xb7, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMSUBADD231PD YMM0, YMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
