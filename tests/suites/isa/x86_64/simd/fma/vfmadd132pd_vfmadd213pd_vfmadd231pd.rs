...
fn test_vfmadd132pd_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0xf1, 0x98, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD132PD XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmadd213pd_ymm0_ymm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0xf5, 0xa8, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD213PD YMM0, YMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmadd231pd_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0xf1, 0xb8, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD231PD XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
