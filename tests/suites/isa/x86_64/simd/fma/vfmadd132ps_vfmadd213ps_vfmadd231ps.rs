...
fn test_vfmadd132ps_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x98, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD132PS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmadd213ps_ymm0_ymm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x75, 0xa8, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD213PS YMM0, YMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vfmadd231ps_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0xb8, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VFMADD231PS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
