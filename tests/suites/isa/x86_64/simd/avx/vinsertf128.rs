...
fn test_vinsertf128_ymm0_ymm1_mem_lane1() {
    ...
    let code = [
        0xc4, 0xe3, 0x75, 0x18, 0x05, 0x00, 0x40, 0x00, 0x00,
        0x01, // VINSERTF128 YMM0, YMM1, [rip + 0x4000], 1
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vinsertf128_ymm2_ymm3_mem_lane0() {
    ...
    let code = [
        ...
        0x00, // VINSERTF128 YMM2, YMM3, [rip + 0x4000], 0
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vinsertf128_ymm7_ymm6_mem_lane0() {
    ...
    let code = [
        0xc4, 0xe3, 0x4d, 0x18, 0x3d, 0x00, 0x40, 0x00, 0x00,
        0x00, // VINSERTF128 YMM7, YMM6, [rip + 0x4000], 0
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vinsertf128_ymm7_ymm6_mem_lane1() {
    ...
    let code = [
        0xc4, 0xe3, 0x4d, 0x18, 0x3d, 0x00, 0x40, 0x00, 0x00,
        0x01, // VINSERTF128 YMM7, YMM6, [rip + 0x4000], 1
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vinsertf128_lane_swap() {
    // Swap the lanes by inserting them in opposite positions
    let code = [
        0xc4, 0xe3, 0x7d, 0x18, 0xc0,
        0x01, // VINSERTF128 YMM0, YMM0, XMM0, 1 - insert lower to upper
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
