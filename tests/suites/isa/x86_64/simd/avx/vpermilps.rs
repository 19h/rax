...
fn test_vpermilps_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0x0c, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VPERMILPS XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vpermilps_imm_xmm0_mem_0x1b() {
    let code = [
        0xc4, 0xe3, 0x79, 0x04, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00,
        0x1b, // VPERMILPS XMM0, [0x3000], 0x1B
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vpermilps_imm_ymm0_mem_0xe4() {
    let code = [
        0xc4, 0xe3, 0x7d, 0x04, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00,
        0xe4, // VPERMILPS YMM0, [0x3000], 0xE4
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
