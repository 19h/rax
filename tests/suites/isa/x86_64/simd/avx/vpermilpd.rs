...
fn test_vpermilpd_xmm0_xmm1_mem() {
    let code = [
        0xc4, 0xe2, 0x71, 0x0d, 0x04, 0x25, 0x00, 0x30, 0x00,
        0x00, // VPERMILPD XMM0, XMM1, [0x3000]
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vpermilpd_imm_xmm0_mem_0x1() {
    let code = [
        0xc4, 0xe3, 0x79, 0x05, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00,
        0x01, // VPERMILPD XMM0, [0x3000], 0x1
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vpermilpd_imm_ymm0_mem_0x5() {
    let code = [
        0xc4, 0xe3, 0x7d, 0x05, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00,
        0x05, // VPERMILPD YMM0, [0x3000], 0x5
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
