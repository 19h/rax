...
fn test_vdpps_xmm0_xmm1_mem_ff() {
    ...
    let code = [
        0xc4, 0xe3, 0x71, 0x40, 0x05, 0x00, 0x40, 0x00, 0x00,
        0xff, // VDPPS XMM0, XMM1, [rip+0x4000], 0xFF
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);

    // Test data: [1.0, 2.0, 3.0, 4.0]
    // Dot product with itself: 1+4+9+16 = 30
    let test_data: [u8; 16] = [
...
fn test_vdpps_xmm1_xmm2_mem_f1() {
    ...
    let code = [
        0xc4, 0xe3, 0x69, 0x40, 0x0d, 0x00, 0x40, 0x00, 0x00,
        0xf1, // VDPPS XMM1, XMM2, [rip+0x4000], 0xF1
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vdpps_xmm8_xmm9_mem_ff() {
    ...
    let code = [
        0xc4, 0x63, 0x31, 0x40, 0x05, 0x00, 0x40, 0x00, 0x00,
        0xff, // VDPPS XMM8, XMM9, [rip+0x4000], 0xFF
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vdpps_ymm0_ymm1_mem_ff() {
    ...
    let code = [
        0xc4, 0xe3, 0x75, 0x40, 0x05, 0x00, 0x40, 0x00, 0x00,
        0xff, // VDPPS YMM0, YMM1, [rip+0x4000], 0xFF
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);

    // Test data: lower lane [1,2,3,4], upper lane [1,2,3,4]
    let test_data: [u8; 32] = [
        0x00, 0x00, 0x80, 0x3f, // 1.0
        0x00, 0x00, 0x00, 0x40, // 2.0
        0x00, 0x00, 0x40, 0x40, // 3.0
...
fn test_vdpps_ymm8_ymm9_mem_f1() {
    ...
    let code = [
        0xc4, 0x63, 0x35, 0x40, 0x05, 0x00, 0x40, 0x00, 0x00,
        0xf1, // VDPPS YMM8, YMM9, [rip+0x4000], 0xF1
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vdpps_3d_dot_product() {
    // Common 3D graphics pattern: 3-element dot product (0x71)
    let code = [
        0xc4, 0xe3, 0x71, 0x40, 0xc2, 0x71, // VDPPS XMM0, XMM1, XMM2, 0x71
        0xf4, // HLT
    ];
...
fn test_vdpps_broadcast_result() {
    // Broadcast dot product result to all elements (0xFF)
    let code = [
        0xc4, 0xe3, 0x71, 0x40, 0xc2, 0xff, // VDPPS XMM0, XMM1, XMM2, 0xFF
        0xf4, // HLT
    ];
...
fn test_vdpps_multiple_sequential() {
    // Multiple dot products in sequence
    let code = [
        0xc4, 0xe3, 0x71, 0x40, 0xc2, 0xff, // VDPPS XMM0, XMM1, XMM2, 0xFF
...
fn test_vdpps_alternating_elements() {
    // Test alternating element patterns
    let code = [
