...
fn test_vmaskmovps_load_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2c, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM0, XMM1, [rip + 0x4000]
        0xf4, // HLT
    ];
...
fn test_vmaskmovps_load_xmm1_xmm2_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x69, 0x2c, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM1, XMM2, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm2_xmm3_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x61, 0x2c, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM2, XMM3, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm3_xmm4_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x59, 0x2c, 0x1d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM3, XMM4, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm4_xmm5_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x51, 0x2c, 0x25, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM4, XMM5, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm5_xmm6_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x49, 0x2c, 0x2d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM5, XMM6, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm6_xmm7_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x41, 0x2c, 0x35, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM6, XMM7, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm7_xmm0_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x79, 0x2c, 0x3d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM7, XMM0, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm8_xmm9_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x31, 0x2c, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM8, XMM9, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_xmm9_xmm10_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x29, 0x2c, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM9, XMM10, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);

    let test_data: [u8; 16] = [
        0x00, 0x00, 0x40, 0x40, 0x00, 0x00, 0x40, 0x40, 0x00, 0x00, 0x40, 0x40, 0x00, 0x00, 0x40,
        0x40,
    ]; // 3.0
    mem.write_slice(&test_data, GuestAddress(ALIGNED_ADDR))
        .unwrap();
...
fn test_vmaskmovps_load_xmm10_xmm11_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x21, 0x2c, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM10, XMM11, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);

    let test_data: [u8; 16] = [
        0x00, 0x00, 0x80, 0x40, 0x00, 0x00, 0x80, 0x40, 0x00, 0x00, 0x80, 0x40, 0x00, 0x00, 0x80,
        0x40,
    ]; // 4.0
    mem.write_slice(&test_data, GuestAddress(ALIGNED_ADDR))
        .unwrap();
...
fn test_vmaskmovps_load_xmm15_xmm14_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x09, 0x2c, 0x3d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM15, XMM14, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);

    let test_data: [u8; 16] = [
        0x00, 0x00, 0xa0, 0x40, 0x00, 0x00, 0xa0, 0x40, 0x00, 0x00, 0xa0, 0x40, 0x00, 0x00, 0xa0,
        0x40,
    ]; // 5.0
    mem.write_slice(&test_data, GuestAddress(ALIGNED_ADDR))
        .unwrap();
...
fn test_vmaskmovps_load_ymm0_ymm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x75, 0x2c, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS YMM0, YMM1, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_ymm1_ymm2_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x6d, 0x2c, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS YMM1, YMM2, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_ymm2_ymm3_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x65, 0x2c, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS YMM2, YMM3, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_load_ymm8_ymm9_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x35, 0x2c, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS YMM8, YMM9, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovps_store_mem_xmm0_xmm1() {
    ...
    let code = [
        0xc4, 0xe2, 0x79, 0x2e, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], XMM0, XMM1
        0xf4, // HLT
    ];
...
fn test_vmaskmovps_store_mem_xmm1_xmm2() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2e, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], XMM1, XMM2
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_xmm2_xmm3() {
    ...
    let code = [
        0xc4, 0xe2, 0x69, 0x2e, 0x1d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], XMM2, XMM3
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_xmm3_xmm4() {
    ...
    let code = [
        0xc4, 0xe2, 0x61, 0x2e, 0x25, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], XMM3, XMM4
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_xmm8_xmm9() {
    ...
    let code = [
        0xc4, 0x62, 0x39, 0x2e, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], XMM8, XMM9
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_ymm0_ymm1() {
    ...
    let code = [
        0xc4, 0xe2, 0x7d, 0x2e, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], YMM0, YMM1
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_ymm1_ymm2() {
    ...
    let code = [
        0xc4, 0xe2, 0x75, 0x2e, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], YMM1, YMM2
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_store_mem_ymm8_ymm9() {
    ...
    let code = [
        0xc4, 0x62, 0x3d, 0x2e, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4000], YMM8, YMM9
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_load_xmm0_xmm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2d, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD XMM0, XMM1, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_xmm1_xmm2_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x69, 0x2d, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD XMM1, XMM2, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_xmm2_xmm3_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x61, 0x2d, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD XMM2, XMM3, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_xmm8_xmm9_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x31, 0x2d, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD XMM8, XMM9, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_ymm0_ymm1_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x75, 0x2d, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD YMM0, YMM1, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_ymm1_ymm2_mem() {
    ...
    let code = [
        0xc4, 0xe2, 0x6d, 0x2d, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD YMM1, YMM2, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_ymm8_ymm9_mem() {
    ...
    let code = [
        0xc4, 0x62, 0x35, 0x2d, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD YMM8, YMM9, [rip + 0x4000]
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_store_mem_xmm0_xmm1() {
    ...
    let code = [
        0xc4, 0xe2, 0x79, 0x2f, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], XMM0, XMM1
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_xmm1_xmm2() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2f, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], XMM1, XMM2
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_xmm2_xmm3() {
    ...
    let code = [
        0xc4, 0xe2, 0x69, 0x2f, 0x1d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], XMM2, XMM3
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_xmm8_xmm9() {
    ...
    let code = [
        0xc4, 0x62, 0x39, 0x2f, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], XMM8, XMM9
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_ymm0_ymm1() {
    ...
    let code = [
        0xc4, 0xe2, 0x7d, 0x2f, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], YMM0, YMM1
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_ymm1_ymm2() {
    ...
    let code = [
        0xc4, 0xe2, 0x75, 0x2f, 0x15, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], YMM1, YMM2
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovpd_store_mem_ymm8_ymm9() {
    ...
    let code = [
        0xc4, 0x62, 0x3d, 0x2f, 0x0d, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4000], YMM8, YMM9
        0xf4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vmaskmovps_load_store_roundtrip() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2c, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPS XMM0, XMM1, [rip + 0x4000]
        0xc4, 0xe2, 0x71, 0x2e, 0x0d, 0x10, 0x40, 0x00,
        0x00, // VMASKMOVPS [rip + 0x4010], XMM1, XMM0
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_vmaskmovpd_load_store_roundtrip() {
    ...
    let code = [
        0xc4, 0xe2, 0x71, 0x2d, 0x05, 0x00, 0x40, 0x00,
        0x00, // VMASKMOVPD XMM0, XMM1, [rip + 0x4000]
        0xc4, 0xe2, 0x71, 0x2f, 0x0d, 0x10, 0x40, 0x00,
        0x00, // VMASKMOVPD [rip + 0x4010], XMM1, XMM0
        0xf4, // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
