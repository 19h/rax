...
//! Flags affected: None
//!
//! Reference: Intel SDM Vol. 2D, WRMSR—Write to Model Specific Register.
...
fn test_wrmsr_basic() {
    ...
    // 0F 30 = WRMSR
    let code = [
        0x48, 0xC7, 0xC1, 0x3B, 0x00, 0x00, 0x00, // MOV RCX, IA32_TSC_ADJUST
    ...
    assert_eq!(regs.rcx, 0x3B, "ECX should be preserved");
...
fn test_wrmsr_high_rcx_ignored() {
    ...
    let code = [
        0x48, 0xB9, 0x3B, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF,
        0xFF, // MOV RCX, 0xFFFFFFFF_0000003B
    ...
    let _regs = run_until_hlt(&mut vcpu).unwrap();
    // Should write IA32_TSC_ADJUST, ignoring high 32 bits of RCX.
}

#[test]
fn test_wrmsr_lock_and_rex2_prefixes_raise_ud_without_consuming_inputs() {
    for code in [&[0xF0, 0x0F, 0x30, 0xF4][..], &[0xD5, 0x80, 0x30, 0xF4]] {
        ...
        assert!(faulted_to_handler(&mut vcpu), "{code:02X?}");
...
fn test_wrmsr_different_msrs() {
    ...
    let code = [
        ...
        0x48, 0x31, 0xD2, // XOR RDX, RDX
        0x0F, 0x30, // WRMSR (IA32_TSC_ADJUST)
        0x48, 0xC7, 0xC1, 0x81, 0x00, 0x00, 0xC0, // MOV RCX, IA32_STAR
        0x48, 0xC7, 0xC0, 0x22, 0x00, 0x00, 0x00, // MOV RAX, 0x22
        0x0F, 0x30, // WRMSR (IA32_STAR)
        0xF4, // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
...
fn make_cpl3(vcpu: &mut rax::isa::x86_64::X86_64Vcpu) {
...
fn faulted_to_handler(vcpu: &mut rax::isa::x86_64::X86_64Vcpu) -> bool {
    let _ = vcpu.step();
...
#[test]
fn test_wrmsr_real_mode_ignores_stale_cs_rpl_and_roundtrips() {
    let value = 0xCAFE_BABE_DEAD_BEEF;
    let mut input = Registers::default();
    input.rcx = 0x3B; // IA32_TSC_ADJUST
    input.rax = value as u32 as u64;
    input.rdx = value >> 32;
    let (mut vcpu, _) = setup_vm_no_idt(&[0x0F, 0x30, 0x0F, 0x32], Some(input));
    let mut sregs = vcpu.get_sregs().unwrap();
    ...
    assert!(vcpu.step().unwrap().is_none());
    let regs = vcpu.get_regs().unwrap();
    assert_eq!((regs.rdx << 32) | regs.rax, value);
}
...
fn test_wrmsr_cpl0_still_works() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x3B, "WRMSR at CPL 0 should run to HLT normally");
}
...
fn test_msr_unimplemented_write_faults_without_committing_inputs() {
    ...
    assert!(
        faulted_to_handler(&mut vcpu),
        "unknown WRMSR must deliver #GP(0)"
    );
    let regs = vcpu.get_regs().unwrap();
    assert_eq!(regs.rcx, 0x100);
    assert_eq!(regs.rax, 0x1111_2222_3333_4444);
    assert_eq!(regs.rdx, 0x5555_6666_7777_8888);
}
