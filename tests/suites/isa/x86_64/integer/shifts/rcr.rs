...
fn test_rcr_al_1_with_lsb() {
    ...
    assert_eq!(regs.rax & 0xFF, 0x40, "AL: 0x81 RCR 1 (CF=0) = 0x40");
...
fn test_rcr_al_1_lsb_and_cf() {
    ...
    assert_eq!(regs.rax & 0xFF, 0xC0, "AL: 0x81 RCR 1 (CF=1) = 0xC0");
    assert!(cf_set(regs.rflags), "CF: receives LSB (was 1)");
}
...
fn test_rcr_al_cl() {
    ...
    assert_eq!(regs.rax & 0xFF, 0x11, "AL: 0x10 RCR 4 (CF=1) = 0x11");
}
...
#[test]
fn test_rcr_al_raw_multi_count_preserves_undefined_of() {
    ...
    let code = [
        0xc0, 0xd8, 0x0a, // RCR AL, 10
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rax = 0x01;
    regs.rflags = 0x2 | flags::bits::OF; // CF clear, OF set
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFF, 0x00, "AL: effective rotate count is 1");
    assert!(cf_set(regs.rflags), "CF: receives the original LSB (1)");
...
fn test_rcr_bl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rbx & 0xFF, 0x62, "BL: 0xC5 RCR 1 (CF=0) = 0x62");
...
fn test_rcr_cl_reg() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 0x73, "CL: 0xCC RCR 2 (CF=1)");
}
...
fn test_rcr_dl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFF, 0x0F, "DL: 0xF0 RCR 4 (CF=0) = 0x0F");
}
...
fn test_rcr_ax_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFF, 0x8123, "AX: 0x1234 RCR 4 (CF=0)");
}
...
fn test_rcr_ax_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFF, 0x6812, "AX: 0x1234 RCR 8 (CF=0)");
}
...
fn test_rcr_ax_full_rotation() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFF, 0x1234, "AX: full 17-bit rotation");
}
...
fn test_rcr_cx() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFF, 0xAABC, "CX: 0xABCD RCR 4 (CF=0)");
}
...
fn test_rcr_dx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFF, 0x00FF, "DX: 0xFF00 RCR 8 (CF=0)");
}
...
fn test_rcr_eax_1() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x43218765, "EAX: RCR 1 (CF=0)");
}
...
fn test_rcr_eax_1_cf_set() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0xC3218765, "EAX: RCR 1 (CF=1)");
}
...
fn test_rcr_eax_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0xF0123456, "EAX: RCR 8 (CF=0)");
}
...
fn test_rcr_eax_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0xACF01234, "EAX: RCR 16 (CF=0)");
}
...
fn test_rcr_ebx() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rbx & 0xFFFFFFFF, 0x00000000, "EBX: RCR 1 (CF=0)");
    assert!(cf_set(regs.rflags), "CF: LSB was 1");
}
...
fn test_rcr_ecx() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0x2ABCDEF0, "ECX: RCR 4 (CF=0)");
}
...
fn test_rcr_edx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFFFFFF, 0x000000FF, "EDX: RCR 24 (CF=0)");
}
...
fn test_rcr_esi() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi & 0xFFFFFFFF, 0xC0000000, "ESI: RCR 1 (CF=1)");
}
...
fn test_rcr_edi() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi & 0xFFFFFFFF, 0xCF012345, "EDI: RCR 12 (CF=0)");
}
...
fn test_rcr_rax_1() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x4321876543218765, "RAX: RCR 1 (CF=0)");
}
...
fn test_rcr_rax_1_cf_set() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xC321876543218765, "RAX: RCR 1 (CF=1)");
}
...
fn test_rcr_rax_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xE0123456789ABCDE, "RAX: RCR 8 (CF=0)");
}
...
fn test_rcr_rax_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xBDE0123456789ABC, "RAX: RCR 16 (CF=0)");
}
...
fn test_rcr_rax_32bits() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x3579BDE012345678, "RAX: RCR 32 (CF=0)");
}
...
fn test_rcr_rbx() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rbx, 0x0000000000000000, "RBX: RCR 1 (CF=0)");
    assert!(cf_set(regs.rflags), "CF: LSB was 1");
}
...
fn test_rcr_rcx() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x2ABCDEF012345678, "RCX: RCR 4 (CF=0)");
}
...
fn test_rcr_rdx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx, 0x00000000000000FF, "RDX: RCR 56 (CF=0)");
}
...
fn test_rcr_rsi() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi, 0xC000000000000000, "RSI: RCR 1 (CF=1)");
}
...
fn test_rcr_rdi() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi, 0xDE0123456789ABCD, "RDI: RCR 12 (CF=0)");
}
...
fn test_rcr_r8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r8, 0x7F6E5D4C3B2A1908, "R8: RCR 1 (CF=0)");
}
...
fn test_rcr_r9_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r9, 0x9BDE0123456789AB, "R9: RCR 16 (CF=0)");
}
...
fn test_rcr_r10_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r10, 0xE0123456789ABCDE, "R10: RCR 8 (CF=0)");
}
...
fn test_rcr_r15() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r15, 0x9111111111111111, "R15: RCR 1 (CF=1)");
}
...
fn test_rcr_mem8() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u8(&mem), 0x40, "Memory: 0x81 RCR 1 (CF=0) = 0x40");
    assert!(cf_set(regs.rflags), "CF: LSB was 1");
}
...
fn test_rcr_mem16() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u16(&mem), 0x8123, "Memory: 0x1234 RCR 4 (CF=0)");
}
