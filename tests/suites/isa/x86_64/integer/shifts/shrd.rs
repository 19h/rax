...
fn test_shrd_ax_bx_1bit() {
    ...
    regs.rbx = 0x0001; // Will put 1 in MSB position
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(
        regs.rax & 0xFFFF,
        0x8001,
        "AX: 0x0002 SHRD 1 with MSB from source"
    );
    assert!(!cf_set(regs.rflags), "CF: bit shifted out was 0");
...
fn test_shrd_cx_dx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFF, 0xC567, "CX: 0x5678 SHRD 4 from 0x9ABC");
}
...
fn test_shrd_dx_si_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFF, 0x2111, "DX: 0x1111 SHRD 4 from 0x2222");
}
...
fn test_shrd_eax_carry_flag() {
    let code = [0x0f, 0xac, 0xd8, 0x04, 0xf4]; // SHRD EAX, EBX, 4
    let mut regs = Registers::default();
    regs.rax = 0x0000000F; // LSBs set
    regs.rbx = 0x00000000;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert!(cf_set(regs.rflags), "CF: LSBs were shifted out");
}
...
fn test_shrd_eax_1bit() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x00000002;
    regs.rbx = 0x00000001; // MSB will be set
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x80000001, "EAX: SHRD 1 with MSB");
...
fn test_shrd_ecx_edx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0x22211111, "ECX: SHRD 12");
}
...
fn test_shrd_edx_esi_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFFFFFF, 0x0000FFFF, "EDX: SHRD 16");
}
...
fn test_shrd_esi_edi_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi & 0xFFFFFFFF, 0x44AA55AA, "ESI: SHRD 8");
}
...
fn test_shrd_edi_ebx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi & 0xFFFFFFFF, 0xF0123456, "EDI: SHRD 4");
}
...
fn test_shrd_eax_zero_count() {
    ...
    let initial = regs.rax & 0xFFFFFFFF;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, initial, "EAX: unchanged");
}
...
fn test_shrd_rax_rbx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x0123456789ABCDEF, "RAX: SHRD 4");
}
...
fn test_shrd_rax_rbx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x10123456789ABCDE, "RAX: SHRD 8");
}
...
fn test_shrd_rax_1bit() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x8000000000000001, "RAX: SHRD 1");
    assert!(of_set(regs.rflags), "OF: sign changed");
}
...
fn test_shrd_rax_carry() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert!(cf_set(regs.rflags), "CF: bits shifted out");
}
...
fn test_shrd_rax_16bits() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x123456789ABCDEF0;
    regs.rbx = 0xAAAABBBBCCCCDDDD;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xDDDD123456789ABC, "RAX: SHRD 16");
}
...
fn test_shrd_rax_32bits() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x7654321012345678, "RAX: SHRD 32");
}
...
fn test_shrd_rdx_rsi_cl() {
    ...
    regs.rsi = 0x0000000000000000;
    regs.rcx = 0x10;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx, 0x0000FFFFFFFFFFFF, "RDX: SHRD 16");
}
...
fn test_shrd_rsi_rdi_imm8() {
    ...
    let mut regs = Registers::default();
    regs.rsi = 0xAA55AA55AA55AA55;
    regs.rdi = 0x1122334455667788;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi, 0x88AA55AA55AA55AA, "RSI: SHRD 8");
}
...
fn test_shrd_rdi_rbx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi, 0x00123456789ABCDE, "RDI: SHRD 4");
}
...
fn test_shrd_r8_r9_imm8() {
    ...
    let mut regs = Registers::default();
    regs.r8 = 0x123456789ABCDEF0;
    regs.r9 = 0xFEDCBA9876543210;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r8, 0x10123456789ABCDE, "R8: SHRD 8");
}
...
fn test_shrd_r10_r11_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r10, 0x5AAAAAAAAAAAAAAA, "R10: SHRD 4");
}
...
fn test_shrd_r12_r13_imm8() {
    ...
    regs.r13 = 0x1111222233334444;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r12, 0x4444123456789ABC, "R12: SHRD 16");
}
...
fn test_shrd_r14_r15_cl() {
    ...
    let mut regs = Registers::default();
    regs.r14 = 0xF0F0F0F0F0F0F0F0;
    regs.r15 = 0x0F0F0F0F0F0F0F0F;
    regs.rcx = 0x04;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r14, 0xFF0F0F0F0F0F0F0F, "R14: SHRD 4");
}
...
fn test_shrd_rax_zero_count() {
    ...
    regs.rbx = 0xFEDCBA9876543210;
    regs.rflags = 0x2 | flags::bits::CF;
    let initial = regs.rax;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, initial, "RAX: unchanged");
}
...
fn test_shrd_mem16_imm8() {
    ...
    let code = [
        ...
        0xac,
        0x14,
        0x25, // SHRD word ptr [disp32], DX, imm8
    ...
    assert_eq!(
        read_mem_u16(&mem),
        0xD123,
        "Memory: 0x1234 SHRD 4 from 0xABCD"
    );
}
...
fn test_shrd_mem32_cl() {
    ...
    let code = [
        ...
        0xad,
        0x14,
        0x25, // SHRD dword ptr [disp32], EDX, CL
        (DATA_ADDR & 0xFF) as u8,
        ((DATA_ADDR >> 8) & 0xFF) as u8,
        ((DATA_ADDR >> 16) & 0xFF) as u8,
        ((DATA_ADDR >> 24) & 0xFF) as u8,
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u32(&mem), 0x01123456, "Memory: SHRD 8");
}
...
fn test_shrd_mem64_imm8() {
    ...
    let code = [
        ...
        0xac,
        0x14,
        0x25, // SHRD qword ptr [disp32], RDX, imm8
        (DATA_ADDR & 0xFF) as u8,
        ((DATA_ADDR >> 8) & 0xFF) as u8,
        ((DATA_ADDR >> 16) & 0xFF) as u8,
        ((DATA_ADDR >> 24) & 0xFF) as u8,
    ...
    let mut regs = Registers::default();
    regs.rdx = 0xAAAABBBBCCCCDDDD;
    let (mut vcpu, mem) = setup_vm(&code, Some(regs));
    write_mem_u64(&mem, 0x123456789ABCDEF0);
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u64(&mem), 0xDDDD123456789ABC, "Memory: SHRD 16");
}
...
fn test_shrd_sf_flag() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x00000002;
    regs.rbx = 0x00000001; // Will make result negative
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_shrd_pf_flag() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x12345678;
    regs.rbx = 0xFF000000; // Low byte will be 0xFF (even parity)
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert!(pf_set(regs.rflags), "PF: even parity in low byte");
}
...
fn test_shrd_all_ones() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0xFFFFFFFF, "All ones stays all ones");
}
