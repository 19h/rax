...
#[test]
fn sse_reciprocal_estimate_packed_forms() {
    let a = f32x4([0.0, 0.0, 0.0, 0.0]);
    let b = f32x4([3.0, 5.0, 7.0, 11.0]);

    check_sse(
        "rcpps",
        &sse_program(&[0x0F, 0x53, 0xC1]),
        sse_scratch(a, b),
    );
    check_sse(
        "rsqrtps",
        &sse_program(&[0x0F, 0x52, 0xC1]),
        sse_scratch(a, b),
    );
}

#[test]
fn sse_reciprocal_estimate_scalar_forms() {
    let a = f32x4([1.0, 21.0, 22.0, 23.0]);
    let b = f32x4([3.0, 5.0, 7.0, 11.0]);

    check_sse(
        "rcpss",
        &sse_program(&[0xF3, 0x0F, 0x53, 0xC1]),
        sse_scratch(a, b),
    );
    check_sse(
        "rsqrtss",
        &sse_program(&[0xF3, 0x0F, 0x52, 0xC1]),
        sse_scratch(a, b),
    );
}
...
fn sse_reciprocal_estimate_memory_forms() {
    ...
    check_sse(
        "rcpps_mem",
        &sse_memory_source_program(&[0x0F, 0x53, 0x47, 0x10]),
        sse_scratch(a, b),
    );
    check_sse(
        "rsqrtps_mem",
        &sse_memory_source_program(&[0x0F, 0x52, 0x47, 0x10]),
        sse_scratch(a, b),
    );
    check_sse(
        "rcpss_mem",
        &sse_memory_source_program(&[0xF3, 0x0F, 0x53, 0x47, 0x10]),
        sse_scratch(a, b),
    );
    check_sse(
        "rsqrtss_mem",
        &sse_memory_source_program(&[0xF3, 0x0F, 0x52, 0x47, 0x10]),
        sse_scratch(a, b),
    );
}
...
fn avx_reciprocal_estimate_packed_forms() {
    ...
    let mut rcp = avx_start();
    rcp.extend_from_slice(&[0xC5, 0xFC, 0x10, 0x0F]); // vmovups ymm1, [rdi]
    rcp.extend_from_slice(&[0xC5, 0xF8, 0x53, 0xD1]); // vrcpps xmm2, xmm1
    rcp.extend_from_slice(&[0xC5, 0xF8, 0x53, 0x5F, 0x10]); // vrcpps xmm3, [rdi+16]
    rcp.extend_from_slice(&[0xC5, 0xFC, 0x53, 0xE1]); // vrcpps ymm4, ymm1
    rcp.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x17]); // vmovups [rdi], xmm2
    rcp.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x5F, 0x10]); // vmovups [rdi+16], xmm3
    rcp.extend_from_slice(&[0xC5, 0xFC, 0x11, 0x67, 0x20]); // vmovups [rdi+32], ymm4
    rcp.push(HLT);
    check_avx_mem("avx_vrcpps_forms", &rcp, s);

    let mut rsqrt = avx_start();
    rsqrt.extend_from_slice(&[0xC5, 0xFC, 0x10, 0x0F]); // vmovups ymm1, [rdi]
    rsqrt.extend_from_slice(&[0xC5, 0xF8, 0x52, 0xD1]); // vrsqrtps xmm2, xmm1
    rsqrt.extend_from_slice(&[0xC5, 0xF8, 0x52, 0x5F, 0x10]); // vrsqrtps xmm3, [rdi+16]
    rsqrt.extend_from_slice(&[0xC5, 0xFC, 0x52, 0xE1]); // vrsqrtps ymm4, ymm1
    rsqrt.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x17]); // vmovups [rdi], xmm2
    rsqrt.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x5F, 0x10]); // vmovups [rdi+16], xmm3
    rsqrt.extend_from_slice(&[0xC5, 0xFC, 0x11, 0x67, 0x20]); // vmovups [rdi+32], ymm4
    rsqrt.push(HLT);
    check_avx_mem("avx_vrsqrtps_forms", &rsqrt, s);
}
...
fn avx_reciprocal_estimate_scalar_forms() {
    ...
    let mut code = avx_start();
    code.extend_from_slice(&[0xC5, 0xF8, 0x10, 0x47, 0x20]); // vmovups xmm0, [rdi+32]
    code.extend_from_slice(&[0xC5, 0xF8, 0x10, 0x0F]); // vmovups xmm1, [rdi]
    code.extend_from_slice(&[0xC5, 0xFA, 0x53, 0xD1]); // vrcpss xmm2, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xFA, 0x53, 0x5F, 0x10]); // vrcpss xmm3, xmm0, [rdi+16]
    code.extend_from_slice(&[0xC5, 0xFA, 0x52, 0xE1]); // vrsqrtss xmm4, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xFA, 0x52, 0x6F, 0x10]); // vrsqrtss xmm5, xmm0, [rdi+16]
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x17]); // vmovups [rdi], xmm2
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x5F, 0x10]); // vmovups [rdi+16], xmm3
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x67, 0x20]); // vmovups [rdi+32], xmm4
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x6F, 0x30]); // vmovups [rdi+48], xmm5
    code.push(HLT);
    check_avx_mem("avx_vrcp_vrsqrt_scalar_forms", &code, s);
}
...
#[test]
fn evex_zmm_masked_byte_word_arithmetic_forms() {
    let mut s = [0u8; 64];
    ...
    let mut code = opmask_start();
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, imm64
    code.extend_from_slice(&0xA5A5_5A5A_C3C3_3C3Cu64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0xBB, 0x3C, 0x3C, 0xA5, 0xA5]); // mov ebx, 0xa5a53c3c
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x17]); // vmovdqu8 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x1F]); // vmovdqu8 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x27]); // vmovdqu16 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x2F]); // vmovdqu16 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0xFC, 0xD1]); // vpaddb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0xF8, 0xD9]); // vpsubb zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x4A, 0xFD, 0xE1]); // vpaddw zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xCA, 0xF9, 0xE9]); // vpsubw zmm5{k2}{z}, zmm0, zmm1
    ...
    code.push(HLT);
    check_evex_mem("evex_zmm_masked_byte_word_arithmetic_forms", &code, s);
}
...
fn evex_zmm_vpbroadcast_gpr_mask_merge_zero_forms() {
    ...
    code.extend_from_slice(&[0xC5, 0xF9, 0x92, 0xD3]); // kmovb k2, ebx
    code.extend_from_slice(&[0xB8, 0x55, 0x66, 0x77, 0x88]); // mov eax, 0x88776655
    ...
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x7A, 0xD0]); // vpbroadcastb zmm2{k1}, eax
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x7A, 0xD8]); // vpbroadcastb zmm3{k1}{z}, eax
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x71, 0xD3, 0x01]); // vpsrlw zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x7B, 0xE0]); // vpbroadcastw zmm4{k1}, eax
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x7B, 0xE8]); // vpbroadcastw zmm5{k1}{z}, eax
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x71, 0xD5, 0x01]); // vpsrlw zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x7C, 0xF0]); // vpbroadcastd zmm6{k1}, eax
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x7C, 0xF8]); // vpbroadcastd zmm7{k1}{z}, eax
    ...
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCA, 0x7C, 0xC8]); // vpbroadcastq zmm9{k2}{z}, rax
    code.extend_from_slice(&[0x62, 0xD1, 0xB5, 0x48, 0x73, 0xD1, 0x01]); // vpsrlq zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_vpbroadcast_gpr_mask_merge_zero_forms",
        &code,
...
#[test]
fn evex_zmm_vdbpsadbw_mask_merge_zero_forms() {
    let mut s = [0u8; 64];
    for i in 0..64 {
        s[i] = (0x5Bu8)
    ...
    code.extend_from_slice(&[0xB8, 0x5A, 0x5A, 0xA5, 0xA5]); // mov eax, 0xa5a55a5a
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xC8]); // kmovd k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x07]); // vmovdqu8 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu8 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x17]); // vmovdqu8 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x1F]); // vmovdqu8 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x42, 0xD1, 0x05]); // vdbpsadbw zmm2{k1}, zmm0, zmm1, 5
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x42, 0xD9, 0x06]); // vdbpsadbw zmm3{k1}{z}, zmm0, zmm1, 6
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x71, 0xD3, 0x01]); // vpsrlw zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3

    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x27]); // vmovdqu8 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x2F]); // vmovdqu8 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x42, 0x67, 0x01, 0x07]); // vdbpsadbw zmm4{k1}, zmm0, [rdi+64], 7
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x42, 0x6F, 0x01, 0x01]); // vdbpsadbw zmm5{k1}{z}, zmm0, [rdi+64], 1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x71, 0xD5, 0x01]); // vpsrlw zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x7F, 0x17]); // vmovdqu8 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_vdbpsadbw_mask_merge_zero_forms", &code, s);
}
...
#[test]
fn evex_zmm_count_unary_mask_merge_zero_register_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s.iter_mut().enumerate() {
    ...
    let mut code = opmask_start();
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0xa55aa55a3c3cc3c3
    code.extend_from_slice(&0xA55A_A55A_3C3C_C3C3u64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0xBB, 0xA5, 0x5A, 0xA5, 0x5A]); // mov ebx, 0x5aa55aa5
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xD3]); // kmovd k2, ebx
    code.extend_from_slice(&[0xB9, 0x5A, 0xA5, 0x00, 0x00]); // mov ecx, 0xa55a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xD9]); // kmovw k3, ecx
    code.extend_from_slice(&[0xBA, 0x5A, 0x00, 0x00, 0x00]); // mov edx, 0x5a
    code.extend_from_slice(&[0xC5, 0xF9, 0x92, 0xE2]); // kmovb k4, edx
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x54, 0xD0]); // vpopcntb zmm2{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x54, 0xD8]); // vpopcntb zmm3{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x54, 0xE0]); // vpopcntw zmm4{k2}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x54, 0xE8]); // vpopcntw zmm5{k2}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x4B, 0x55, 0xF0]); // vpopcntd zmm6{k3}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xCB, 0x55, 0xF8]); // vpopcntd zmm7{k3}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4C, 0x55, 0xC0]); // vpopcntq zmm8{k4}, zmm0
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCC, 0x55, 0xC8]); // vpopcntq zmm9{k4}{z}, zmm0
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm10, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm11, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x4B, 0x44, 0xD0]); // vplzcntd zmm10{k3}, zmm0
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xCB, 0x44, 0xD8]); // vplzcntd zmm11{k3}{z}, zmm0
    code.extend_from_slice(&[0x62, 0x51, 0x2D, 0x48, 0xEF, 0xD3]); // vpxord zmm10, zmm10, zmm11
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD2]); // vpxord zmm2, zmm2, zmm10
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm12, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm13, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4C, 0x44, 0xE0]); // vplzcntq zmm12{k4}, zmm0
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCC, 0x44, 0xE8]); // vplzcntq zmm13{k4}{z}, zmm0
    code.extend_from_slice(&[0x62, 0x51, 0x1D, 0x48, 0xEF, 0xE5]); // vpxord zmm12, zmm12, zmm13
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm12
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_count_unary_mask_merge_zero_register_forms",
        &code,
        s,
    );
}
...
#[test]
fn evex_zmm_abs_mask_merge_zero_register_forms() {
    let mut s = [0u8; 64];
    for i in 0..64 {
        s[i] = (0x80u8)
            .wrapping_add((i as u8).wrapping_mul(11))
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x1C, 0xD0]); // vpabsb zmm2{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x1C, 0xD8]); // vpabsb zmm3{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x4A, 0x1D, 0xE0]); // vpabsw zmm4{k2}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xCA, 0x1D, 0xE8]); // vpabsw zmm5{k2}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x4B, 0x1E, 0xF0]); // vpabsd zmm6{k3}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xCB, 0x1E, 0xF8]); // vpabsd zmm7{k3}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4C, 0x1F, 0xC0]); // vpabsq zmm8{k4}, zmm0
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCC, 0x1F, 0xC8]); // vpabsq zmm9{k4}{z}, zmm0
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_abs_mask_merge_zero_register_forms", &code, s);
}
...
fn evex_zmm_minmax_dword_mask_merge_zero_register_forms() {
    ...
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu32 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x17]); // vmovdqu32 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x1F]); // vmovdqu32 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x39, 0xD1]); // vpminsd zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x39, 0xD9]); // vpminsd zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x72, 0xD3, 0x01]); // vpsrld zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x27]); // vmovdqu32 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x2F]); // vmovdqu32 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x3D, 0xE1]); // vpmaxsd zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x3D, 0xE9]); // vpmaxsd zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x72, 0xD5, 0x01]); // vpsrld zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x37]); // vmovdqu32 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x3F]); // vmovdqu32 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x3B, 0xF1]); // vpminud zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x3B, 0xF9]); // vpminud zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x45, 0x48, 0x72, 0xD7, 0x01]); // vpsrld zmm7, zmm7, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7E, 0x48, 0x6F, 0x0F]); // vmovdqu32 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x49, 0x3F, 0xC1]); // vpmaxud zmm8{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xC9, 0x3F, 0xC9]); // vpmaxud zmm9{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xD1, 0x35, 0x48, 0x72, 0xD1, 0x01]); // vpsrld zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x7F, 0x17]); // vmovdqu32 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_minmax_dword_mask_merge_zero_register_forms",
        &code,
        s,
    );
}
...
#[test]
fn evex_zmm_minmax_qword_mask_merge_zero_register_forms() {
    let s = evex_qword_scratch(|i| {
        let value =
            0x8000_0000_0000_0000u64.wrapping_add((i as u64).wrapping_mul(0x1357_9BDF_2468_ACE1));
        value.rotate_left(((i * 9) % 63) as u32)
            ^ if i % 2 == 0 {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu64 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x39, 0xD1]); // vpminsq zmm2{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x39, 0xD9]); // vpminsq zmm3{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xE5, 0x48, 0x73, 0xD3, 0x01]); // vpsrlq zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x3D, 0xE1]); // vpmaxsq zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x3D, 0xE9]); // vpmaxsq zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xD5, 0x48, 0x73, 0xD5, 0x01]); // vpsrlq zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x3B, 0xF1]); // vpminuq zmm6{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x3B, 0xF9]); // vpminuq zmm7{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xC5, 0x48, 0x73, 0xD7, 0x01]); // vpsrlq zmm7, zmm7, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4A, 0x3F, 0xC1]); // vpmaxuq zmm8{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCA, 0x3F, 0xC9]); // vpmaxuq zmm9{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xD1, 0xB5, 0x48, 0x73, 0xD1, 0x01]); // vpsrlq zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_minmax_qword_mask_merge_zero_register_forms",
        &code,
        s,
    );
}
...
fn evex_zmm_minmax_byte_mask_merge_zero_register_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x17]); // vmovdqu8 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x1F]); // vmovdqu8 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x38, 0xD1]); // vpminsb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x38, 0xD9]); // vpminsb zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x71, 0xD3, 0x01]); // vpsrlw zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x27]); // vmovdqu8 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x2F]); // vmovdqu8 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x3C, 0xE1]); // vpmaxsb zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x3C, 0xE9]); // vpmaxsb zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x71, 0xD5, 0x01]); // vpsrlw zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x37]); // vmovdqu8 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x3F]); // vmovdqu8 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0xDA, 0xF1]); // vpminub zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0xDA, 0xF9]); // vpminub zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x45, 0x48, 0x71, 0xD7, 0x01]); // vpsrlw zmm7, zmm7, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0x7F, 0x48, 0x6F, 0x07]); // vmovdqu8 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7F, 0x48, 0x6F, 0x0F]); // vmovdqu8 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0x49, 0xDE, 0xC1]); // vpmaxub zmm8{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0xC9, 0xDE, 0xC9]); // vpmaxub zmm9{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xD1, 0x35, 0x48, 0x71, 0xD1, 0x01]); // vpsrlw zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x7F, 0x17]); // vmovdqu8 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_minmax_byte_mask_merge_zero_register_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_minmax_word_mask_merge_zero_register_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0xB8, 0x0F, 0x0F, 0xA5, 0xA5]); // mov eax, 0xa5a50f0f
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xC8]); // kmovd k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x07]); // vmovdqu16 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu16 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x17]); // vmovdqu16 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x1F]); // vmovdqu16 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0xEA, 0xD1]); // vpminsw zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0xEA, 0xD9]); // vpminsw zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x71, 0xD3, 0x01]); // vpsrlw zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x27]); // vmovdqu16 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x2F]); // vmovdqu16 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0xEE, 0xE1]); // vpmaxsw zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0xEE, 0xE9]); // vpmaxsw zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x71, 0xD5, 0x01]); // vpsrlw zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x37]); // vmovdqu16 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x48, 0x6F, 0x3F]); // vmovdqu16 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x3A, 0xF1]); // vpminuw zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x3A, 0xF9]); // vpminuw zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x45, 0x48, 0x71, 0xD7, 0x01]); // vpsrlw zmm7, zmm7, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x48, 0x6F, 0x07]); // vmovdqu16 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x48, 0x6F, 0x0F]); // vmovdqu16 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x49, 0x3E, 0xC1]); // vpmaxuw zmm8{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xC9, 0x3E, 0xC9]); // vpmaxuw zmm9{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xD1, 0x35, 0x48, 0x71, 0xD1, 0x01]); // vpsrlw zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    ...
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_minmax_word_mask_merge_zero_register_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_vnni_mask_merge_zero_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s.iter_mut().enumerate() {
        *byte = (0x91u8)
            .wrapping_add((i as u8).wrapping_mul(17))
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x27]); // vmovdqu32 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x2F]); // vmovdqu32 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x50, 0xD1]); // vpdpbusd zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x51, 0xD9]); // vpdpbusds zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x52, 0xE1]); // vpdpwssd zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x53, 0xE9]); // vpdpwssds zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x7F, 0x17]); // vmovdqu32 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_vnni_mask_merge_zero_forms", &code, s);
}
...
#[test]
fn evex_zmm_vbmi_mask_merge_zero_register_forms() {
    let mut s = [0u8; 64];
    for i in 0..64 {
        s[i] = (0xA7u8)
    ...
    let mut code = opmask_start();
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0xa5a55a5ac3c33c3c
    code.extend_from_slice(&0xA5A5_5A5A_C3C3_3C3Cu64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x07]); // vmovdqu8 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x48, 0xFC, 0xC8]); // vpaddb zmm1, zmm0, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x17]); // vmovdqu8 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x1F]); // vmovdqu8 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x8D, 0xD1]); // vpermb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x8D, 0xD9]); // vpermb zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x27]); // vmovdqu8 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x2F]); // vmovdqu8 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x49, 0x83, 0xE1]); // vpmultishiftqb zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xC9, 0x83, 0xE9]); // vpmultishiftqb zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x7F, 0x17]); // vmovdqu8 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_vbmi_mask_merge_zero_register_forms", &code, s);
}
...
fn evex_zmm_fp32_mask_merge_zero_arith_unary_logical_forms() {
    ...
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x48, 0x04, 0xC8, 0xB1]); // vpermilps zmm1, zmm0, 0xb1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x17]); // vmovdqu32 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x1F]); // vmovdqu32 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x59, 0xD1]); // vmulps zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x5E, 0xD9]); // vdivps zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6C, 0x48, 0x57, 0xD3]); // vxorps zmm2, zmm2, zmm3

    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x27]); // vmovdqu32 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x2F]); // vmovdqu32 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x51, 0xE0]); // vsqrtps zmm4{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x51, 0xE9]); // vsqrtps zmm5{k1}{z}, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5C, 0x48, 0x57, 0xE5]); // vxorps zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6C, 0x48, 0x57, 0xD4]); // vxorps zmm2, zmm2, zmm4

    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x37]); // vmovdqu32 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x3F]); // vmovdqu32 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x54, 0xF1]); // vandps zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x57, 0xF9]); // vxorps zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x4C, 0x48, 0x57, 0xF7]); // vxorps zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6C, 0x48, 0x57, 0xD6]); // vxorps zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x7F, 0x17]); // vmovdqu32 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_fp32_mask_merge_zero_arith_unary_logical_forms",
        &code,
        s,
    );
}
...
fn evex_zmm_fp64_mask_merge_zero_arith_unary_logical_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x4A, 0x59, 0xD1]); // vmulpd zmm2{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xCA, 0x5E, 0xD9]); // vdivpd zmm3{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0x57, 0xD3]); // vxorpd zmm2, zmm2, zmm3

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x4A, 0x5D, 0xE1]); // vminpd zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xCA, 0x5F, 0xE9]); // vmaxpd zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0x57, 0xE5]); // vxorpd zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0x57, 0xD4]); // vxorpd zmm2, zmm2, zmm4

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x4A, 0x51, 0xF0]); // vsqrtpd zmm6{k2}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xCA, 0x51, 0xF9]); // vsqrtpd zmm7{k2}{z}, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xCD, 0x48, 0x57, 0xF7]); // vxorpd zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0x57, 0xD6]); // vxorpd zmm2, zmm2, zmm6

    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFD, 0x4A, 0x54, 0xC1]); // vandpd zmm8{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x71, 0xFD, 0xCA, 0x57, 0xC9]); // vxorpd zmm9{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0xBD, 0x48, 0x57, 0xC1]); // vxorpd zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0xED, 0x48, 0x57, 0xD0]); // vxorpd zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_fp64_mask_merge_zero_arith_unary_logical_forms",
        &code,
        s,
    );
}
...
fn evex_scalar_fp32_mask_merge_zero_arith_forms() {
    ...
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0xBB, 0x01, 0x00, 0x00, 0x00]); // mov ebx, 1
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xD3]); // kmovw k2, ebx
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x48, 0x04, 0xC8, 0xB1]); // vpermilps zmm1, zmm0, 0xb1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm2, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm3, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x09, 0x58, 0xD1]); // vaddss xmm2{k1}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x89, 0x5C, 0xD9]); // vsubss xmm3{k1}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xE8, 0x57, 0xD3]); // vxorps xmm2, xmm2, xmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm4, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm5, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x0A, 0x59, 0xE1]); // vmulss xmm4{k2}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x8A, 0x5E, 0xE9]); // vdivss xmm5{k2}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xD8, 0x57, 0xE5]); // vxorps xmm4, xmm4, xmm5
    code.extend_from_slice(&[0xC5, 0xE8, 0x57, 0xD4]); // vxorps xmm2, xmm2, xmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_scalar_fp32_mask_merge_zero_arith_forms", &code, s);
}
...
fn evex_scalar_fp64_mask_merge_zero_arith_forms() {
    ...
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xD3]); // kmovw k2, ebx
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x48, 0x01, 0xC8, 0xB1]); // vpermpd zmm1, zmm0, 0xb1
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm2, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xE5, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm3, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x09, 0x58, 0xD1]); // vaddsd xmm2{k1}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x89, 0x5C, 0xD9]); // vsubsd xmm3{k1}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xE9, 0x57, 0xD3]); // vxorpd xmm2, xmm2, xmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm4, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xD5, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm5, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x0A, 0x59, 0xE1]); // vmulsd xmm4{k2}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x8A, 0x5E, 0xE9]); // vdivsd xmm5{k2}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xD9, 0x57, 0xE5]); // vxorpd xmm4, xmm4, xmm5
    code.extend_from_slice(&[0xC5, 0xE9, 0x57, 0xD4]); // vxorpd xmm2, xmm2, xmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_scalar_fp64_mask_merge_zero_arith_forms", &code, s);
}
...
fn evex_scalar_fp32_sqrt_minmax_mask_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm2, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm3, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x09, 0x51, 0xD1]); // vsqrtss xmm2{k1}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x89, 0x51, 0xD9]); // vsqrtss xmm3{k1}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xE8, 0x57, 0xD3]); // vxorps xmm2, xmm2, xmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm4, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x55, 0x48, 0x72, 0xD0, 0x01]); // vpsrld zmm5, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x0A, 0x5D, 0xE1]); // vminss xmm4{k2}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x8A, 0x5F, 0xE9]); // vmaxss xmm5{k2}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xD8, 0x57, 0xE5]); // vxorps xmm4, xmm4, xmm5
    code.extend_from_slice(&[0xC5, 0xE8, 0x57, 0xD4]); // vxorps xmm2, xmm2, xmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_scalar_fp32_sqrt_minmax_mask_forms", &code, s);
}
...
fn evex_scalar_fp64_sqrt_minmax_mask_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm2, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xE5, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm3, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x09, 0x51, 0xD1]); // vsqrtsd xmm2{k1}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x89, 0x51, 0xD9]); // vsqrtsd xmm3{k1}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xE9, 0x57, 0xD3]); // vxorpd xmm2, xmm2, xmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm4, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xD5, 0x48, 0x73, 0xD0, 0x01]); // vpsrlq zmm5, zmm0, 1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x0A, 0x5D, 0xE1]); // vminsd xmm4{k2}, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x8A, 0x5F, 0xE9]); // vmaxsd xmm5{k2}{z}, xmm0, xmm1
    code.extend_from_slice(&[0xC5, 0xD9, 0x57, 0xE5]); // vxorpd xmm4, xmm4, xmm5
    code.extend_from_slice(&[0xC5, 0xE9, 0x57, 0xD4]); // vxorpd xmm2, xmm2, xmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_scalar_fp64_sqrt_minmax_mask_forms", &code, s);
}
...
#[test]
fn evex_zmm_gfni_mask_merge_zero_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s.iter_mut().enumerate() {
        *byte = (0x3Du8)
            .wrapping_add((i as u8).wrapping_mul(17))
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x1F]); // vmovdqu8 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x27]); // vmovdqu8 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0xCF, 0xD1]); // vgf2p8mulb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0xC9, 0xCE, 0xD9, 0x5A]); // vgf2p8affineqb zmm3{k1}{z}, zmm0, zmm1, 0x5a
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x49, 0xCF, 0xE1, 0xA5]); // vgf2p8affineinvqb zmm4{k1}, zmm0, zmm1, 0xa5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x7F, 0x17]); // vmovdqu8 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_gfni_mask_merge_zero_forms", &code, s);
}
...
#[test]
fn evex_zmm_insert_extract_i64x4_f64x4_memory_forms() {
    let s = evex_qword_scratch(|i| {
        0x3141_5926_5358_9793u64
            .wrapping_add((i as u64).wrapping_mul(0x2718_2818_2845_9045))
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x48, 0x3A, 0x4F, 0x02, 0x01]); // vinserti64x4 zmm1, zmm0, [rdi+64], 1
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x48, 0x3B, 0x0F, 0x01]); // vextracti64x4 [rdi], zmm1, 1
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x48, 0x1A, 0x57, 0x02, 0x01]); // vinsertf64x4 zmm2, zmm0, [rdi+64], 1
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x48, 0x1B, 0x57, 0x01, 0x01]); // vextractf64x4 [rdi+32], zmm2, 1
    code.push(HLT);
    check_evex_mem("evex_zmm_insert_extract_i64x4_f64x4_memory_forms", &code, s);
}

#[test]
fn evex_zmm_insert_extract_i32x8_f32x8_memory_forms() {
    let s = evex_dword_scratch(|i| {
        0xC001_D00Du32
            .wrapping_add((i as u32).wrapping_mul(0x1020_4081))
            .rotate_left((i % 7) as u32)
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x48, 0x1A, 0x4F, 0x02, 0x01]); // vinsertf32x8 zmm1, zmm0, [rdi+64], 1
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x48, 0x1B, 0x0F, 0x01]); // vextractf32x8 [rdi], zmm1, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x17]); // vmovdqu32 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x6D, 0x48, 0x3A, 0x5F, 0x02, 0x00]); // vinserti32x8 zmm3, zmm2, [rdi+64], 0
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x48, 0x3B, 0x5F, 0x01, 0x00]); // vextracti32x8 [rdi+32], zmm3, 0
    code.push(HLT);
    check_evex_mem("evex_zmm_insert_extract_i32x8_f32x8_memory_forms", &code, s);
}
...
#[test]
fn evex_zmm_vpcompress_vpexpand_bw_register_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s.iter_mut().enumerate() {
        *byte = (0x53u8)
            .wrapping_add((i as u8).wrapping_mul(23))
    ...
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xD3]); // kmovd k2, ebx
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x07]); // vmovdqu8 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x63, 0xC2]); // vpcompressb zmm2{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x62, 0xDA]); // vpexpandb zmm3{k1}{z}, zmm2
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x63, 0xC4]); // vpcompressw zmm4{k2}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x62, 0xEC]); // vpexpandw zmm5{k2}{z}, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x7F, 0x17]); // vmovdqu8 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_vpcompress_vpexpand_bw_register_forms", &code, s);
}
...
#[test]
fn evex_zmm_two_table_bw_mask_merge_zero_register_forms() {
    let mut s = [0u8; 64];
    for lane in 0..32 {
        let value = (((lane * 3) + if lane % 2 == 0 { 32 } else { 0 }) & 0x3f) as u16;
        s[lane * 2..lane * 2 + 2].copy_from_slice(&value.to_le_bytes());
    }
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu64 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x5F, 0x01]); // vmovdqu64 zmm3, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0x75, 0x49, 0x75, 0xC2]); // vpermi2b zmm0{k1}, zmm1, zmm2
    code.extend_from_slice(&[0x62, 0xF2, 0x75, 0xC9, 0x7D, 0xDA]); // vpermt2b zmm3{k1}{z}, zmm1, zmm2
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x48, 0xEF, 0xC3]); // vpxord zmm0, zmm0, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x6F, 0x01]); // vmovdqu64 zmm5, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x7F, 0x01]); // vmovdqu64 zmm7, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0xD5, 0x4A, 0x75, 0xE6]); // vpermi2w zmm4{k2}, zmm5, zmm6
    code.extend_from_slice(&[0x62, 0xF2, 0xD5, 0xCA, 0x7D, 0xFE]); // vpermt2w zmm7{k2}{z}, zmm5, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE7]); // vpxord zmm4, zmm4, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x48, 0xEF, 0xC4]); // vpxord zmm0, zmm0, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x07]); // vmovdqu64 [rdi], zmm0
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_two_table_bw_mask_merge_zero_register_forms",
        &code,
        s,
    );
}
...
fn evex_zmm_two_table_dq_mask_merge_zero_register_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x5F, 0x01]); // vmovdqu64 zmm3, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0x75, 0x49, 0x76, 0xC2]); // vpermi2d zmm0{k1}, zmm1, zmm2
    code.extend_from_slice(&[0x62, 0xF2, 0x75, 0xC9, 0x7E, 0xDA]); // vpermt2d zmm3{k1}{z}, zmm1, zmm2
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x48, 0xEF, 0xC3]); // vpxord zmm0, zmm0, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x6F, 0x01]); // vmovdqu64 zmm5, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x7F, 0x01]); // vmovdqu64 zmm7, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0xD5, 0x4A, 0x76, 0xE6]); // vpermi2q zmm4{k2}, zmm5, zmm6
    code.extend_from_slice(&[0x62, 0xF2, 0xD5, 0xCA, 0x7E, 0xFE]); // vpermt2q zmm7{k2}{z}, zmm5, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE7]); // vpxord zmm4, zmm4, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x48, 0xEF, 0xC4]); // vpxord zmm0, zmm0, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x07]); // vmovdqu64 [rdi], zmm0
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_two_table_dq_mask_merge_zero_register_forms",
        &code,
        s,
    );
}
...
#[test]
fn evex_zmm_int_extend_mask_merge_zero_register_forms() {
    let source = evex_pmov_source();
    let s = evex_qword_scratch(|i| source[i]);

    let mut code = opmask_start();
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0xa5a55a5a
    code.extend_from_slice(&0xa5a55a5au64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x07]); // vmovdqu64 ymm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x0F]); // vmovdqu64 xmm1, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x20, 0xD0]); // vpmovsxbw zmm2{k1}, ymm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x20, 0xD8]); // vpmovsxbw zmm3{k1}{z}, ymm0
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD3]); // vpxorq zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x31, 0xE1]); // vpmovzxbd zmm4{k1}, xmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x31, 0xE9]); // vpmovzxbd zmm5{k1}{z}, xmm1
...
#[test]
fn evex_zmm_int_narrow_mask_merge_zero_register_forms() {
    let source = evex_pmov_source();
    let s = evex_qword_scratch(|i| source[i]);
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x17]); // vmovdqu64 ymm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x1F]); // vmovdqu64 ymm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0x49, 0x10, 0xC2]); // vpmovuswb ymm2{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0xC9, 0x10, 0xC3]); // vpmovuswb ymm3{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x28, 0xEF, 0xD3]); // vpxorq ymm2, ymm2, ymm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x27]); // vmovdqu64 ymm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x2F]); // vmovdqu64 ymm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0x49, 0x20, 0xC4]); // vpmovswb ymm4{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0xC9, 0x20, 0xC5]); // vpmovswb ymm5{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x28, 0xEF, 0xE5]); // vpxorq ymm4, ymm4, ymm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x28, 0xEF, 0xD4]); // vpxorq ymm2, ymm2, ymm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x37]); // vmovdqu64 ymm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x3F]); // vmovdqu64 ymm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0x49, 0x30, 0xC6]); // vpmovwb ymm6{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7E, 0xC9, 0x30, 0xC7]); // vpmovwb ymm7{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xCD, 0x28, 0xEF, 0xF7]); // vpxorq ymm6, ymm6, ymm7
...
#[test]
fn evex_zmm_vcmp_fp_writemask_register_forms() {
    let lhs = [1.0f64, -2.0, 3.5, -4.5, 0.0, 16.0, -32.0, 128.0];
    let rhs = [2.0f64, -4.0, 3.5, -2.5, -0.0, 8.0, -64.0, 256.0];

    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &rhs.map(f64::to_bits));
    code.extend_from_slice(&[0xB8, 0x5A, 0x5A, 0x00, 0x00]); // mov eax, 0x5a5a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu64 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0xC2, 0xD1, 0x01]); // vcmpltps k2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x49, 0xC2, 0xD9, 0x02]); // vcmplepd k3{k1}, zmm0, zmm1
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x93, 0xC2]); // kmovq rax, k2
    ...
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_vcmp_fp_writemask_register_forms",
        &code,
        evex_qword_scratch(|i| lhs[i].to_bits()),
    );
}
...
fn evex_zmm_rcp14_rsqrt14ps_memory_forms() {
    ...
    append_seed_rdi_plus_64_qwords(&mut rcp, &source);
    rcp.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x48, 0x4C, 0x57, 0x01]); // vrcp14ps zmm2, [rdi+64]
    rcp.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    rcp.push(HLT);
    check_evex_mem("evex_zmm_vrcp14ps_memory_form", &rcp, zero_scratch());

    let mut rsqrt = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut rsqrt, &source);
    rsqrt.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x48, 0x4E, 0x57, 0x01]); // vrsqrt14ps zmm2, [rdi+64]
    rsqrt.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    rsqrt.push(HLT);
    check_evex_mem("evex_zmm_vrsqrt14ps_memory_form", &rsqrt, zero_scratch());
}

#[test]
fn evex_zmm_rcp14_rsqrt14pd_memory_forms() {
    let source = [
        3.0f64.to_bits(),
        5.0f64.to_bits(),
        7.0f64.to_bits(),
    ...
    let mut rcp = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut rcp, &source);
    rcp.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x48, 0x4C, 0x57, 0x01]); // vrcp14pd zmm2, [rdi+64]
    rcp.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    rcp.push(HLT);
    check_evex_mem("evex_zmm_vrcp14pd_memory_form", &rcp, zero_scratch());

    let mut rsqrt = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut rsqrt, &source);
    rsqrt.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x48, 0x4E, 0x57, 0x01]); // vrsqrt14pd zmm2, [rdi+64]
    rsqrt.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    rsqrt.push(HLT);
    check_evex_mem("evex_zmm_vrsqrt14pd_memory_form", &rsqrt, zero_scratch());
}
...
fn evex_xmm_ymm_rcp14_rsqrt14_vl_forms() {
    ...
    let mut xmm_ps = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut xmm_ps, &ps_source);
    xmm_ps.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x47, 0x04]); // vmovdqu64 xmm0, [rdi+64]
    xmm_ps.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4C, 0xD0]); // vrcp14ps xmm2, xmm0
    xmm_ps.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4E, 0x5F, 0x04]); // vrsqrt14ps xmm3, [rdi+64]
    ...
    xmm_pd.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x47, 0x04]); // vmovdqu64 xmm0, [rdi+64]
    xmm_pd.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4C, 0xD0]); // vrcp14pd xmm2, xmm0
    xmm_pd.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4E, 0x5F, 0x04]); // vrsqrt14pd xmm3, [rdi+64]
    ...
    ymm_ps.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x47, 0x02]); // vmovdqu64 ymm0, [rdi+64]
    ymm_ps.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x28, 0x4C, 0xD0]); // vrcp14ps ymm2, ymm0
    ymm_ps.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x28, 0x4E, 0x5F, 0x02]); // vrsqrt14ps ymm3, [rdi+64]
    ...
    ymm_pd.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x28, 0x6F, 0x47, 0x02]); // vmovdqu64 ymm0, [rdi+64]
    ymm_pd.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x28, 0x4C, 0xD0]); // vrcp14pd ymm2, ymm0
    ymm_pd.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x28, 0x4E, 0x5F, 0x02]); // vrsqrt14pd ymm3, [rdi+64]
...
fn evex_scalar_rcp14_rsqrt14ss_forms() {
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &source);
    code.extend_from_slice(&[0xC5, 0xF8, 0x10, 0x47, 0x20]); // vmovups xmm0, [rdi+32]
    code.extend_from_slice(&[0xC5, 0xF8, 0x10, 0x4F, 0x40]); // vmovups xmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4D, 0xD1]); // vrcp14ss xmm2, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4D, 0x67, 0x14]); // vrcp14ss xmm4, xmm0, [rdi+80]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4F, 0xD9]); // vrsqrt14ss xmm3, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x08, 0x4F, 0x6F, 0x14]); // vrsqrt14ss xmm5, xmm0, [rdi+80]
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x17]); // vmovups [rdi], xmm2
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x67, 0x10]); // vmovups [rdi+16], xmm4
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x5F, 0x20]); // vmovups [rdi+32], xmm3
    code.extend_from_slice(&[0xC5, 0xF8, 0x11, 0x6F, 0x30]); // vmovups [rdi+48], xmm5
    code.push(HLT);
    check_evex_mem("evex_scalar_vrcp14_vrsqrt14ss_forms", &code, scratch);
}

#[test]
fn evex_scalar_rcp14_rsqrt14sd_forms() {
    let source = [
        3.0f64.to_bits(),
        5.0f64.to_bits(),
        7.0f64.to_bits(),
    ...
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &source);
    code.extend_from_slice(&[0xC5, 0xF9, 0x10, 0x47, 0x20]); // vmovupd xmm0, [rdi+32]
    code.extend_from_slice(&[0xC5, 0xF9, 0x10, 0x4F, 0x40]); // vmovupd xmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4D, 0xD1]); // vrcp14sd xmm2, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4D, 0x67, 0x0A]); // vrcp14sd xmm4, xmm0, [rdi+80]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4F, 0xD9]); // vrsqrt14sd xmm3, xmm0, xmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x08, 0x4F, 0x6F, 0x0A]); // vrsqrt14sd xmm5, xmm0, [rdi+80]
    code.extend_from_slice(&[0xC5, 0xF9, 0x11, 0x17]); // vmovupd [rdi], xmm2
    code.extend_from_slice(&[0xC5, 0xF9, 0x11, 0x67, 0x10]); // vmovupd [rdi+16], xmm4
    code.extend_from_slice(&[0xC5, 0xF9, 0x11, 0x5F, 0x20]); // vmovupd [rdi+32], xmm3
    code.extend_from_slice(&[0xC5, 0xF9, 0x11, 0x6F, 0x30]); // vmovupd [rdi+48], xmm5
    code.push(HLT);
    check_evex_mem("evex_scalar_vrcp14_vrsqrt14sd_forms", &code, scratch);
}
...
fn evex_zmm_variable_permute_mask_merge_zero_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x36, 0xD1]); // vpermd zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x36, 0xD9]); // vpermd zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x16, 0xE1]); // vpermps zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x16, 0xE9]); // vpermps zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x16, 0xF1]); // vpermpd zmm6{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x16, 0xF9]); // vpermpd zmm7{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
...
#[test]
fn evex_zmm_immediate_shuffle_mask_merge_zero_forms() {
    let mut code = opmask_start();
    code.extend_from_slice(&[0xB8, 0x3C, 0x3C, 0xA5, 0xA5]); // mov eax, 0xa5a53c3c
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xC8]); // kmovd k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0x70, 0xD0, 0x1B]); // vpshufd zmm2{k1}, zmm0, 0x1b
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0x70, 0xD8, 0x4E]); // vpshufd zmm3{k1}{z}, zmm0, 0x4e
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x49, 0x70, 0xE0, 0xB1]); // vpshufhw zmm4{k1}, zmm0, 0xb1
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0xC9, 0x70, 0xE8, 0x4E]); // vpshufhw zmm5{k1}{z}, zmm0, 0x4e
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x49, 0x70, 0xF0, 0x93]); // vpshuflw zmm6{k1}, zmm0, 0x93
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0xC9, 0x70, 0xF8, 0x39]); // vpshuflw zmm7{k1}{z}, zmm0, 0x39
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_immediate_shuffle_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_fp_shuffle_mask_merge_zero_forms() {
    let mut code = opmask_start();
    code.extend_from_slice(&[0xB8, 0x5A, 0xA5, 0x00, 0x00]); // mov eax, 0xa55a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0xBB, 0x5A, 0x00, 0x00, 0x00]); // mov ebx, 0x5a
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0xC6, 0xD1, 0x93]); // vshufps zmm2{k1}, zmm0, zmm1, 0x93
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0xC6, 0xD9, 0x4E]); // vshufps zmm3{k1}{z}, zmm0, zmm1, 0x4e
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x4A, 0xC6, 0xE1, 0x55]); // vshufpd zmm4{k2}, zmm0, zmm1, 0x55
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xCA, 0xC6, 0xE9, 0xAA]); // vshufpd zmm5{k2}{z}, zmm0, zmm1, 0xaa
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x23, 0xF1, 0x4E]); // vshuff32x4 zmm6{k1}, zmm0, zmm1, 0x4e
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x23, 0xF9, 0xB1]); // vshuff32x4 zmm7{k1}{z}, zmm0, zmm1, 0xb1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x73, 0xFD, 0x4A, 0x23, 0xC1, 0xB1]); // vshuff64x2 zmm8{k2}, zmm0, zmm1, 0xb1
    code.extend_from_slice(&[0x62, 0x73, 0xFD, 0xCA, 0x23, 0xC9, 0x4E]); // vshuff64x2 zmm9{k2}{z}, zmm0, zmm1, 0x4e
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_fp_shuffle_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_permute_imm_mask_merge_zero_forms() {
    let mut code = opmask_start();
    code.extend_from_slice(&[0xB8, 0x5A, 0xA5, 0x00, 0x00]); // mov eax, 0xa55a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0xBB, 0x5A, 0x00, 0x00, 0x00]); // mov ebx, 0x5a
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x04, 0xD0, 0x1B]); // vpermilps zmm2{k1}, zmm0, 0x1b
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x04, 0xD8, 0x4E]); // vpermilps zmm3{k1}{z}, zmm0, 0x4e
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x4A, 0x05, 0xE0, 0x55]); // vpermilpd zmm4{k2}, zmm0, 0x55
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0xCA, 0x05, 0xE8, 0xAA]); // vpermilpd zmm5{k2}{z}, zmm0, 0xaa
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x4A, 0x00, 0xF0, 0x1B]); // vpermq zmm6{k2}, zmm0, 0x1b
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0xCA, 0x00, 0xF8, 0x4E]); // vpermq zmm7{k2}{z}, zmm0, 0x4e
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x73, 0xFD, 0x4A, 0x01, 0xC0, 0xB1]); // vpermpd zmm8{k2}, zmm0, 0xb1
    code.extend_from_slice(&[0x62, 0x73, 0xFD, 0xCA, 0x01, 0xC8, 0x4E]); // vpermpd zmm9{k2}{z}, zmm0, 0x4e
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_permute_imm_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_duplicate_shuffle_mask_merge_zero_forms() {
    let mut code = opmask_start();
    code.extend_from_slice(&[0xB8, 0x5A, 0xA5, 0x00, 0x00]); // mov eax, 0xa55a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0xBB, 0x5A, 0x00, 0x00, 0x00]); // mov ebx, 0x5a
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x49, 0x12, 0xD0]); // vmovsldup zmm2{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0xC9, 0x12, 0xD8]); // vmovsldup zmm3{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x49, 0x16, 0xE0]); // vmovshdup zmm4{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0xC9, 0x16, 0xE8]); // vmovshdup zmm5{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x4A, 0x12, 0xF0]); // vmovddup zmm6{k2}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0xCA, 0x12, 0xF8]); // vmovddup zmm7{k2}{z}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_duplicate_shuffle_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
fn evex_zmm_permil_variable_mask_merge_zero_forms() {
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x0C, 0xD1]); // vpermilps zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x0C, 0xD9]); // vpermilps zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x0D, 0xE1]); // vpermilpd zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x0D, 0xE9]); // vpermilpd zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_permil_variable_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_byte_word_shuffle_mask_merge_zero_forms() {
    let control = evex_pshufb_control_source();
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x00, 0xD1]); // vpshufb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x00, 0xD9]); // vpshufb zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x0F, 0xE1, 0x07]); // vpalignr zmm4{k1}, zmm0, zmm1, 7
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x0F, 0xE9, 0x0B]); // vpalignr zmm5{k1}{z}, zmm0, zmm1, 11
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0x60, 0xF1]); // vpunpcklbw zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0x60, 0xF9]); // vpunpcklbw zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0x4A, 0x69, 0xC1]); // vpunpckhwd zmm8{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0xCA, 0x69, 0xC9]); // vpunpckhwd zmm9{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_byte_word_shuffle_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_broadcast_mask_merge_zero_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0xB8, 0xA5, 0xA5, 0x00, 0x00]); // mov eax, 0xa5a5
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0xBB, 0x5A, 0x00, 0x00, 0x00]); // mov ebx, 0x5a
    code.extend_from_slice(&[0xC5, 0xF9, 0x92, 0xD3]); // kmovb k2, ebx
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x47, 0x01]); // vmovdqu64 zmm0, [rdi+64]

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x18, 0x57, 0x10]); // vbroadcastss zmm2{k1}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x18, 0x5F, 0x10]); // vbroadcastss zmm3{k1}{z}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x65, 0x48, 0x72, 0xD3, 0x01]); // vpsrld zmm3, zmm3, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x19, 0x67, 0x08]); // vbroadcastsd zmm4{k2}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x19, 0x6F, 0x08]); // vbroadcastsd zmm5{k2}{z}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xD5, 0x48, 0x73, 0xD5, 0x01]); // vpsrlq zmm5, zmm5, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x19, 0xF0]); // vbroadcastf32x2 zmm6{k1}, xmm0
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x19, 0xF8]); // vbroadcastf32x2 zmm7{k1}{z}, xmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x45, 0x48, 0x72, 0xD7, 0x01]); // vpsrld zmm7, zmm7, 1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    ...
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x49, 0x1A, 0x47, 0x04]); // vbroadcastf32x4 zmm8{k1}, [rdi+64]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xC9, 0x1A, 0x4F, 0x04]); // vbroadcastf32x4 zmm9{k1}{z}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xD1, 0x35, 0x48, 0x72, 0xD1, 0x01]); // vpsrld zmm9, zmm9, 1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    ...
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm10, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm11, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x49, 0x5B, 0x57, 0x02]); // vbroadcasti32x8 zmm10{k1}, [rdi+64]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xC9, 0x5B, 0x5F, 0x02]); // vbroadcasti32x8 zmm11{k1}{z}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xD1, 0x25, 0x48, 0x72, 0xD3, 0x01]); // vpsrld zmm11, zmm11, 1
    code.extend_from_slice(&[0x62, 0x51, 0x2D, 0x48, 0xEF, 0xD3]); // vpxord zmm10, zmm10, zmm11
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD2]); // vpxord zmm2, zmm2, zmm10

    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm12, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm13, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4A, 0x5B, 0x67, 0x02]); // vbroadcasti64x4 zmm12{k2}, [rdi+64]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCA, 0x5B, 0x6F, 0x02]); // vbroadcasti64x4 zmm13{k2}{z}, [rdi+64]
    code.extend_from_slice(&[0x62, 0xD1, 0x95, 0x48, 0x73, 0xD5, 0x01]); // vpsrlq zmm13, zmm13, 1
    code.extend_from_slice(&[0x62, 0x51, 0x1D, 0x48, 0xEF, 0xE5]); // vpxord zmm12, zmm12, zmm13
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm12
    ...
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_broadcast_mask_merge_zero_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_pack_saturate_mask_merge_zero_register_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0xB8, 0xA5, 0x5A, 0xA5, 0x5A]); // mov eax, 0x5aa55aa5
    code.extend_from_slice(&[0xC5, 0xFB, 0x92, 0xC8]); // kmovd k1, eax
    code.extend_from_slice(&[0x48, 0xBB]); // mov rbx, 0xa55aa55a3c3cc3c3
    code.extend_from_slice(&0xA55A_A55A_3C3C_C3C3u64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xD3]); // kmovq k2, rbx
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu64 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x49, 0x6B, 0xD1]); // vpackssdw zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xC9, 0x6B, 0xD9]); // vpackssdw zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0x4A, 0x63, 0xE1]); // vpacksswb zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x7D, 0xCA, 0x63, 0xE9]); // vpacksswb zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x2B, 0xF1]); // vpackusdw zmm6{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x2B, 0xF9]); // vpackusdw zmm7{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0x4A, 0x67, 0xC1]); // vpackuswb zmm8{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0xCA, 0x67, 0xC9]); // vpackuswb zmm9{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_pack_saturate_mask_merge_zero_register_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_funnel_shift_imm_mask_merge_zero_forms() {
    let s = evex_qword_scratch(|i| {
        0x1020_4080_0102_0305u64
            .wrapping_mul((i as u64) + 3)
            .rotate_left((i * 7) as u32)
    });
    let source = [
    ...
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &source);
    code.extend_from_slice(&[0xB8, 0xAA, 0xAA, 0x00, 0x00]); // mov eax, 0xaaaa
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x4F, 0x01]); // vmovdqu64 zmm1, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x17]); // vmovdqu32 zmm2, [rdi]
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x71, 0xD1, 0x05]); // vpshldd zmm2{k1}, zmm0, zmm1, 5
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0xC9, 0x73, 0xD9, 0x09]); // vpshrdd zmm3{k1}{z}, zmm0, zmm1, 9
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0x49, 0x71, 0xE1, 0x07]); // vpshldq zmm4{k1}, zmm0, zmm1, 7
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0xC9, 0x73, 0xE9, 0x0B]); // vpshrdq zmm5{k1}{z}, zmm0, zmm1, 11
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0xEF, 0xE5]); // vpxorq zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD4]); // vpxorq zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
...
#[test]
fn evex_zmm_funnel_shift_count_mask_merge_zero_forms() {
    let s = evex_qword_scratch(|i| {
    ...
    let source = [
        0xA5A5_0001_5A5A_0000u64,
        0x1357_0005_2468_001F,
        0xFACE_0007_CAFE_0020,
        0x89AB_000D_7654_003F,
        0x0F1E_0011_2D3C_0023,
        0x8877_001B_6655_002D,
        0x1357_0003_9BDF_0035,
        0xFFFF_000F_AAAA_003E,
    ];
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x71, 0xD1]); // vpshldvd zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x73, 0xD9]); // vpshrdvd zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x49, 0x71, 0xE1]); // vpshldvq zmm4{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xC9, 0x73, 0xE9]); // vpshrdvq zmm5{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0xEF, 0xE5]); // vpxorq zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD4]); // vpxorq zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_funnel_shift_count_mask_merge_zero_forms",
        &code,
        s,
    );
}
...
#[test]
fn evex_zmm_valign_mask_merge_zero_forms() {
    let s = evex_qword_scratch(|i| {
        0x7E57_DA7A_515E_DA7Au64
            .wrapping_add((i as u64).wrapping_mul(0x1111_2222_3333_4444))
            .rotate_left((i * 5) as u32)
    });
    let source = [
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x17]); // vmovdqu32 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF3, 0x7D, 0x49, 0x03, 0xD1, 0x05]); // valignd zmm2{k1}, zmm0, zmm1, 5
    code.extend_from_slice(&[0x62, 0xF3, 0xFD, 0xC9, 0x03, 0xD9, 0x03]); // valignq zmm3{k1}{z}, zmm0, zmm1, 3
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD3]); // vpxorq zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_valign_mask_merge_zero_forms", &code, s);
}
...
#[test]
fn evex_zmm_packed_convert_mask_merge_zero_forms() {
    let s = evex_dword_scratch(|i| ((i as f32) + 1.25).to_bits());
    let mem_words = [
        1.5f64.to_bits(),
        (-2.25f64).to_bits(),
    ...
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &mem_words);
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0x5aa5
    code.extend_from_slice(&0x5aa5u64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0xBB, 0x5A, 0x00, 0x00, 0x00]); // mov ebx, 0x5a
    code.extend_from_slice(&[0xC5, 0xF9, 0x92, 0xD3]); // kmovb k2, ebx
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x48, 0x6F, 0x07]); // vmovdqu32 zmm0, [rdi]
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x5A, 0xD0]); // vcvtps2pd zmm2{k1}, ymm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x5A, 0xD8]); // vcvtps2pd zmm3{k1}{z}, ymm0
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD3]); // vpxorq zmm2, zmm2, zmm3

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x4A, 0x5A, 0xE1]); // vcvtpd2ps ymm4{k2}, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xCA, 0x5A, 0xE9]); // vcvtpd2ps ymm5{k2}{z}, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0xEF, 0xE5]); // vpxorq zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD4]); // vpxorq zmm2, zmm2, zmm4

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x5B, 0xF0]); // vcvtdq2ps zmm6{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x5B, 0xF8]); // vcvtdq2ps zmm7{k1}{z}, zmm0
    ...
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0x49, 0x5B, 0xC0]); // vcvtps2dq zmm8{k1}, zmm0
    code.extend_from_slice(&[0x62, 0x71, 0x7D, 0xC9, 0x5B, 0xC8]); // vcvtps2dq zmm9{k1}{z}, zmm0
    code.extend_from_slice(&[0x62, 0x51, 0xBD, 0x48, 0xEF, 0xC1]); // vpxorq zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0xED, 0x48, 0xEF, 0xD0]); // vpxorq zmm2, zmm2, zmm8

    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem("evex_zmm_packed_convert_mask_merge_zero_forms", &code, s);
}
...
#[test]
fn evex_scalar_convert_mask_merge_zero_forms() {
    let mut s = [0u8; 64];
    for i in 0..16 {
        s[i] = 0x31u8.wrapping_add((i as u8).wrapping_mul(11));
        s[16 + i] = 0xA5u8.wrapping_sub((i as u8).wrapping_mul(7));
    }
    s[32..36].copy_from_slice(&9.5f32.to_le_bytes());
    s[40..48].copy_from_slice(&(-6.25f64).to_le_bytes());

    let mut code = opmask_start();
    code.extend_from_slice(&[0x31, 0xC0]); // xor eax, eax
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x47, 0x01]); // vmovdqu64 xmm0, [rdi+16]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x4F, 0x01]); // vmovdqu64 xmm1, [rdi+16]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x17]); // vmovdqu64 xmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x1F]); // vmovdqu64 xmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x09, 0x5A, 0x57, 0x08]); // vcvtss2sd xmm2{k1}, xmm0, [rdi+32]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x89, 0x5A, 0x5F, 0x08]); // vcvtss2sd xmm3{k1}{z}, xmm0, [rdi+32]
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x08, 0xEF, 0xD3]); // vpxorq xmm2, xmm2, xmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x27]); // vmovdqu64 xmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x2F]); // vmovdqu64 xmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xF7, 0x09, 0x5A, 0x67, 0x05]); // vcvtsd2ss xmm4{k1}, xmm1, [rdi+40]
    code.extend_from_slice(&[0x62, 0xF1, 0xF7, 0x89, 0x5A, 0x6F, 0x05]); // vcvtsd2ss xmm5{k1}{z}, xmm1, [rdi+40]
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x08, 0xEF, 0xE5]); // vpxorq xmm4, xmm4, xmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x08, 0xEF, 0xD4]); // vpxorq xmm2, xmm2, xmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x7F, 0x17]); // vmovdqu64 [rdi], xmm2
    code.push(HLT);
    check_evex_mem("evex_scalar_convert_mask_merge_zero_forms", &code, s);
}

#[test]
fn evex_scalar_signed_fp_gpr_conversion_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s[..16].iter_mut().enumerate() {
        *byte = 0x23u8.wrapping_add((i as u8).wrapping_mul(13));
    }

    let mem_words = [
        12.75f32.to_bits() as u64,
        (-9.75f64).to_bits(),
        0,
        (-123_456_789i32 as u32) as u64,
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &mem_words);
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x07]); // vmovdqu64 xmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7E, 0x08, 0x2A, 0x4F, 0x16]); // vcvtsi2ss xmm1, xmm0, dword ptr [rdi+88]
    code.extend_from_slice(&[0x62, 0xF1, 0xFF, 0x08, 0x2A, 0x57, 0x0C]); // vcvtsi2sd xmm2, xmm0, qword ptr [rdi+96]
    code.extend_from_slice(&[0x62, 0xF1, 0xF5, 0x08, 0xEF, 0xCA]); // vpxorq xmm1, xmm1, xmm2
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x2C, 0x47, 0x10]); // vcvttss2si r8, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x2D, 0x4F, 0x10]); // vcvtss2si r9, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x2C, 0x57, 0x09]); // vcvttsd2si r10, qword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x2D, 0x5F, 0x09]); // vcvtsd2si r11, qword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x7F, 0x0F]); // vmovdqu64 [rdi], xmm1
    code.extend_from_slice(&[0x4C, 0x89, 0x47, 0x10]); // mov [rdi+16], r8
    code.extend_from_slice(&[0x4C, 0x89, 0x4F, 0x18]); // mov [rdi+24], r9
    code.extend_from_slice(&[0x4C, 0x89, 0x57, 0x20]); // mov [rdi+32], r10
    code.extend_from_slice(&[0x4C, 0x89, 0x5F, 0x28]); // mov [rdi+40], r11
    code.push(HLT);
    check_evex_mem("evex_scalar_signed_fp_gpr_conversion_forms", &code, s);
}

#[test]
fn evex_scalar_unsigned_fp_gpr_rounding_forms() {
    let s = [0u8; 64];
    let mem_words = [
        42.75f32.to_bits() as u64,
        12_345.75f64.to_bits(),
        0x46C0,
        0,
        0,
        0,
    ...
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &mem_words);
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x78, 0x47, 0x10]); // vcvttss2usi r8, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x79, 0x4F, 0x10]); // vcvtss2usi r9, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x78, 0x57, 0x09]); // vcvttsd2usi r10, qword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x79, 0x5F, 0x09]); // vcvtsd2usi r11, qword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0x75, 0xFE, 0x08, 0x78, 0x67, 0x28]); // vcvttsh2usi r12, word ptr [rdi+80]
    code.extend_from_slice(&[0x62, 0x75, 0xFE, 0x08, 0x79, 0x6F, 0x28]); // vcvtsh2usi r13, word ptr [rdi+80]
    ...
    code.extend_from_slice(&[0x4C, 0x89, 0x67, 0x20]); // mov [rdi+32], r12
    code.extend_from_slice(&[0x4C, 0x89, 0x6F, 0x28]); // mov [rdi+40], r13
    code.push(HLT);
    check_evex_mem("evex_scalar_unsigned_fp_gpr_rounding_forms", &code, s);
}

#[test]
fn evex_scalar_fp16_signed_gpr_conversion_forms() {
    let mut s = [0u8; 64];
    for (i, byte) in s[..16].iter_mut().enumerate() {
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &mem_words);
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x07]); // vmovdqu64 xmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF5, 0x7E, 0x08, 0x2A, 0x4F, 0x12]); // vcvtsi2sh xmm1, xmm0, dword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0xF5, 0xFE, 0x08, 0x2A, 0x57, 0x0A]); // vcvtsi2sh xmm2, xmm0, qword ptr [rdi+80]
    code.extend_from_slice(&[0x62, 0xF1, 0xF5, 0x08, 0xEF, 0xCA]); // vpxorq xmm1, xmm1, xmm2
    code.extend_from_slice(&[0x62, 0x75, 0xFE, 0x08, 0x2C, 0x47, 0x20]); // vcvttsh2si r8, word ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x75, 0xFE, 0x08, 0x2D, 0x4F, 0x20]); // vcvtsh2si r9, word ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x7F, 0x0F]); // vmovdqu64 [rdi], xmm1
    code.extend_from_slice(&[0x4C, 0x89, 0x47, 0x10]); // mov [rdi+16], r8
    code.extend_from_slice(&[0x4C, 0x89, 0x4F, 0x18]); // mov [rdi+24], r9
    code.push(HLT);
    check_evex_mem("evex_scalar_fp16_signed_gpr_conversion_forms", &code, s);
}

#[test]
fn evex_scalar_fp_gpr_register_sae_rounding_forms() {
    let mut s = [0u8; 64];
    s[0..4].copy_from_slice(&0x0000_5F80u32.to_le_bytes());
    let mem_words = [
        12.75f32.to_bits() as u64,
    ...
    code.extend_from_slice(&[0xC5, 0xF8, 0xAE, 0x17]); // vldmxcsr [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x47, 0x04]); // vmovdqu64 xmm0, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x08, 0x6F, 0x4F, 0x05]); // vmovdqu64 xmm1, [rdi+80]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x38, 0x2D, 0xC0]); // vcvtss2si r8, xmm0, {rd-sae}
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x38, 0x2D, 0xC9]); // vcvtsd2si r9, xmm1, {rd-sae}
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x38, 0x79, 0xD0]); // vcvtss2usi r10, xmm0, {rd-sae}
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x38, 0x79, 0xD9]); // vcvtsd2usi r11, xmm1, {rd-sae}
    code.extend_from_slice(&[0x4C, 0x89, 0x07]); // mov [rdi], r8
    code.extend_from_slice(&[0x4C, 0x89, 0x4F, 0x08]); // mov [rdi+8], r9
    code.extend_from_slice(&[0x4C, 0x89, 0x57, 0x10]); // mov [rdi+16], r10
    code.extend_from_slice(&[0x4C, 0x89, 0x5F, 0x18]); // mov [rdi+24], r11
    code.push(HLT);
    check_evex_mem("evex_scalar_fp_gpr_register_sae_rounding_forms", &code, s);
}

#[test]
fn evex_scalar_fp_gpr_mxcsr_rounding_forms() {
    let mut s = [0u8; 64];
    s[0..4].copy_from_slice(&0x0000_5F80u32.to_le_bytes());
    let mem_words = [
        12.25f32.to_bits() as u64,
        12.25f64.to_bits(),
        0,
        0,
        0,
    ...
    append_seed_rdi_plus_64_qwords(&mut code, &mem_words);
    code.extend_from_slice(&[0xC5, 0xF8, 0xAE, 0x17]); // vldmxcsr [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x2D, 0x47, 0x10]); // vcvtss2si r8, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x2D, 0x4F, 0x09]); // vcvtsd2si r9, qword ptr [rdi+72]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x08, 0x79, 0x57, 0x10]); // vcvtss2usi r10, dword ptr [rdi+64]
    code.extend_from_slice(&[0x62, 0x71, 0xFF, 0x08, 0x79, 0x5F, 0x09]); // vcvtsd2usi r11, qword ptr [rdi+72]
    code.extend_from_slice(&[0x4C, 0x89, 0x07]); // mov [rdi], r8
    code.extend_from_slice(&[0x4C, 0x89, 0x4F, 0x08]); // mov [rdi+8], r9
    code.extend_from_slice(&[0x4C, 0x89, 0x57, 0x10]); // mov [rdi+16], r10
    code.extend_from_slice(&[0x4C, 0x89, 0x5F, 0x18]); // mov [rdi+24], r11
    code.push(HLT);
    check_evex_mem("evex_scalar_fp_gpr_mxcsr_rounding_forms", &code, s);
}
...
#[test]
fn evex_zmm_packed_multiply_mask_merge_zero_register_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0xB8, 0x5A, 0xA5, 0x00, 0x00]); // mov eax, 0xa55a
    code.extend_from_slice(&[0xC5, 0xF8, 0x92, 0xC8]); // kmovw k1, eax
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm2, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x40, 0xD1]); // vpmulld zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x40, 0xD9]); // vpmulld zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x40, 0xE1]); // vpmullq zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x40, 0xE9]); // vpmullq zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_packed_multiply_mask_merge_zero_register_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_masked_move_load_store_byte_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0x5aa55aa55aa55aa5
    code.extend_from_slice(&0x5aa55aa55aa55aa5u64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x07]); // vmovdqu8 zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x57, 0x01]); // vmovdqu8 zmm2, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x5F, 0x01]); // vmovdqu8 zmm3, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x49, 0x6F, 0x17]); // vmovdqu8 zmm2{k1}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0xC9, 0x6F, 0x1F]); // vmovdqu8 zmm3{k1}{z}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD3]); // vpxorq zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x49, 0x7F, 0x47, 0x01]); // vmovdqu8 [rdi+64]{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7F, 0x48, 0x6F, 0x67, 0x01]); // vmovdqu8 zmm4, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD4]); // vpxorq zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_masked_move_load_store_byte_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_masked_move_load_store_fp_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0xa55aa55a
    code.extend_from_slice(&0xa55aa55au64.to_le_bytes());
    code.extend_from_slice(&[0xC4, 0xE1, 0xFB, 0x92, 0xC8]); // kmovq k1, rax
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x48, 0x10, 0x07]); // vmovups zmm0, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x48, 0x10, 0x57, 0x01]); // vmovups zmm2, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x48, 0x10, 0x5F, 0x01]); // vmovups zmm3, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x10, 0x17]); // vmovups zmm2{k1}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0xC9, 0x10, 0x1F]); // vmovups zmm3{k1}{z}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD3]); // vpxorq zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x48, 0x10, 0x67, 0x01]); // vmovupd zmm4, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x48, 0x10, 0x6F, 0x01]); // vmovupd zmm5, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x49, 0x10, 0x27]); // vmovupd zmm4{k1}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0xC9, 0x10, 0x2F]); // vmovupd zmm5{k1}{z}, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xDD, 0x48, 0xEF, 0xE5]); // vpxorq zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD4]); // vpxorq zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x49, 0x11, 0x47, 0x01]); // vmovups [rdi+64]{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0xFD, 0x49, 0x11, 0x47, 0x01]); // vmovupd [rdi+64]{k1}, zmm0
    code.extend_from_slice(&[0x62, 0xF1, 0x7C, 0x48, 0x10, 0x77, 0x01]); // vmovups zmm6, [rdi+64]
    code.extend_from_slice(&[0x62, 0xF1, 0xED, 0x48, 0xEF, 0xD6]); // vpxorq zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_masked_move_load_store_fp_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
...
#[test]
fn evex_zmm_blend_select_mask_merge_zero_register_forms() {
    let mut code = opmask_start();
    append_seed_rdi_plus_64_qwords(&mut code, &evex_shuffle_source());
    code.extend_from_slice(&[0x48, 0xB8]); // mov rax, 0xa55aa55a3c3cc3c3
    code.extend_from_slice(&0xA55A_A55A_3C3C_C3C3u64.to_le_bytes());
    ...
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm3, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x49, 0x66, 0xD1]); // vpblendmb zmm2{k1}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xC9, 0x66, 0xD9]); // vpblendmb zmm3{k1}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD3]); // vpxord zmm2, zmm2, zmm3
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm4, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm5, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0x4A, 0x66, 0xE1]); // vpblendmw zmm4{k2}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0xFD, 0xCA, 0x66, 0xE9]); // vpblendmw zmm5{k2}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x5D, 0x48, 0xEF, 0xE5]); // vpxord zmm4, zmm4, zmm5
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm4
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x37]); // vmovdqu64 zmm6, [rdi]
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x6F, 0x3F]); // vmovdqu64 zmm7, [rdi]
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0x4B, 0x64, 0xF1]); // vpblendmd zmm6{k3}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF2, 0x7D, 0xCB, 0x64, 0xF9]); // vpblendmd zmm7{k3}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0xF1, 0x4D, 0x48, 0xEF, 0xF7]); // vpxord zmm6, zmm6, zmm7
    code.extend_from_slice(&[0x62, 0xF1, 0x6D, 0x48, 0xEF, 0xD6]); // vpxord zmm2, zmm2, zmm6
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x07]); // vmovdqu64 zmm8, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x0F]); // vmovdqu64 zmm9, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4C, 0x64, 0xC1]); // vpblendmq zmm8{k4}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCC, 0x64, 0xC9]); // vpblendmq zmm9{k4}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0x3D, 0x48, 0xEF, 0xC1]); // vpxord zmm8, zmm8, zmm9
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD0]); // vpxord zmm2, zmm2, zmm8
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x17]); // vmovdqu64 zmm10, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x1F]); // vmovdqu64 zmm11, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0x4B, 0x65, 0xD1]); // vblendmps zmm10{k3}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0x7D, 0xCB, 0x65, 0xD9]); // vblendmps zmm11{k3}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0x2D, 0x48, 0xEF, 0xD3]); // vpxord zmm10, zmm10, zmm11
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD2]); // vpxord zmm2, zmm2, zmm10
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x27]); // vmovdqu64 zmm12, [rdi]
    code.extend_from_slice(&[0x62, 0x71, 0xFE, 0x48, 0x6F, 0x2F]); // vmovdqu64 zmm13, [rdi]
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0x4C, 0x65, 0xE1]); // vblendmpd zmm12{k4}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x72, 0xFD, 0xCC, 0x65, 0xE9]); // vblendmpd zmm13{k4}{z}, zmm0, zmm1
    code.extend_from_slice(&[0x62, 0x51, 0x1D, 0x48, 0xEF, 0xE5]); // vpxord zmm12, zmm12, zmm13
    code.extend_from_slice(&[0x62, 0xD1, 0x6D, 0x48, 0xEF, 0xD4]); // vpxord zmm2, zmm2, zmm12
    code.extend_from_slice(&[0x62, 0xF1, 0xFE, 0x48, 0x7F, 0x17]); // vmovdqu64 [rdi], zmm2
    code.push(HLT);
    check_evex_mem(
        "evex_zmm_blend_select_mask_merge_zero_register_forms",
        &code,
        evex_shuffle_scratch(),
    );
}
