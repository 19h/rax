...
fn test_fcmovb_when_cf_set() {
    // Set CF with STC, then FCMOVB should move
    let code = [
    ...
    assert_eq!(read_f64(&mem, 0x3000), 1.0); // Should have moved ST(1) to ST(0)
}

#[test]
fn test_fcmovb_when_cf_clear() {
    // Clear CF with CLC, then FCMOVB should not move
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0 -> ST(0)
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0 -> ST(0), ST(1)=1.0
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 2.0); // Should remain ST(0)
}

#[test]
fn test_fcmovb_st2() {
    // Test FCMOVB with ST(2)
    let code = [
...
fn test_fcmovb_after_cmp() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 5.0); // Should move because CF=1
}
...
fn test_fcmove_when_zf_set() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 1.0); // Should move
}

#[test]
fn test_fcmove_when_zf_clear() {
    // Clear ZF by comparing different values
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
        0xB8, 0x01, 0x00, 0x00, 0x00, // MOV EAX, 1
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 2.0); // Should NOT move
}
...
fn test_fcmove_st3() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 1.0); // Should move ST(3)
}
...
fn test_fcmovu_when_pf_set() {
    // Set PF by testing a value with even parity
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
...
fn test_fcmovu_when_pf_clear() {
    // Clear PF by testing a value with odd parity
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
...
fn test_fcmovnb_after_cmp_greater() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 5.0); // Should move
}
...
#[test]
fn test_fcmov_chain() {
    // Chain multiple conditional moves
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
        0xDD, 0x04, 0x25, 0x10, 0x20, 0x00, 0x00, // FLD 3.0
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 3.0); // Should remain ST(0)
}

#[test]
fn test_fcmov_preserves_other_registers() {
    // Verify that FCMOV only affects ST(0)
    let code = [
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
        0xDD, 0x04, 0x25, 0x10, 0x20, 0x00, 0x00, // FLD 3.0
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 2.0); // ST(0) after move
    assert_eq!(read_f64(&mem, 0x3008), 2.0); // ST(1) unchanged
    assert_eq!(read_f64(&mem, 0x3010), 1.0); // ST(2) unchanged
}

#[test]
fn test_fcmov_with_arithmetic() {
    // Use FCMOV with arithmetic operations
    let code = [
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 20.0); // 10.0 + 10.0 after move
}

#[test]
fn test_fcmov_all_conditions() {
    // Test that each condition works independently
    let code = [
        // Test FCMOVB
        0xDD, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // FLD 1.0
        0xDD, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00, // FLD 2.0
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_f64(&mem, 0x3000), 1.0); // FCMOVB result
    assert_eq!(read_f64(&mem, 0x3008), 1.0); // FCMOVE result
}
