...
fn test_adcx_32bit_no_carry() {
    ...
    assert_eq!(regs.rflags & 0x01, 0, "CF should be clear (no carry out)");
}
...
fn test_adcx_32bit_with_carry_in() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 9, "EAX should be 9");
}
...
fn test_adcx_32bit_carry_out() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0, "EAX should be 0 (wrapped)");
    assert_ne!(regs.rflags & 0x01, 0, "CF should be set (carry out)");
}
...
fn test_adcx_32bit_chain() {
    ...
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0, "ECX should be 0 (propagated carry)");
    assert_ne!(regs.rflags & 0x01, 0, "CF should still be set");
}
...
fn test_adcx_64bit_no_carry() {
    ...
    assert_eq!(regs.rflags & 0x01, 0, "CF should be clear");
}
...
fn test_adcx_64bit_with_carry() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 9, "RAX should be 9");
}
...
fn test_adcx_64bit_large_values() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0xC000000000000000, "RAX should be 0xC000000000000000");
}
...
fn test_adox_32bit_preserves_cf() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_ne!(regs.rflags & 0x01, 0, "CF should still be set (ADOX doesn't modify CF)");
}
...
fn test_multiprecision_128bit_add() {
    // Add two 128-bit numbers using both ADCX and ADOX
    // Number 1: 0x0000000200000001
    // Number 2: 0x0000000400000003
    // Result:   0x0000000600000004
    let code = [
        0xf8, // CLC
        0x31, 0xc0, // XOR EAX, EAX (clear OF)
        // Low 64 bits: 1 + 3 = 4
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 4, "RAX (low 64) should be 4");
    assert_eq!(regs.rcx, 6, "RCX (high 64) should be 6");
}

#[test]
fn test_multiprecision_dual_chain() {
    // Using both CF and OF chains for parallel addition
    let code = [
        0xf8, // CLC
        0x31, 0xc0, // XOR EAX, EAX (clear OF)
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0, "EAX should be 0");
    assert_eq!(regs.rcx & 0xFFFFFFFF, 1, "ECX should be 1");
    assert_eq!(regs.rsi & 0xFFFFFFFF, 1, "ESI should be 1 (carry propagated)");
}
...
fn test_adcx_memory_operand() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x00000000, "Result should wrap");
    assert_ne!(regs.rflags & 0x01, 0, "CF should be set");
}
...
fn test_adox_memory_operand() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 30, "EAX should be 30");
}
...
fn test_256bit_addition_pattern() {
    // Add two 256-bit numbers (4 x 64-bit limbs)
    let code = [
        0xf8, // CLC
    ...
    assert_eq!(regs.rax, 0, "Limb 0 should be 0 (wrapped)");
    assert_eq!(regs.rcx, 1, "Limb 1 should be 1 (carry propagated)");
    assert_eq!(regs.rsi, 1, "Limb 2 should be 1 (carry propagated)");
}

#[test]
fn test_interleaved_adcx_adox() {
    // Pattern for dual-chain multi-precision arithmetic
    let code = [
        0xf8, // CLC
        0x31, 0xc0, // XOR EAX, EAX (clear OF)
    ...
    assert_eq!(regs.rcx & 0xFFFFFFFF, 9, "ECX should be 9");
    assert_eq!(regs.rsi & 0xFFFFFFFF, 13, "ESI should be 13");
}
...
fn test_adox_32bit_repeated_operations() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 64, "EAX should be 64");
}
...
fn test_adcx_power_of_two() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x02000000, "EAX should be 0x02000000");
}
...
fn test_adox_power_of_two() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x04000000, "EAX should be 0x04000000");
}
...
fn test_adcx_64bit_power_of_two() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x0000000200000000, "RAX should be 0x0000000200000000");
}
...
fn test_adox_64bit_power_of_two() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x0000000400000000, "RAX should be 0x0000000400000000");
}
...
fn test_adcx_carry_chain_4_limbs() {
    ...
    assert_eq!(regs.rsi & 0xFFFFFFFF, 0, "ESI should be 0");
    assert_ne!(regs.rflags & 0x01, 0, "CF should still be set");
}
...
fn test_adox_overflow_chain_4_limbs() {
    ...
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0, "ECX should be 0");
    assert_eq!(regs.rsi & 0xFFFFFFFF, 0, "ESI should be 0");
    assert_ne!(regs.rflags & 0x800, 0, "OF should still be set");
}
...
fn test_adcx_adox_alternating() {
    ...
    assert_eq!(regs.rcx & 0xFFFFFFFF, 9, "ECX should be 9");
    assert_eq!(regs.rsi & 0xFFFFFFFF, 13, "ESI should be 13");
    assert_eq!(regs.r8 & 0xFFFFFFFF, 17, "R8D should be 17");
}
...
fn test_adcx_with_initial_carry_propagation() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 1, "EAX should be 1 (carry in)");
}
...
fn test_adox_with_initial_overflow_propagation() {
    ...
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 1, "EAX should be 1 (overflow in)");
}
