...
fn test_adc_al_imm8_no_carry() {
    ...
    assert_eq!(regs.rax & 0xFF, 15, "ADC AL, 5: 10 + 5 + 0 = 15");
}
...
fn test_adc_al_imm8_overflow_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 0x80, "ADC AL, 0: 0x7F + 0 + 1 = 0x80");
...
fn test_adc_rm8_imm8_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 15, "ADC CL, 10: 5 + 10 + 0 = 15");
}
...
fn test_adc_rm8_imm8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 16, "ADC CL, 10: 5 + 10 + 1 = 16");
}
...
fn test_adc_rm8_imm8_memory_with_carry() {
    ...
    assert_eq!(read_mem_u8(&mem), 36, "ADC [RBX], 10: 25 + 10 + 1 = 36");
}
...
fn test_adc_rm64_imm32_register_with_carry() {
    ...
    assert_eq!(regs.rcx, 0x100000000, "ADC RCX, -1: 0x100000000 + (-1) + 1");
}
...
fn test_adc_rm32_imm8_positive_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 111, "ADC ECX, 10: 100 + 10 + 1 = 111");
}
...
fn test_adc_rm32_imm8_negative_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 91, "ADC ECX, -10: 100 + (-10) + 1 = 91");
}
...
fn test_adc_rm8_r8_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 15, "ADC AL, CL: 10 + 5 + 0 = 15");
}
...
fn test_adc_rm8_r8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFF, 16, "ADC AL, CL: 10 + 5 + 1 = 16");
}
...
fn test_adc_rm8_r8_memory_with_carry() {
    ...
    let _ = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u8(&mem), 51, "ADC [RBX], CL: 30 + 20 + 1 = 51");
}
...
fn test_adc_rm32_r32_register_no_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x23456789, "ADC EAX, ECX with CF=0");
}
...
fn test_adc_rm32_r32_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x80000001, "ADC EAX, ECX: 0x7FFFFFFF + 1 + 1");
}
...
fn test_adc_rm64_r64_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0, "ADC RAX, RCX: max + 0 + 1 = 0 with overflow");
...
fn test_adc_rm32_r32_memory_with_carry() {
    ...
    let _ = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u32(&mem), 0x12346679, "ADC [RBX], ECX with CF=1");
}
...
fn test_adc_r8_rm8_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 16, "ADC CL, AL: 5 + 10 + 1 = 16");
}
...
fn test_adc_r8_rm8_memory_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFF, 51, "ADC CL, [RBX]: 20 + 30 + 1 = 51");
}
...
fn test_adc_r32_rm32_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x33333334, "ADC ECX, EAX: with CF=1");
}
...
fn test_adc_r64_rm64_register_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x3333333333333334, "ADC RCX, RAX with CF=1");
}
...
fn test_adc_r32_rm32_memory_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x12346679, "ADC ECX, [RBX] with CF=1");
}
...
fn test_adc_chain_64bit_addition() {
    // Test adding two 64-bit values using 32-bit ADC chain
    ...
    // But using 32-bit operations:
    // Low: 0x00000001 + 0x00000001 = 0x00000002 (no carry)
    // High: 0xFFFFFFFF + 0x00000000 + 0 (no carry from low) = 0xFFFFFFFF
    regs.rax = 0x00000001; // EAX = low word of first number
    regs.rcx = 0x00000001; // ECX = low word of second number
    regs.rbx = 0xFFFFFFFF; // EBX = high word of first number
    regs.rdx = 0x00000000; // EDX = high word of second number
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
...
fn test_adc_chain_with_carry_propagation() {
    // Test carry propagation through ADC chain
    ...
    regs.rax = 0xFFFFFFFF; // Low word of first number
    regs.rcx = 0x00000001; // Low word of second number
    regs.rbx = 0x00000000; // High word of first number
    regs.rdx = 0x00000000; // High word of second number
    let (mut vcpu, _) = setup_vm(&code, Some(regs));

    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0, "Low 32 bits: 0xFFFFFFFF + 1 = 0 (with carry)");
    assert_eq!(regs.rbx, 1, "High 32 bits: 0 + 0 + 1 (carry) = 1");
}
...
fn test_adc_overflow_with_carry() {
    // Signed overflow: 0x7E + 0x00 + 1 = 0x7F (no overflow)
    // But 0x7F + 0x00 + 1 = 0x80 (overflow)
...
fn test_adc_r8_extended_with_carry() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r8, 151, "ADC R8D, 100: 50 + 100 + 1 = 151");
}
