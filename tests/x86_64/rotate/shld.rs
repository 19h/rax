...
fn test_shld_cx_dx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFF, 0x6789, "CX: 0x5678 SHLD 4 from 0x9ABC");
}
...
fn test_shld_dx_si_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFF, 0x1112, "DX: 0x1111 SHLD 4 from 0x2222");
}
...
fn test_shld_eax_1bit() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x80000000, "EAX: 0x40000000 SHLD 1");
...
fn test_shld_ecx_edx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0x11111222, "ECX: SHLD 12");
}
...
fn test_shld_edx_esi_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx & 0xFFFFFFFF, 0xFFFF0000, "EDX: SHLD 16");
}
...
fn test_shld_esi_edi_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi & 0xFFFFFFFF, 0x55AA5511, "ESI: SHLD 8");
}
...
fn test_shld_edi_ebx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi & 0xFFFFFFFF, 0x12345678, "EDI: SHLD 4");
}
...
fn test_shld_rax_rbx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x23456789ABCDEF0F, "RAX: SHLD 4");
}
...
fn test_shld_rax_rbx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x3456789ABCDEF0FE, "RAX: SHLD 8");
}
...
fn test_shld_rax_1bit() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x8000000000000000, "RAX: SHLD 1");
    assert!(of_set(regs.rflags), "OF: sign changed");
}
...
fn test_shld_rax_16bits() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x56789ABCDEF0AAAA, "RAX: SHLD 16");
}
...
fn test_shld_rax_32bits() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rax, 0x9ABCDEF0FEDCBA98, "RAX: SHLD 32");
}
...
fn test_shld_rcx_rdx_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rcx, 0x1111111111111222, "RCX: SHLD 12");
}
...
fn test_shld_rdx_rsi_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdx, 0xFFFFFFFFFFFF0000, "RDX: SHLD 16");
}
...
fn test_shld_rsi_rdi_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rsi, 0x55AA55AA55AA5511, "RSI: SHLD 8");
}
...
fn test_shld_rdi_rbx_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.rdi, 0x123456789ABCDEFF, "RDI: SHLD 4");
}
...
fn test_shld_r8_r9_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r8, 0x3456789ABCDEF0FE, "R8: SHLD 8");
}
...
fn test_shld_r10_r11_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r10, 0xAAAAAAAAAAAAAAA5, "R10: SHLD 4");
}
...
fn test_shld_r12_r13_imm8() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r12, 0x56789ABCDEF01111, "R12: SHLD 16");
}
...
fn test_shld_r14_r15_cl() {
    ...
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(regs.r14, 0x0F0F0F0F0F0F0F00, "R14: SHLD 4");
}
...
fn test_shld_mem32_cl() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u32(&mem), 0x345678AB, "Memory: SHLD 8");
}
...
fn test_shld_mem64_imm8() {
    ...
    run_until_hlt(&mut vcpu).unwrap();
    assert_eq!(read_mem_u64(&mem), 0x56789ABCDEF0AAAA, "Memory: SHLD 16");
}
...
fn test_shld_sf_flag() {
    let code = [0x0f, 0xa4, 0xd8, 0x01, 0xf4]; // SHLD EAX, EBX, 1
    let mut regs = Registers::default();
    regs.rax = 0x40000000; // Will become negative
    regs.rbx = 0x00000000;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_shld_pf_flag() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x12345678;
    regs.rbx = 0x000000FF; // Low byte will be 0xFF (even parity)
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_shld_masked_count_32bit() {
    ...
    let regs = run_until_hlt(&mut vcpu).unwrap();
    // 36 & 0x1F = 4
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x2345678A, "Count masked to 4");
}
