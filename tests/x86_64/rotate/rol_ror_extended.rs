...
fn test_rol_all_counts_8bit() {
    ...
    for count in 0..=8 {
        ...
        assert_eq!(regs.rax & 0xFF, expected, "ROL AL by {} failed", count);
    }
}
...
fn test_ror_all_counts_8bit() {
    ...
    for count in 0..=8 {
        ...
        assert_eq!(regs.rax & 0xFF, expected, "ROR AL by {} failed", count);
    }
}
...
fn test_rol_boundary_8bit() {
    ...
    for &value in &test_values {
        ...
        assert_eq!(regs.rax & 0xFF, expected, "ROL 0x{:02X} by 1", value);
    }
}
...
fn test_ror_boundary_8bit() {
    ...
    for &value in &test_values {
        ...
        let expected = ((value >> 1) | (value << 7)) & 0xFF;
        assert_eq!(regs.rax & 0xFF, expected, "ROR 0x{:02X} by 1", value);
    }
}
...
fn test_rol_boundary_32bit() {
    ...
    for &value in &test_values {
        ...
        assert_eq!(regs.rax & 0xFFFFFFFF, expected, "ROL 0x{:08X} by 1", value);
    }
}
...
fn test_ror_boundary_32bit() {
    ...
    for &value in &test_values {
        ...
        let expected = ((value >> 1) | (value << 31)) & 0xFFFFFFFF;
        assert_eq!(regs.rax & 0xFFFFFFFF, expected, "ROR 0x{:08X} by 1", value);
    }
}
...
fn test_rol_boundary_64bit() {
    ...
    for &value in &test_values {
        ...
        assert_eq!(regs.rax, expected, "ROL 0x{:016X} by 1", value);
    }
}
...
fn test_ror_boundary_64bit() {
    ...
    for &value in &test_values {
        ...
        let expected = (value >> 1) | (value << 63);
        assert_eq!(regs.rax, expected, "ROR 0x{:016X} by 1", value);
    }
}
...
fn test_rol_count_masking_8bit() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x12;
    regs.rcx = 0x1F; // 31 - should be masked
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
    ...
    assert_eq!(regs.rax & 0xFF, regs2.rax & 0xFF, "Count masking for 8-bit");
}
...
fn test_rol_count_masking_32bit() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x12345678;
    regs.rcx = 0x24; // 36 - should be masked to 4 (36 & 0x1F = 4)
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    let code2 = [0xc1, 0xc0, 0x04, 0xf4];
    let mut regs2 = Registers::default();
    regs2.rax = 0x12345678;
    let (mut vcpu2, _) = setup_vm(&code2, Some(regs2));
    let regs2 = run_until_hlt(&mut vcpu2).unwrap();
...
fn test_ror_count_masking_32bit() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x12345678;
    regs.rcx = 0x24; // 36 - should be masked to 4
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_rol_count_masking_64bit() {
    ...
    regs.rcx = 0x48; // 72 - should be masked to 8 (72 & 0x3F = 8)
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    let code2 = [0x48, 0xc1, 0xc0, 0x08, 0xf4];
    let mut regs2 = Registers::default();
    regs2.rax = 0x123456789ABCDEF0;
    let (mut vcpu2, _) = setup_vm(&code2, Some(regs2));
    let regs2 = run_until_hlt(&mut vcpu2).unwrap();

    assert_eq!(regs.rax, regs2.rax, "Count masking for 64-bit");
}
...
fn test_ror_count_masking_64bit() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x123456789ABCDEF0;
    regs.rcx = 0x48; // 72 - should be masked to 8
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_ror_of_transitions() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0x01; // Will become 0x80 (MSB set)
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFF, 0x80, "Result");
    // OF should be set
    assert!(of_set(regs.rflags), "OF for ROR with MSB change");
}
...
fn test_rol_alternating_pattern() {
    ...
    let mut regs = Registers::default();
    regs.rax = 0xAA; // 1010_1010
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFF, 0xAA, "Alternating pattern ROL 4");
}
...
fn test_ror_alternating_pattern() {
    ...
    assert_eq!(regs.rax & 0xFF, 0xAA, "Alternating pattern ROR 4");
}
...
fn test_rol_sequential_rotations() {
    ...
    assert_eq!(regs.rax & 0xFF, regs2.rax & 0xFF, "Sequential rotations");
}
...
fn test_rol_ror_identity() {
    ...
    assert_eq!(regs.rax & 0xFF, 0x42, "ROL then ROR returns original");
}
