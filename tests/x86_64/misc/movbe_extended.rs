...
fn test_movbe_16bit_register_to_memory() {
    ...
    let code = [
        0x66, 0xb8, 0x12, 0x34,        // MOV EAX, 0x3412 (16-bit immediate with prefix)
        0x66, 0x0f, 0x38, 0xf1, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE [0x2000], AX
        0xf4,                          // HLT
    ];
    ...
    let value = read_mem_at_u16(&mem, 0x2000);
...
fn test_movbe_32bit_register_to_memory() {
    ...
    let code = [
        ...
        0x0f, 0x38, 0xf1, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE [0x2000], EAX
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    let _ = run_until_hlt(&mut vcpu).unwrap();
    ...
    let value = read_mem_at_u32(&mem, 0x2000);
...
fn test_movbe_64bit_register_to_memory() {
    ...
    let code = [
        ...
        0x48, 0x0f, 0x38, 0xf1, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE [0x2000], RAX
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    let _ = run_until_hlt(&mut vcpu).unwrap();
    ...
    let value = read_mem_at_u64(&mem, 0x2000);
...
fn test_movbe_register_to_memory_preserves_source() {
    ...
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x12345678, "Source register unchanged");
}
...
fn test_movbe_different_registers() {
    ...
    let code = [
        ...
        0x0f, 0x38, 0xf1, 0x1d, 0x00, 0x20, 0x00, 0x00, // MOVBE [0x2000], EBX
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    let _ = run_until_hlt(&mut vcpu).unwrap();
...
fn test_movbe_multiple_stores() {
    ...
    let code = [
        ...
        0xbb, 0xAA, 0xBB, 0xCC, 0xDD, // MOV EBX, 0xDDCCBBAA
        0x0f, 0x38, 0xf1, 0x1d, 0x04, 0x20, 0x00, 0x00, // MOVBE [0x2004], EBX
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    let _ = run_until_hlt(&mut vcpu).unwrap();

    let value1 = read_mem_at_u32(&mem, 0x2000);
    let value2 = read_mem_at_u32(&mem, 0x2004);
...
fn test_movbe_16bit_memory_to_register() {
    ...
    let code = [
        0x66, 0x0f, 0x38, 0xf0, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE AX, [0x2000]
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u16(&mem, 0x2000, 0x1234);

    let regs = run_until_hlt(&mut vcpu).unwrap();

    // Should swap bytes: 0x1234 from memory -> 0x3412 in register
    assert_eq!(regs.rax & 0xFFFF, 0x3412, "16-bit value should be byte-swapped");
}
...
fn test_movbe_32bit_memory_to_register() {
    ...
    let code = [
        0x0f, 0x38, 0xf0, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE EAX, [0x2000]
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    ...
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x78563412, "32-bit value should be byte-swapped");
}
...
fn test_movbe_64bit_memory_to_register() {
    ...
    let code = [
        0x48, 0x0f, 0x38, 0xf0, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE RAX, [0x2000]
        0xf4,                          // HLT
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x0123456789ABCDEF);

    let regs = run_until_hlt(&mut vcpu).unwrap();

    // Should swap bytes
    assert_eq!(regs.rax, 0xEFCDAB8967452301, "64-bit value should be byte-swapped");
}
...
fn test_movbe_memory_to_register_overwrites_dest() {
    ...
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x78563412, "Destination overwritten with swapped value");
}
...
fn test_movbe_roundtrip_32bit() {
    ...
    let code = [
        0xb8, 0x12, 0x34, 0x56, 0x78, // MOV EAX, 0x78563412
        0x0f, 0x38, 0xf1, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE [0x2000], EAX (stores swapped)

        0x31, 0xc0,                    // XOR EAX, EAX (clear EAX)
        0x0f, 0x38, 0xf0, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE EAX, [0x2000] (loads and swaps)
        0xf4,                          // HLT
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    let regs = run_until_hlt(&mut vcpu).unwrap();
    ...
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x78563412, "Roundtrip restores original value");
}
...
fn test_movbe_roundtrip_64bit() {
    ...
    assert_eq!(regs.rax, 0x7766554433221100, "64-bit roundtrip restores value");
}
...
fn test_movbe_preserves_other_registers() {
    ...
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x11111111, "EAX preserved");
}

#[test]
fn test_movbe_16bit_partial_register() {
    // MOVBE with 16-bit should zero-extend to 32-bit
    let code = [
        0xb8, 0xFF, 0xFF, 0xFF, 0xFF, // MOV EAX, 0xFFFFFFFF
        0x66, 0x0f, 0x38, 0xf0, 0x05, 0x00, 0x20, 0x00, 0x00, // MOVBE AX, [0x2000]
        0xf4,                          // HLT
    ...
    // Lower 16 bits should be swapped, upper 16 bits may be zero-extended
    assert_eq!(regs.rax & 0xFFFF, 0x3412, "16-bit value byte-swapped");
}
...
fn test_movbe_sequential_operations() {
    ...
    let code = [
        ...
        0x0f, 0x38, 0xf1, 0x1d, 0x08, 0x20, 0x00, 0x00, // MOVBE [0x2008], EBX
    ...
    let mem_val1 = read_mem_at_u32(&mem, 0x2000);
    let mem_val2 = read_mem_at_u32(&mem, 0x2008);
    assert_eq!(mem_val1, 0x44332211, "First value stored correctly");
    assert_eq!(mem_val2, 0x88776655, "Second value stored correctly");

    // Verify loaded value
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x44332211, "Value loaded and swapped");
}
