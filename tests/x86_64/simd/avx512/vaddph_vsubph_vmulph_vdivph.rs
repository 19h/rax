...
fn test_vmulph_xmm_by_one() {
    // Test multiplication by 1.0 (identity)
    let code = [
...
fn test_fp16_all_sizes() {
    // Test all vector sizes for VADDPH
    let code = [
...
fn test_fp16_rounding_modes() {
    // Test different rounding modes with ZMM
    let code = [
...
fn test_fp16_high_registers() {
    // Test with high registers (R8-R15)
    let code = [
