...
fn test_vpsrlvq_ymm5_ymm6_mem() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // Maximum shift counts (63 and beyond)
    let data = vec![
...
fn test_vpsrlvd_zero_shifts() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = vec![0x00; 32]; // All zero shift counts
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_vpsrlvq_max_shifts() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // All shift counts = 63 (max useful for 64-bit)
    let data = vec![
        0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
fn test_vpsrlvd_overflow_shifts() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // Shift counts > 31 (should zero out elements)
    let data = vec![
...
fn test_vpsrlvq_overflow_shifts() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // Shift counts > 63 (should zero out elements)
    let data = vec![
        0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
