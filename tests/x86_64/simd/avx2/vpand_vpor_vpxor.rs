...
fn test_vpand_ymm0_ymm1_mem() {
    ...
    mem.write_slice(&[0x0F; 32], GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_vpor_ymm0_ymm1_mem() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0xF0; 32], GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_logical_ops_mem_unaligned() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0xFF; 33], GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
