...
fn test_vpavgb_ymm6_ymm7_ymm8_max_values() {
    // Test averaging 0xFF and 0xFF (should give 0xFF with rounding)
    let code = [
...
fn test_vpavgb_rounding_up() {
    // Test rounding: (1 + 2 + 1) / 2 = 2 (rounds up from 2.0)
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
fn test_vpavgb_rounding_even_odd() {
    // Test rounding with various even/odd combinations
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
fn test_vpavgb_overflow_prevention() {
    // Average of two max values: (0xFF + 0xFF + 1) / 2 = 0xFF
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
fn test_vpavgw_ymm6_ymm7_ymm8_max_values() {
    // Test averaging 0xFFFF and 0xFFFF
    let code = [
...
fn test_vpavgw_overflow_prevention() {
    // Average of two max values: (0xFFFF + 0xFFFF + 1) / 2 = 0xFFFF
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
fn test_vpavgb_chain_multiple_ops() {
    // Chain multiple VPAVGB operations
    let code = [
        0xc5, 0xf5, 0xe0, 0xc2, // VPAVGB YMM0, YMM1, YMM2
...
fn test_vpavgw_chain_multiple_ops() {
    // Chain multiple VPAVGW operations
    let code = [
        0xc5, 0xf5, 0xe3, 0xc2, // VPAVGW YMM0, YMM1, YMM2
...
fn test_vpavgb_zero_and_max() {
    // Average of 0x00 and 0xFF: (0 + 255 + 1) / 2 = 128
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
