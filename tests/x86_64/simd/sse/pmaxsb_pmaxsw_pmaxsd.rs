...
//
// Performs SIMD compare of packed signed integers and returns maximum values.
//
// PMAXSB: Maximum of 16 packed signed byte integers (SSE4.1)
// PMAXSW: Maximum of 8 packed signed word integers (SSE2/SSE)
// PMAXSD: Maximum of 4 packed signed dword integers (SSE4.1)
//
// Opcodes:
// 66 0F 38 3C /r      PMAXSB xmm1, xmm2/m128   - Max of packed signed bytes
// 66 0F EE /r         PMAXSW xmm1, xmm2/m128   - Max of packed signed words
// 66 0F 38 3D /r      PMAXSD xmm1, xmm2/m128   - Max of packed signed dwords

const ALIGNED_ADDR: u64 = 0x3000;
const ALIGNED_ADDR2: u64 = 0x3100;

// ============================================================================
// PMAXSB Tests - 16x Signed Byte Maximum
// ============================================================================

#[test]
fn test_pmaxsb_all_zeros() {
    let code = [0x48, 0xb8];
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x0f, 0x38, 0x3c, 0xc1, // PMAXSB XMM0, XMM1
        0xf4, // HLT
    ]);
...
#[test]
fn test_pmaxsb_positive_negative() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x7F; 16], GuestAddress(ALIGNED_ADDR)).unwrap(); // +127
    mem.write_slice(&[0x80; 16], GuestAddress(ALIGNED_ADDR2)).unwrap(); // -128
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsb_mixed_values() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
#[test]
fn test_pmaxsb_negative_values() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0xFF; 16], GuestAddress(ALIGNED_ADDR)).unwrap(); // -1
    mem.write_slice(&[0xFE; 16], GuestAddress(ALIGNED_ADDR2)).unwrap(); // -2
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsb_extremes() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x7F; 16], GuestAddress(ALIGNED_ADDR)).unwrap(); // Max positive
    mem.write_slice(&[0x01; 16], GuestAddress(ALIGNED_ADDR2)).unwrap(); // 1
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsb_xmm2_xmm3() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x0f, 0x38, 0x3c, 0xd3, // PMAXSB XMM2, XMM3
        0xf4,
    ]);
...
fn test_pmaxsb_from_memory() {
    ...
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0x38, 0x3c, 0x03, // PMAXSB XMM0, [RBX]
        0xf4,
    ]);

    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x11; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x22; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsb_alternating_sign() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
// ============================================================================
// PMAXSW Tests - 8x Signed Word Maximum
// ============================================================================

#[test]
fn test_pmaxsw_all_zeros() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
#[test]
fn test_pmaxsw_positive_negative() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
#[test]
fn test_pmaxsw_negative_values() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
fn test_pmaxsw_xmm4_xmm5() {
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x0f, 0xee, 0xe5, // PMAXSW XMM4, XMM5
        0xf4,
    ]);
...
fn test_pmaxsw_from_memory() {
    ...
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00,
        0x66, 0x0f, 0xee, 0x03, // PMAXSW XMM0, [RBX]
        0xf4,
    ]);
...
// ============================================================================
// PMAXSD Tests - 4x Signed Dword Maximum
// ============================================================================

#[test]
fn test_pmaxsd_all_zeros() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
#[test]
fn test_pmaxsd_positive_negative() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
#[test]
fn test_pmaxsd_negative_values() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
...
fn test_pmaxsd_from_memory() {
    ...
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00,
        0x66, 0x0f, 0x38, 0x3d, 0x03, // PMAXSD XMM0, [RBX]
        0xf4,
    ]);
...
fn test_pmaxsb_xmm8_xmm9() {
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x45, 0x0f, 0x38, 0x3c, 0xc1, // PMAXSB XMM8, XMM9
        0xf4,
    ]);

    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x01; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x02; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsw_xmm10_xmm11() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x45, 0x0f, 0xee, 0xd3, // PMAXSW XMM10, XMM11
        0xf4,
    ]);
...
fn test_pmaxsd_xmm12_xmm13() {
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x45, 0x0f, 0x38, 0x3d, 0xe5, // PMAXSD XMM12, XMM13
        0xf4,
    ]);
...
#[test]
fn test_pmaxsb_pmaxsw_sequence() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
#[test]
fn test_pmaxsb_same_register() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0x38, 0x3c, 0xc0, // PMAXSB XMM0, XMM0
        0xf4,
    ]);

    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x42; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pmaxsw_same_register() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0xee, 0xc0, // PMAXSW XMM0, XMM0
        0xf4,
    ]);
...
#[test]
fn test_pmaxsd_same_register() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0x38, 0x3d, 0xc0, // PMAXSD XMM0, XMM0
        0xf4,
    ]);
