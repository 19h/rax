...
fn test_paddsb_positive_values() {
    ...
    mem.write_slice(&[0x20; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_negative_values() {
    ...
    mem.write_slice(&[0xF0; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_saturate_positive_max_large() {
    ...
    mem.write_slice(&[0x64; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x64; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_saturate_negative_min_large() {
    ...
    mem.write_slice(&[0x9C; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x9C; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_mixed_positive_negative() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // 50 + (-30) = 20
    mem.write_slice(&[0x32; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0xE2; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_xmm_self() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x40; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_paddsb_sequential_operations() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x20; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x10; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
