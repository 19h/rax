...
fn test_pcmpgtb_negative_vs_positive() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x80; 16], GuestAddress(ALIGNED_ADDR)).unwrap(); // -128
    mem.write_slice(&[0x01; 16], GuestAddress(ALIGNED_ADDR2)).unwrap(); // 1
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_pcmpgtb_positive_vs_negative() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x01; 16], GuestAddress(ALIGNED_ADDR)).unwrap(); // 1
    mem.write_slice(&[0xFF; 16], GuestAddress(ALIGNED_ADDR2)).unwrap(); // -1
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_pcmpgtb_from_memory() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x10; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
...
fn test_pcmpgtb_xmm8_xmm9() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x7F; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x01; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
