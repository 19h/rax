...
//
// PMINUB: Minimum of 16 packed unsigned byte integers (SSE2)
// PMINUW: Minimum of 8 packed unsigned word integers (SSE4.1)
// PMINUD: Minimum of 4 packed unsigned dword integers (SSE4.1)
//
// Opcodes:
// 66 0F DA /r         PMINUB xmm1, xmm2/m128   - Min of packed unsigned bytes
...
// ============================================================================
// PMINUB Tests - 16x Unsigned Byte Minimum
// ============================================================================

#[test]
fn test_pminub_all_zeros() {
    let code = [0x48, 0xb8];
...
fn test_pminub_max_values() {
    ...
    mem.write_slice(&[0x7F; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}

#[test]
fn test_pminub_xmm2_xmm3() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[0x48, 0xbb]);
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x0f, 0xda, 0xd3, // PMINUB XMM2, XMM3
        0xf4,
    ]);
...
fn test_pminub_from_memory() {
    ...
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0xda, 0x03, // PMINUB XMM0, [RBX]
        0xf4,
    ]);
...
fn test_pminub_identical() {
    ...
    mem.write_slice(&[0x42; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_pminub_xmm8_xmm9() {
    ...
    full_code.extend_from_slice(&[
        ...
        0x66, 0x45, 0x0f, 0xda, 0xc1, // PMINUB XMM8, XMM9
        0xf4,
    ]);
...
#[test]
fn test_pminub_pminuw_sequence() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
#[test]
fn test_pminub_same_register() {
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, // MOVDQA XMM0, [RAX]
        0x66, 0x0f, 0xda, 0xc0, // PMINUB XMM0, XMM0
        0xf4,
    ]);
