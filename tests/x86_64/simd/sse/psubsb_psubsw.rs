...
fn test_psubsb_positive_values() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // 0x30 - 0x10 = 0x20
    mem.write_slice(&[0x30; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
...
fn test_psubsb_negative_values() {
    ...
    mem.write_slice(&[0xE0; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_psubsb_mixed_positive_negative() {
    ...
    mem.write_slice(&[0x1E; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_psubsb_sequential_operations() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    mem.write_slice(&[0x50; 16], GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&[0x10; 16], GuestAddress(ALIGNED_ADDR2)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
