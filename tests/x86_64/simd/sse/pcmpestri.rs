...
fn test_pcmpestri_equal_any_basic() {
    // Control byte: 0x00 = unsigned bytes, equal any
    let code = [0x48, 0xb8];
...
fn test_pcmpestri_partial_lengths() {
    // Test with lengths shorter than actual data
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    // Data has more than specified length, but only specified bytes are used
...
fn test_pcmpestri_sword_length_8() {
    // Control byte: 0x03 = signed words
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
...
fn test_pcmpestri_max_word_length() {
    // Control byte: 0x01 = unsigned words
    let code = [0x48, 0xb8];
    let mut full_code = code.to_vec();
    full_code.extend_from_slice(&ALIGNED_ADDR.to_le_bytes());
    full_code.extend_from_slice(&[
