...
fn test_movq_same_register() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0xCAFEBABEDEADBEEF);
...
fn test_movq_mm0_m64() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x0011223344556677);
...
fn test_movq_mm1_m64() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2008, 0x8877665544332211);
...
fn test_movq_m64_mm0() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x123456789ABCDEF0);
...
fn test_movq_store_all_zeros() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x0000000000000000);
    write_mem_at_u64(&mem, 0x3000, 0xFFFFFFFFFFFFFFFF); // Pre-fill with ones
...
fn test_movq_store_all_ones() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0xFFFFFFFFFFFFFFFF);
    write_mem_at_u64(&mem, 0x3000, 0x0000000000000000); // Pre-fill with zeros
...
fn test_movq_mem_to_mem() {
    // Test moving data from one memory location to another via MMX register
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
        0x0f, 0x7f, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00, // MOVQ [0x3000], MM0
        0xf4,
...
fn test_movq_multiple_transfers() {
    // Test multiple sequential MOVQ operations
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
        0x0f, 0x7f, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00, // MOVQ [0x3000], MM0
        0x0f, 0x6f, 0x0c, 0x25, 0x08, 0x20, 0x00, 0x00, // MOVQ MM1, [0x2008]
    ...
    let result1 = read_mem_at_u64(&mem, 0x3000);
    let result2 = read_mem_at_u64(&mem, 0x3008);
...
fn test_movq_register_chain() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0xBEEFCAFEDEADC0DE);
...
fn test_movq_circular_move() {
    // Test moving values in a circular pattern
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
        0x0f, 0x6f, 0x0c, 0x25, 0x08, 0x20, 0x00, 0x00, // MOVQ MM1, [0x2008]
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0xAAAAAAAAAAAAAAAA);
    write_mem_at_u64(&mem, 0x2008, 0xBBBBBBBBBBBBBBBB);
...
fn test_movq_high_bit_set() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x8000000000000000);
...
fn test_movq_one() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x0000000000000001);
...
fn test_movq_power_of_two() {
    ...
    for (i, &value) in test_values.iter().enumerate() {
        ...
        let (mut vcpu, mem) = setup_vm(&code, None);
        write_mem_at_u64(&mem, 0x2000, value);
...
fn test_movq_sequential_bytes() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x0001020304050607);
...
fn test_movq_repeating_bytes() {
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    write_mem_at_u64(&mem, 0x2000, 0x1212121212121212);
...
fn test_movq_all_registers() {
    // Test loading all MMX registers with different values
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
        0x0f, 0x6f, 0x0c, 0x25, 0x08, 0x20, 0x00, 0x00, // MOVQ MM1, [0x2008]
    ...
    let (mut vcpu, mem) = setup_vm(&code, None);
    for i in 0..8 {
        write_mem_at_u64(&mem, 0x2000 + i * 8, 0x1111 * (i + 1) as u64);
    }
...
fn test_movq_overwrite() {
    // Test overwriting a register value
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
...
fn test_movq_load_store_independence() {
    // Verify that loading doesn't affect other registers
    let code = vec![
        0x0f, 0x6f, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // MOVQ MM0, [0x2000]
        0x0f, 0x6f, 0x0c, 0x25, 0x08, 0x20, 0x00, 0x00, // MOVQ MM1, [0x2008]
        0x0f, 0x6f, 0x14, 0x25, 0x10, 0x20, 0x00, 0x00, // MOVQ MM2, [0x2010]
    ...
    write_mem_at_u64(&mem, 0x2000, 0xAAAAAAAAAAAAAAAA);
    write_mem_at_u64(&mem, 0x2008, 0xBBBBBBBBBBBBBBBB);
    write_mem_at_u64(&mem, 0x2010, 0xCCCCCCCCCCCCCCCC);

    run_until_hlt(&mut vcpu).unwrap();

    let result0 = read_mem_at_u64(&mem, 0x3000);
    let result1 = read_mem_at_u64(&mem, 0x3008);
    let result2 = read_mem_at_u64(&mem, 0x3010);
