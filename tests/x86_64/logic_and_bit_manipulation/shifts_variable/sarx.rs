...
fn test_sarx_32bit_shift_negative_by_1() {
    // Shift negative number by 1 (sign extends)
    let code = [
        0xc4, 0xe2, 0x72, 0xf7, 0xc3, // SARX EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x80000000; // -2147483648 in signed 32-bit
    regs.rcx = 1;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_sarx_32bit_all_shift_counts_positive() {
    ...
    for count in 0..=31 {
        ...
        let mut regs = Registers::default();
        regs.rbx = 0x7FFFFFFF; // Max positive 32-bit
        regs.rcx = count;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_sarx_32bit_all_shift_counts_negative() {
    ...
    for count in 0..=31 {
        ...
        let mut regs = Registers::default();
        regs.rbx = 0x80000000; // Min negative 32-bit
        regs.rcx = count;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_sarx_32bit_negative_shift_to_all_ones() {
    // Shifting negative number fully results in all ones
    let code = [
        0xc4, 0xe2, 0x72, 0xf7, 0xc3, // SARX EAX, EBX, ECX
        0xf4,
    ];
...
fn test_sarx_32bit_sign_extension_pattern() {
    // Test sign extension with specific patterns
    let test_cases = vec![
...
fn test_sarx_64bit_shift_positive_by_1() {
    ...
    assert_eq!(regs.rax, 0x0000000000000080, "256 >> 1 = 128 (positive)");
}
...
fn test_sarx_64bit_shift_negative_by_1() {
    ...
    let mut regs = Registers::default();
    regs.rbx = 0x8000000000000000; // Min negative 64-bit
    regs.rcx = 1;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax, 0xC000000000000000, "Sign bit should be extended");
}
...
fn test_sarx_64bit_all_shift_counts_positive() {
    ...
    for count in 0..=63 {
        ...
        let mut regs = Registers::default();
        regs.rbx = 0x7FFFFFFFFFFFFFFF; // Max positive 64-bit
        regs.rcx = count;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();

        let expected = (0x7FFFFFFFFFFFFFFFi64 >> count) as u64;
        assert_eq!(regs.rax, expected, "Positive value >> {} failed", count);
    }
}
...
fn test_sarx_64bit_all_shift_counts_negative() {
    ...
    for count in 0..=63 {
        ...
        assert_eq!(regs.rax, expected, "Negative value >> {} failed", count);
    }
}
...
fn test_sarx_64bit_sign_extension_pattern() {
    ...
    let test_cases = vec![
        ...
        (0xF000000000000000u64, 16, 0xFFFF000000000000u64),
...
fn test_sarx_64bit_preserves_all_flags() {
    ...
    assert_eq!(regs.rax, 0xFFFF800000000000, "Shift result correct");
...
fn test_sarx_32bit_all_ones_stays_all_ones() {
    // All ones (negative) shifted should stay all ones
...
fn test_sarx_64bit_all_ones_stays_all_ones() {
    // All ones (negative) shifted should stay all ones (64-bit)
...
fn test_sarx_32bit_positive_no_sign_extension() {
    // Positive numbers should not get sign extended
    let code = [
        0xc4, 0xe2, 0x72, 0xf7, 0xc3, // SARX EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x7FFFFFFF; // Max positive
    regs.rcx = 1;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
...
fn test_sarx_64bit_r14_r15_r13() {
    ...
    assert_eq!(regs.r14, 0xFF80000000000000, "64-bit extended registers");
}
...
fn test_sarx_source_preservation() {
    ...
    assert_eq!(regs.rbx & 0xFFFFFFFF, 0x80000000, "Source unchanged");
...
fn test_sarx_vs_shrx_difference() {
    // Demonstrate difference between SARX (sign-extend) and SHRX (zero-extend)
    let code = [
        0xc4, 0xe2, 0x72, 0xf7, 0xc3, // SARX EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xF0000000; // Negative
    regs.rcx = 4;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();
