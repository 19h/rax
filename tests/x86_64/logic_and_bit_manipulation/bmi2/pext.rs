use crate::common::*;
use rax::cpu::Registers;

// PEXT - Parallel Bits Extract (BMI2)
// Extracts bits from the source operand at positions specified by the mask (second source).
// For each set bit in the mask, the corresponding bit from the source is extracted and
// packed contiguously into the destination, starting from the LSB.
// This is the inverse operation of PDEP.
//
// Opcodes:
// VEX.NDS.LZ.F3.0F38.W0 F5 /r   PEXT r32, r32, r/m32   - Parallel extract of bits
// VEX.NDS.LZ.F3.0F38.W1 F5 /r   PEXT r64, r64, r/m64   - Parallel extract of bits

#[test]
fn test_pext_eax_ebx_ecx_all_mask() {
    // PEXT EAX, EBX, ECX - mask all ones (identity)
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x12345678;
    regs.rcx = 0xFFFFFFFF; // all mask bits set
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0x12345678, "EAX should equal source (identity with full mask)");
}

#[test]
fn test_pext_eax_ebx_ecx_zero_mask() {
    // PEXT EAX, EBX, ECX - mask all zeros
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x12345678;
    regs.rcx = 0x00000000; // no mask bits set
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0, "EAX should be zero (no bits extracted)");
}

#[test]
fn test_pext_eax_ebx_ecx_single_bit_mask() {
    // PEXT with single bit mask
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0b0000_1000; // bit 3 set
    regs.rcx = 0b0000_1000; // extract bit 3
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0b0000_0001, "EAX should have extracted bit at position 0");
}

#[test]
fn test_pext_eax_ebx_ecx_alternating_mask() {
    // PEXT with alternating mask 0101...0101
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x55555555; // alternating bits
    regs.rcx = 0x55555555; // extract every other bit
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    // Extracts 16 alternating bits, packs them contiguously
    assert_eq!(regs.rax & 0xFFFFFFFF, 0x0000FFFF, "EAX should have 16 bits packed");
}

#[test]
fn test_pext_eax_ebx_ecx_low_nibble() {
    // Extract specific nibble
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x12345678;
    regs.rcx = 0x0000000F; // mask for bits 0-3
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0x8, "EAX should contain extracted nibble");
}

#[test]
fn test_pext_rax_rbx_rcx_64bit() {
    // PEXT RAX, RBX, RCX - 64-bit version
    let code = [
        0xc4, 0xe2, 0xf2, 0xf5, 0xc3, // PEXT RAX, RBX, RCX (W1)
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xFF00000000000000;
    regs.rcx = 0xFF00000000000000; // extract high byte
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax, 0xFF, "RAX should have extracted high byte at low position");
}

#[test]
fn test_pext_sparse_mask() {
    // PEXT with sparse mask
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x80001001; // bits 0, 12, 31 set
    regs.rcx = 0x80001001; // extract those bits
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0b111, "EAX should have 3 bits packed at low positions");
}

#[test]
fn test_pext_with_extended_registers() {
    // PEXT R8D, R9D, R10D
    let code = [
        0xc4, 0x42, 0x33, 0xf5, 0xc1, // PEXT R8D, R9D, R10D
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.r9 = 0x00FF0000; // bits 16-23 set
    regs.r10 = 0x00FF0000; // extract bits 16-23
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.r8 & 0xFFFFFFFF, 0xFF, "R8D should have extracted byte");
}

#[test]
fn test_pext_mem32() {
    // PEXT EAX, EBX, [mem]
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // PEXT EAX, EBX, [DATA_ADDR]
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x000F0000;
    let (mut vcpu, mem) = setup_vm(&code, Some(regs));
    write_mem_u32(&mem, 0x000F0000); // mask bits 16-19
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xF, "EAX should have extracted nibble from memory mask");
}

#[test]
fn test_pext_mem64() {
    // PEXT RAX, RBX, [mem]
    let code = [
        0xc4, 0xe2, 0xf2, 0xf5, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // PEXT RAX, RBX, [DATA_ADDR]
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x00FF000000000000;
    let (mut vcpu, mem) = setup_vm(&code, Some(regs));
    write_mem_u64(&mem, 0x00FF000000000000); // mask bits 48-55
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax, 0xFF, "RAX should have extracted byte from memory mask");
}

#[test]
fn test_pext_preserves_sources() {
    // PEXT should not modify source operands
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x12345678;
    regs.rcx = 0xAAAAAAAA;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rbx & 0xFFFFFFFF, 0x12345678, "EBX should be unchanged");
    assert_eq!(regs.rcx & 0xFFFFFFFF, 0xAAAAAAAA, "ECX should be unchanged");
}

#[test]
fn test_pext_sequential_extracts() {
    // Test extracting sequential bit ranges
    let test_cases = vec![
        (0x0000000F, 0x0000000F, 0x0000000F),
        (0x000000FF, 0x000000FF, 0x000000FF),
        (0x00000FFF, 0x00000FFF, 0x00000FFF),
        (0x0000FFFF, 0x0000FFFF, 0x0000FFFF),
    ];

    for (src, mask, expected) in test_cases {
        let code = [
            0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
            0xf4,
        ];
        let mut regs = Registers::default();
        regs.rbx = src;
        regs.rcx = mask;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();

        assert_eq!(regs.rax & 0xFFFFFFFF, expected, "PEXT(0x{:X}, 0x{:X}) should be 0x{:X}", src, mask, expected);
    }
}

#[test]
fn test_pext_extract_nibbles() {
    // Extract alternating nibbles
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x0D0C0B0A;
    regs.rcx = 0x0F0F0F0F; // every other nibble
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xDCBA, "Should extract alternating nibbles packed");
}

#[test]
fn test_pext_bit_gather() {
    // Gather scattered bits
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x01010101; // one bit per byte
    regs.rcx = 0x01010101; // extract those bits
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xF, "Should gather 4 scattered bits");
}

#[test]
fn test_pext_power_of_two_masks() {
    // Test with power of 2 masks
    for bit_pos in 0..32 {
        let code = [
            0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
            0xf4,
        ];
        let mut regs = Registers::default();
        regs.rbx = 1u64 << bit_pos;
        regs.rcx = 1u64 << bit_pos;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();

        assert_eq!(regs.rax & 0xFFFFFFFF, 1, "Should extract single bit to position 0");
    }
}

#[test]
fn test_pext_inverse_of_pdep() {
    // PEXT is the inverse of PDEP with the same mask
    let code_pdep = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PDEP EAX, EBX, ECX
        0xf4,
    ];
    let code_pext = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];

    let value = 0x12345678u64;
    let mask = 0x0F0F0F0Fu64;

    // Apply PDEP
    let mut regs = Registers::default();
    regs.rbx = value;
    regs.rcx = mask;
    let (mut vcpu, _) = setup_vm(&code_pdep, Some(regs));
    let regs_pdep = run_until_hlt(&mut vcpu).unwrap();
    let deposited = regs_pdep.rax & 0xFFFFFFFF;

    // Apply PEXT to result
    let mut regs = Registers::default();
    regs.rbx = deposited;
    regs.rcx = mask;
    let (mut vcpu, _) = setup_vm(&code_pext, Some(regs));
    let regs_pext = run_until_hlt(&mut vcpu).unwrap();

    // Should get back the masked bits compacted
    let expected = (value & mask).count_ones();
    assert_eq!((regs_pext.rax & 0xFFFFFFFF).count_ones(), expected, "PEXT(PDEP(x, mask), mask) should preserve bit count");
}

#[test]
fn test_pext_byte_compaction() {
    // Compact bytes with gaps
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x0A0B0C0D;
    regs.rcx = 0x00FF00FF; // two bytes separated
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0x0B0D, "Should compact separated bytes");
}

#[test]
fn test_pext_zero_source() {
    // Zero source always produces zero
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0;
    regs.rcx = 0xFFFFFFFF;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0, "Zero source should produce zero");
}

#[test]
fn test_pext_64bit_high_positions() {
    // Extract from high positions in 64-bit
    let code = [
        0xc4, 0xe2, 0xf2, 0xf5, 0xc3, // PEXT RAX, RBX, RCX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xFFFF000000000000;
    regs.rcx = 0xFFFF000000000000; // extract high 16 bits
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax, 0xFFFF, "Should extract high bits to low positions");
}

#[test]
fn test_pext_pattern_extraction() {
    // Extract specific patterns
    let test_cases = vec![
        (0x11111111, 0x11111111, 0x0000000F),  // extract every 8th bit
        (0x33333333, 0x33333333, 0x000000FF),  // extract every 4th-5th bits
        (0x0F0F0F0F, 0x0F0F0F0F, 0x0000FFFF),  // extract nibbles
    ];

    for (src, mask, expected) in test_cases {
        let code = [
            0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
            0xf4,
        ];
        let mut regs = Registers::default();
        regs.rbx = src;
        regs.rcx = mask;
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();

        assert_eq!(regs.rax & 0xFFFFFFFF, expected, "PEXT(0x{:X}, 0x{:X}) should be 0x{:X}", src, mask, expected);
    }
}

#[test]
fn test_pext_consecutive_mask_bits() {
    // Consecutive mask bits act like right shift + mask
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x12345678;
    regs.rcx = 0x0000FFFF; // lower 16 bits consecutive
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0x00005678, "Consecutive mask extracts lower bits");
}

#[test]
fn test_pext_field_unpacking() {
    // Unpack multiple fields using mask
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0x0F0000F0; // two 4-bit fields
    regs.rcx = 0x0F0000F0; // extract both
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xFF, "Should unpack fields contiguously");
}

#[test]
fn test_pext_mask_population_count() {
    // Result can have at most popcount(mask) bits
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xFFFFFFFF; // all source bits set
    regs.rcx = 0x000000FF; // 8 mask bits
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xFF, "Result limited by mask popcount");
}

#[test]
fn test_pext_interleaved_bytes() {
    // Extract interleaved bytes
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xAABBCCDD;
    regs.rcx = 0x00FF00FF; // bytes at positions 0 and 2
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xBBDD, "Should extract interleaved bytes");
}

#[test]
fn test_pext_bit_reversal_aid() {
    // PEXT can help with bit manipulation patterns
    let code = [
        0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
        0xf4,
    ];
    let mut regs = Registers::default();
    regs.rbx = 0xF0F0F0F0;
    regs.rcx = 0xF0F0F0F0; // extract high nibbles of each byte
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rax & 0xFFFFFFFF, 0xFFFF, "Should extract all high nibbles");
}

#[test]
fn test_pext_single_byte_from_dword() {
    // Extract specific byte from dword
    for byte_idx in 0..4 {
        let code = [
            0xc4, 0xe2, 0x72, 0xf5, 0xc3, // PEXT EAX, EBX, ECX
            0xf4,
        ];
        let mut regs = Registers::default();
        regs.rbx = 0x03020100;
        regs.rcx = 0xFFu64 << (byte_idx * 8);
        let (mut vcpu, _) = setup_vm(&code, Some(regs));
        let regs = run_until_hlt(&mut vcpu).unwrap();

        assert_eq!(regs.rax & 0xFFFFFFFF, byte_idx as u64, "Should extract byte {}", byte_idx);
    }
}
