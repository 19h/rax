use crate::common::{run_until_hlt, setup_vm, write_mem_at_u32, write_mem_at_u16};
...
fn test_bound_16bit_within_bounds() {
    ...
    let code = [
        0x66, 0xb8, 0x05, 0x00, // MOV AX, 5 (index)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
...
fn test_bound_16bit_at_lower_bound() {
    ...
    let code = [
        0x66, 0xb8, 0x00, 0x00, // MOV AX, 0 (at lower bound)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
...
fn test_bound_16bit_at_upper_bound() {
    ...
    let code = [
        0x66, 0xb8, 0x0a, 0x00, // MOV AX, 10 (at upper bound)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
...
fn test_bound_16bit_below_lower_bound() {
    ...
    let code = [
        0x66, 0xb8, 0xff, 0xff, // MOV AX, -1 (below lower bound)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
...
fn test_bound_16bit_above_upper_bound() {
    ...
    let code = [
        0x66, 0xb8, 0x0b, 0x00, // MOV AX, 11 (above upper bound)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
...
fn test_bound_32bit_within_bounds() {
    ...
    let code = [
        ...
        0x67, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND EAX, [0x2000]
        0x48, 0xc7, 0xc3, 0x01, 0x00, 0x00, 0x00, // MOV RBX, 1
        0xf4,
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_bound_32bit_negative_bounds() {
    ...
    write_mem_at_u32(&mem, 0x2000, 0xffffff9c); // -100
...
fn test_bound_array_access_pattern() {
    ...
    let code = [
        0xb8, 0x07, 0x00, 0x00, 0x00, // MOV EAX, 7 (array index)
        0x67, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND EAX, [0x2000] (bounds)
...
fn test_bound_loop_index_check() {
    ...
    let code = [
        0xb8, 0x00, 0x00, 0x00, 0x00, // MOV EAX, 0 (loop counter)
        // Loop start
        0x67, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND EAX, [0x2000]
        0x48, 0xff, 0xc0, // INC RAX
        0x48, 0x83, 0xf8, 0x05, // CMP RAX, 5
        0x72, 0xf3, // JB loop (back 13 bytes)
...
fn test_bound_base_displacement() {
    ...
    let code = [
        ...
        0x67, 0x62, 0x83, 0x00, 0x01, 0x00, 0x00, // BOUND EAX, [RBX + 0x100]
...
fn test_bound_invalid_zero_length() {
    ...
    let code = [
        0x66, 0xb8, 0x06, 0x00, // MOV AX, 6 (not 5)
        0x66, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND AX, [0x2000]
        0x48, 0xc7, 0xc3, 0x02, 0x00, 0x00, 0x00, // MOV RBX, 2
        0xf4,
    ];
...
fn test_bound_16bit_max_bounds() {
    ...
    write_mem_at_u16(&mem, 0x2000, 0);
    write_mem_at_u16(&mem, 0x2002, 0xFFFF);
...
fn test_bound_32bit_max_bounds() {
    ...
    write_mem_at_u32(&mem, 0x2000, 0);
    write_mem_at_u32(&mem, 0x2004, 0xFFFFFFFF);
...
fn test_bound_with_different_registers() {
    ...
    let code = [
        ...
        0x66, 0x62, 0x1c, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND BX, [0x2000]
        0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00, // MOV RAX, 1
        0xf4,
    ];
    let (mut vcpu, mem) = setup_vm(&code, None);
...
fn test_bound_cx_register() {
    let code = [
        0x66, 0xb9, 0x08, 0x00, // MOV CX, 8
        0x66, 0x62, 0x0c, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND CX, [0x2000]
...
fn test_bound_dx_register() {
    let code = [
        0x66, 0xba, 0x03, 0x00, // MOV DX, 3
        0x66, 0x62, 0x14, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND DX, [0x2000]
...
fn test_bound_string_length_check() {
    ...
    let code = [
        0xb8, 0x03, 0x00, 0x00, 0x00, // MOV EAX, 3 (string index)
        0x67, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND EAX, [0x2000]
...
fn test_bound_buffer_access_validation() {
    ...
    let code = [
        0xb8, 0x14, 0x00, 0x00, 0x00, // MOV EAX, 20 (buffer offset)
        0x67, 0x62, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00, // BOUND EAX, [0x2000]
