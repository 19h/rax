...
fn test_popfq_restore_carry() {
    ...
    assert!(cf_set(regs.rflags), "CF should be restored");
}
...
fn test_popfq_restore_zero() {
    ...
    assert!(zf_set(regs.rflags), "ZF should be restored");
}
...
fn test_popfq_restore_sign() {
    ...
    assert!(sf_set(regs.rflags), "SF should be restored");
}
...
fn test_pushfq_popfq_roundtrip() {
    ...
    assert!(cf_set(regs.rflags), "CF should be restored by POPFQ");
    assert_eq!(regs.rsp, 0x1000, "RSP should be balanced");
}
...
fn test_pushfq_at_stack_boundary() {
    ...
    let mut regs = Registers::default();
    regs.rsp = 0x08; // Near bottom of memory
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp, 0x00, "RSP decremented");
}
...
fn test_popfq_restore_overflow() {
    ...
    assert!(of_set(regs.rflags), "OF should be restored");
}
...
fn test_popfq_restore_direction_flag() {
    ...
    assert!(df_set(regs.rflags), "DF should be restored");
}
...
fn test_popfq_restore_parity() {
    ...
    assert!(pf_set(regs.rflags), "PF should be restored");
}
