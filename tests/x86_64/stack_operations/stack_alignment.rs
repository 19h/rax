...
fn test_stack_alignment_16_byte() {
    ...
    regs.rsp = 0x1000; // 16-byte aligned
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp & 0x0F, 0, "RSP is 16-byte aligned");
}
...
fn test_stack_misalignment_8_byte() {
    ...
    let mut regs = Registers::default();
    regs.rsp = 0x1008; // 8-byte aligned, not 16-byte aligned
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_ne!(regs.rsp & 0x0F, 0, "RSP is not 16-byte aligned");
    assert_eq!(regs.rsp & 0x07, 0, "RSP is 8-byte aligned");
}
...
fn test_single_push_breaks_alignment() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "RSP offset by 8 from 16-byte boundary");
}
...
fn test_odd_pushes_misalign() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "Three pushes misalign stack");
}
...
fn test_four_pushes_aligned() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Four pushes maintain alignment");
}
...
fn test_enter_with_even_allocation() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "ENTER 16,0 results in misalignment");
}
...
fn test_enter_with_alignment_padding() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "ENTER 8,0 maintains alignment");
}
...
fn test_push_imm_alignment() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "PUSH immediate breaks alignment");
}
...
fn test_multiple_push_imm_pattern() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Eight pushes maintain alignment");
}
...
fn test_sub_rsp_16_aligned() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "SUB RSP, 16 maintains alignment");
}
...
fn test_sub_rsp_8_misaligned() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "SUB RSP, 8 breaks alignment");
}
...
fn test_sub_rsp_32_aligned() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "SUB RSP, 32 maintains alignment");
}
...
fn test_sub_rsp_48_aligned() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "SUB RSP, 48 maintains alignment");
}
...
fn test_sub_rsp_odd_value() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "SUB RSP, 24 breaks alignment");
}
...
fn test_align_stack_with_and() {
    ...
    let mut regs = Registers::default();
    regs.rsp = 0x1008; // Misaligned
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp & 0x0F, 0, "AND aligns stack to 16 bytes");
    assert_eq!(regs.rsp, 0x1000, "Stack aligned down");
}
...
fn test_align_already_aligned_stack() {
    ...
    let mut regs = Registers::default();
    regs.rsp = 0x1000; // Already aligned
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp, 0x1000, "Already aligned stack unchanged");
}
...
fn test_function_prologue_with_saved_regs() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "Prologue with saved reg misaligned");
}
...
fn test_function_prologue_aligned_locals() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Aligned prologue with 8-byte adjust");
}
...
fn test_push_pop_alignment_restoration() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "PUSH/POP restores alignment");
}
...
fn test_sub_add_alignment_restoration() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "SUB/ADD restores alignment");
}
...
fn test_nested_function_alignment() {
    ...
    // Outer: PUSH(8) + SUB(8) = 16
    // Inner: PUSH(8) + SUB(8) = 16
    // Total: 32 bytes, aligned
    assert_eq!(regs.rsp & 0x0F, 0, "Nested functions maintain alignment");
}
...
fn test_alignment_with_parameter_passing() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Six parameters maintain alignment");
}
...
fn test_alignment_with_odd_parameters() {
    ...
    assert_eq!(regs.rsp & 0x0F, 8, "Three parameters break alignment");
}
...
fn test_check_multiple_alignment_points() {
    ...
    // After 4 pushes: aligned
    assert_eq!(regs.rsp & 0x0F, 0, "Four pushes result in alignment");
}
...
fn test_pushfq_popfq_alignment() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "PUSHFQ/POPFQ preserves alignment");
}
...
fn test_large_stack_frame_alignment() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Large allocation maintains alignment");
    assert_eq!(regs.rsp, 0xF000, "4KB frame allocated");
}
...
fn test_alignment_after_mixed_operations() {
    ...
    assert_eq!(regs.rsp & 0x0F, 0, "Mixed operations maintain alignment");
}
