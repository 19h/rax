...
fn test_add_rsp_8() {
    ...
    assert_eq!(regs.rsp, 0x1008, "RSP incremented by 8");
}
...
fn test_add_rsp_16() {
    ...
    assert_eq!(regs.rsp, 0x1010, "RSP incremented by 16");
}
...
fn test_add_rsp_32() {
    ...
    assert_eq!(regs.rsp, 0x1020, "RSP incremented by 32");
}
...
fn test_add_rsp_large() {
    ...
    assert_eq!(regs.rsp, 0x2000, "RSP incremented by 4096");
}
...
fn test_add_rsp_cleanup_params() {
    ...
    assert_eq!(regs.rsp, 0x1000, "Stack cleaned up after parameter push");
}
...
fn test_sub_rsp_16() {
    ...
    assert_eq!(regs.rsp, 0x0FF0, "RSP decremented by 16");
}
...
fn test_sub_rsp_32() {
    ...
    assert_eq!(regs.rsp, 0x0FE0, "RSP decremented by 32");
}
...
fn test_sub_rsp_large_frame() {
    ...
    assert_eq!(regs.rsp, 0xF000, "Large stack frame allocated");
}
...
fn test_sub_rsp_for_locals() {
    ...
    assert_eq!(regs.rsp, 0x1000 - 40, "Frame + locals allocated");
}
...
fn test_mov_rsp_rax() {
    ...
    assert_eq!(regs.rsp, 0x2000, "RSP set from RAX");
}
...
fn test_mov_rsp_rbx() {
    ...
    assert_eq!(regs.rsp, 0x3000, "RSP set from RBX");
}
...
fn test_mov_rsp_rbp() {
    ...
    assert_eq!(regs.rsp, 0x2000, "RSP set from RBP (like LEAVE part 1)");
}
...
fn test_mov_rsp_r8() {
    ...
    assert_eq!(regs.rsp, 0x4000, "RSP set from R8");
}
...
fn test_mov_rax_rsp() {
    ...
    assert_eq!(regs.rax, 0x1000, "RAX gets RSP value");
...
fn test_mov_rbx_rsp() {
    ...
    assert_eq!(regs.rbx, 0x1000, "RBX gets RSP value");
}
...
fn test_mov_rbp_rsp() {
    ...
    assert_eq!(regs.rbp, 0x1000, "RBP gets RSP value (prologue pattern)");
}
...
fn test_save_restore_rsp() {
    ...
    assert_eq!(regs.rsp, 0x1000, "RSP saved and restored");
}
...
fn test_lea_rsp_rsp_plus_8() {
    ...
    assert_eq!(regs.rsp, 0x1008, "RSP adjusted by LEA +8");
}
...
fn test_lea_rsp_rsp_minus_8() {
    ...
    assert_eq!(regs.rsp, 0x0FF8, "RSP adjusted by LEA -8");
}
...
fn test_lea_rsp_rsp_plus_16() {
    ...
    assert_eq!(regs.rsp, 0x1010, "RSP adjusted by LEA +16");
}
...
fn test_lea_rsp_rbp_minus_offset() {
    ...
    assert_eq!(regs.rsp, 0x1FF0, "RSP set via LEA from RBP");
}
...
fn test_inc_rsp() {
    ...
    assert_eq!(regs.rsp, 0x1001, "RSP incremented by 1");
}
...
fn test_dec_rsp() {
    ...
    assert_eq!(regs.rsp, 0x0FFF, "RSP decremented by 1");
}
...
fn test_multiple_inc_rsp() {
    ...
    assert_eq!(regs.rsp, 0x1004, "RSP incremented 4 times");
}
...
fn test_xchg_rsp_rax() {
    ...
    assert_eq!(regs.rsp, 0x2000, "RSP gets RAX value");
    assert_eq!(regs.rax, 0x1000, "RAX gets old RSP value");
}
...
fn test_xchg_rsp_rbx() {
    ...
    assert_eq!(regs.rsp, 0x3000, "RSP gets RBX value");
    assert_eq!(regs.rbx, 0x1000, "RBX gets old RSP value");
}
...
fn test_rsp_alloc_dealloc_cycle() {
    ...
    assert_eq!(regs.rsp, 0x1000, "RSP restored after alloc/dealloc");
}
...
fn test_rsp_with_stack_operations() {
    ...
    // Value was written to 0xFF0 but that's now above RSP
    let val = read_mem_at_u64(&vm, 0x0FF0);
...
fn test_rsp_lea_cleanup() {
    ...
    assert_eq!(regs.rsp, 0x1000, "Stack cleaned up via LEA");
}
...
fn test_add_rsp_rax() {
    ...
    assert_eq!(regs.rsp, 0x1100, "RSP += RAX");
}
...
fn test_sub_rsp_rbx() {
    ...
    assert_eq!(regs.rsp, 0x0FB0, "RSP -= RBX");
}
...
fn test_and_rsp_align() {
    ...
    assert_eq!(regs.rsp, 0x1000, "RSP aligned to 16 bytes");
}
...
fn test_mov_rsp_from_memory() {
    ...
    write_mem_at_u64(&vm, DATA_ADDR, 0x2000);

    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp, 0x2000, "RSP loaded from memory");
}
...
fn test_rsp_zero() {
    ...
    assert_eq!(regs.rsp, 0, "RSP can be zeroed");
}
...
fn test_rsp_max_value() {
    ...
    assert_eq!(regs.rsp, 0xFFFFFFFFFFFFFFFF, "RSP can be set to max value");
}
...
fn test_rsp_wraparound_add() {
    ...
    let mut regs = Registers::default();
    regs.rsp = 0xFFFFFFFFFFFFFFF0;
    let (mut vcpu, _) = setup_vm(&code, Some(regs));
    let regs = run_until_hlt(&mut vcpu).unwrap();

    assert_eq!(regs.rsp, 0, "RSP wraps around on overflow");
}
...
fn test_rsp_complex_calculation() {
    ...
    assert_eq!(regs.rsp, 0x0FE0, "Complex RSP calculation");
}
...
fn test_rsp_in_loop_simulation() {
    ...
    assert_eq!(regs.rsp, 0x1000, "RSP balanced after loop simulation");
}
