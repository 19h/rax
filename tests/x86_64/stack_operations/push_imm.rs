...
fn test_push_imm8_sequence() {
    ...
    assert_eq!(regs.rsp, 0x1000 - 40, "Five values pushed");
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 5, "Top of stack");
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 4);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 16), 3);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 24), 2);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 32), 1);
}
...
fn test_push_imm32_sequence() {
    ...
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 3);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 2);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 16), 1);
}
...
fn test_push_imm_mixed_sizes() {
    ...
    assert_eq!(read_mem_at_u64(&vm, 0x1000 - 24), 0x33);
    assert_eq!(read_mem_at_u64(&vm, 0x1000 - 16), 0x22222222);
    assert_eq!(read_mem_at_u64(&vm, 0x1000 - 8), 0x11);
}
...
fn test_push_imm8_pop_roundtrip() {
    ...
    assert_eq!(regs.rax, 0x42, "Value popped into RAX");
...
fn test_push_imm32_pop_roundtrip() {
    ...
    assert_eq!(regs.rax, 0x12345678, "Value popped into RAX");
    assert_eq!(regs.rsp, 0x1000, "Stack balanced");
}
...
fn test_push_imm_negative_pop() {
    ...
    assert_eq!(regs.rax, 0xFFFFFFFFFFFFFFFF, "-1 in RAX");
}
...
fn test_push_imm_preserves_registers() {
    ...
    assert_eq!(regs.rax, 0xAA, "RAX unchanged");
    assert_eq!(regs.rbx, 0xBB, "RBX unchanged");
}
...
fn test_push_imm_large_params() {
    ...
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 0x3000);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 0x2000);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 16), 0x1000);
}
...
fn test_push_imm_at_low_stack() {
    ...
    assert_eq!(regs.rsp, 0x08, "RSP at low address");
    let val = read_mem_at_u64(&vm, 0x08);
...
fn test_push_imm_rapid_sequence() {
    ...
    assert_eq!(regs.rsp, 0x2000 - 128, "16 values pushed");
    for i in 0..16 {
...
fn test_push_imm_powers_of_two() {
    ...
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 64);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 32);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 16), 16);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 24), 8);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 32), 4);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 40), 2);
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 48), 1);
}
...
fn test_push_imm_ascii_values() {
    ...
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 0x43, "'C'");
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 0x42, "'B'");
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 16), 0x41, "'A'");
}
...
fn test_push_imm_boolean_values() {
    ...
    assert_eq!(read_mem_at_u64(&vm, regs.rsp), 1, "true");
    assert_eq!(read_mem_at_u64(&vm, regs.rsp + 8), 0, "false");
}
...
fn test_push_imm_stack_based_calc() {
    ...
    assert_eq!(regs.rax, 17, "3 * 4 + 5 = 17");
}
