...
The x86 VCPU also uses the scalar AArch64 trampoline for eligible x86-lifted SMIR. Legacy x86 GPR encodings RAX-R15 map to X0-X15. The bridge maps x86 CF/ZF/SF/OF to PSTATE.C/Z/N/V and merges only those four flags back into RFLAGS; PF, AF, control flags, and reserved bits remain from the pre-region snapshot. Eligibility rejects live PF/AF definitions or consumers, unsupported registers/virtual temporaries, memory, and flag contracts whose AArch64 carry convention is not normalized (notably live subtraction-borrow outputs, SBB, and generic CF-based unsigned conditions).

The architecture-specific scalar whitelist admits exact register-only BMI1 BLS, ADX, BT/BTS/BTR/BTC, and CLC/STC/CMC lowering in addition to the shared scalar set. BT and its update forms are admissible at 16/32/64 bits; the update lowerings explicitly merge 16-bit results into x86 destinations. Other destination-producing operations are limited to 32/64-bit forms unless their lowerer has a separately validated x86 partial-register merge: AArch64 W-register writes zero-extend, while x86 8/16-bit writes preserve the destination's upper bits. Unsupported subregister shapes fail before native execution.
